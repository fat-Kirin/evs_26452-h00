...
)
{
    ...
    temp = Find_Max_Norm16(OldauOut, L);
    Copy_Scale_sig(OldauOut, OldauOut, L, temp);
    *Q_old = add(*Q_old, temp);
    ...
    *Q_sig=*Q_old;  /*fixing output to new Q_old */
    ...
    IF( EQ_16(use_bfi_win,1))
    {
        ...
        {
            /* rescale oldgapsynth */
            Copy_Scale_sig( oldgapsynth, oldgapsynth, L, *Q_sig );
            p3=SS2+sub(temp,1);
            p4=oldgapsynth+n;
            FOR (i=0 ; i < temp; i++)
            {
    ...
    *Qsubfr=tmp;
    ...
    IF( EQ_16(last_L_frame,L_FRAME))
    {
        ...
        ELSE
        {
            ...
            test();
            test();
            IF ( (EQ_16(bwidth, NB)&&GE_32(output_Fs,16000))||(GT_16(bwidth,NB)&&GT_32(output_Fs,16000)))
            {
                /* mix cubic and CLDFB resampled buffers in case of resampling to higher frequency rates */
                buf_offset = i_mult2(SWITCH_GAP_LENGTH_8k, delta);
                ...
                FOR(i=0; i<out_filt_length; i++)
                {
                    ...
                    L_tmp = L_mult(a, tmp_buf_switch2[i]);
                    tmp_buf_switch[buf_offset + i] = mac_r(L_tmp, b, synth_subfr_out[buf_offset + i]);
                    move16();
                }
            }
            ELSE
            {
                ...
                Copy( tmp_buf_switch2, tmp_buf_switch+i_mult2(SWITCH_GAP_LENGTH_8k,delta), out_filt_length );
            }
        }
