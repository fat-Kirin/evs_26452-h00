...
)
{
    ...
    FOR( i = 0; i < len; i++ )
    {
        s = s_max(s, abs_s(input_fx[i]));
    }
    FOR( i = 0; i < INTERP_3_2_MEM_LEN; i++ )
    {
        s = s_max(s, abs_s(mem_fx[i]));
    }
    s = s_max(sub(norm_s(s), 2), 0);
    Copy_Scale_sig(input_fx, input_fx_temp, len, s);
    Scale_sig(mem_fx, INTERP_3_2_MEM_LEN, s);
    out1 = out1_buff;
    ...
    Scale_sig(out_fx, i_mult(3, shr(len,1)), -s);
    Scale_sig(mem_fx, INTERP_3_2_MEM_LEN, -s);
    return;
}
...
)
{
    ...
    Word16 sc, input_fx[960];
    sc = s_min( s_min(sub(Find_Max_Norm16(input, len), 2), sub(Find_Max_Norm16(mem_fx, 12), 2)), sub(Find_Max_Norm16(lp_mem_fx, 6), 2) );
    Scale_sig(lp_mem_fx, 6, sc);
    Copy_Scale_sig(input, input_fx, len,  sc);
    Scale_sig(mem_fx, 12, sc);
    out1 = out1_buff;
