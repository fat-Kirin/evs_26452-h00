...
void tcx_ltp_get_lpc(Word16 *x, Word16 L, Word16 *A, Word16 order)
{
    ...
    IF (s < 0)
    {
...

        {
            ...
            FOR (j = 0; j < L; j++)
            {
                ...
                move16();
                r64 = W_mac0_16_16(r64, tmpbuf[j], tmpbuf[j]);
            }
            r = W_sat_l(r64);
        }
        r = L_max(r, L_shl(100, shl(s, 1)));
    ...
    FOR (i = 1; i <= order; i++)
    {
        ...
        tmp = sub(L, i);
        {
            ...
            FOR (j = 0; j < tmp; j++)
            {
                r64 = W_mac0_16_16(r64, p[j], p[j+i]);
            }
            r = W_sat_l(r64);
        }
        r = L_shl(r, s2);
...
)
{
    ...
    if (EQ_16(frac_max, 6))win=&inter6_2tcx2[frac][0];
                            ...
                            )
{
    ...
    IF (ltp_param != 0 && ltp_param[0] != 0)
    {
        ...
        *gain = imult1616(add(ltp_param[2], 1), 0x1400);
        ...
        IF( GT_16(*pitch_int,PIT_MAX_MAX))
        {
            /*pitch out of range due to bit error */
