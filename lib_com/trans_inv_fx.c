...
)
{
    ...
    IF(LE_32(brate, HQ_32k))
    {
        ...
        IF( EQ_16(framelength, L_FRAME8k))
        {
            num_subsubframes = 4;
            move16();
        ...
        *memfilt_lb_fx = shr(*fxptr2, q_sig16);
        ...
        FOR (j = 0; j < NUMSF; j++)                 /* 8 present subframes */
        {
            ...
            {
                if( EQ_16(i, subframelength_s2))
                {
                    *fx32ptr5 = tmp_fxL1;
                    move32();
                }

                if( EQ_16(i, subframelength_s34))
                {
                    *fx32ptr6 = tmp_fxL1;
                    move32();
                }
                ...
                IF( L_mult0(*fxptr2, *(fxptr2-1)) <= 0 )
                {
                    ...
                    test();
                    if( (firstnzcr > 0) && (GT_16(cntnzcr, maxnzcr[tmp_fx2])))
                    {
            ...
            *sptr2 = sptr2_loc;
            ...
            firstnzcr = 1;
            move16();
            IF((LT_16(j, NUMSF_M1))&&(L_mult0(*fxptr2,*(fxptr2-1))<=0)) /* zcr between 2 subframes */
            {
        ...
        *sptr1 = sptr1_loc;
        ...
        move16();
        test();
        if( *prevflag != 0 || LT_32(max_es_fx, L_mult0(subframelength, 2500)))
        {
            maxind = 0;
            move16();
        }
        ...
        *prevflag = 0;
        ...
        FOR (i = 0; i < maxind; i++)                /* only subbands before max energy subband are handled */
        {
            ...
            test();
            test();
            IF( (  GT_32(tmp_fxL1, L_add(mean_prev_nc_fx_loc,125000)))||/* less then 20% energy in 3/4 of the subframe -> starting onset in the last quarter */
                (( GT_32(tmp_fxL2, L_add(mean_prev_nc_fx_loc,125000))) &&
                 ((LT_16(zcr[i], limzcr))||(LT_32(es_mdct_quart_fx[i],tmp_fxL3))))) /* already an offset, plosif, do not touch */
            {
                maxind = i;
                ...
                *prevflag = 1;
                ...
                FOR(j = sub(i,1); j >= 0; j--)
                {
                    if (GT_32(es_mdct_fx[j], L_shr(es_mdct_fx[i],1)))
                    {
                        maxind = j;
                        move16();
                    }
            ...
            ELSE
            {
                IF (LT_32(es_mdct_fx[i], L_shr(max_es_fx, 4)))
                {
                    g_fx = lim16_fx;
                    move16();
                    cnt5 = add(cnt5,1);

                    IF (LT_32(es_mdct_fx[i], L_shr(max_es_fx, 5)))
                    {
                        g_fx = lim32_fx;
                        move16();
                        cnt2 = add(cnt2, 1);
                    ...
                    IF(LT_32(mean_prev_fx_loc, es_mdct_fx[i]))
                    {
                    ...
                    IF(LT_32(mean_prev_hb_fx_loc, es_mdct_hb_fx[i]))
                    {
                    ...
                    {
                        if(LT_16(min_g_fx[i], 32767)) /* *mean_prev < es_mdct[i]) */
                        {
        ...
        FOR( i = 0; i < NUMSF; i++ )
        {
            if( LT_16(gt_fx[i],  32767))                         /*gt not yet limited by min_g*/
            {
        ...
        test();
        if( (GT_16(wmold_fx,  16384))&&(LT_16(add(cnt2,cnt5),2)))   /* mini either 1 cnt2 (and so also cnt5) or 2 cnt5 */
        {
        ...
        test();
        test();
        IF( ind2 > 0 || LT_16(wmold_fx, 32767)||LT_16(*wmold_hb_fx,32767))
        {
            ptr_fx = imdct_mem_fx;
            qtmp = qmemp1;
            ...
            *mean_plus_es_fx = *plus_es_mdct_fx;
            move32(); /* index [0] */
            if( LT_32(*mean_plus_es_fx, maxcrit_fx))
            {
            ...
            j = 0;
            move16();
            WHILE((LT_32(plus_es_mdct_fx[j], mean_plus_es_fx[j]))&&(LT_32(plus_es_mdct_fx[j],max_plus_es_mdct_fx/8)))
            {
                test();
                j = add(j,1);
            }
        ...
        IF(ind2>0)
        {
            ...
            IF( ind4 > 0 )
            {
                ...
                IF( GT_16(ind3, 2))
                {
                    ...
                    test();
                    IF ((LT_32(tmp_fxL1, eshbmean3_fx))||(sxylb3_fx<0))
                    {
                        ind2 = 0;
                        move16();
                        ind2_sfl = 0;
        ...
        *wmold_hb_fx = preechogain_hb_fx[framelength_m1];
        ...
        if( LT_32(savehalfe_fx, L_shr(mean_prev_fx_loc, 1) ))
        {
            mean_prev_fx_loc = L_shl(tmp_fxL1, 1);
        }

        tmp_fxL1 = L_sub(mean_prev_hb_fx_loc, savehalfe_hb_fx);
        if( LT_32(savehalfe_hb_fx, L_shr(mean_prev_hb_fx_loc, 1) ))
        {
        ...
        if( GT_32(last2_hb_fx, mean_prev_hb_fx_loc))
        {
            mean_prev_hb_fx_loc = L_add(last2_hb_fx, 0);
        }
        *mean_prev_fx = mean_prev_fx_loc;
        move32();
        *mean_prev_hb_fx = L_shr(mean_prev_hb_fx_loc, shl(q_sig16,1));
        move32(); /*save in Q0*/
...
#define N_GUARD_BITS (9+1) /* 9 is enough but we put one extra bit */
    ...
    IF (is_transient)
    {
        ...
        IF (EQ_16(L, L_FRAME48k))
        {
            win = short_window_48kHz_fx;
        }
        ELSE IF (EQ_16(L, L_FRAME32k))
        {
            win = short_window_32kHz_fx;
        }
        ELSE IF( EQ_16(L, L_FRAME16k))
        {
