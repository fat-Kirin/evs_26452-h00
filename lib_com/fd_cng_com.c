...
)
{
    ...
    ELSE
    {
        ...
        WHILE ( GT_16(stop,start))
        {
            ...
            FOR (j=start; j<stop; j++)
            {
                ...
                msNewMinFlag[j] = 0;
                move16();
                IF ( LT_32(scalar2,msCurrentMin[j])/*0.0 Q31*/)
                {
                    msNewMinFlag[j] = 1;
                    move16();
        ...
        test();
        IF ( GT_16(st->msFrCnt,1)&&LT_16(st->msFrCnt,MSSUBFRLEN))
        {
            FOR (j=0; j<len; j++)
            {
                if ( msNewMinFlag[j] > 0 )
        ...
        ELSE /* sub window complete */
        {
            IF ( GE_16(st->msFrCnt,MSSUBFRLEN))
            {
                ...
                FOR (i=1; i<MSNUMSUBFR; i++)
                {
                    FOR (j=0; j<len; j++)
                    {
                        if ( LT_32(*ptr,msCurrentMinOut[j])/*0.0 Q31*/)
                        {
    ...
    *msPeriodogBufPtr = add(*msPeriodogBufPtr,1);
    move16();
    if ( EQ_16(*msPeriodogBufPtr,MSBUFLEN))
    {
        (*msPeriodogBufPtr) = 0;
        move16();
    }
