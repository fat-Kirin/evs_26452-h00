    ...
    BASOP_SATURATE_ERROR_ON;
        ...
        IF(shift != 0)
        {
            ...
            FOR (i=1; i<L_subfr; i++)
            {
                ...
                ener_64 = W_mac0_16_16(ener_64, stmp, stmp);
            }
        }
        ELSE
        {
            ener_64 = W_mult0_16_16(exc[0+i_subfr], exc[0+i_subfr]);
            FOR (i=1; i<L_subfr; i++)
            {
                ener_64 = W_mac0_16_16(ener_64, exc[i+i_subfr], exc[i+i_subfr]); /* Q_new -> exponent = (15-Q_new)*2+1  */
            }       
        }
