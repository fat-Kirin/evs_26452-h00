...
Word32 ar_div(Word32 num, Word32 denum)
{
    ...
    test();
    IF (LT_32(num, denum)||denum==0)
    {
        return 0;
    }
    ELSE IF (EQ_32(num, denum))
    {
        return 1;
    }
    ELSE
...
static void ar_encode_fx( PARCODEC_FX arInst, Word16 const *model, Word32 symbol )
{
    ...
    FOR( ; ; )
    {
        IF ( LT_32(high, AR_HALF))
        {
            transmission_bits( arInst, 0 );
        }
        ELSE
        {
            IF (GE_32(low, AR_HALF))
            {
            ...
            {
                test();
                IF (GE_32(low, AR_FIRST)&&LT_32(high,AR_THIRD))
                {
...
static Word16 quantize_fx( Word16 val, Word16 D)
{
    ...
    IF (D == 0)
    {
        if (GT_16(abs_s(sub(shl(abs_s(retval_fx), 10), abs_s(val))), abs_s(val)))
        {
            retval_fx = 0;
            move16();
        }
                ...
                Word16 length, Word16 *vout_fx, Word16 pulses, Word16 *pulsesout, Word16* nzposout, Word32 *savedstates, Word32 * lasttrellislevel, Word32 terminate)
{
    ...
    FOR ( i = 0; i < length; i++) /* cycle over symbols */
    {
        FOR ( st = 0; st < STATES; st++) /* cycle over conditions */
        {
            ...
            test();
            if (GT_16(add(quantum1_fx , pused_fx[step_tcq_fx[st][0]][i]), pulses)&&terminate)
            {
                newdist1_fx = MAX_16;
                move16();
            }
            ...
            test();
            if (GT_16(add(quantum2_fx , pused_fx[step_tcq_fx[st][1]][i]), pulses)&&terminate)
            {
                newdist2_fx = MAX_16;
                move16();
            }
    ...
    FOR ( i = 1; i < STATES; i++)
    {
        ...
        test();
        test();
        IF ( (GT_32(dmin_fx, metric_fx[ i][ length])&&EQ_16(pused_fx[i][length],pulses))||
             (NE_16(pused_fx[dminpos][ length], pulses)  && EQ_16(pused_fx[i][ length], pulses)) )
        {
                       ...
                       Word16 *nzpout, Word16 *bcount, Word32 *abuffer, Word16 *mbuffer, Word32 *sbuffer)
{
    ...
    Word16 i, j, m_int, leftp, leftnz, nzpos, direction, pulsesnum, diff, pulsescurr, nzposcurr;
    Word16 sign;
    Word16 flag_g1;
    Word32 t32;
    Word16 SafeExp;
    ...
    IF ( pulsesnum > 0 )
    {
        ...
        WHILE (NE_16(t_fx, pulsesnum))
        {
            ...
            /* new alg */
            {
                ...
                FOR (i = 0; i < size; i++)
                {
                    dist_fx[i] = L_shr(dist_fx[i], s_min(sub(exp_dist[i], Q_temp), 31));
                    move32();
                }
            }

            {
                ...
                IF (magn_fx[pos] == 0 && direction < 0)
                {
                    ...
                    {
                        test();
                        IF (magn_fx[i] != 0 && LT_32(dmin_fx, dist_fx[i]))
                        {
                            pos = L_deposit_l(i);
                            dmin_fx = L_add(dist_fx[i], 0);
                        }
        ...
        FOR (i = 0;  i < size; i++)
        {
            IF (magn_fx[i] > 0)
            {
                ...
                actualt_fx =  L_add(actualt_fx, L_shr( aquants_fx[i], 2) );/*12 */
                nzpos++;
            }
        ...
        *nzpout = nzpos;
        ...
        test();
        test();
        IF ( (NE_16(nzpos, pulsesnum)&&GT_16(nzpos,1))&&noTCQ==0)
        {
                ...
                FOR ( i = 0; i < size; i++)
                {
                    ...
                    IF ( magn_fx[i] != 0 )
                    {
                        FOR ( j = 0; j < leftp; j++)
                        {
                            ...
                            test();
                            IF(NE_16(quantum1_fx, add(j, 1))&&NE_16(quantum2_fx,add(j,1)))
                            {
...
void TCQLSB_fx(Word16 bcount, Word32 *abuffer_fx, Word16 *mbuffer_fx, Word32 *sbuffer_fx, Word16 *dpath)
{
    ...
    metric_fx[1][0] = MAX_32>>8;
    move32();
    metric_fx[2][0] = MAX_32>>8;
    move32();
    metric_fx[3][0] = MAX_32>>8;
    move32();
    ...
    FOR( i = 0; i < 2*TCQ_AMP; i+=2 )
    {
        ...
        FOR ( st = 0; st < 4; st++)
        {
            ...
            tmp1 = L_sub(a1_fx, L_shl(Mult_32_16(L_add(L_shl(q1_fx, 15), sign1_fx), s1_fx16), sub(12+2, exp1)));/*12*/
            tmp2 = L_sub(a2_fx, L_shl(Mult_32_16(L_add(L_shl(q2_fx, 15), sign2_fx), s2_fx16), sub(12+2, exp2)));/*12*/
    ...
    FOR ( st = 1; st < 4; st++)
    {
        IF ( GT_32(dmin_fx, metric_fx[ st][ i/2]))
        {
...
Word32 encode_position_ari_fx(PARCODEC_FX parenc, Word16* quants, Word16 size, Word32* est_bits_frame_fx)
{
    ...
    *est_bits_frame_fx = L_add(*est_bits_frame_fx, bits_fx);
    ...
    ar_make_model_fx(prob, model_num_nz, s_min(pulses, size));
...
Word32 encode_magnitude_usq_fx(ARCODEC_FX* parenc, Word16* magn_fx, Word16 size, Word16 npulses, Word16 nzpos, Word32* est_frame_bits_fx)
{
    ...
    *est_frame_bits_fx = L_add(*est_frame_bits_fx, bits_fx);
    ...
    test();
    IF (EQ_16(npulses, nzpos)||EQ_16(nzpos,1))
    {
        return bits_fx;
    }
    magnp = sub(npulses, 1);
    ...
    FOR ( i = 0; i < size; i++)
    {
        ...
        IF (magn_fx[i] != 0)
        {
            FOR ( j = 0; j < leftp; j++)
            {
                ...
                test();
                IF (EQ_16(prob0_fx, MAX_16)||EQ_16(prob1_fx,MAX_16))
                {
                    CONTINUE;
                }
...
void decode_mangitude_tcq_fx(ARCODEC_FX* pardec, Word16 size, Word16 npulses, Word16 nzpos, Word16* positions, Word16* out, Word32* surplus_fx)
{
    ...
    test();
    IF (NE_16(nzpos, npulses)&&GT_16(nzpos,1))
    {
        /*update the surplus */
...
void decode_signs_fx(ARCODEC_FX* pardec, Word16 size, Word16* out)
{
    ...
    FOR ( i = 0; i < size; i++)
    {
        ...
        {
            Word16 tmp;
            tmp = ar_decode_fx( pardec, uniform_model_fx );
...
Word16 GetScale_fx( Word16 blen, Word32 bits_fx/*Q16*/, Word32 *surplus_fx/*Q16*/)
{
    ...
    p_est = extract_h(L_shl(Pow2( extract_l( L_shr(ab,15) ), ab&0x7FFF ),16));  /* enable saturationof pow2 result  */

    pulses = s_min( p_est, MAX_PULSES );

    FOR( ; pulses >= 0; pulses--)
    {
        estbits_fx = GetBitsFromPulses_fx( pulses, blen);
        IF( GE_32( bits_fx, estbits_fx))
        {
            BREAK;
        }
    }
