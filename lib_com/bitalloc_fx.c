...
)
{
    ...
    FOR ( i=0; i<n; i++ )
    {
        ...
        test();
        IF ( GE_16(sum,sfmsize[j])&&LT_16(r[j],K))
        {
            y[k] = sub(y[k], fac);
            move16();
            r[j] = add(r[j], 1);
    ...
    IF ( GE_16(sum, WID_G2))
    {
        FOR (i=0; i<=N; i++)
        {
            ...
            test();
            test();
            IF ( GE_16(j, SFM_G1)&&LT_16(j,SFM_thr)&&r[j]==0)
            {
                r[j] = 1;
                move16();
                sum = sub(sum, WID_G2);
    ...
    IF ( GE_16(sum, WID_G2))
    {
        FOR (i=0; i<=N; i++)
        {
            ...
            test();
            test();
            IF ( GE_16(j,SFM_G1)&&LT_16(j,SFM_thr)&&EQ_16(r[j],1))
            {
                r[j] = 2;
                move16();
                sum = sub(sum, WID_G2);
...
)
{
    ...
    factor_fx[1] = 24576;/*Q13     move16(); */
    {
        ...
        test();
        IF ( LT_32(R_diff_32_fx[0],393216)&&LT_32(R_diff_32_fx[1],245760))
        {
            IF(Rsum_fx == 0)
            {
                B1 = 0;
