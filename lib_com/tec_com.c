                                  ...
                                  Word16*  pGainTemp_e  /* Q0 */)
{
    ...
    FOR (slot=startPos; slot < stopPos; slot++)
    {
        ...
        IF ( tmp32 > 0 )
        {
            IF  ( GE_32(tmp32,1006632960l/*0.46875 Q31*/))
            {
                        ...
                        )
{
    ...
    test();
    test();
    IF ( (GT_16(lower_limit_gain_e, min_curr_enr_e))||(EQ_16(lower_limit_gain_e,min_curr_enr_e)&&GT_16(lower_limit_gain_m,min_curr_enr_m)))
    {
    ...
    move16();
    IF (EQ_16(code,  LOBUF_NO_SMOOTHING_MODE))
    {
    ...
    FOR (i=i_offset; i<N_TEC_TFA_SUBFR; i++)
    {
        test();
        test();
        IF ( (GT_16(lower_limit_gain_e, gain_e[i]))||(EQ_16(lower_limit_gain_e,gain_e[i])&&GT_16(lower_limit_gain_m,gain_m[i])))
        {
        ...
        test();
        test();
        IF ( (LT_16(upper_limit_gain_e, gain_e[i]))||(EQ_16(upper_limit_gain_e,gain_e[i])&&LT_16(upper_limit_gain_m,gain_m[i])))
        {
    ...
    *corrFlag = 0;
    ...
    IF (code)
    {
        ...
        FOR (i = 1; i < noCols; i++)
        {
            ...
            if ( LT_16(maxLoFix , loTempEnv_ns_Fix[i]))
            {
                maxPosLo = i;
                move16();
            }
        ...
        if (LT_16(abs_s( sub(maxPosHi , maxPosLo)), 2))
        {
            *corrFlag = 2;
            move16();
        }

        {
            ...
            FOR (i = 0; i < 16; i++)
            {
                ...
                {
                    if (  LT_16( max_local_Fix , curr_pos_Fix[-j]))
                    {
                        max_local_Fix = curr_pos_Fix[-j];
                        move16();
                    }

                    if ( GT_16(min_local_Fix, curr_pos_Fix[-j] ))
                    {
            ...
            {
                test();
                if ((LE_16(feature_max_Fix, shl(20, 7))||GE_16(abs_s(sub(pos_feature_max,maxPosHi)),3)))
                {
                    *corrFlag = 0;
                    move16();
                }
...
void tecEnc_TBE_fx(Word16* corrFlag, const Word16* voicing, Word16 coder_type)
{
    ...
    if( GT_16(voice_sum, 19661/*0.6 Q15*/)) /*if( voice_sum > 0.6 * 2 )*/
    {
        *corrFlag = 0;
        move16();
    }
...
void set_TEC_TFA_code_fx(const Word16 corrFlag, Word16* tec_flag, Word16* tfa_flag)
{
    *tec_flag = 0;
    ...
    {
        test();
        if (EQ_16(corrFlag, 1)||EQ_16(corrFlag,2))
        {
            *tec_flag = 1;
            move16();
        }
