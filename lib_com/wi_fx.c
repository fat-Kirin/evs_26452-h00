...

{
    ...
    test();
    IF (EQ_16(X_fx.upper_cut_off_freq_fx, 0x02800)||EQ_16(X_fx.upper_cut_off_freq_fx,4000))/*  0x2800=0.3125 in Q15 (4000Hz) */
    {
        num_erb_fx=NUM_ERB_NB;
        move16();
        erb_fx=&(erb_NB_fx[0]);
    ...
    ELSE IF (EQ_16(X_fx.upper_cut_off_freq_fx, 0x04000)||EQ_16(X_fx.upper_cut_off_freq_fx,6400))/*  0x4000=0.5 in Q15 (6400Hz) */
    {
        test();
    ...
    FOR (i=0 ; i<=n; i++)
    {
        ...
        FOR ( ; j<num_erb_fx; j++)
        {
            IF (LT_32(Ltemp_fx,L_shl(erb_fx[j+1],5)))
            {
...
void erb_slot_fx(
    ...
    Word16 *mfreq_fx,       /* o : ERB frequencies    */
...

{
    ...
    *POWER_IDX = 0;
    ...
    test();
    if (GT_16(tmp,0x3C29)&&GT_16(target[0],-819))
    {
...
)
{
    ...
    *out_of_bound=0;
    ...
    ELSE IF (GE_16(add(spike,range),l))
    {
        tmp = sub(spike,range);
        FOR(i=0; i<tmp; i++)
        {
