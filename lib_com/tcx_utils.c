...
)
{
    ...
    IF (EQ_16(left_overlap_mode, TRANSITION_OVERLAP))
    {
...
)
{
    ...
    test();
    test();
    IF ( EQ_16(last_is_cng, 1)&&left_rect==0)
    {
        IF (!fullbandScale)
        {
    ...
    ELSE IF ( EQ_16(left_rect, 1)&&last_core_bfi==ACELP_CORE)
    {
    ...
    ELSE
    {
        IF ( EQ_16(left_mode, 2))     /* min. overlap */
        {
        ...
        ELSE IF ( EQ_16(left_mode, 3))    /* half OL */
        {
            Word16 w;
...
)
{
    ...
    IF ( EQ_16(right_mode, 2))    /* min. overlap */
    {
    ...
    ELSE IF ( EQ_16(right_mode,3))   /* half OL */
    {
        Word16 w;
              ...
              Word16 *mdct_inv_gains, Word16 *mdct_inv_gains_exp)
{
    ...
    {
        ComplexData[2*i] = L_mult(lpcCoeffs[i], ptwiddle->v.re);
        move32();
        ComplexData[2*i+1] = L_negate(L_mult(lpcCoeffs[i], ptwiddle->v.im));
        move32();
        ptwiddle += step;
    }
    /* zero padding */
