...
)
{
    ...
    IF( GE_16(length,L_FRAME32k))
    {
        /* calculate the switching parameters */
        test();
        test();
        IF( ( NE_16(hqswb_clas,HQ_GEN_SWB)&&LE_32(st_fx->core_brate_fx,HQ_32k))||GT_32(st_fx->core_brate_fx,HQ_32k))
        {
            ...
            st_fx->prev_ener_shb_fx = mult(st_fx->prev_ener_shb_fx,5461); /*Q1*/
        }
        ELSE
        {
            st_fx->prev_ener_shb_fx = 0;
...
)
{
    ...
    har_freq_est2 = 0;
    move16();
    set16_fx( peak_idx, 0, HVQ_MAX_PEAKS_32k);
    ...
    test();
    test();
    IF( !*is_transient && NE_16(*hqswb_clas, HQ_HVQ)&&!(EQ_16(length,L_FRAME16k)&&EQ_32(st_fx->core_brate_fx,HQ_32k)))
    {
    ...
    dequantize_norms_fx( st_fx, start_norm, num_env_bands, *is_transient, ynrm, normqlg2 );
    ...
    test();
    test();
    IF( EQ_16(*hqswb_clas, HQ_GEN_SWB)&&st_fx->bws_cnt1_fx>0&&EQ_32(st_fx->core_brate_fx,HQ_24k40))
    {
    ...
    test();
    test();
    IF ( NE_16(*hqswb_clas, HQ_HARMONIC)||NE_16(*hqswb_clas,HQ_HVQ)||flag_dis==0)
    {
