...
)
{
    ...
    test();
    IF ( (EQ_16( st->nbLostCmpt, 1 ))||st->tcxConceal_recalc_exc)
    {
    ...
    ELSE
    {
        ...
        IF(GE_16(st->last_good_fx, UNVOICED_TRANSITION))
        {
    ...
    test();
    test();
    IF( GT_16(st->last_good_fx, UNVOICED_CLAS)&&!(EQ_16(st->last_good_fx,UNVOICED_TRANSITION)&&EQ_16(st->core_ext_mode,GENERIC)))
    {
        ...
        tmp16 = 0;
        move16();
        if (GT_32(st->output_Fs_fx , 25600))
        {
            tmp16 = 1;
            move16();
        }
        ...
        test();
        test();
        IF( ((EQ_16(st->nbLostCmpt,1))||st->tcxConceal_recalc_exc)&&GE_16(st->rf_frame_type,RF_TCXFD)&&LE_16(st->rf_frame_type,RF_TCXTD2)&&st->use_partial_copy)
        {
        ...
        test();
        IF( LT_16(st->stab_fac_fx ,32767/*1.f Q15*/)&&EQ_16(st->nbLostCmpt,1))
        {
            /* pitch cycle is first low-pass filtered */

            IF (LE_32(st->output_Fs_fx , 16000))
            {
                FOR( i=0 ; i< Tc; i++ )
                {
                    move16();
    ...
    test();
    IF (EQ_16(st->last_good_fx , VOICED_CLAS)||EQ_16(st->last_good_fx,ONSET))
    {
    ...
    ELSE
    {
        IF(GT_16( st->last_good_fx , UNVOICED_TRANSITION))
        {
    ...
    test();
    test();
    IF (EQ_16(st->last_good_fx , UNVOICED_CLAS)&&NE_16(st->core_ext_mode,UNVOICED))
    {
        gain_inov = mult_r(gain_inov,26214/*0.8f Q15*/);
    }
    ELSE IF (!( EQ_16(st->last_good_fx , UNVOICED_CLAS)||EQ_16(st->last_good_fx,UNVOICED_TRANSITION)))
    {
