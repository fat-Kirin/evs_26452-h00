...
)
{
    ...
    test();
    test();
    if( EQ_16(coder_type,INACTIVE)||(EQ_16(st_fx->bpf_off_fx,1)&&NE_16(coder_type,AUDIO)&&NE_16(coder_type,TRANSITION)))
    {
    ...
    test();
    test();
    if( (NE_16(coder_type,AUDIO)||st_fx->Last_GSC_noisy_speech_flag_fx!=0)&&st_fx->Last_GSC_pit_band_idx_fx>0)
    {
    ...
    test();
    if( GT_32(st_fx->total_brate_fx,ACELP_24k40)&&EQ_16(coder_type,INACTIVE))
    {
    ...
    test();
    test();
    IF( EQ_16(st_fx->Opt_AMR_WB_fx,1)&&EQ_16(coder_type,INACTIVE)&&NE_32(st_fx->core_brate_fx,SID_1k75)&&NE_32(st_fx->core_brate_fx,FRAME_NO_DATA))
    {
    ...
    IF ( NE_16(st_fx->last_core_fx,HQ_CORE))
    {
        st_fx->HqVoicing_fx = 1;
        move16();
        test();
        if( !st_fx->Opt_AMR_WB_fx && EQ_16(coder_type,UNVOICED))
        {
...
{
    Word16 xsp_tmp[M];
    IF( EQ_16(st_fx->last_core_fx,AMR_WB_CORE))      /* switching to EVS primary mode */
    {
    ...
    ELSE                                    /* switching to AMR-WB IO mode */
    {
        ...
        set16_fx( st_fx->past_qua_en_fx, -14*1024, GAIN_PRED_ORDER );

        /* HF synthesis memories */
        st_fx->Ng_ener_ST_fx = -51*256;  /*Q8*/     move16();
...
)
{
    ...
    IF( EQ_16(st_fx->core_fx, ACELP_CORE))
    {
        ...
        move16();
        test();
        if( EQ_16(st_fx->bwidth_fx, WB)&&st_fx->bws_cnt_fx==0)
        {
        ...
        move16();
        if(EQ_16(st_fx->prev_mode_fx, HARMONIC))
        {
    ...
    ELSE
    {
        ...
        test();
        test();
        IF( !(GE_16(st_fx->last_inner_frame_fx, L_FRAME16k)&&LE_16(inner_frame_tbl[st_fx->bwidth_fx],L_FRAME16k)&&st_fx->bws_cnt_fx>0&&LT_16(st_fx->bws_cnt_fx,N_WS2N_FRAMES)))
        {
        ...
        test();
        IF(GE_16(inner_frame_tbl[st_fx->bwidth_fx], L_FRAME32k)||LE_32(st_fx->core_brate_fx,HQ_16k40))
        {
            st_fx->prev_weight1_fx = 16384;
            move16();
            test();
            if(EQ_16(st_fx->prev_hqswb_clas_fx, HQ_HARMONIC)||EQ_16(st_fx->prev_hqswb_clas_fx,HQ_HVQ))
            {
                st_fx->prev_weight1_fx = 6554;
                move16();
            }
...
)
{
    ...
    move16();
    st_fx->prev_last_core_fx = st_fx->last_core_fx;

    st_fx->tcxConceal_recalc_exc = 0;
    move16();
    test();
    test();
    IF( (GE_16(st_fx->rf_frame_type,RF_TCXFD)&&LE_16(st_fx->rf_frame_type,RF_TCXTD2)&&st_fx->use_partial_copy&&st_fx->bfi_fx)||!st_fx->bfi_fx)
    {
        test();
        test();
        test();
        if( st_fx->bfi_fx && (LE_16(st_fx->last_good_fx, UNVOICED_TRANSITION))&&(GT_16(st_fx->clas_dec,UNVOICED_TRANSITION))&&st_fx->last_con_tcx)
        {
            st_fx->tcxConceal_recalc_exc = 1;
            move16();
        }
    ...
    if (EQ_16(st_fx->m_frame_type, ACTIVE_FRAME)&&(st_fx->bfi_fx!=1||st_fx->use_partial_copy!=0))
    {
    ...
    Copy_Scale_sig( synth + NS2SA_fx2(st_fx->output_Fs_fx,ACELP_LOOK_NS+DELAY_BWE_TOTAL_NS), st_fx->old_synth_sw_fx, NS2SA_fx2(st_fx->output_Fs_fx,FRAME_SIZE_NS-ACELP_LOOK_NS-DELAY_BWE_TOTAL_NS), st_fx->Q_old_postdec );

    test();
    test();
    test();
    IF( ((LE_32(st_fx->core_brate_fx,SID_2k40))&&EQ_16(st_fx->cng_type_fx,FD_CNG))
...
void update_decoder_LPD_cng( Decoder_State_fx *st, Word16 coder_type, Word16 *timeDomainBuffer, Word16 *A, Word16 *bpf_noise_buf)
{
    ...
    bufferCopyFx( synth+st->L_frame_fx-(st->L_frame_fx/2), st->old_syn_Overl, st->L_frame_fx/2,0 /*Qf_syn*/, -1 /*Qf_old_xnq*/, 0 , 0 /*Q_old_xnq*/);
    ...
    assert(st->L_frame_fx < L_EXC_MEM_DEC);
    ...
    FOR (i=0; i<M; i++)
    {
        ...
        move16();
        move16();
        st->lsf_adaptive_mean_fx[i] = add(    mult_r(st->lsfoldbfi1_fx[i], 10923/*1.0f/3.0f Q15*/),    add( mult_r(st->lsfoldbfi0_fx[i], 10923/*1.0f/3.0f Q15*/), mult_r(lsf[i], 10923/*1.0f/3.0f Q15*/)  ) );
        st->lsfoldbfi1_fx[i] = st->lsfoldbfi0_fx[i];
        st->lsfoldbfi0_fx[i] = lsf[i];
    }
