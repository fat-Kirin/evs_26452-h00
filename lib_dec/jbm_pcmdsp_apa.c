...
Word8 apa_set_rate( apa_state_t * ps, Word32 rate, Word16 num_channels )
{
    ...
    if(EQ_32(ps->rate, 16000))
        ps->win_incrementor = 2;
    move16();
    if(EQ_32(ps->rate, 8000))
    ...
    Word16     qualityrise)
{
    ...
    assert(LE_32(L_deposit_h(-2), qualityQ16) && LE_32(qualityQ16, L_deposit_h(3)) );
    assert(qualityred > 0 && LE_16( qualityred, 20 ));
    assert(qualityrise > 0 && LE_16(qualityrise, 20));
                ...
                Word16 *l_out /* o:   number of output samples */)
{
    ...
    *l_out = 0;
    ...
    /* get target length */
    IF(s_or((Word16)EQ_16(ps->l_frm, 480),(Word16)EQ_16(ps->l_frm,960)))
    {
    ...
    IF( LT_16( ps->l_buf_out, ps->l_frm ))
    {
    ...
    ELSE
    {
        ...
        /* no scaling */
        IF( EQ_16( ps->scale, 100 ))
        {
            copy_frm (ps, frm_in, a_out, &l_frm_out);
        }
        /* shrink */
        ELSE IF( LT_16( ps->scale, 100 ))
        {
        ...
        IF( NE_16( l_frm_out, ps->l_frm ))
        {
            test();
            IF( maxScaling != 0U &&
            GT_16( abs_s( sub( ps->l_frm, l_frm_out) ), maxScaling ))
            {
                /* maxScaling exceeded -> discard scaled frame */
                copy_frm (ps, frm_in, a_out, &l_frm_out);
            }
            ELSE IF( GT_32( L_abs( l_frm_out_target ), L_deposit_l(ps->l_frm) ))  /* ignore small difference */
            {
    ...
    IF ( GT_16( add( ps->l_buf_out, l_frm_out), APA_BUF ))
    {
    ...
    *l_out = l_frm_out;
    ...
    test();
    IF( LT_32(L_abs(ps->diffSinceSetScale),
              L_sub(0x7FFFFF, L_deposit_l(sub(l_frm_out, ps->l_frm))))  &&
        LT_16(ps->nFramesSinceSetScale, statsResetThreshold) )
    {
                                ...
                                Word32 * qualityQ16)
{
    ...
    FOR( i=0; i < ps->num_channels; i++ )
    {
        ...
        IF(pitch_cn > 0)
        {
            ...
            IF(LE_16(add(add(shr(i_mult2(pitch, 3), 1), offset), corr_len), s_len))
            {
            ...
            IF(LE_16(add(add(shr(pitch, 1), offset), corr_len), s_len))
            {
...
Word16 apa_getQualityIncreaseForLowEnergy(Word16 energydBQ8)
{
    ...
    /* increase calculated quality of signals with low energy */
    IF(LT_16(energydBQ8, qualIncreaseMaxEnergy))
    {
        qualIncForLowEnergy = energydBQ8;
        move16();
        if(LT_16(qualIncForLowEnergy, qualIncreaseMinEnergy))
        {
            qualIncForLowEnergy = qualIncreaseMinEnergy;
            move16();
        }
        if(GT_16(qualIncForLowEnergy, qualIncreaseMaxEnergy))
        {
                                ...
                                Word16 * synchpos)
{
    ...
    DO
    {
        ...
        {
            test();
            IF( EQ_16(wss,1)&&EQ_16(ps->num_channels,1))
            {
        ...
        if( LT_16(s_start,s_start_old))
        {
            s_start = s_start_old;
            move16();
        }

        IF( GT_16( add(s_start,inlen), add(s_start_old,s_len_old) ))
        {
                          ...
                          const Word16 frm_in[], Word16 maxScaling, Word16 frm_out[], Word16 * l_frm_out)
{
    ...
    if( GE_16( add( s_end, l_seg ), l_frm ))
    {
    ...
    IF( isSilence( frm_in, l_seg, 10 ) )
    {
        ...
        test();
        if( maxScaling != 0U && GT_16( s_end, add( maxScaling, 1 ) ))
        {
            xtract = maxScaling;
            move16();
        }
    ...
    IF(LT_32(qualityQ16, L_add(L_sub(ps->targetQualityQ16,
                               ...
                               L_mult0(ps->good_frame_count, 13107))))
    {
        ...
        if( LT_16( ps->bad_frame_count, ps->qualityred ))
        {
    ...
    ELSE
    {
        ...
        if( LT_16( ps->good_frame_count, ps->qualityrise ))
        {
                          ...
                          const Word16 frm_in[], Word16 frm_out[], Word16 * l_frm_out)
{
    ...
    move16();
    {
        ...
        IF( GE_16( sn_plus_search, sync_start_sub_pmin ))
        {
            /* shrink search region to enforce minimum shift */
            s_end = sync_start_sub_pmin;
            move16();

            IF( LT_16( sn_plus_search, sync_start ))
            {
                s_start = s[n]; /* just do it with normal start position */           move16();
            }
            ELSE IF( EQ_16( n, add(N,1) ))   /* move search region left for last segment */
            {
    ...
    {
        test();
        IF( over[n] != 0 && NE_16( add( xtract[sub(n,1)], l_seg ), xtract[n] ))
        {
