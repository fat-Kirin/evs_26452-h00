...
)
{
    ...
    ELSE
    {
        ...
        IF ( EQ_16(st->last_bwidth_fx,NB))
        {
            ...
            test();
            test();
            if ((GT_32(st->lp_noise, LP_NOISE_THRESH))||
        ...
        move16();
        test();
        if( GT_32(st->lp_noise,LP_NOISE_THRESH)&&st->narrowBand)
        {
            tmp8 = 1;
            move16();
        }
    ...
    FOR (i_subfr = 0; i_subfr < l_frame; i_subfr += l_subfr)
    {
        ...
        IF (disable_bpf == 0 && gainT_sf[sf] > 0)
        {
            ...
            IF (lg > 0)
            {
                ...
                {
                    Word64 tmp64 = W_deposit32_l( tmp );
...
)
{
    ...
    IF(EQ_16(st->VAD,1))
    {
        ...
        FOR (i = 0; i < (st->cldfbSyn_fx->no_channels - st->cldfbSyn_fx->bandsToZero); i++)
        {
            ...
            move16();
            test();
            if(GT_32(nrg_band[i], max_nrg)&&GE_16(i,11))
            {
                max_nrg = nrg_band[i];
                move16();
            }
