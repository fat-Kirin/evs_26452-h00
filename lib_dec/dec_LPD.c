...
)
{
    ...
    test();
    test();
    if(st->use_partial_copy && GE_16(st->rf_frame_type, RF_TCXFD)&&LE_16(st->rf_frame_type,RF_TCXTD2))
    {
        bfi = st->bfi_fx;
        move16();
    }
    ...
    IF( (bfi == 0 ) || ( bfi != 0 && st->use_partial_copy != 0 && EQ_16(st->rf_frame_type,RF_TCXFD)))
    {
        ...
        test();
        test();
        IF(EQ_16(st->use_partial_copy,1)&&(LT_16(st->rf_frame_type,RF_TCXFD)||GT_16(st->rf_frame_type,RF_TCXTD2)))
        {
            IF( EQ_16((Word16)st->envWeighted,1))
            {
            ...
            IF( EQ_16(st->core_fx,ACELP_CORE)&&EQ_16(st->last_core_fx,ACELP_CORE)
                && LT_32(lsp_diff, 52428 )&&GT_32(lsp_diff,3932)&&EQ_16(st->next_coder_type,GENERIC)
                && !st->prev_use_partial_copy && EQ_16(st->last_coder_type_fx,UNVOICED)  && GE_16(st->rf_frame_type,RF_GENPRED)  )
            {
                Copy( &lsp[0], &lsp[M], M );
            }
        ...
        ELSE
        IF ((st->enableTcxLpc !=0 && NE_16(st->core_fx , ACELP_CORE))||(bfi&&st->use_partial_copy&&st->rf_frame_type==RF_TCXFD))
        {
            Word16 tcx_lpc_cdk;
        ...
        ELSE
        {
            ...
            ELSE
            {
                ...
                st->sr_core, &st->mid_lsf_int_fx, st->prev_bfi_fx, &LSF_Q_prediction, &st->safety_net_fx );
                IF(EQ_16(st->prev_use_partial_copy,1)&&EQ_16(st->last_core_fx,ACELP_CORE)&&EQ_16(st->core_fx,ACELP_CORE)
                && GE_16(st->prev_rf_frame_type, RF_GENPRED)  && EQ_16(*coder_type,UNVOICED)  )
                {
    ...
    test();
    test();
    IF( (st->prev_bfi_fx!=0) && (bfi==0) && (EQ_16(*coder_type,VOICED))&&GT_16(st->prev_nbLostCmpt,4))
    {
        st->dec_glr_idx = 1;
        move16();
        st->reset_mem_AR = 1;
    ...
    IF ( st->core_fx == ACELP_CORE )
    {
        IF (st->tcxonly==0)
        {
            ...
            test();
            test();
            IF( (bfi == 0) && ((EQ_16(st->dec_glr_idx, 1))||((st->safety_net_fx==0)&&(shr(enr_new,11)>0)&&(GT_16(shr(enr_new,1),enr_old))))&&(st->prev_bfi_fx!=0))
            {
                Word16 reset_q = 0;
            ...
            ELSE
            {
                ...
                IF ( bfi==0 && st->acelp_cfg.midLpc )
                {
                    ...
                    IF (st->prev_bfi_fx)
                    {
                        ...
                        IF ( LT_16(enr_new, shr(enr_old, 2)))
                        {
                            ...
                            test();
                            test();
                            if ( EQ_16(st->clas_dec, UNVOICED_CLAS)||EQ_16(st->clas_dec,SIN_ONSET)||EQ_16(st->clas_dec,INACTIVE_CLAS)||EQ_16(*coder_type,GENERIC)||EQ_16(*coder_type,TRANSITION))
                            {
                                st->relax_prev_lsf_interp_fx = 1;
                                move16();
                            }
                    ...
                    test();
                    test();
                    if (st->stab_fac_fx == 0 && st->old_bfi_cnt_fx > 0 && NE_16(st->clas_dec, VOICED_CLAS)&&NE_16(st->clas_dec,ONSET)&&st->relax_prev_lsf_interp_fx==0)
                    {
        ...
        test();
        test();
        IF( st->narrowBand || (EQ_32(st->sr_core, 12800))||(EQ_32(st->sr_core,16000)))
        {
    ...
    test();
    IF (EQ_16(st->core_fx, TCX_10_CORE)||EQ_16(st->core_fx,TCX_20_CORE))
    {
        test();
        test();
        IF(st->enablePlcWaveadjust ||                       /* bfi      */
           (GE_32(st->last_total_brate_fx, HQ_48k)  &&  /* recovery */
            EQ_16(st->last_codec_mode, MODE2) ) )
        {
            ...
            test();
            test();
            IF ((bfi || st->prev_bfi_fx) && st->plcInfo.Pitch_fx && (EQ_16(st->plcInfo.concealment_method, TCX_NONTONAL)))
            {
    ...
    IF( LE_32( st->sr_core, 16000))
    {
        test();
        test();
        test();
        IF ( EQ_16(st->core_fx, TCX_20_CORE)||EQ_16(st->core_fx,TCX_10_CORE)||(st->tcxonly&&st->bfi_fx))
        {
    ...
    test();
    IF( bfi && NE_16(st->last_core_bfi , ACELP_CORE))
    {
    ...
    test();
    test();
    IF(!bfi && GE_16(st->clas_dec, VOICED_TRANSITION)&&LT_16(st->clas_dec,INACTIVE_CLAS))
    {
        Word16 offset;
    ...
    IF( st->tcxonly == 0 )
    {
        ...
        IF (EQ_16(st->bwidth_fx,NB)&&st->enableTcxLpc!=0&&NE_16(st->core_fx,ACELP_CORE))
        {
