...
)
{
    ...
    IF( NE_16(dim, 1))
    {
        ...
        test();
        IF( GE_32(entry.index, entry.size)||st_fx->ber_occured_in_pvq!=0)
        {
            st_fx->ber_occured_in_pvq = 1;
            move16();
            st_fx->BER_detect = 1;
            move16();
            entry.index = 0;
            move16(); /* a zero index will essentially disable PVQ index decompostion complexity */
        }
