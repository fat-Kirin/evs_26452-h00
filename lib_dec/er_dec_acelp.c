...
)
{
    ...
    IF( GE_16(st->last_good_fx , UNVOICED_TRANSITION ))
    {
        ...
        test();
        test();/*test();*/
        IF (LE_16(st->last_good_fx , UNVOICED_TRANSITION)
            && (EQ_16(coder_type   , GENERIC            ) )
            &&  GT_32(pc         , 6*2*32768/*6(15Q16)*/ )  /*&& (stab_fac <= 0.5f)*/
           )
        {
            gain = 0;
            move16();
    ...
    test();
    IF( (EQ_16(st->last_good_fx, UNVOICED_TRANSITION))&&(EQ_16(coder_type,GENERIC)))
    {
    ...
    test();
    if ((EQ_16(st->last_good_fx ,UNVOICED_CLAS))&&(NE_16(coder_type,UNVOICED))) /* Attenuate somewhat on unstable unvoiced */
    {
    ...
    *Qf_mem_syn = Qf_syn;
    ...
    IF(LT_16(st->last_good_fx,UNVOICED_TRANSITION))
    {
