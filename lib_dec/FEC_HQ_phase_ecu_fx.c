...
static void spec_ana_fx(const Word16*, Word16*, Word32*, Word16*, Word16*, const Word16, const Word16, Word16*);
...
)
{
    ...
    /* check burst error */
    IF (EQ_16(output_frame, L_FRAME48k)) /* output_frame = (sampling frequency)/50 */
    {
    ...
    *ph_dith = 0;
    ...
    IF (GE_16(output_frame, L_FRAME32k)) /*currently est_mus_content only calculated for SWB and FB */
    {
        roundEstMusContent = 0;
        move16();
        if (GE_16(est_mus_content, FEC_HQ_ECU_POINT5)) /* est_mus_content is in [0.0, 1.0]. */
        {
    ...
    IF (GT_16(burst_len, burst_phdith_thresh))
    {
...
#error The implementation of phase_dith=min(1.0, (burst_len - burst_phdith_thresh)/burst_phdith_rampup_len)) is incorrect
        ...
        *ph_dith = 32767;  /* 1.0 in Q15. N.B. 2*PI is not included. */       move16();
        ...
        if (LT_16(tmp16, burst_phdith_rampup_len))
        {
    ...
    attDegreeFrames = 0;
    move16();
    IF (GT_16(burst_len, burst_att_thresh))
    {
        att_always = 1;
        move16();
         ...
         */
        if (GT_16(attDegreeFrames, OFF_FRAMES_LIMIT))
        {
    ...
    test();
    test();
    IF (LE_16(burst_len, 1)||(EQ_16(burst_len,2)&&last_fec!=0))
    {
...
#error trans_ana_fx-- The following implementation is incorrect
        ...
        ELSE
        {
            ...
            IF (EQ_16(att_per_frame, ATT_PER_FRAME)) /* Select the corresponding lookup-table. */
            {
    ...
    *pX = *pX0;
    ...
    IF (GT_16(len, 2))
    {
        ...
        IF (GE_16(xAt0, xAt1))
        {
            ii = -1;
            move16();
            IF (GE_16(xAt1, xAt2)) /* x[1] is not extremum -> overwrite with x[0] */
            {
        ...
        *cInd = 0;
        ...
        FOR (;;)
        {
            ...
            IF (GT_16(*(++pX), tempMag))
            {
                IF ( GT_16(*pX, threshold))  /* threshold = leftMin + sel */
                {
                    tempLoc = ii;
                    move16();
                    tempMag = *pX;
        ...
        /* Check end point */
        IF (GT_16(x[lenMinus1], tempMag))
        {
            IF (GT_16(x[lenMinus1], threshold)) /* threshold = leftMin + sel */
            {
    ...
    ELSE /* This is a monotone function where an endpoint is the only peak */
    {
        ...
        peakMag[0] = x[xInd];
        move16();
        IF (GT_16(peakMag[0], add(minMag, sel)))
        {
            plocs[0] = *(indarr + xInd);
            move16();
            *cInd = 1;
...
)
{
    ...
    *Q = s_max(0, sub(Exp16Array(Lprot, prevsynth), 1));
    ...
    IF (GT_16(output_frame, inner_frame_tbl_fx[bwidth_fx]))
    {
        stop_band_start = shl(128, bwidth_fx);
        stop_band_length = sub(Lprot, shl(stop_band_start,1));
        stop_band_start = add(stop_band_start, 1);
        set16_fx( xfp + stop_band_start, 0, stop_band_length );
    }
    ...
    IF ( n >= 0 )
    {
        ...
        pXfp = pXfp1 + *pPlocs;
        IF (EQ_16(*pPlocs, Lprot2)) /* Only the very last peak is possible the peak at Lprot2 index position. */
        {
...
)
{
    ...
    {
        test();
        IF(GT_16(Tf_abs[cpt],old)&&GT_16(Tf_abs[cpt],new_s))
        {
            Word16 tmp2;
    ...
    ELSE
    {
        DO
        {
            ...
            *pt3++ = indmax;
            ...
            maxi=sub(maxi,1);
        } WHILE( maxi>0 && LT_32(cumsum, L_tmp));
...
)
{
    ...
    test();
    IF (EQ_16(output_frame,L_FRAME32k)||EQ_16(output_frame,L_FRAME16k))
    {
        n=R1_16-R2_16;
        move16();
...
)
{
    ...
    test();
    test();
    IF ( (GE_32(st_fx->total_brate_fx,48000)&&
          ( GE_16(output_frame, L_FRAME16k)  && !prev_bfi && (!old_is_transient[0] || old_is_transient[1] ) && (NE_16(ph_ecu_HqVoicing,0) || ( ((NE_16(st_fx->env_stab_plc_fx ,0)) && (LT_16(corr,19661))) || (!(NE_16(st_fx->env_stab_plc_fx ,0)) && (GT_16(corr, 27853))  ))))) ||
         (LT_32(st_fx->total_brate_fx,48000)   && ( ( ph_ecu_HqVoicing || GT_16(corr, 27853)) && !prev_bfi && (!old_is_transient[0] || old_is_transient[1]) )) )
    {
    ...
    ELSE
    {
        ...
        mag_chg_1st, Xavg, beta_mute, st_fx->bwidth_fx, output_frame );
    }
