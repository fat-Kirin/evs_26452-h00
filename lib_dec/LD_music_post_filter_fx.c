...
static void spectrum_mod_dct_fx(const Word16 Qdct,Word16 data[], const Word32 lf_E[], Word32 lf_EO[],
                                const Word32 noiseE[], const Word16 minGain,
...
)
{
    ...
    *Old_ener_Q = Qdct;
     ...
     *------------------------------------------------------------------------*/
    music_flag2 = stab_est_fx( LG_etot, lt_diff_etot, mem_etot
    ...
    *last_nonfull_music = s_min( 51, *last_nonfull_music );
    ...
    IF( GT_16(music_flag2,3))
    {
        min_band = 2;
        move16();
        local_min_gain = 8231;        /*Q15->0.25119f;*/     move16();
    }
    ELSE IF( EQ_16(music_flag2,3))
    {
        min_band = 3;
        move16();
        local_min_gain = 8231;        /*Q15->0.25119f; */    move16();
    }
    ELSE IF( EQ_16(music_flag2,2))
    {
        min_band = 4;
        move16();
        local_min_gain = 11626;       /*Q15->0.35481f; */     move16();
    }
    ELSE IF( EQ_16(music_flag2,1))
    {
        min_band = 4;
        move16();
    ...
    FOR(i = 1; i < DCT_L_POST-1; i++)
    {
        ...
        Ltmp = L_mac(L_mult(10813, old_tmp16), 10813, old_tmp16_1);

        old_tmp16 = old_tmp16_1;
        move16();
    ...
    spectrum_mod_dct_fx( Qdct, dtc_out, lf_E, lf_EO, bckr, local_min_gain, lp_gbin, music_flag2, min_band, MAX_GN, MAX_band );

    i = 0;
    move16();
    IF( GE_16(music_flag2,1))
    {
    ...
    {
        IF( GT_16(*last_nonfull_music,40))
        {
            ...
            ELSE IF( GE_32(core_brate,ACELP_12k65))
            {
                max_ovf_2k = 4096;      /*1.0 Q12*/ move16();
                max_ovf_4k = 5120;     /*1.25 Q12*/ move16();

                IF( GT_32(core_brate,ACELP_15k85))
                {
                    max_ovf_4k = 4096;  /*1.0 Q12*/ move16();
                    max_ovf_6k = 5120; /*1.25 Q12*/ move16();
        ...
        ELSE IF( GT_16(*last_nonfull_music,25))
        {
    ...
    IF ( music_flag != 0 ) /* prevent subtraction on clean speech */
    {
        ...
        FOR (; i < MAX_band; i++)
        {
            ...
            FOR (; freq <= mfreq_loc_LD_fx[i]; freq += BIN_16kdct_fx)
            {
                ...
                /*if (noiseE[i] >= 0.5f)*/
                IF (GT_32(noiseE[i], dot5_scaled))/* Do not alter if noise E very low */
                {
