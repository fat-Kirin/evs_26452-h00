...
)
{
     ...
     *-----------------------------------------------------------------*/
    IF( EQ_16(codec_type, MODE1))
    {
        test();
        test();
        test();
        IF ( ( Opt_AMR_WB || EQ_16(coder_type,GENERIC)||EQ_16(coder_type,TRANSITION))&&LT_32(core_brate,ACELP_11k60))
        {
            lp_flag = LOW_PASS;
            move16();
        }
        ELSE IF ( GE_32(core_brate,ACELP_11k60))
        {
