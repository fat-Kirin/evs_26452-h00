...
)
{
    ...
    test();
    test();
    IF ( ((EQ_16(st_fx->L_frame_fx, L_FRAME16k))&&(LE_16(lsf_q[M-1],WB_LIMIT_LSF_FX)))||((LT_16(st_fx->L_frame_fx,L_FRAME16k))&&(GT_16(lsf_q[M-1],WB_LIMIT_LSF_FX))))
    {
        st_fx->BER_detect = 1;
        move16();
    }
...
)
{
    ...
    ELSE
    {
        ...
        ELSE IF ( EQ_16(st_fx->nelp_mode_dec_fx,1))
        {
    ...
    test();
    test();
    IF ( ( st_fx->prev_bfi_fx && (EQ_16(coder_type,TRANSITION))&&(EQ_16(tc_subfr,sub(L_frame,L_SUBFR)))))
    {
        lsf_diff = 1205;
...
)
{
    ...
    test();
    test();
    IF((EQ_16(coder_type_org, GENERIC))&&(EQ_32(int_fs,INT_FS_16k))&&(mode2_flag==0))
    {
...
)
{
    ...
    test();
    test();
    IF ( prev_bfi || ( EQ_16(*mid_lsf_int, 1)&&bad_spacing))
    {
        FOR (j=0; j<M; j++)
        {
