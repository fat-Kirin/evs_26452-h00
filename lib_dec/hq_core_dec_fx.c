...
)
{
    ...
    move16();
    if (EQ_16(st_fx->tcx_cfg.tcx_curr_overlap_mode, FULL_OVERLAP))
    {
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        test();
        IF ( !(core_switching_flag) && GT_32(st_fx->core_brate_fx, MINIMUM_RATE_TO_ENCODE_VOICING_FLAG))
        {
        ...
        ELSE
        {
            ...
            move16();
            if ( GT_32(st_fx->core_brate_fx, MINIMUM_RATE_TO_ENCODE_VOICING_FLAG))
            {
    ...
    IF( EQ_16(st_fx->bfi_fx, 1))
    {
        ...
        IF ( GE_16(output_frame, L_FRAME16k)) /* Apply phase ecu for WB, SWB and FB */
        {
            ...
            hq_ecu_fx( st_fx->prev_good_synth_fx, t_audio_q, &st_fx->time_offs_fx, st_fx->X_sav_fx, &st_fx->Q_X_sav, &st_fx->num_p_fx, st_fx->plocs_fx, st_fx->plocsi_fx, st_fx->env_stab_fx,
                       &st_fx->last_fec_fx, st_fx->ph_ecu_HqVoicing_fx, &st_fx->ph_ecu_active_fx, gapsynth_fx, st_fx->prev_bfi_fx, st_fx->old_is_transient_fx, st_fx->mag_chg_1st_fx,
    ...
    {
        IF( EQ_16(st_fx->bws_cnt1_fx,N_NS2W_FRAMES))
        {
            ener_match = 32767;
            move16(); /*Q15*/
        }
    ...
    ELSE
    {
        Copy_Scale_sig_32_16(t_audio_q, st_fx->t_audio_q_fx, L_FRAME, -13);
    }
    ...
    test();
    IF (EQ_16(output_frame, L_FRAME8k)||st_fx->bfi_fx==0)
    {
        test();
        IF( NE_16(inner_frame, output_frame)&&EQ_16(st_fx->bfi_fx,1))
        {
            Inverse_Transform( t_audio_q, &Q_audio, wtda_audio, is_transient, output_frame, output_frame );
        }
        ELSE
        {
    ...
    IF ( EQ_16(output_frame, L_FRAME8k))
    {
        ...
        test();
        test();
        IF( (EQ_16(st_fx->prev_bfi_fx, 1)||EQ_16(st_fx->bfi_fx,1))&&st_fx->old_is_transient_fx[2]==0&&EQ_16(st_fx->last_core_fx,HQ_CORE)&&EQ_16(st_fx->last_codec_mode,MODE1))
        {
        ...
        test();
        test();
        IF ( (st_fx->bfi_fx == 0 && st_fx->prev_bfi_fx == 0) || !(GE_16(output_frame, L_FRAME16k)))
        {
            preecho_sb_fx( st_fx->core_brate_fx, wtda_audio, Q_audio, synth, *Q_synth, output_frame, &st_fx->memfilt_lb_fx,
                           &st_fx->mean_prev_hb_fx, &st_fx->smoothmem_fx, &st_fx->mean_prev_fx, &st_fx->mean_prev_nc_fx, &st_fx->wmold_hb_fx, &st_fx->prevflag_fx, &st_fx->pastpre_fx, st_fx->bwidth_fx );
        }
    }
    ELSE
    {
        test();
        IF (EQ_16(st_fx->bfi_fx, 1)&&GE_16(output_frame,L_FRAME16k))
        {
            /* PHASE_ECU active */
            Q_audio = 15;
            move16();
    ...
    IF (GE_16(output_frame, L_FRAME16k))
    {
        IF (EQ_16(st_fx->ph_ecu_HqVoicing_fx, 1))
        {
            st_fx->oldHqVoicing_fx = 1;
            move16();
            Copy( gapsynth_fx, st_fx->oldgapsynth_fx, L_FRAME48k );
        }
        ELSE
        {
    ...
    if( EQ_16(st_fx->nbLostCmpt, FRAMECTTOSTART_MDCT))
    {
        st_fx->HqVoicing_fx = 0;
        move16();
    }
    ...
    IF( EQ_16(output_frame, L_FRAME8k))
    {
        Copy32( wtda_audio, st_fx->oldIMDCTout_fx, L_FRAME8k/2 );
...

    {
        ...
        test();
        IF( EQ_16(st_fx->last_core_fx,HQ_CORE)&&NE_16(st_fx->L_frame_fx,st_fx->last_L_frame_fx))
        {
