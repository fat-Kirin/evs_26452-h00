...
)
{
    ...
    test();
    test();
    if (st->last_con_tcx && (NE_16(st->L_frameTCX_past, st->L_frame_fx))&&(st->last_core_fx!=0))
    {
    ...
    test();
    test();
    IF ( st->prev_bfi_fx && st->last_con_tcx && LT_16(st->L_frame_fx, L_FRAME16k))
    {
    ...
    IF( st->last_con_tcx && st->old_enr_LP )
    {
        ...
        IF(LT_16(st->old_enr_LP, enr_LP))
        {
            ratio = div_s(st->old_enr_LP,enr_LP); /* Q15 */
            IF (LT_16(ratio, 26215))
            {
    ...
    *(exc+st->L_frame_fx) = 0;
     ...
     *------------------------------------------------------------------------*/
    test();
    if(st->prev_bfi_fx && EQ_16(coder_type,VOICED))
    {
    ...
    FOR (i_subfr = 0; i_subfr < st->L_frame_fx; i_subfr += L_SUBFR)
    {
        ...
        test();
        IF( EQ_16(st->use_partial_copy,1)&&EQ_16(st->rf_frame_type,RF_NELP))
        {
            IF( i_subfr == 0 )
            {
        ...
        ELSE
        {
            ...
            test();
            IF( EQ_16(st->use_partial_copy,1)&&st->acelp_cfg.gains_mode[i_subfr/L_SUBFR]==0)
            {
            ...
            IF ( acelp_cfg.ltp_bits != 0 )
            {
                ...
                test();
                IF( EQ_16(st->pit_res_max,6)&&!(st->use_partial_copy))
                {
                    IF ( EQ_16(T0_res, shr(st->pit_res_max, 1)))
                    {
            ...
            ( EQ_16(st->rf_frame_type,RF_ALLPRED)  ||
            ( EQ_16(st->rf_frame_type,RF_GENPRED)  &&
            ( EQ_16(i_subfr,L_SUBFR)  || EQ_16(i_subfr,3*L_SUBFR)  )) ) )
            {
             ...
             *-------------------------------------------------------*/
            test();
            IF( EQ_16(acelp_cfg.gains_mode[i_subfr/L_SUBFR],7)&&!st->use_partial_copy)
            {
            ...
            IF ( EQ_16(st->core_fx,ACELP_CORE)&&EQ_16(st->last_core_fx,ACELP_CORE)&&(EQ_16(st->use_partial_copy,1)||EQ_16(st->prev_use_partial_copy,1)))
            {
                IF( i_subfr > 0 && GT_16(gain_pit,20152)&&GT_16(st->prev_tilt_code_dec_fx,6553)&&EQ_16(st->next_coder_type,VOICED)
                &&( EQ_16(st->use_partial_copy,1)  || EQ_16(st->prev_use_partial_copy, 1)  ) )
                {
                    gain_pit = mult(gain_pit,sub(26214, mult(i_subfr,51)));
                }
                ELSE IF( !st->prev_use_partial_copy && EQ_16(st->last_coder_type_fx,UNVOICED)&&NE_16(st->next_coder_type,UNVOICED)&&LT_32(gain_code,gain_code_pre))
         ...
         *----------------------------------------------------------*/
        test();
        if( st->enableGplc && EQ_16( shr(i_subfr,6), sub(st->nb_subfr,1) ))
        {
            st->T0_4th = T0;
            move16();
        }
...

    {
        ...
        test();
        test();
        IF ( (EQ_16(st->clas_dec,ONSET))||((GE_16(st->last_good_fx,VOICED_TRANSITION))&&(LT_16(st->last_good_fx,INACTIVE_CLAS))))
        {
            force_scale_syn = 1;
            move16();
        }
