                    ...
                    )
{
    ...
    if (EQ_16(st_fx->last_nelp_mode_dec_fx,1)&&NE_16(st_fx->bwidth_fx,st_fx->last_bwidth_fx))
    {
    ...
    test();
    test();
    IF ( EQ_16(coder_type,UNVOICED)&&EQ_16(st_fx->bwidth_fx,NB))
    {
        IF (NE_16(st_fx->last_nelp_mode_dec_fx,1))
        {
            BP1_ORDER = 7;
            move16();
    ...
    ELSE IF ( EQ_16(coder_type,UNVOICED)&&(EQ_16(st_fx->bwidth_fx,WB)||EQ_16(st_fx->bwidth_fx,SWB)))
    {
        BP1_ORDER =4;
        move16();
        IF (NE_16(st_fx->last_nelp_mode_dec_fx,1))
    ...
    IF (bfi == 0)
    {
        ...
        test();
        test();
        IF ( EQ_16(coder_type,UNVOICED)&&(EQ_16(st_fx->bwidth_fx,WB)||EQ_16(st_fx->bwidth_fx,SWB)))
        {
            test();
            IF(EQ_16(st_fx->rf_frame_type,RF_NELP)&&EQ_16(st_fx->use_partial_copy,1))
            {
    ...
    IF (EQ_16(st_fx->bwidth_fx,WB)||EQ_16(st_fx->bwidth_fx,SWB))
    {
        ...
        /* This is done to avoid internal overflow observed in the wb bp filter below, similar to encoder */
        normalize_arr(Gains, &qGain, 10, 4);
    }

    generate_nelp_excitation_fx(&(st_fx->nelp_dec_seed_fx), Gains, ptr, gain_fac);
    /* o: ptr = nelp_exc in Q=qGain, qGain = 0 always for NB */
    ...
    IF ( EQ_16(coder_type,UNVOICED )&&(EQ_16(st_fx->bwidth_fx,WB)||EQ_16(st_fx->bwidth_fx,SWB)))
    {
        BP1_ORDER =4;
        move16();
        Scale_sig(st_fx->bp1_filt_mem_wb_dec_fx, 2*BP1_ORDER, qGain); /* bring filter prev memory from Q0 to qGain */
        ...
        Scale_sig(st_fx->bp1_filt_mem_wb_dec_fx, 2*BP1_ORDER, -qGain); /* bring filter prev memory from qGain to Q0 */
