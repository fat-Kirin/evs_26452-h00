                                            ...
                                            )
{
    ...
    IF (NE_16(self->nSamples,nSamples))
    {
        self->secondLastBlockData.blockIsValid = 0;
        move16();
        self->lastBlockData.blockIsValid = 0;
                                                      ...
                                                      )
{
    ...
    IF (!self->lastBlockData.tonalConcealmentActive || NE_16(self->lastBlockData.nSamples,nNewSamples))
    {
        IF (LE_16(nNewSamples,L_FRAME_MAX))
        {
            /* Shift the buffers */
                                                   ...
                                                   )
{
    ...
    ELSE
    {
        ...
        move16();
        test();
        if((LE_16(nNewSamples,2*L_FRAME_MAX))&&(nNewSamples>0))
        {
            newBlockIsValid = 1;
            move16();
        }
        ...
        Word32 const pitchLag)
{
    ...
    IF ( GT_16(self->nSamplesCore, self->nSamples))
    {
        ...
        Word16 * const numIndices)
{
    ...
    IF (self->lastBlockData.blockIsValid && self->secondLastBlockData.blockIsValid
        && (EQ_16(self->lastBlockData.nSamples,nSamples) ) && (EQ_16(self->secondLastBlockData.nSamples,nSamples) )
...
{
    IF (EQ_16(nNewSamples,self->nSamples))
    {
