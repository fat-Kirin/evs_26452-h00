                              ...
                              JB4_INPUTBUFFER_ELEMENT *replacedElement )
{
    ...
    *replacedElement = NULL;
    ...
    IF(GE_16(size, sub(h->capacity, 1)))
    {
        return -1;
    }
    ...
    IF( size == 0 )
    {
        ...
        if( EQ_16( h->writePos, h->capacity ))
        {
            h->writePos = 0;
            move16();
        }
...
Word16 JB4_INPUTBUFFER_IsFull( const JB4_INPUTBUFFER_HANDLE h )
{
    ...
    ret = 0;
    move16();
    IF( EQ_16( JB4_INPUTBUFFER_Size( h ), sub( h->capacity, 1 )))
    {
        ret = 1;
        move16();
    }
