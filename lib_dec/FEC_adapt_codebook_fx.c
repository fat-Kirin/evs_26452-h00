...
)
{
    ...
    test();
    test();
    IF ( GT_16(P0,PIT_MAX)&&EQ_16(L_frame,L_FRAME))
    {
        P0 = PIT_MAX;
...
)
{
    ...
    IF(GT_16(Terr,abs_s(add(sub(Tlist[1],Tc), P0))))
    {
    ...
    test();
    test();
    IF (LE_16(Terr, i_mult(tmp2, diff_pit))&&
    ...
    if( NE_16(last_L_frame, L_FRAME16k))
    {
        Do_WI = 0;
        move16();
    }
...
)
{
    ...
    IF( GE_32(L_min_energy, MAX_32))
    {
        return 0;
    }

    IF(GT_16(nb_min,16))  /* inv_sqi & sqi are built for a maximum of nb_min-2 = 14 values*/
    {
        return 0;
    }
    /*--------------------------------------------------------------------
     ...
     * --------------------------------------------------------------------*/
    test();
    IF (EQ_16(nb_min, 1)||EQ_16(abs_s(point_to_remove),1))
    {
        nb_min = 1;
        move16();
    ...
    ELSE
    {
        ...
        FOR (i = 2; i <= nb_min; i++)
        {
            ...
            IF (LT_16(points_by_pos[i-1], points_by_pos[i-2]))
            {
    ...
    IF (point_to_add > 0)/* Samples insertion */
    {
        ...
        /* Copy remaining data */
        remaining_len = s_max(0, remaining_len);
