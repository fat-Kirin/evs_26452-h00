...
)
{
    ...
    ELSE IF(EQ_16(coder_type,1)) /* 8/4/4/4 (EVS) */
    {
        IF (i_subfr == 0)
        {
    ...
    ELSE IF(EQ_16(coder_type,2)) /* 8/5/8/5 (EVS) */
    {
        test();
        IF ( ( i_subfr == 0 ) || ( EQ_16(i_subfr,shl(L_subfr,1))))
        {
            Mode2_abs_pit_dec( T0, T0_frac, T0_res, pt_indice, pit_min, pit_fr1b, pit_min, pit_res_max);
        }
        ELSE
    ...
    ELSE IF(EQ_16(coder_type,8)) /* 8/5/5/5 (RF all pred mode) */
    {
        IF (i_subfr == 0)
        {
            Mode2_abs_pit_dec( T0, T0_frac, T0_res, pt_indice, pit_min, pit_fr1b, pit_min, pit_res_max);
    ...
    ELSE IF(EQ_16(coder_type,9)) /* 8/0/8/0 (RF gen pred mode) */
    {
        IF (i_subfr == 0)
        {
            Mode2_abs_pit_dec( T0, T0_frac, T0_res, pt_indice, pit_min, pit_fr1b, pit_min, pit_res_max);
...
)
{
    ...
    ELSE IF( EQ_16(nBits, 8))     /* absolute decoding with 8 bits */
    {
    ...
    ELSE IF( EQ_16(nBits, 6))     /* relative decoding with 6 bits */
    {
        delta_pit_dec_fx( 4, pitch_index, T0, T0_frac, *T0_min );
    }
    ELSE IF( EQ_16(nBits, 5))     /* relative decoding with 5 bits */
    {
        IF( EQ_16(delta,8))
        {
...
)
{
    ...
    {
        {
            IF( LT_16(pitch_index,shl((PIT16k_FR2_EXTEND_10b-PIT16k_MIN_EXTEND),2)))
            {
    ...
    {
        {
            IF (LT_16(pitch_index,(PIT16k_FR2_EXTEND_9b-PIT16k_MIN_EXTEND)*4))
            {
                *T0 = add(PIT16k_MIN_EXTEND, shr(pitch_index,2));
                move16();
                *T0_frac = sub(pitch_index, shl(sub(*T0, PIT16k_MIN_EXTEND),2));
            ...
            ELSE IF (LT_16(pitch_index,( (PIT16k_FR2_EXTEND_9b-PIT16k_MIN_EXTEND)*4 + (PIT16k_FR1_EXTEND_9b-PIT16k_FR2_EXTEND_9b)*2)))
            {
...
)
{
    ...
    IF( limit_flag == 0 )
    {
        IF(EQ_16(fr_steps,2))
        {
            IF(LT_16(pitch_index,PIT_FR1_8b_MINUS_PIT_MIN_X2))
            {
        ...
        ELSE IF( EQ_16(fr_steps,4))
        {
            ...
            ELSE IF (LT_16(pitch_index,PIT_DECODE_1))  /*( (PIT_FR2_9b-PIT_MIN)*4 + (PIT_FR1_9b-PIT_FR2_9b)*2) = 440*/
            {
    ...
    ELSE IF( EQ_16(limit_flag, 1))   /* extended Q range */
    {
        IF( EQ_16(fr_steps,2))
        {
        ...
        ELSE IF( EQ_16(fr_steps,4))
        {
            IF( LT_16(pitch_index, PIT_FR2_EXT9b_MINUS_PIT_MIN_EXT_X4))
            {
            ...
            ELSE IF( LT_16(pitch_index,add(PIT_FR2_EXT9b_MINUS_PIT_MIN_EXT_X4, PIT_FR1_EXT9b_MINUS_PIT_FR2_EXT9b_X2)))
            {
    ...
    ELSE  /* limit_flag == 2 */
    {
        IF( EQ_16(fr_steps,2))
        {
            IF( LT_16(pitch_index,PIT_FR1_DEXT8b_MINUS_PIT_MIN_DEXT_X2))
            {
        ...
        ELSE IF( EQ_16(fr_steps,4))
        {
            ...
            ELSE IF( LT_16(pitch_index,PIT_DECODE_9))
            {
