...
)
{
    ...
    test();
    test();
    IF (EQ_16(bfi_cnt , 1)&&GE_16(last_good,UNVOICED_TRANSITION)&&LT_16(last_good,ONSET))
    {
        move16();
        no_subfr_pred = 4;
        ...
        test();
        test();
        IF (LT_32(L_abs(dy[imax]), Mpy_32_16_1(*old_fpitch, 4915/*0.15f Q15*/))&&((EQ_16(limitation,1))||(LT_32(L_abs(dy[imax]),L_abs(mdy)))))
        {
