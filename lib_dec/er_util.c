                       ...
                       Word16        currentFrameLevel_e)   /* exponent of currentFrameLevel */
{
    ...
    IF (LT_16(shl(currentFrameLevel, currentFrameLevel_e),PLC_MIN_CNG_LEV))
    {
...

{
    ...
    IF( GT_16(last_good , UNVOICED_TRANSITION))
    {
    ...
    ELSE
    {
        ...
        IF (EQ_16(st->nbLostCmpt,1))
        {
            ...
            IF ( !(st->rf_flag && st->use_partial_copy && (EQ_16(st->rf_frame_type, RF_TCXTD1)||EQ_16(st->rf_frame_type,RF_TCXTD2))))
            {
                ...
                test();
                test();
                IF ((EQ_16(st->last_core_fx,TCX_20_CORE))&&(EQ_16(st->second_last_core,TCX_20_CORE))
                && ((LE_32(st->old_fpitch,L_deposit_h(shr(st->L_frame_fx,1))))
                || (LE_16(st->tcxltp_last_gain_unmodified,13107/*0.4f Q15*/)))
                ...
                && (EQ_32(st->old_fpitch, st->tcxltp_second_last_pitch))
                && !st->last_tns_active && !st->second_last_tns_active)
                {
                    ...
                    IF ((GT_16(numIndices,10))
                        || ((GT_16(numIndices,5) )
                            && (LT_32(L_abs(L_sub(st->tcxltp_third_last_pitch,st->tcxltp_second_last_pitch)),32768l/*0.5f Q16*/) ))
                        || ((numIndices > 0) && ((LE_16(st->last_good_fx,UNVOICED_TRANSITION))||(LE_16(st->tcxltp_last_gain_unmodified,13107/*0.4f Q15*/)))
                            && (LT_32(L_abs(L_sub(st->tcxltp_third_last_pitch,st->tcxltp_second_last_pitch)),32768l/*0.5f Q16*/) )))
                    {
                        core = TCX_20_CORE;
                        move16();
                    ...
                    ELSE IF (LE_16(st->last_good_fx,UNVOICED_TRANSITION)||LE_16(st->tcxltp_last_gain_unmodified,13107/*0.4f Q15*/))
                    {
                        core = TCX_20_CORE;
                        move16();
                    }
                ...
                {
                    test();
                    if (LE_16(st->last_good_fx,UNVOICED_TRANSITION)||LE_16(st->tcxltp_last_gain_unmodified,13107/*0.4f Q15*/))
                    {
