    ...
    IF( core_switching_flag && EQ_16(st_fx->last_L_frame_fx, st_fx->last_L_frame_ori_fx)&&((EQ_16(st_fx->last_core_fx,ACELP_CORE))||(EQ_16(st_fx->last_core_fx,AMR_WB_CORE))))
    {
        ...
        *----------------------------------------------------------------*/
        ...
        Rescale_mem( st_fx->Q_exc, &st_fx->prev_Q_syn, &st_fx->Q_syn, st_fx->mem_syn2_fx, st_fx->mem_syn_clas_estim_fx,
                     4, &st_fx->mem_deemph_fx, st_fx->pst_old_syn_fx, &st_fx->pst_mem_deemp_err_fx,&st_fx->agc_mem_fx[1], &st_fx->pfstat , 1, 0, NULL );

        syn_12k8_fx( 2*L_SUBFR, Aq, exc, synth_intFreq, st_fx->mem_syn2_fx, 1, st_fx->Q_exc, st_fx->Q_syn );

        IF(st_fx->pfstat.on && (EQ_16(st_fx->last_bwidth_fx,NB)))
        {
            ...
            nb_post_filt( 2*L_SUBFR, &(st_fx->pfstat), &tmp_noise, 0, synth_intFreq, Aq, pitch_buf_tmp, AUDIO, st_fx->BER_detect, 0 );
        }
        ...
        test();
        IF( st_fx->pfstat.on && (NE_16(st_fx->last_bwidth_fx,NB)))
        {
            Copy( st_fx->pfstat.mem_pf_in+L_SYN_MEM-M, bpf_error_signal, M );
            Copy( synth_intFreq, bpf_error_signal + M, L_SUBFR );
            Residu3_fx ( Aq, bpf_error_signal + M, exc, L_SUBFR, 1 );
            E_UTIL_synthesis ( 1, Aq, exc, bpf_error_signal, L_SUBFR, st_fx->pfstat.mem_stp+L_SYN_MEM-M, 0, M );
            scale_st ( synth_intFreq, bpf_error_signal, &st_fx->pfstat.gain_prec, L_SUBFR );
        ...
        *----------------------------------------------------------------*/
        ...
        test();
        test();
        IF( decode_bwe && !(( EQ_16(output_frame, L_FRAME16k)&&EQ_16(st_fx->last_L_frame_fx,L_FRAME16k))||EQ_16(output_frame,L_FRAME8k)))
        {
            ...
            ELSE IF( EQ_16(st_fx->last_L_frame_fx, L_FRAME))
            {
            ...
            {
                d1m = sub(i,1);
                gain = 0; /* force muting */    move16();
                st_fx->BER_detect = 1;
                move16();
            }

            i = NS2SA_fx2(st_fx->output_Fs_fx, FRAME_SIZE_NS-ACELP_LOOK_NS-DELAY_BWE_TOTAL_NS);
            move16();
            Copy( st_fx->old_synth_sw_fx, hb_synth_tmp, i );
            set16_fx( hb_synth_tmp + i, 0, NS2SA_fx2(st_fx->output_Fs_fx, 10000000L) - i );
            fir_fx( hb_synth_tmp, hp_filter, hb_synth_tmp, tmp_mem2, shr(output_frame,1), fdelay, 1, 0 );
