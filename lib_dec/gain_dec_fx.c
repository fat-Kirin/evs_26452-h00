...
)
{
    ...
    test();
    test();
    IF( (EQ_16(tc_subfr_fx,3*L_SUBFR )&&EQ_16(i_subfr_fx,3*L_SUBFR)&&EQ_16(L_frame_fx,L_FRAME))||
        (EQ_16(tc_subfr_fx,4*L_SUBFR ) && EQ_16(i_subfr_fx,4*L_SUBFR)  && EQ_16(L_frame_fx,L_FRAME16k)) )
    {
        /* decode pitch gain */
    ...
    ELSE
    {
        ...
        test();
        if( EQ_16(coder_type_fx,INACTIVE)&&EQ_16(nBits,6))
        {
            nBits = sub(nBits, 1);
        }
