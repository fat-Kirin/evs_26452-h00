...
)
{
    ...
    Word16 last_bitalloc_max_band[2];
    Word32 L_tmp;
    ...
    set16_fx(last_bitalloc_max_band, 0, 2);
    ...
    IF( EQ_16(st_fx->bwidth_fx, SWB)&&(EQ_32(L_bwe_br,HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20)))
    {
        ...
        *is_transient_fx = 0;
        move16();
        if ( EQ_16(hqswb_clas_fx, HQ_TRANSIENT))
        {
            *is_transient_fx = 1;
            move16();
        }
    ...
    {
        /* Max: 45.0(737279,Q14) at 32kHz, highest band, Min: -6.600037(-108135,Q14) at 8kHz(NB),8kbps, is_transient (-6.7f(-109772,Q14) is safty-threshold) */
        test();
        IF( GT_32(L_band_energy[k], 737279L)||LT_32(L_band_energy[k],-109772L))
        {
            st_fx->BER_detect = 1;
            move16();
            set32_fx( L_yout, 0x0L, inner_frame );
            return;
        }
    }
    ...
    IF( LT_16(bit_budget_fx, 2))
    {
        st_fx->BER_detect = 1;
        move16();
        set32_fx( L_yout, 0x0, inner_frame );
        return;
    }
    ...
    test();
    test();
    IF ( *is_transient_fx == 0 && EQ_16(inner_frame, L_FRAME8k)&&LE_32(st_fx->core_brate_fx,ACELP_13k20))
    {
        /* decode the last p2a_bands-1 subbands bit-allocation index of the previous frame */
        j = 0;
        FOR(i = 0; i < 2; i++)
        {
            last_bitalloc_max_band[i] = get_next_indice_fx( st_fx, 1 );
        }
        lowband = 6;
        move16();
        trans_bit_fx = 2;
    ...
    ELSE IF( *is_transient_fx == 0 && EQ_16(inner_frame, L_FRAME16k))
    {
        ...
        FOR( i = sub(bands_fx,p2a_bands_fx); i < bands_fx; i++ )
        {
            ...
            {
                tmp = sub(bands_fx, p2a_bands_fx);
                IF(EQ_16(last_bitalloc_max_band[sub(i, add(tmp, 1))], 1))
                {
                    tmp = sub(tmp,lowband);
    ...
    test();
    IF( !(GE_16(st_fx->last_inner_frame_fx, L_FRAME16k)&&st_fx->bws_cnt_fx>0))
    {
        k1_fx =	sub(bands_fx,2);
        if(NE_16(*is_transient_fx,1))
        {
    ...
    IF( GE_16(st_fx->last_inner_frame_fx,L_FRAME32k))
    {
        set16_fx(st_fx->prev_SWB_fenv_fx, st_fx->prev_ener_shb_fx ,SWB_FENV);
    }

    updat_prev_frm_fx( L_y2, L_yout, L_bwe_br, length_fx, inner_frame, bands_fx, st_fx->bwidth_fx, *is_transient_fx, hqswb_clas_fx, &st_fx->prev_hqswb_clas_fx,
                       st_fx->prev_SWB_peak_pos_fx, prev_SWB_peak_pos_tmp_fx, &st_fx->prev_frm_hfe2_fx, &st_fx->prev_stab_hfe2_fx, st_fx->bws_cnt_fx );
