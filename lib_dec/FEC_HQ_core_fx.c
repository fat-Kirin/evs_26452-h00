...
)
{
    ...
    test();
    test();
    if( EQ_16(st_fx->prev_old_bfi_fx, 1)&&EQ_16(st_fx->nbLostCmpt,1)&&EQ_16(output_frame,L_FRAME8k))
    {
        st_fx->nbLostCmpt = add(st_fx->nbLostCmpt, 1);
    }

    /* Frequency-domain FEC */
    IF ( EQ_16(st_fx->nbLostCmpt, 1)) /* First erasure frame */
    {
        IF ( is_transient == 0 )
        {
            IF (LT_16(energy_diff_fx, 1024))
            {
    ...
    {
        test();
        IF( LT_16(energy_diff_fx, 1024)&&is_transient==0)
        {
            num_pgf = 4;
            move16();
        }
        ...
        IF( GE_16(st_fx->nbLostCmpt, mute_start))
        {
            /* Scaling */
            FOR ( i=0; i < output_frame; i++ )
            {
...
)
{
    ...
    IF(EQ_16(output_frame, L_FRAME8k))
    {
        ...
        test();
        test();
        IF((c_switching_flag)||((EQ_16(st_fx->last_core_fx, ACELP_CORE))&&(EQ_16(st_fx->core_fx,HQ_CORE))))
        {
        ...
        set16_fx(st_fx->Norm_gain_fx, 32767, SFM_N_NB);/*15*/
        ...
        /* Moving Average */
        st_fx->energy_MA_Curr_fx[0] = s_max(1,add(mult_r(26214, st_fx->energy_MA_Curr_fx[0]), mult_r(6554, st_fx->energy_MA_Curr_fx[1])));
        ...
        IF (LT_16(st_fx->diff_energy_fx, ED_THRES_12P_fx))
        {
            stat_mode_curr = 1;
            move16();
        }
        ...
        IF(EQ_16(st_fx->stat_mode_old_fx, stat_mode_curr))
        {
        ...
        *mean_en_high_fx = mult(*mean_en_high_fx, inv_tbl_fx[num_Sb-k]);
        ...
        test();
        test();
        IF ((LT_16(Min_ind, 5))&&(LT_16(abs_s(sub(Min_ind,st_fx->old_Min_ind_fx)),2))&&(LT_16(st_fx->diff_energy_fx,ED_THRES_90P_fx))&&(!st_fx->bfi_fx)&&(!st_fx->prev_bfi_fx)&&(!st_fx->prev_old_bfi_fx)
            &&(!is_transient)&&(!st_fx->old_is_transient_fx[1]) && (st_fx->prev_last_core_fx==HQ_CORE) && (st_fx->last_core_fx==HQ_CORE))
        {
...
)
{
    ...
    *false_flag = 0;
    move16();
    test();
    if (LT_16(tmp, 8192)||GT_16(tmp,24576))
    {
        *false_flag = 1;
        move16();
    }
...
)
{
    ...
    test();
    IF ((GT_16(mean_en_high_fx, 64))||(LT_16(mean_en_high_fx,16)))
    {
        oldout_pha_idx = 1;
        move16();
    }
...
)
{
    ...
    test();
    test();
    IF( (EQ_16(st_fx->nbLostCmpt, 1))&&(EQ_16(st_fx->phase_mat_flag_fx,1))&&(EQ_16(st_fx->phase_mat_next_fx,0)))
    {
    ...
    ELSE IF((EQ_16(st_fx->prev_bfi_fx, 1))&&(st_fx->bfi_fx==0)&&(EQ_16(st_fx->phase_mat_next_fx,1)))
    {
    ...
    ELSE IF((EQ_16(st_fx->prev_bfi_fx, 1))&&(EQ_16(st_fx->bfi_fx,1))&&(EQ_16(st_fx->phase_mat_next_fx,1)))
    {
    ...
    ELSE
    {
        ...
        test();
        IF (st_fx->bfi_fx == 0 && EQ_16(st_fx->prev_bfi_fx, 1))
        {
            test();
            IF((EQ_16(st_fx->stat_mode_out_fx, 1))||(LT_16(st_fx->diff_energy_fx,ED_THRES_50P_fx)))/* Q11 */
            {
                Word16 tmp;

                tmp = 0;
                if( GT_16(st_fx->old_bfi_cnt_fx,1))
                {
                    tmp = 1;
                }
                Repetition_smoothing_nextgood_fx( wtda_audio_fx, out_fx, st_fx->oldIMDCTout_fx, st_fx->old_out_fx, tmp, N_LEAD_NB);
                *Q_synth = 0;
                move16();
            }
            ELSE IF(GT_16(st_fx->old_bfi_cnt_fx, 1))
            {
        ...
        {
            test();
            IF( (EQ_16(st_fx->stat_mode_out_fx, 1))||(LT_16(st_fx->diff_energy_fx,ED_THRES_50P_fx)))
            {
