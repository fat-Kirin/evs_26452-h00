...
)
{
    ...
    IF(GT_16(st_fx->tilt_wb_fx, 16384))
    {
    ...
    test();
    test();
    IF( NE_16(st_fx->last_core_fx, HQ_CORE)&&EQ_16(st_fx->last_codec_mode,MODE1)&&
        ...
        (GT_32(st_fx->enerLL_fx, L_shr(st_fx->prev_enerLL_fx, 1))  && LT_32(L_shr(st_fx->enerLL_fx, 1), st_fx->prev_enerLL_fx) ) )
    {
        ...
        {
            test();
            IF(NE_16(st_fx->prev_coder_type_fx, coder_type)&&GT_16(mult_r(SWB_fenv_fx[i],16384),st_fx->prev_SWB_fenv_fx[i]))
            {
        ...
        IF(LT_16(st_fx->attenu_fx, 29491))
        {
    ...
    ELSE
    {
        ...
        test();
        test();
        IF( NE_32(st_fx->core_brate_fx, st_fx->last_core_brate_fx)||(GT_32(st_fx->enerLH_fx,L_shr(st_fx->prev_enerLH_fx,1))&&LT_32(L_shr(st_fx->enerLH_fx,1),st_fx->prev_enerLH_fx))||
        (GT_32(st_fx->enerLL_fx, L_shr(st_fx->prev_enerLL_fx, 1))  && LT_32(L_shr(st_fx->enerLL_fx, 1), st_fx->prev_enerLL_fx) ) )
        {
            FOR(i=0; i<SWB_FENV; i++)
            {
    ...
    *Qpost = sub(new_input_fx_exp,15);
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        ELSE
        {
            ...
            ,st_fx->last_extl_fx, st_fx->tilt_wb_fx
                                      );
            move16();
        }
    }
    ...
    test();
    IF( NE_16(st_fx->last_extl_fx, WB_BWE)||st_fx->bfi_fx)
    {
...
)
{
    ...
    *Qpost = sub(new_input_fx_exp,15);
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        {
            /* estimate parameters */
            mode = para_pred_bws_fx( st_fx, ysynth_fx, SWB_fenv_fx, Q_syn, coder_type );
            move16();
        }
        ELSE
        {
    ...
    {
        /* SHB FEC */
        IF( NE_16(st_fx->prev_mode_fx, TRANSIENT))
        {
    ...
    test();
    IF ( EQ_16(st_fx->prev_frica_flag_fx, 1)&&frica_flag==0)
    {
        FOR( i = 0; i < L_SUBFR; i++ )
        {
    ...
    ELSE IF( EQ_16(frica_flag, 1)&&st_fx->prev_frica_flag_fx==0)
    {
        time_reduce_pre_echo_fx( synth_fx, hb_synth_fx, st_fx->prev_td_energy_fx, l_subfr, *Qpost, Q_syn_hb );
    }
    ELSE
    {
