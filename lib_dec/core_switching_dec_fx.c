...
)
{
    ...
    test();
    test();
    IF( GE_16(st_fx->bws_cnt1_fx, N_NS2W_FRAMES))
    {
        st_fx->bws_cnt1_fx = 0;
        move16();
    }
    ELSE IF( GT_32(st_fx->total_brate_fx, ACELP_9k60)&&LT_32(st_fx->last_core_brate_fx,ACELP_9k60)
             && EQ_16(st_fx->bwidth_fx, SWB)  && EQ_16(st_fx->last_bwidth_fx, WB)  )
    {
    ...
    {
        IF( LT_16(st_fx->bwidth_fx, st_fx->last_bwidth_fx))
        {
    ...
    test();
    test();
    IF( GE_16(st_fx->bws_cnt_fx, N_WS2N_FRAMES))
    {
        st_fx->bws_cnt_fx = 0;
        move16();
    }
    ELSE IF( LT_32(st_fx->total_brate_fx, ACELP_9k60)&&GT_32(st_fx->last_core_brate_fx,ACELP_9k60)
             && LT_16(st_fx->bwidth_fx, st_fx->last_bwidth_fx)  && EQ_16(st_fx->bwidth_fx, WB)  )
    {
    ...
    {
        IF( GT_16(st_fx->bwidth_fx, st_fx->last_bwidth_fx))
        {
...
)
{
    ...
    test();
    test();
    IF( st_fx->last_bwidth_fx == 0 && LE_16(st_fx->extl_fx, SWB_CNG))
    {
        st_fx->prev_ener_shb_fx = 0;
        move16();
        set16_fx(st_fx->prev_SWB_fenv_fx, 0, SWB_FENV);
    }
    ELSE if(((EQ_16(st_fx->core_fx, ACELP_CORE)&&EQ_16(st_fx->last_core_fx,HQ_CORE))||(EQ_16(st_fx->core_fx,st_fx->last_core_fx)&&NE_16(st_fx->extl_fx,st_fx->last_extl_fx)))&&GE_16(st_fx->last_bwidth_fx,SWB))
    {
    ...
    IF(EQ_16(st_fx->last_core_fx, HQ_CORE)
       || ( EQ_16(st_fx->last_core_fx, ACELP_CORE) 
            && !(EQ_16(st_fx->last_extl_fx, WB_TBE)  || EQ_16(st_fx->last_extl_fx, SWB_TBE)  || EQ_16(st_fx->last_extl_fx, FB_TBE) ) && GT_32(st_fx->core_brate_fx, ACELP_8k00) ))
    {
...
)
{
    ...
    IF( EQ_16(st_fx->last_codec_mode,MODE2))
    {
        ...
        st_fx->pst_lp_ener_fx = round_fx(L_shl(Mpy_32_16_1(st_fx->lp_error_ener, 0x6054), 2+8)); /* convert from 15Q16, log2 -> 7Q8 10*log10  */
        st_fx->pst_mem_deemp_err_fx = 0;
        move16();
        ...
        {
            st_fx->old_bwe_delay_fx = NS2SA_fx2( st_fx->output_Fs_fx, MAX_DELAY_TBE_NS - DELAY_SWB_TBE_12k8_NS );
        }
        ELSE
        {
            st_fx->old_bwe_delay_fx = NS2SA_fx2( st_fx->output_Fs_fx, MAX_DELAY_TBE_NS - DELAY_SWB_TBE_16k_NS );
        }
        set16_fx( st_fx->hb_prev_synth_buffer_fx, 0, NS2SA(48000, DELAY_BWE_TOTAL_NS) );
        st_fx->prev_hb_synth_fx_exp = 31;
        move16();
        ...
        IF( GE_32(st_fx->output_Fs_fx,16000))
        {
        ...
        IF( GT_32(st_fx->output_Fs_fx,16000))
        {
            set32_fx( st_fx->prev_coeff_out_fx, 0, L_HQ_WB_BWE );
        }
        ...
        IF( EQ_16(st_fx->last_core_fx,TCX_20_CORE)||EQ_16(st_fx->last_core_fx,TCX_10_CORE))
        {
            ...
            set16_fx( st_fx->prev_SWB_peak_pos_fx, 0, SPT_SHORTEN_SBNUM );
        ...
        IF(st_fx->prev_bfi_fx!=0)
        {
            ...
            IF( !st_fx->last_con_tcx && st_fx->last_core_bfi == ACELP_CORE && EQ_16(st_fx->core_fx,HQ_CORE))
            {
    ...
    /*FEC*/
    IF( LE_16(st_fx->L_frame_fx,L_FRAME16k))
    {
        test();
        IF( LE_16(st_fx->last_L_frame_fx,L_FRAME16k)&&NE_16(st_fx->core_fx,HQ_CORE))
        {
            IF( NE_16(st_fx->L_frame_fx,st_fx->last_L_frame_fx))
            {
                IF (GT_16(st_fx->L_frame_fx,st_fx->last_L_frame_fx))
                {
    ...
    IF( (EQ_16(st_fx->core_fx,ACELP_CORE)||EQ_16(st_fx->core_fx,AMR_WB_CORE))&&EQ_16(st_fx->last_core_fx,HQ_CORE))
    {
        IF(EQ_16(st_fx->L_frame_fx, L_FRAME16k))
        {
            ...
            Copy( TRWB2_Ave_fx, st_fx->lsfoldbfi1_fx, M );
            Copy( TRWB2_Ave_fx, st_fx->lsfoldbfi0_fx, M );
            Copy( TRWB2_Ave_fx, st_fx->lsf_adaptive_mean_fx, M );
            lsf2lsp_fx( st_fx->lsf_old_fx, st_fx->lsp_old_fx, M, INT_FS_16k );
        }
        ELSE
        {
            Copy( TRWB_Ave_fx, st_fx->lsf_old_fx, M ); /* init of LSP */
            Copy( TRWB_Ave_fx, st_fx->lsfoldbfi1_fx, M );
            Copy( TRWB_Ave_fx, st_fx->lsfoldbfi0_fx, M );
            Copy( TRWB_Ave_fx, st_fx->lsf_adaptive_mean_fx, M );
        ...
        ELSE IF (LT_16(st_fx->L_frame_fx,L_FRAME16k))
        {
        ...
        IF( GE_32(st_fx->output_Fs_fx, 16000L))
        {
            hf_synth_reset_fx( &st_fx->seed2_fx, st_fx->mem_hf_fx, st_fx->mem_syn_hf_fx, st_fx->mem_hp400_fx, st_fx->mem_hp_interp_fx, st_fx->delay_syn_hf_fx );
        }
        set16_fx( st_fx->old_syn_12k8_16k_fx, 0, NS2SA(16000, DELAY_FD_BWE_ENC_NS) );
    ...
    test();
    test();
    IF( EQ_16(st_fx->core_fx,HQ_CORE)&&(EQ_16(st_fx->last_core_fx,ACELP_CORE)||st_fx->last_core_fx==AMR_WB_CORE))
    {
        set32_fx( st_fx->prev_env_fx, 0, SFM_N_WB );
        set32_fx( st_fx->prev_normq_fx, 0, SFM_N_WB );
    ...
    IF( EQ_32(st_fx->core_brate_fx,FRAME_NO_DATA))
    {
        st_fx->VAD = 0;
        move16();
    ...
    ELSE IF( LE_32(st_fx->core_brate_fx,SID_2k40))
    {
        st_fx->VAD = 0;
        move16();
    ...
    test();
    test();
    IF( NE_16(st_fx->core_fx,AMR_WB_CORE)&&st_fx->VAD&&LE_32(st_fx->total_brate_fx,CNA_MAX_BRATE))
    {
        st_fx->flag_cna = 1;
        move16();
    }
    ELSE IF( EQ_16(st_fx->core_fx,AMR_WB_CORE)&&st_fx->VAD&&LE_32(st_fx->total_brate_fx,ACELP_8k85))
    {
        st_fx->flag_cna = 1;
        move16();
    }
    ELSE IF( st_fx->VAD || ( EQ_16(st_fx->cng_type_fx,FD_CNG)&&EQ_16(st_fx->L_frame_fx,L_FRAME16k)))
    {
        st_fx->flag_cna = 0;
        move16();
    }

    if( EQ_16(st_fx->core_fx,AMR_WB_CORE))
    {
    ...
    test();
    test();
    IF( st_fx->hFdCngDec_fx && ((NE_16(st_fx->last_L_frame_fx,st_fx->L_frame_fx))||
                                (NE_16(st_fx->hFdCngDec_fx->hFdCngCom->frameSize,st_fx->L_frame_fx)) ||
                                st_fx->ini_frame_fx == 0 || NE_16(st_fx->bwidth_fx, st_fx->last_bwidth_fx) ))
    {

        IF( NE_16(st_fx->core_fx,AMR_WB_CORE))
        {
            Word32 tmp;

            tmp = st_fx->total_brate_fx;
            move32();
            test();
            if( EQ_16(st_fx->rf_flag,1)&&EQ_32(st_fx->total_brate_fx,ACELP_13k20))
            {
                tmp = ACELP_9k60;
                move32();
            }
            configureFdCngDec(st_fx->hFdCngDec_fx, st_fx->bwidth_fx, tmp, st_fx->L_frame_fx );
        }
        ELSE
        {
        ...
        IF ( NE_16(st_fx->last_L_frame_fx,st_fx->L_frame_fx)&&LE_16(st_fx->L_frame_fx,L_FRAME16k)&&LE_16(st_fx->last_L_frame_fx,L_FRAME16k))
        {
            ...
            test();
            IF( LE_32(st_fx->total_brate_fx,SID_2k40)&&LE_32(st_fx->last_total_brate_fx,SID_2k40))
            {
                lerp( st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth, st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth, st_fx->L_frame_fx*2, st_fx->last_L_frame_fx*2 );
                IF( EQ_16(st_fx->L_frame_fx, L_FRAME))
                {
                    Word16 n;
                    FOR( n=0; n < st_fx->L_frame_fx*2; n++ )
                    {
                        st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth[n] = mult_r( st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth[n], 20480 );
                    }
                }
                ELSE
                {
                    ...
                    FOR( n=0; n < st_fx->L_frame_fx*2; n++ )
                    {
                        st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth[n] = mult_r( shl(st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth[n],1), 26214 );
                    }
                }
            }
        }
...
)
{
    ...
    *Qsynth=add(*Qsynth,tmp);
    ...
    test();
    IF( EQ_16(st_fx->core_fx, ACELP_CORE)&&st_fx->bfi_fx)
    {
    ...
    IF( EQ_16(st_fx->core_fx, HQ_CORE))
    {
        ...
        test();
        test();
        IF ( core_switching_flag && EQ_16(st_fx->last_L_frame_fx, st_fx->last_L_frame_ori_fx)&&(EQ_16(st_fx->last_core_fx,ACELP_CORE)||EQ_16(st_fx->last_core_fx,AMR_WB_CORE)))
        {
            acelp_core_switch_dec_fx( st_fx, synth_subfr_out, synth_subfr_bwe, output_frame, core_switching_flag, mem_synth, &Qsubfr );
        }
        test();
        test();
        IF( core_switching_flag && EQ_16(st_fx->last_core_fx, HQ_CORE)&&st_fx->prev_bfi_fx)
        {
        ...
        *Qsynth=Qtmp;
        ...
        IF ( core_switching_flag && EQ_16(st_fx->last_L_frame_fx, st_fx->last_L_frame_ori_fx)&&(EQ_16(st_fx->last_core_fx,ACELP_CORE)||EQ_16(st_fx->last_core_fx,AMR_WB_CORE)))
        {
            ...
            core_switching_OLA_fx( mem_synth, st_fx->last_L_frame_fx, st_fx->output_Fs_fx, synth, synth_subfr_out, synth_subfr_bwe, output_frame, st_fx->bwidth_fx, Qsynth, &Qsubfr );
        }
        ELSE IF ( core_switching_flag && EQ_16(st_fx->last_core_fx, HQ_CORE)&&st_fx->prev_bfi_fx) /* HQ | ACELP | TRANSITION  with ACELP frame lost */
        {
        ...
        ELSE IF ( ( !core_switching_flag && EQ_16(st_fx->core_fx, HQ_CORE)&&(EQ_16(st_fx->last_core_fx,ACELP_CORE)||EQ_16(st_fx->last_core_fx,AMR_WB_CORE)))  /* ACELP | TRANSITION | HQ with TRANSITION lost */
                  || (core_switching_flag && st_fx->prev_bfi_fx && NE_16(st_fx->last_L_frame_fx, st_fx->last_L_frame_ori_fx) ) )
        {
        ...
        st_fx->bwe_non_lin_prev_scale_fx = L_deposit_l(0);
        IF ( !(EQ_16(inner_frame_tbl[st_fx->bwidth_fx], L_FRAME16k)&&EQ_32(st_fx->core_brate_fx,HQ_32k)))
        {
            set32_fx( st_fx->prev_env_fx, 0, SFM_N_WB );
            set32_fx( st_fx->prev_normq_fx, 0, SFM_N_WB );
        }
    ...
    ELSE
    {
        IF ( EQ_16(st_fx->last_core_fx, HQ_CORE)) /*  MDCT to ACELP transition */
        {
    ...
    test();
    test();
    IF(st_fx->bws_cnt_fx == 0 || (st_fx->bws_cnt_fx > 0 && NE_16(coder_type, INACTIVE)&&NE_16(coder_type,AUDIO)))
    {
        st_fx->attenu_fx = 3277;
        move16();
    }

    IF( ( NE_16(st_fx->last_extl_fx, SWB_BWE)&&EQ_16(st_fx->extl_fx,SWB_BWE))||(NE_16(st_fx->last_extl_fx,FB_BWE)&&EQ_16(st_fx->extl_fx,FB_BWE))||
        ((EQ_16(st_fx->last_core_fx, HQ_CORE)  || EQ_16(st_fx->last_extl_fx, SWB_TBE) ) && st_fx->extl_fx < 0 && NE_16(st_fx->core_fx, HQ_CORE) )
        || (EQ_16(st_fx->last_core_fx,ACELP_CORE)  && EQ_16(st_fx->core_fx,ACELP_CORE) 
            && ((NE_16(st_fx->prev_coder_type_fx,INACTIVE)  && EQ_16(coder_type,INACTIVE) ) || (NE_16(st_fx->prev_coder_type_fx,AUDIO)  && EQ_16(coder_type,AUDIO) ))
            && st_fx->bws_cnt_fx > 0)
      )
    {
    ...
    test();
    IF( NE_16(st_fx->last_extl_fx, WB_BWE)&&EQ_16(st_fx->extl_fx,WB_BWE))
    {
        set16_fx(st_fx->L_old_wtda_swb_fx, 0, output_frame);
    ...
    test();
    test();
    IF( (( EQ_16(st_fx->extl_fx, SWB_TBE)||EQ_16(st_fx->extl_fx,FB_TBE)||EQ_16(st_fx->extl_fx,SWB_CNG))&&
         ( NE_16(st_fx->L_frame_fx, st_fx->last_L_frame_fx)  || ( NE_16(st_fx->last_extl_fx, SWB_TBE)   && NE_16(st_fx->last_extl_fx, FB_TBE)  ) || EQ_16(st_fx->last_core_fx, HQ_CORE)  )) ||
        ( LT_16(st_fx->bwidth_fx, st_fx->last_bwidth_fx)  && NE_16(st_fx->last_extl_fx, SWB_TBE)  ) || st_fx->old_ppp_mode_fx
        || ((EQ_16(st_fx->prev_coder_type_fx, AUDIO)  || EQ_16(st_fx->prev_coder_type_fx, INACTIVE) ) && st_fx->bws_cnt_fx > 0)
        || (st_fx->bws_cnt_fx == 0 && EQ_16(st_fx->prev_bws_cnt_fx, N_WS2N_FRAMES)) )
    {
    ...
    test();
    test();
    IF(EQ_32(st_fx->output_Fs_fx,48000)&&((GT_32(st_fx->last_core_brate_fx,SID_2k40))&&(EQ_32(st_fx->core_brate_fx,FRAME_NO_DATA)||EQ_32(st_fx->core_brate_fx,SID_2k40))))
    {
    ...
    test();
    test();
    IF( EQ_16(st_fx->extl_fx, FB_TBE)&&(NE_16(st_fx->last_extl_fx,FB_TBE)||NE_16(st_fx->L_frame_fx,st_fx->last_L_frame_fx)))
    {
    ...
    test();
    IF( NE_16(st_fx->last_extl_fx, WB_TBE)&&EQ_16(st_fx->extl_fx,WB_TBE))
    {
...
)
{
    ...
    test();
    IF( EQ_16(st_fx->last_core_fx, HQ_CORE)&&st_fx->prev_bfi_fx)
    {
    ...
    test();
    test();
    IF( !( ( EQ_16(inner_frame_tbl_fx[st_fx->bwidth_fx], L_FRAME16k)&&EQ_16(st_fx->last_L_frame_fx,L_FRAME16k))||EQ_16(inner_frame_tbl_fx[st_fx->bwidth_fx],L_FRAME8k)))
    {
