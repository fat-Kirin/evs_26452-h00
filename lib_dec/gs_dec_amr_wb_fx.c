...
)
{
    ...
    IF( LT_32(core_brate,ACELP_12k65))
    {
        ...
        test();
        test();
        IF((EQ_16(coder_type,INACTIVE)||EQ_16(clas,VOICED_TRANSITION))&&LT_16(temp,20))
        {
            FOR(i = 0; i < CRIT_NOIS_BAND; i++)
            {
...
)
{
    ...
    IF( ( locattack == 0 && LE_32(core_brate, ACELP_12k65))&&
        ( (LT_32(core_brate, ACELP_8k85)  && NE_16(clas, AUDIO_CLAS)  &&
           (EQ_16(clas, UNVOICED_CLAS)  || EQ_16(clas, VOICED_TRANSITION) )) || EQ_16(coder_type, INACTIVE)  ) )
    {
        /*------------------------------------------------------------*
        ...
        *seed_tcx = extract_l(L_mult0(pitch_buf_fx[0],pitch_buf_fx[3]));
        ...
        test();
        test();
        IF( EQ_16(coder_type, INACTIVE)&&GT_16(Last_ener_fx,-3*256)&&EQ_16(last_coder_type_fx,UNVOICED)&&rate_switching_reset==0) /* 3.0 x 256 to Go to Q8 */
        {
