...
static void find_max_mem_dec( Decoder_State_fx* st_fx, Word16* n_mem, Word16 *n_mem2, Word16 *n_mem3 );
...
)
{
    ...
    *n_mem2 = norm_s(max2);
    ...
    if(LT_32(st_fx->total_brate_fx, ACELP_24k40))
    {
        max = s_max(max, max2);
    }
    ...
    IF( EQ_16(st_fx->extl_fx, FB_TBE))
    {
        FOR ( i = 0; i < LPC_SHB_ORDER; i++ )
        {
            tempQ15 = abs_s( st_fx->fb_state_lpc_syn_fx[i] );
            max = s_max(max, tempQ15);
        }
        /* FB de-emph memory */
        tempQ15 = abs_s( st_fx->fb_tbe_demph_fx);
        max = s_max(max, tempQ15);
    }
    ...
    *n_mem = s_max( tempQ15, 0 );
    ...
    IF( EQ_32(st_fx->output_Fs_fx, 48000))
    {
    ...
    IF( EQ_32(st_fx->output_Fs_fx, 16000))
    {
        FOR ( i = 0; i < (2*ALLPASSSECTIONS_STEEP+1); i++ )
        {
            tempQ15 = abs_s( st_fx->mem_resamp_HB_32k_fx[i] );
            max3 = s_max(max3, tempQ15);
        }
    }
    /* estimate the norm for 16-bit memories  */
...
)
{
    ...
    set16_fx(st_fx->genSHBsynth_state_lsyn_filt_shb_local_fx, 0, 2*ALLPASSSECTIONS_STEEP);  /* Interp all pass memory */

    st_fx->syn_dm_phase_fx = 0;
    move16();
    st_fx->prev_fbbwe_ratio_fx = 32767/*1.0f Q15*/;

    /* these are fd-bwe constants */
    st_fx->prev_wb_bwe_frame_pow_fx = 4194l/*0.001f Q22*/; /* Q22 */
    st_fx->prev_swb_bwe_frame_pow_fx = 4194l/*0.001f Q22*/; /* Q22 */
...
void ResetSHBbuffer_Dec_fx( Decoder_State_fx* st_fx /* i/o: SHB encoder structure */ )
{
    ...
    IF( EQ_16(st_fx->extl_fx, FB_TBE))
    {
        ...
        move16();
        fb_tbe_reset_synth_fx( st_fx->fbbwe_hpf_mem_fx, st_fx->fbbwe_hpf_mem_fx_Q,&st_fx->prev_fbbwe_ratio_fx );
    }
    ...
    set16_fx( st_fx->state_lsyn_filt_shb_fx,0, 2 * ALLPASSSECTIONS_STEEP );
    set16_fx( st_fx->state_lsyn_filt_dwn_shb_fx,0, 2 * ALLPASSSECTIONS_STEEP );
    set16_fx( st_fx->state_32and48k_WB_upsample_fx, 0, 2 * ALLPASSSECTIONS_STEEP );
    ...
    ELSE
    {
        ...
        FOR( ; i<LPC_SHB_ORDER; i++)
        {
            st_fx->lsp_prevfrm_fx[i] = 0;
            move16();
        }
    }
    ...
    set16_fx(st_fx->old_tbe_synth_fx, 0, L_SHB_TRANSITION_LENGTH);
    st_fx->tilt_swb_fec_fx = 0;
    move16();
...
)
{
    ...
    IF( st_fx->bws_cnt_fx == 0 )
    {
        ...
        IF( !st_fx->bfi_fx )
        {
            IF(EQ_16(st_fx->use_partial_copy,1))
            {
                IF(NE_16(st_fx->last_extl_fx, WB_TBE))
                {
                    st_fx->GainFrame_prevfrm_fx = 0;
                    st_fx->lsp_prevfrm_fx[0] = 3277/*0.1f Q15*/;
                    move16();
                    FOR (i=1; i<LPC_SHB_ORDER_LBR_WB; i++)
                    {
                        st_fx->lsp_prevfrm_fx[i] = add(st_fx->lsp_prevfrm_fx[i-i], 3277/*0.1f Q15*/);
                    }
                }
                Copy( st_fx->lsp_prevfrm_fx, lsf_wb, LPC_SHB_ORDER_LBR_WB );
                set16_fx( GainShape, RECIP_ROOT_EIGHT_FX, NUM_SHB_SUBFR/2 );

                IF( EQ_16(st_fx->rf_frame_type,RF_NELP))
                {
                    /* Frame gain */
                    ...
                    IF( EQ_16(st_fx->core_fx,ACELP_CORE)&&EQ_16(st_fx->last_core_fx,ACELP_CORE)
                        && !st_fx->prev_use_partial_copy && EQ_16(st_fx->prev_coder_type_fx,UNVOICED) 
                        && NE_32(GainFrame,st_fx->GainFrame_prevfrm_fx) && EQ_16(st_fx->last_extl_fx, WB_TBE)  )
                    {
                ...
                ELSE
                {
                    ...
                    move16();
                    /* Frame gain */
                    SWITCH (st_fx->rf_indx_tbeGainFr)
                    {
                    case 0:
                        GainFrame = 131072; /* 0.5f in Q18 */
                        IF(LE_32(st_fx->GainFrame_prevfrm_fx, 327680l/*1.25 Q18*/)) temp = 26214/*0.8 Q15*/;
                        move16();
                        BREAK;
                    case 1:
                        GainFrame = 524288; /* 2.0f in Q18 */
                        IF(GT_32(st_fx->GainFrame_prevfrm_fx, 327680l/*1.25 Q18*/)&&LE_32(st_fx->GainFrame_prevfrm_fx,786432l/*3 Q18*/)) temp = 26214/*0.8 Q15*/;
                        move16();
                        test();
                        BREAK;
                    case 2:
                        GainFrame = 1048576;/* 4.0f in Q18 */
                        IF(GT_32(st_fx->GainFrame_prevfrm_fx, 786432l/*3 Q18*/)&&LE_32(st_fx->GainFrame_prevfrm_fx,1572864l/*6 Q18*/)) temp = 26214/*0.8 Q15*/;
                        move16();
                        test();
                        BREAK;
                    case 3:
                        GainFrame = 2097152;/* 8.0f in Q18 */
                        IF(GT_32(st_fx->GainFrame_prevfrm_fx, 1572864l/*6 Q18*/)&&LE_32(st_fx->GainFrame_prevfrm_fx,4194304l/*16 Q18*/)) temp = 26214/*0.8 Q15*/;
                        move16();
                        test();
                        BREAK;
                    default:
                    ...
                    IF(EQ_16(st_fx->last_extl_fx, WB_TBE))
                    {
                    ...
                    IF(EQ_16(st_fx->core_fx,ACELP_CORE)&&EQ_16(st_fx->last_core_fx,ACELP_CORE))
                    {
                        IF(!st_fx->prev_use_partial_copy && EQ_16(st_fx->last_coder_type_fx, VOICED)&&EQ_16(st_fx->rf_frame_type,RF_GENPRED)
                        && LT_16(st_fx->prev_tilt_code_dec_fx,1497)  && GT_16(st_fx->prev_tilt_code_dec_fx,200)  )
                        {
        ...
        ELSE
        {
            IF( EQ_32( st_fx->extl_brate_fx, WB_TBE_0k35))
            {
                Copy( st_fx->lsp_prevfrm_fx, lsf_wb, LPC_SHB_ORDER_LBR_WB );
            }
            ELSE
        ...
        {
            IF( GT_32( L_shr( curr_frame_pow, 1 ), st_fx->prev_wb_bwe_frame_pow_fx ))
            {
        ...
        IF( EQ_32(st_fx->output_Fs_fx, 32000))
        {
            FOR ( i = 0; i < 2*ALLPASSSECTIONS_STEEP; i++ )
            {
        ...
        Copy( error + L_FRAME16k - L_SHB_TRANSITION_LENGTH, st_fx->old_tbe_synth_fx, L_SHB_TRANSITION_LENGTH );
        ...
        IF( EQ_32(st_fx->output_Fs_fx, 32000))  /* 32kHz sampling rate, but only WB output - interpolate */
        {
        ...
        ELSE IF( EQ_32(st_fx->output_Fs_fx, 48000))
        {
    ...
    IF( EQ_32( st_fx->extl_brate_fx, WB_TBE_0k35 ) )
    {
        Copy( lsf_wb, st_fx->lsp_prevfrm_fx, LPC_SHB_ORDER_LBR_WB );
    }
    ELSE
    {
...
)
{
    ...
    test();
    IF( st_fx->bfi_fx && NE_16(st_fx->clas_dec,UNVOICED_CLAS))
    {
        tilt_swb_fec = st_fx->tilt_swb_fec_fx;
        move16();
    }
    ...
    IF( ( GT_16(st_fx->tilt_wb_fx, 10240)&&EQ_16(st_fx->clas_dec,UNVOICED_CLAS))||GT_16(st_fx->tilt_wb_fx,20480))
    {
        ...
        IF( (st_fx->prev_fractive_fx == 0 &&
             (LT_32( st_fx->prev_enerLH_fx, L_shl( st_fx->enerLH_fx, 1 ) )  && GT_32( st_fx->prev_enerLH_fx, L_shr( st_fx->enerLH_fx, 1 ) ) 
              && LT_32( st_fx->prev_enerLL_fx, L_shl( st_fx->enerLL_fx, 1 ) )  && GT_32( st_fx->prev_enerLL_fx, L_shr( st_fx->enerLL_fx, 1 ) ) ))
            || (EQ_16(st_fx->prev_fractive_fx,1)  &&
                GT_32(L_shr(st_fx->prev_enerLH_fx,2), Mult_32_16(st_fx->enerLH_fx,24576)) )  /* 24576 in Q13*/
            || (GT_32(L_shr(st_fx->enerLL_fx,1), Mult_32_16(st_fx->enerLH_fx, 24576))   &&  /*24576 = 1.5 in Q14*/
                LT_16(st_fx->tilt_wb_fx, 20480)  )/* 20480 = 10 in Q11*/
          )
        {
            is_fractive = 0;
            move16();
    ...
    IF( st_fx->bws_cnt_fx > 0 )
    {
        ...
        test();
        test();
        IF( (NE_16(st_fx->last_extl_fx, SWB_TBE)&&NE_16(st_fx->last_extl_fx,FB_TBE)&&
        ...
        ELSE
        {
            if(GT_16(st_fx->prev_GainShape_fx, 11587))
            {
        ...
        set16_fx( shb_res_gshape, 3277/*0.2f Q14*/, NB_SUBFR16k );  /* Q14 */
    }
    ELSE /* No bandwidth switching */
    {
        ...
        IF( NE_16(st_fx->last_extl_fx, SWB_TBE)&&NE_16(st_fx->last_extl_fx,FB_TBE))
        {
            f_fx = 1489;    /*Q15*/     move16();
            inc_fx = 1489;  /*Q15*/     move16();
            FOR (i=0; i<LPC_SHB_ORDER; i++)
            {
                st_fx->lsp_prevfrm_fx[i] = f_fx;
                move16();
        ...
        IF( !st_fx->bfi_fx )
        {
            ...
            {
                IF(NE_16(st_fx->last_extl_fx, SWB_TBE))
                {
                    st_fx->GainFrame_prevfrm_fx = 0;
                    move16();
                    move16();
                ...
                {
                    /* Frame gain */
                    GainFrame = L_mac( SHB_GAIN_QLOW_FX, st_fx->rf_indx_tbeGainFr, SHB_GAIN_QDELTA_FX );
                    move32();/*Q18*/
                    ...
                    IF( EQ_16(st_fx->core_fx,ACELP_CORE)&&EQ_16(st_fx->last_core_fx,ACELP_CORE)
                        && !st_fx->prev_use_partial_copy && EQ_16(st_fx->prev_coder_type_fx,UNVOICED) 
                        && NE_32(GainFrame,st_fx->GainFrame_prevfrm_fx)  && NE_16(st_fx->next_coder_type,GENERIC)  && EQ_16(st_fx->last_extl_fx, SWB_TBE) )
                    {
                        /*GainFrame = 0.2f*GainFrame + 0.8f*st_fx->GainFrame_prevfrm_fx;*/
                        GainFrame = L_add(Mult_32_16(st_fx->GainFrame_prevfrm_fx, 26214), Mult_32_16(GainFrame, 6553));
                    }
                ...
                ELSE
                {
                    ...
                    case 3:
                        GainFrame = 2097152;/* 8.0f in Q18 */
                        IF(GT_32(st_fx->GainFrame_prevfrm_fx, 1572864l/*6 Q18*/)&&LE_32(st_fx->GainFrame_prevfrm_fx,4194304l/*16Q18*/)) temp = 26214/*0.8 Q15*/;
                        move16();
                        test();
                        BREAK;
                    default:
                    ...
                    IF(EQ_16(st_fx->last_extl_fx, SWB_TBE))
                    {
                        GainFrame = L_add(Mult_32_16(st_fx->GainFrame_prevfrm_fx, temp), Mult_32_16(GainFrame, sub(32767,temp)));
                    }
                    IF(EQ_16(st_fx->core_fx,ACELP_CORE)&&EQ_16(st_fx->last_core_fx,ACELP_CORE))
                    {
                        IF(!st_fx->prev_use_partial_copy && EQ_16(st_fx->last_coder_type_fx, VOICED)&&EQ_16(st_fx->rf_frame_type,RF_GENPRED)&&GT_32(GainFrame,2097152)&&LT_32(GainFrame,3059606))
                        {
                            GainFrame = Mult_32_16(GainFrame,9830);
                        }
                    }
        ...
        ELSE /* FER concealment of TBE parameters */
        {
            ...
            ELSE
            {
                ...
                IF( GT_16( tilt_swb_fec, 8<<11 ))   /* tilt_swb_fec in Q11 */
                {
                    ...
                    {
                        GainFrame = Mult_32_16(st_fx->GainFrame_prevfrm_fx, 19661/*0.6f Q15*/);
                    }
                    ELSE IF( EQ_16(st_fx->nbLostCmpt, 2))
                    {
                        GainFrame = Mult_32_16(st_fx->GainFrame_prevfrm_fx, 11469/*0.35f Q15*/);
                    }
                    ELSE
                    {
                        GainFrame = Mult_32_16(st_fx->GainFrame_prevfrm_fx, 6554/*0.2f Q15*/);
                    }
            ...
            test();
            IF(EQ_32(st_fx->total_brate_fx,ACELP_24k40)||EQ_32(st_fx->total_brate_fx,ACELP_32k))
            {
                IF(EQ_16(st_fx->codec_mode, MODE1))
                {
                    /*scale = st->prev1_shb_ener_sf/root_a(st->prev2_shb_ener_sf * st->prev3_shb_ener_sf); */
                    L_tmp = L_mult(extract_l(st_fx->prev2_shb_ener_sf_fx), extract_l(st_fx->prev3_shb_ener_sf_fx)); /*Q1*/
                    tmp = round_fx(root_a_fx(L_tmp, 1, &exp)); /* Q = 15-exp */
                    tmp1 = extract_l(st_fx->prev1_shb_ener_sf_fx);  /*Q0*/
                    i = sub(norm_s(tmp1), 1);
                    tmp1 = shl(tmp1, i); /* Qi */
                    if(tmp == 0)
                    {
                    ...
                    ELSE
                    {
                        scale = div_s(tmp1, tmp);   /* Q15 - Q(15-exp) + Qi  = Qexp+i */
                        scale = s_max(scale, 0);
                        /*scale = st->prev_res_shb_gshape * min(scale, 1.0f); */
                        tmp = shl(scale, 15 - exp - i); /*Q15*/
                    }
                    scale = mult_r(st_fx->prev_res_shb_gshape_fx, tmp); /* Q14 */

                    test();
                    IF( GT_32( L_shr(st_fx->prev2_shb_ener_sf_fx, 1), st_fx->prev1_shb_ener_sf_fx )||
                        GT_32( L_shr(st_fx->prev3_shb_ener_sf_fx, 1), st_fx->prev2_shb_ener_sf_fx )  )
                    {
                        /* shb_ener_sf_32 = 0.5f * scale * st_fx->prev1_shb_ener_sf_fx; */
                        shb_ener_sf_32 = Mult_32_16( st_fx->prev1_shb_ener_sf_fx, scale );

                        if( GT_16(st_fx->nbLostCmpt, 1))
                        {
                            /* shb_ener_sf_32 *= 0.5f; */
                    ...
                    ELSE
                    {
                        ...
                        L_tmp = L_mult(scale, scale);  /* Q29 */
                        shb_ener_sf_32 = L_shl(Mult_32_16(st_fx->prev1_shb_ener_sf_fx, round_fx(L_tmp)), 2);
                    }
                }
                ELSE
                {
            ...
            IF(EQ_16(st_fx->codec_mode, MODE1))
            {
                set16_fx( shb_res_gshape, 3277/*0.2f Q14*/, 5 );  /* Q14 */
            }
            ELSE
            {
                set16_fx( shb_res_gshape, 16384/*1.0f Q14*/, 5 );  /* Q14 */
            }
        }
    }
    ...
    IF ( (st_fx->rf_flag != 0) || EQ_32(st_fx->total_brate_fx, ACELP_9k60))
    {
        FOR( i = 0; i < NUM_SHB_SUBFR / 4; i++ )
        {
    ...
    test();
    IF( EQ_16(coder_type, VOICED)||GT_16(mean_vf,13107/*0.4f Q15*/ ))
    {
        FOR( i = 1; i < NB_SUBFR; i++ )
        {
            L_tmp = L_mult(voice_factors[i], 26214/*0.8f Q15*/);
            vf_modified[i] = mac_r(L_tmp, voice_factors[i-1], 6554/*0.2f Q15*/);
            move16();
        }
        IF( NE_16(st_fx->L_frame_fx, L_FRAME))
        {
            L_tmp = L_mult(voice_factors[4], 26214/*0.8f Q15*/);
            vf_modified[4] = mac_r(L_tmp, voice_factors[3], 6554/*0.2f Q15*/);
            move16();
        }
    }
    ...
    test();
    IF( EQ_16(st_fx->last_extl_fx, SWB_TBE)||EQ_16(st_fx->last_extl_fx,FB_TBE))
    {
    ...
    test();
    test();
    IF( st_fx->bws_cnt_fx == 0 && st_fx->bws_cnt1_fx == 0 && st_fx->prev_use_partial_copy == 0 && st_fx->use_partial_copy == 0 )
    {
        lsf_diff[0] = 16384;
        move16();   /*Q15*/
        ...
        IF(NE_16(st_fx->last_extl_fx,SWB_TBE)&&NE_16(st_fx->last_extl_fx,FB_TBE))
        {
            FOR( i=1; i<LPC_SHB_ORDER-1; i++)
            {
                st_fx->prev_lsf_diff_fx[i-1] = shr(lsf_diff[i], 1);
                move16();
            }
        }
        ...
        IF( LE_32(st_fx->total_brate_fx,ACELP_16k40))
        {
            ...
            test();
            test();
            IF(!(GT_16(st_fx->prev_tilt_para_fx,5120)&&(EQ_16(coder_type,TRANSITION)||LT_16(tilt_para,1024)))&&
               !(((LT_16(st_fx->prev_tilt_para_fx,3072)  && GE_16(st_fx->prev_coder_type_fx,VOICED))) && GT_16(tilt_para,5120) ))
            {
                FOR( i = 1; i < LPC_SHB_ORDER-1; i++ )
                {
                    IF(LT_16(lsf_diff[i],st_fx->prev_lsf_diff_fx[i-1]))
                    {
                        tmp = mult(26214,lsf_diff[i]);

                        test();
                        IF( st_fx->prev_lsf_diff_fx[i-1] <= 0 || tmp < 0 ) /* safety check in case of bit errors */
                        {
                            st_fx->BER_detect = 1;
                            move16();
                            tmp = 0;
                        ...
                        ELSE
                        {
                            tmp = div_s(tmp,st_fx->prev_lsf_diff_fx[i-1]);
                        }
                    ...
                    ELSE
                    {
                        tmp = mult(26214,st_fx->prev_lsf_diff_fx[i-1]);
        ...
        Copy( lsf_diff + 1, st_fx->prev_lsf_diff_fx, LPC_SHB_ORDER-2 );
    ...
    find_max_mem_dec( st_fx, &n_mem, &n_mem2, &n_mem3 );  /* for >=24.4, use n_mem2 lpc_syn, shb_20sample, and mem_stp_swb_fx memory */
    ...
    IF( GT_32(st_fx->total_brate_fx, ACELP_32k))
    {
        FOR( j = 0; j < 4; j++ )
        {
    ...
    GenShapedSHBExcitation_fx( shaped_shb_excitation + L_SHB_LAHEAD, lpc_shb, White_exc16k,
                               ...
                               &(st_fx->fb_tbe_demph_fx), &Q_bwe_exc, &Q_bwe_exc_fb,Q_shb, n_mem2, st_fx->prev_Q_bwe_syn, st_fx->total_brate_fx, st_fx->prev_bfi_fx );
    ...
    *Q_white_exc = Q_bwe_exc_fb;
    ...
    prev_pow = L_shl(1407374848l/*0.00001f Q47*/, tmp);   /* 2*(Q_bwe_exc) */
    ...
    FOR(i = 0; i < NUM_SHB_SUBGAINS; i++)
    {
        ...
        IF( L_tmp != 0 )
        {
            ...
            L_ener = L_add(L_ener, L_shr(ener_tmp[i], 2));/* 2*Q_bwe_exc */
        }
    }
    ...
    IF( st_fx->bws_cnt_fx > 0 )
    {
        ...
        if( GT_16( st_fx->tilt_swb_fx, 16384 ))
        {
        ...
        IF( is_fractive == 0 )
        {
            IF( GT_16( st_fx->tilt_wb_fx, 2048 )) /*assuming st_fx->tilt_wb_fx in Q11*/
            {
                st_fx->tilt_wb_fx = 2048;
                move16();
            }
            ELSE IF( LT_16( st_fx->tilt_wb_fx, 1024 ))
            {
                st_fx->tilt_wb_fx = 1024;
                move16();
            }
            test();
            if( st_fx->prev_fractive_fx == 1 && GT_16( st_fx->tilt_wb_fx, 1024 ))
            {
                st_fx->tilt_wb_fx = 1024;
                move16();
            }
        ...
        ELSE
        {
            IF(GT_16(st_fx->tilt_wb_fx, 8192))
            {
                IF(st_fx->prev_fractive_fx == 0)
                {
        ...
        IF(ener != 0)
        {
            ...
            test();
            IF( GT_32(L_tmp, st_fx->enerLH_fx)) /*st_fx->Q_syn2*/
            {
        ...
        test();
        IF( EQ_16((is_fractive & st_fx->prev_fractive_fx), 1)&&GT_32(GainFrame,GainFrame_prevfrm_fx))
        {
        ...
        ELSE
        {
            ...
            test();
            test();
            IF((LT_32(L_shr(st_fx->prev_enerLH_fx, 1), st_fx->enerLH_fx)&& GT_32(st_fx->prev_enerLH_fx,L_shr(st_fx->enerLH_fx,1)))
            && (LT_32(L_shr(st_fx->prev_enerLL_fx, 1), st_fx->enerLL_fx)  && GT_32(st_fx->prev_enerLL_fx, L_shr(st_fx->enerLL_fx, 1)) ) && (is_fractive ^ st_fx->prev_fractive_fx) == 0)
            {
            ...
            {
                test();
                IF(is_fractive == 0 && EQ_16(st_fx->prev_fractive_fx, 1))
                {
    ...
    ELSE
    {
        ...
        IF(EQ_16(st_fx->nbLostCmpt, 1))
        {
            ...
            test();
            test();
            IF( NE_16(st_fx->clas_dec, UNVOICED_CLAS)&&NE_16(st_fx->clas_dec,UNVOICED_TRANSITION)&&LT_16(st_fx->tilt_swb_fec_fx,16384)&&
            ((GT_32(st_fx->enerLL_fx, L_shr(st_fx->prev_enerLL_fx, 1))  && LT_32(L_shr(st_fx->enerLL_fx, 1), st_fx->prev_enerLL_fx) )|| (GT_32(st_fx->enerLH_fx, L_shr(st_fx->prev_enerLH_fx, 1))  && LT_32(L_shr(st_fx->enerLH_fx, 1), st_fx->prev_enerLH_fx))))
            {
                ...
                ELSE IF(GT_32(L_shr(prev_ener_ratio_fx, 1), GainFrame))
                {
                ...
                test();
                IF(GT_16(tilt_swb_fec, st_fx->tilt_swb_fec_fx)&&st_fx->tilt_swb_fec_fx>0)
                {
            ...
            ELSE IF( (NE_16(st_fx->clas_dec, UNVOICED_CLAS)||GT_16(st_fx->tilt_swb_fec_fx,16384))&&GT_32(L_shr(prev_ener_ratio_fx,2),GainFrame)&&
        ...
        ELSE IF( GT_16(st_fx->nbLostCmpt, 1))
        {
            ...
            test();
            test();
            IF(GT_32(L_shr(prev_ener_ratio_fx, 2), GainFrame)&&((EQ_16(st_fx->codec_mode,MODE1)&&GT_32(st_fx->enerLL_fx,st_fx->prev_enerLL_fx)&&GT_32(st_fx->enerLH_fx,st_fx->prev_enerLH_fx))||EQ_16(st_fx->codec_mode,MODE2)))
            {
                test();
                IF( GT_16(tilt_swb_fec, 20480)&&GT_16(st_fx->tilt_swb_fec_fx,20480))
                {
            ...
            ELSE IF(GT_32(prev_ener_ratio_fx, GainFrame)&&((EQ_16(st_fx->codec_mode,MODE1)&&GT_32(st_fx->enerLL_fx,st_fx->prev_enerLL_fx)&&GT_32(st_fx->enerLH_fx,st_fx->prev_enerLH_fx))||EQ_16(st_fx->codec_mode,MODE2)))
            {
                test();
                IF( GT_16(tilt_swb_fec, 20480)&&GT_16(st_fx->tilt_swb_fec_fx,20480))
                {
    ...
    test();
    test();
    IF( ((GE_32(st_fx->total_brate_fx, ACELP_24k40)&&EQ_16(st_fx->prev_coder_type_fx,coder_type)&&NE_16(coder_type,UNVOICED))
         || (LE_32(st_fx->total_brate_fx, ACELP_16k40)  && (EQ_16(st_fx->prev_coder_type_fx, coder_type)  || (EQ_16(st_fx->prev_coder_type_fx, VOICED)  && EQ_16(coder_type, GENERIC) ) || (EQ_16(st_fx->prev_coder_type_fx, GENERIC)   && EQ_16(coder_type, VOICED) ))))
        && GT_16(pitch_fx, 4480 /*70 in Q6*/)  && LT_16(st_fx->extl_fx, FB_TBE) )
    {
        FOR(i=0; i<NUM_SHB_SUBGAINS; i++)
        {
    ...
    IF( tmp > 0 )  /* shifting prev */
    {
        ...
        st_fx->prev_swb_bwe_frame_pow_fx = L_shr(st_fx->prev_swb_bwe_frame_pow_fx, tmp);
        st_fx->prev_frame_pow_exp = curr_frame_pow_exp;
        move16();
    }
    ELSE            /* shifting curr */
    ...
    IF( !st_fx->bfi_fx && st_fx->prev_bfi_fx )
    {
        ...
        test();
        test();
        IF( ( GT_32( L_shr( curr_frame_pow, 1 ), st_fx->prev_swb_bwe_frame_pow_fx ))&&
            ( GT_32( st_fx->prev_swb_bwe_frame_pow_fx, L_tmp )  ) && EQ_16(st_fx->prev_coder_type_fx,UNVOICED)  )
        {
        ...
        FOR( j = 0; j < 8; j++ )
        {
            ...
            IF( temp > 0 )
            {
                /* scale <= temp, due to scale = sqrt( st->prev_swb_bwe_frame_pow_fx/curr_frame_pow ), temp = sqrt( scale, 1.f/8.f )
                   and curr_frame_pow > st->prev_swb_bwe_frame_pow_fx -> scale <= 1.0, sqrt(scale, 1.f/8.f) >= scale  */
                IF( LT_16( scale, temp ))
                {
                    scale = div_s( scale, temp );
                }
    ...
    IF( st_fx->bfi_fx )
    {
        ...
        IF (EQ_16(st_fx->nbLostCmpt,1))
        {
            ...
            test();
            test();
            IF( GT_32(curr_frame_pow, st_fx->prev_swb_bwe_frame_pow_fx)&&
                NE_16(st_fx->prev_coder_type_fx, UNVOICED) &&
                NE_16(st_fx->last_good_fx,UNVOICED_CLAS)  )
            {
            ...
            ELSE IF( LT_32(curr_frame_pow,  L_shr(st_fx->prev_swb_bwe_frame_pow_fx,1))&&EQ_16(st_fx->nbLostCmpt,1)&&
                     (GT_32(st_fx->enerLL_fx, L_shr( st_fx->prev_enerLL_fx,1)) || GT_32(st_fx->enerLH_fx, L_shr(st_fx->prev_enerLH_fx,1))  ) &&
                     (EQ_16(st_fx->prev_coder_type_fx ,UNVOICED)  || EQ_16(st_fx->last_good_fx, UNVOICED_CLAS)  || GT_16(st_fx->tilt_swb_fec_fx , 10240) ))
            {
                L_tmp = root_a_over_b_fx( st_fx->prev_swb_bwe_frame_pow_fx, curr_frame_pow_exp, curr_frame_pow, curr_frame_pow_exp, &exp );
                scale = round_fx( L_shl( L_tmp, sub(exp,3) ) ); /*Q12*/
                L_tmp = root_a_fx( L_tmp, 31 - exp, &exp );
        ...
        ELSE IF (GT_16(st_fx->nbLostCmpt,1))
        {
            ...
            ELSE IF( LT_32(curr_frame_pow,  L_shr(st_fx->prev_swb_bwe_frame_pow_fx,1))&&
                     (GT_32(st_fx->enerLL_fx, L_shr( st_fx->prev_enerLL_fx,1))  || GT_32(st_fx->enerLH_fx, L_shr(st_fx->prev_enerLH_fx,1)) ) &&
                     (st_fx->prev_coder_type_fx == UNVOICED || st_fx->last_good_fx == UNVOICED_CLAS || GT_16(st_fx->tilt_swb_fec_fx , 10240)) )
            {
                L_tmp = root_a_over_b_fx( st_fx->prev_swb_bwe_frame_pow_fx, curr_frame_pow_exp, curr_frame_pow, curr_frame_pow_exp, &exp );
        ...
        FOR( j = 0; j < 8; j++ )
        {
            ...
            IF( temp > 0 )
            {
                ...
                ELSE
                {
                    tmp1 =  sub(norm_s(scale), 1);
                    tmp2 = norm_s(temp);
                    scale = div_s( shl(scale, tmp1), shl(temp, tmp2) );
                    scale = shr(scale, add(sub(tmp1, tmp2), 3));
                }
            }
            ELSE
            {
...

    {
        ...
        FOR( i = 0; i < L_FRAME16k; i++ )
        {
            prev_ener_shb64 = W_mac0_16_16( prev_ener_shb64, shaped_shb_excitation[i], shaped_shb_excitation[i] ); /* Q0 */
        }
    ...
    IF( sc != 0 )
    {
        ...
        IF( st_fx->output_Fs_fx == 16000 )
        {
            Scale_sig(st_fx->mem_resamp_HB_32k_fx, 2*ALLPASSSECTIONS_STEEP + 1, sc);
        }
    }
    ...
    Copy( error + L_FRAME32k - L_SHB_TRANSITION_LENGTH, st_fx->old_tbe_synth_fx, L_SHB_TRANSITION_LENGTH );
    ...
    IF( EQ_32(st_fx->output_Fs_fx, 48000))
    {
        IF( EQ_32(st_fx->extl_fx,FB_TBE))
        {
            ...
            *Q_white_exc = sub(add(*Q_white_exc, tmp),13); /* *Q_white_exc + 18 + tmp -15 -16 */
            ...
            FOR(i=0; i<L_FRAME16k; i++)
            {
                White_exc16k[i] = shl(White_exc16k[i],sub(tmp,1));
                move16();
            }
    ...
    ELSE IF( EQ_32(st_fx->output_Fs_fx, 32000))
    {
        IF( NE_16(synth_scale_fx,32767)) /* 1.0 in Q15 */
        {
            FOR( i = 0; i < L_FRAME32k; i++ )
    ...
    ELSE IF( EQ_32(st_fx->output_Fs_fx, 16000))
    {
        IF( NE_16(synth_scale_fx,32767)) /* 1.0 in Q15 */
        {
            FOR( i = 0; i < L_FRAME32k; i++ )
    ...
    const   Word16 grid_in[]                /* i   : Input LSF smoohthing grid */ )
{
    ...
    FOR( i = 0; i < NUM_MAP_LSF; i++ )
    {
        lsf_smooth[i] = sub( mult_r( grid_smoothing_fx[i], grid[i] ),
                             mult_r( lsf_map[i], add( grid_smoothing_fx[i], -32768/*-1.0f Q15*/ ) ) );
        move16();
    }
...
)
{
    ...
    ELSE /* SWB TBE DEC */
    {
        ...
        test();
        test();
        IF( (st_fx->rf_flag == 0) && !((EQ_32(st_fx->total_brate_fx, ACELP_9k60))||((st_fx->total_brate_fx==0)&&((EQ_32(st_fx->last_total_brate_fx,ACELP_9k60))||(EQ_32(st_fx->last_total_brate_fx,ACELP_13k20)&&EQ_16(st_fx->rf_flag_last,1))))))

        {
            ...
            test();
            test();
            IF ( EQ_32(extl_brate, SWB_TBE_1k6)||EQ_32(extl_brate,FB_TBE_1k8)||EQ_32(extl_brate,SWB_TBE_2k8)||EQ_32(extl_brate,FB_TBE_3k0))
            {
                IF(EQ_16(st_fx->codec_mode,MODE2))
                {
            ...
            Dequant_lower_LSF_fx( lsf_idx, lsf_q );
...

{
    ...
    test();
    test();
    IF ( EQ_16(st->extl_fx,FB_TBE)&&!st->bfi_fx)
    {
    ...
    ELSE if ( EQ_16(st->extl_fx,FB_TBE)&&st->bfi_fx)
    {
    ...
    /* FB TBE synthesis */
    synthesise_fb_high_band_fx( fb_exc,Q_fb_exc,fb_synth,fb_exc_energy,ratio, st->L_frame_fx, st->bfi_fx, &(st->prev_fbbwe_ratio_fx), st->fbbwe_hpf_mem_fx, st->fbbwe_hpf_mem_fx_Q,hb_synth_exp);
...
)
{
    ...
    test();
    test();
    IF ( (EQ_16(st_fx->rf_flag,1)||EQ_32(st_fx->total_brate_fx,ACELP_9k60))&&EQ_16(st_fx->bwidth_fx,WB))
    {
        /* WB LSF */
    ...
    ELSE IF ( ( GE_32( st_fx->total_brate_fx, ACELP_9k60 ))&&(LE_32(st_fx->total_brate_fx,ACELP_32k))&&
              ( ( EQ_16( st_fx->bwidth_fx, SWB )  ) || ( EQ_16( st_fx->bwidth_fx, FB )  ) ) )
    {
        test();
        IF( (st_fx->rf_flag == 0) && (GT_32(st_fx->total_brate_fx, ACELP_9k60)))
        {
            FOR (i = 0; i < NUM_Q_LSF; i++)
            {
...
)
{
    ...
    {
        speech_buf_16k2[i]   = negate(speech_buf_16k2[i]);
        speech_buf_16k2[i+1] = speech_buf_16k2[i+1];
    }
    ...
    {
        interpolate_3_over_1_allpass_fx( output, L_FRAME16k, upsampled_synth, up_mem, allpass_poles_3_ov_2 );
...
)
{
    ...
    IF( EQ_16(bandwidth, WB))
    {
        ...
        wb_tbe_extras_reset_synth_fx( st_fx->state_lsyn_filt_shb_fx, st_fx->state_lsyn_filt_dwn_shb_fx, st_fx->state_32and48k_WB_upsample_fx, st_fx->mem_resamp_HB_fx );

        set16_fx( st_fx->mem_genSHBexc_filt_down_shb_fx, 0, 7 );
        set16_fx( st_fx->state_lpc_syn_fx, 0, 10 );
    ...
    ELSE IF( EQ_16(bandwidth, SWB)||EQ_16(bandwidth,FB))
    {
        ...
        set16_fx( st_fx->mem_resamp_HB_32k_fx, 0, 2*ALLPASSSECTIONS_STEEP+1 );
