...
)
{
    ...
    ELSE
    {
        ...
        IF ( frame_cnt == 0 && EQ_16(st->last_core_bs_fx, ACELP_CORE) )
        {
    ...
    test();
    IF ( (EQ_16(L_frame, shr(st->L_frame_fx, 1)))&&(st->tcxonly))
    {
        IGFDecUpdateInfo(
            &st->hIGFDec,
            IGF_GRID_LB_SHORT
    ...
    ELSE
    {
        test();
        test();
        IF ((EQ_16(st->last_core_fx, ACELP_CORE))||(left_rect&&st->bfi_fx))
        {
            IGFDecUpdateInfo(
                &st->hIGFDec,
                IGF_GRID_LB_TRAN
    ...
    }
    {
        ...
        ELSE IF (EQ_16(TCX_20_CORE, st->core_fx)||EQ_16(frame_cnt,1))
        {
    ...
    IF (bfi == 0 && st->tcxonly == 0)
    {
        ...
        IF (LT_32(st->total_brate_fx, ACELP_13k20)||st->rf_flag!=0)
        {
    ...
    IF (bfi==0 && (fac_ns > 0))
    {
        ...
        move16();
        test();
        if ( GE_16(st->bits_frame, 256)&&st->rf_flag==0)
        {
            tmp1 = 1;
            move16();
        }
    ...
    IF (st->enablePlcWaveadjust)
    {
        IF (bfi)
        {
            IF (EQ_16(st->nbLostCmpt, 1))
            {
                ...
                test();
                test();
                IF (EQ_16(st->plcInfo.Transient[0],1)
                    && EQ_16(st->plcInfo.Transient[1], 1)
                    && EQ_16(st->plcInfo.Transient[2], 1))
                {
                    Word16 sum_word16 = 0;
                    move16();
            ...
            if (GT_16(L_frameTCX, st->L_frameTCX))
            {
                st->plcInfo.concealment_method = TCX_TONAL;
                move16();
            }
    ...
    {
        test();
        IF( !st->enablePlcWaveadjust || EQ_16(temp_concealment_method, TCX_TONAL))
        {
            Word16 f, tmp;
            ...
            test();
            test();
            IF ( (frame_cnt == 0) && (EQ_16(L_frameTCX, shr(st->L_frameTCX, 1)))
                 && (st->tcxonly) && (!st->tonal_mdct_plc_active) && (EQ_16(st->nbLostCmpt, 1))
    ...
    ELSE IF (GT_16(L_spec, L_frameTCX))
    {
    ...
    test();
    test();
    IF ( bfi && (!st->enablePlcWaveadjust || EQ_16(temp_concealment_method, TCX_TONAL))
         &&  st->igf && (frame_cnt == 0) && (EQ_16(L_frameTCX, shr(st->L_frameTCX, 1)))
         && (st->tcxonly) && (!st->tonal_mdct_plc_active) && (EQ_16(st->nbLostCmpt, 1))
         && (tcx_cfg->tcx_last_overlap_mode != FULL_OVERLAP)
         && (tcx_cfg->tcx_curr_overlap_mode != FULL_OVERLAP) )
    {
    ...
    test();
    test();
    IF(!st->enablePlcWaveadjust || !bfi || (EQ_16(temp_concealment_method, TCX_TONAL)))
    {
        ...
        test();
        test();
        IF (( bfi != 0) && ( !st->enablePlcWaveadjust || EQ_16(temp_concealment_method, TCX_TONAL)))
        {
    ...
    *-----------------------------------------------------------*/
    test();
    test();
    IF (st->igf && !((EQ_16(L_frame, shr(st->L_frame_fx, 1)))&&(st->tcxonly)))
    {
        Word16 igfGridIdx;

        test();
        test();
        IF ((EQ_16(st->last_core_fx, ACELP_CORE))||(left_rect&&bfi))
        {
    ...
    IF(st->igf)
    {
        ...
        IF(st->hIGFDec.flatteningTrigger != 0 && fUseTns == 0)
        {
            ...
            {
                IF (EQ_16(chk_sparse[j-IGF_START_MN], 2))
                {
    ...
    test();
    IF ((EQ_16(L_frame, shr(st->L_frame_fx, 1)))&&(st->tcxonly!=0))
    {
        Word16 L = L_frameTCX;
        move16();
        ...
        test();
        test();
        if ((tcx_cfg->fIsTNSAllowed != 0 && fUseTns != 0 && bfi == 0) || (GT_16(L_spec, L_frameTCX)))
        {
            L = L_spec;
            move16();
        }
                     ...
                     )
{
    ...
    test();
    test();
    if (!st_fx->bfi_fx && st_fx->prev_bfi_fx && EQ_16(st_fx->last_core_fx,ACELP_CORE))
    {
                  ...
                  Word16 *acelp_zir)
{
    ...
    test();
    IF ((EQ_16(L_frameTCX, shr(st->L_frameTCX, 1)))&&(st->tcxonly!=0))
    {
        ...
        */
        ...
        IF ((bfi == 0 && tcx_cfg->tcx_last_overlap_mode != FULL_OVERLAP) || (bfi!=0 && (tcx_cfg->tcx_last_overlap_mode != FULL_OVERLAP) && (tcx_cfg->tcx_curr_overlap_mode != FULL_OVERLAP)))
        {
            ...
            move16();
            if (EQ_16(tcx_cfg->tcx_last_overlap_mode, MIN_OVERLAP))
            {
    ...
    ELSE   /* frame is TCX-20 or not TCX-only */
    {
        ...
        IF (NE_16(st->tcx_cfg.tcx_last_overlap_mode, TRANSITION_OVERLAP))
        {
    ...
    IF ((frame_cnt != 0) || (st->last_core_bfi > ACELP_CORE))
    {
        test();
        test();
        IF (((EQ_16(L_frameTCX, shr(st->L_frameTCX, 1)))&&(st->tcxonly!=0))||(EQ_16(st->tcx_cfg.tcx_last_overlap_mode,TRANSITION_OVERLAP)))
        {
            test();
            test();
            test();
            ...
            ELSE
            {
                ...
                move16();
                test();
                if ((index == 0) || (EQ_16(tcx_cfg->tcx_last_overlap_mode, MIN_OVERLAP)))
                {
    ...
    test();
    IF ( (aldo == 0) &&
         ((EQ_16(L_frameTCX, shr(st->L_frameTCX, 1))  && frame_cnt > 0) ||
          NE_16(L_frameTCX, shr(st->L_frameTCX, 1)) ) )
    {
        ...
        IF (EQ_16(tcx_cfg->tcx_curr_overlap_mode, FULL_OVERLAP))
        {
            FOR (i=0; i<nz; i++)
            {
