            ...
            ELSE					/* good frame  */
            {
                ...
                *-----------------------------------------------------------------*/
                ...
                IF( ( (GE_16(last_good,VOICED_TRANSITION)&&LT_16(last_good,INACTIVE_CLAS)&&(EQ_16(clas,UNVOICED_CLAS)||EQ_16(clas,INACTIVE_CLAS)))||
                      EQ_32(last_core_brate,SID_1k75)  || EQ_32(last_core_brate,SID_2k40)  || EQ_32(last_core_brate,FRAME_NO_DATA) ) && prev_bfi )
                {
        ...
        IF( prev_bfi && NE_16(coder_type,TRANSITION))
        {
            IF( L_enr_q == 0 )
            {
                ...
                ELSE IF( GE_16(last_good,VOICED_TRANSITION)&&LT_16(last_good,INACTIVE_CLAS)&&GE_16(clas,VOICED_TRANSITION)&&LT_16(clas,INACTIVE_CLAS))
                {
                    ...
                    {
                        test();
                        IF( avoid_lpc_burst_on_recovery && GT_16(enr_LP, 160))
                        {
                            exp = norm_s(enr_LP);
                            tmp = shl(enr_LP, exp);
                ...
                test();
                test();
                IF( (GE_16(last_good,VOICED_TRANSITION)&&LT_16(last_good,INACTIVE_CLAS)&&GE_16(clas,VOICED_TRANSITION)&&LT_16(clas,INACTIVE_CLAS))
                    || force_scaling )
                {
