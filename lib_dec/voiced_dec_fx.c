...
)
{
    ...
    pl = s_min(rint_new_fx(st_fx->old_pitch_buf_fx[(2*NB_SUBFR)-1]),MAX_LAG_PIT);
    delta_lag_D = (Word16) get_next_indice_fx( st_fx, 5 );


    l = s_min(MAX_LAG_PIT,add(pl,sub(delta_lag_D,11)));
    ...
    IF (NE_16(temp_pl_fx,temp_l_fx))
    {
        FOR(k=0; k<NB_SUBFR; k++)
        {
            ...
            pitch[k] = s_min(MAX_LAG_PIT << 6, s_max(19 << 6, shl(interp_delay_fx[0], 2)));
            move16();/*Q6 */
        }
    }
    ELSE
    {
        set16_fx(pitch, s_min(MAX_LAG_PIT << 6, s_max(19 << 6, shl(temp_l_fx,6))), NB_SUBFR);/*Q6 */
    }
    ...
    test();
    if (LE_16(shl(pl,1),PIT_MAX)&&LE_16(pl,temp))
    {
        pl = shl(pl,1);
    }

    /* Restoring PPP memories when the last frame is non-PPP or full-rate PPP */
    IF (NE_16(st_fx->last_ppp_mode_dec_fx,1))
    {
    ...
    IF( CURRP_Q_D_FX->lag_fx <= 0 )
    {
        CURRP_Q_D_FX->lag_fx = 1;
        move16();
        st_fx->BER_detect = 1;
        move16();
    }
    ...
    IF(GE_16(sub(upper_cut_off_freq,shr((Word16)L_mult(diff,CURRP_Q_D_FX->nH_fx),1)),diff))
    {
