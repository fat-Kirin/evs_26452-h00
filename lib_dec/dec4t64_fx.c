...
)
{
    ...
    IF ( !Opt_AMR_WB )
    {
        ...
        D_ACELP_indexing( code, config, NB_TRACK_FCB_4T, indexing_indices, &st_fx->BER_detect );
    }
    ELSE
    {
        FOR (i=0; i<L_SUBFR; i++)
...
)
{
    ...
    {
        IF (LT_32((*code_index),PI_offset_fx[pulse_num][i]))
        {
            BREAK;
        }
    }
    ...
    IF(LT_32(j,PI_select_table_fx[16][pulse_pos_num]))
    {
        k = L_deposit_l(0);
        mn9 = L_add(j, 0);
    }
    ELSE
    {
        L_tmp = L_deposit_l(0);
        L_tmp1 = L_add(j, 0);
        WHILE(GE_32(L_tmp1,PI_select_table_fx[16][pulse_pos_num]))
        {
            L_tmp = L_add(L_tmp,1);
    ...
    test();
    IF ( (LT_16( pulse_pos_num,pulse_num))&&(GT_16(pulse_pos_num,1)))
    {
...
)
{
    ...
    FOR (i=0; i<pos_num-1; i++)
    {
        ...
        if (GT_32(k,PI_select_table_fx[L_sub(17,l)][temp]))
        {
            l = L_sub(l,1);
        }
