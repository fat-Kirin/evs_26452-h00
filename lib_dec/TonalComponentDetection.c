                           ...
                           Word16 * powerSpec_exp)           /* o: exponent of estimated power spectrum */
{
    ...
    *powerSpec_exp = add(add(mdctSpec_exp, mdctSpec_exp), 5);
    ...
    IF (LT_16(add(16-3, norm_s(floorPowerSpectrum)), k)) /*extra 3 bits of headroom for MA filter in getEnvelope*/
    {
                  ...
                  Word16 /*float*/        * const   pF0)            /*o   -  Q10*/
{
    ...
    /* Use only F0 >= 100 Hz */
    test();
    IF ((pitchLag > 0) && (LE_16(round_fx(pitchLag) , shr(nSamplesCore,1))))
    {
                      ...
                      Word16  /*float*/ *     pF0)              /*I/O  - Q10 range: {0}, [4..18) */
{
    ...
    IF (F0 > 0 && nHarmonics != 0)
    {
        ...
        i = 1;
        move16();
        IF (EQ_16(imult1616(sortedDiff[0],pHarmonicIndexes[0]),1))
        {
            /* Find how many distances between peaks have length 1 */
            FOR (; i < tmp; i++)
            {
                if (EQ_16(sortedDiff[i],1))
                {
        ...
        IF (LT_16(nSameDiff,3))
        {
            /* Find the most common difference */
            FOR (i = nSameDiff; i < tmp; i++)
            {
                IF (EQ_16(sortedDiff[i], sortedDiff[i-1]))
                {
                    nSameDiff=add(nSameDiff,1);
                }
                ELSE
                {
                    IF (GT_16(nSameDiff, nMostCommonDiff))
                    {
                        nMostCommonDiff = nSameDiff;
                        move16();
                    ...
                    ELSE {
                        test();
                        IF (EQ_16(nSameDiff, nMostCommonDiff)&&(abs_s(sub(iMostCommonDiff,pHarmonicIndexes[0]))>abs_s(sub(sortedDiff[i-1],pHarmonicIndexes[0]))))
                        {
                            nMostCommonDiff = nSameDiff;
                            move16();
                            iMostCommonDiff = sortedDiff[i-1];
        ...
        IF (GE_16(nMostCommonDiff, MAX_PEAKS_FROM_PITCH/2))
        {
            iMult = 1;
            move16();
            FOR (i = 0; i < tmp; i++)
            {
                IF (EQ_16(diff[i], iMostCommonDiff))
                {
                ...
                test();
                test();
                IF (GT_16(sub(nHarmonics,2),i)&&(EQ_16(diff[i],diff[i+1]))&&(EQ_16(add(diff[i],diff[i+1]),iMostCommonDiff)))
                {
                    iMult = pHarmonicIndexes[i];
                    move16();
                    BREAK;
                          ...
                          )
{
    ...
    {
        IF (GT_32(smoothedSpectrum[k],envelope[k]))
        {
                                       ...
                                       Word16 * thresholdModification)
{
    ...
    if (LT_32(L_tmp, 16384l/*0.25f Q16*/))
    {
        pitchIsStable = 1;
        move16();
    }
                                ...
                                Word16 * thresholdModification)  /* IN */
{
    ...
    WHILE ( LE_16(k, tmp_loop1))
    {
        ...
        IF (GT_32(L_shr(smoothedSpectrum[k], 1), L_shl(Mpy_32_16_1(envelope[k]/*Q28,powerSpec_exp*/, thresholdModification[k]/*Q10*/), 4)))
        {
            /* The check that bin at k is bigger than bins at k-1 and k+1 is needed to avoid deadlocks when the thresholds are low. */
            /* It removes some true peaks, especially if non weighted sum is used for the smoothed spectrum. */
            biggerNeighbor = L_max(powerSpectrum[k-1], powerSpectrum[k+1]);
