...
void reconfig_decoder_LPD( Decoder_State_fx *st, Word16 bits_frame, Word16 bandwidth_mode, Word32 bitrate, Word16 L_frame_old)
{
...

    {
        ...
        test();
        IF ((LT_16(bandwidth_mode,SWB))&&!(st->tcxonly))
        {
            ...
            FOR (i = 0 ; i < scaleTableSize ; i++)
            {
                ...
                IF ( (EQ_16(bandwidth_mode,scaleTcxTable[i].bwmode)) &&
                     (GE_32 (bitrate,scaleTcxTable[i].bitrateFrom) ) &&
                     (LT_32 (bitrate,scaleTcxTable[i].bitrateTo) )
                   )
                {
                    if( st->rf_flag )
                    {
    ...
    IF ( st->ini_frame_fx != 0 )
    {
        ...
        test();
        test();
        IF( NE_16 (st->fscale,st->fscale_old)
           ...
           correct sampling rate in open_decoder_LPD() */
        {
            ...
            test();
            test();
            IF( (st->prev_bfi_fx && EQ_16(st->last_core_bfi,ACELP_CORE))||EQ_16(st->last_core_fx,ACELP_CORE))
            {
            ...
            test();
            IF( st->prev_bfi_fx && EQ_16(st->last_core_bfi,ACELP_CORE))
            {
        ...
        {
            IF (LE_16(st->last_L_frame_fx,L_FRAME16k))
            {
