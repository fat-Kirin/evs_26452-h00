...
)
{
    ...
    test();
    test();
    IF((EQ_32(last_core_brate, SID_1k75)||EQ_32(last_core_brate,ACELP_6k60)||EQ_32(last_core_brate,ACELP_8k85)
        || EQ_32(last_core_brate, ACELP_12k65) || EQ_32(last_core_brate, ACELP_14k25) || EQ_32(last_core_brate, ACELP_15k85)
        || EQ_32(last_core_brate, ACELP_18k25) || EQ_32(last_core_brate, ACELP_19k85) || EQ_32(last_core_brate, ACELP_23k05)
        || EQ_32(last_core_brate, ACELP_23k85)) && !Opt_AMR_WB && EQ_16(codec_mode, MODE2)
       && GT_16(L_frame,L_FRAME) )
    {
    ...
    IF(EQ_16(codec_mode, MODE2))
    {
        ...
        ELSE /*synthesis is bigger, scale it down*/
        {
            ...
            test();
            IF(tmp_scale_syn > 0 || EQ_16(mode , 1/*CLASSIFIER_TCX*/)) /*dont scale up syn, but scale mem_syn, adequately*/
            {
    ...
    *Q_mem_syn  = add(add(Q_syn, tmp_scale_syn), tmp_scale);
    ...
    test();
    IF(EQ_16(codec_mode,MODE2)&&EQ_16(mode,1/*CLASSIFIER_TCX*/))
    {
    ...
    *------------------------------------------------------------------------*/
    ...
    test();
    test();
    IF (( EQ_16(codec_mode , MODE1)&&(LT_32(bitrate,ACELP_11k60)
                                            || LE_16(coder_type , UNVOICED)  || Opt_AMR_WB)) ||
        (EQ_16(codec_mode , MODE2) && NE_16(bfi,1)  && !tcxonly ))

    {
        ...
        *------------------------------------------------------------------------*/
        ...
        ELSE IF( EQ_16(coder_type,INACTIVE)&&!Opt_AMR_WB)
        {
            *clas = INACTIVE_CLAS;
            move16();
        }
        ELSE
        {
            ...
            *------------------------------------------------------------------------*/
            ...
            test();
            test();
            IF ( EQ_16(codec_mode , MODE2)&&tmp16!=0&&EQ_16(mode,CLASSIFIER_TCX))
            {
            ...
            if ( (NE_16(coder_type,VOICED)&&LT_32(bitrate,ACELP_11k60))||Opt_AMR_WB)
            {
                ...
                case VOICED_TRANSITION:
                    IF(LT_16(fmerit1, 12780/*0.39f Q15*/))
                    {
                        ...
                        move16();
                                        ...
                                        ELSE IF(LT_16(fmerit1, 20644/*0.63f Q15*/)
                            && (LT_16(ener,-3840)  || EQ_16(codec_mode,MODE2)))
                    {
                ...
                case INACTIVE_CLAS:
                    IF( GT_16(fmerit1, 18350/*0.56f Q15*/))
                    {
                        result = ONSET;
                        move16();
                    }
                    ELSE IF( GT_16(fmerit1, 14746/*0.45f Q15*/))
                    {
        ...
        IF(EQ_16(codec_mode,MODE1))
        {
            ...
            test();
            test();
            if( GT_16(*decision_hyst,16)&&LT_16(*clas,VOICED_CLAS)&&EQ_16(coder_type,AUDIO))
            {
                *clas = VOICED_CLAS;
                move16();
            }
        ...
        IF( EQ_16(*clas,VOICED_CLAS))
        {
            ...
            test();
            test();
            IF(    (EQ_16(codec_mode,MODE2)&&EQ_16(coder_type,VOICED))
                   || (EQ_16(codec_mode,MODE1) && (Opt_AMR_WB || (NE_16(coder_type,GENERIC) && NE_16(coder_type,TRANSITION) ) ) )
              )
            {
        ...
        IF(EQ_16(codec_mode, MODE1))
        {
             ...
             *---------------------------------------------------------------------------------*/
            test();
            if( EQ_16(coder_type, INACTIVE)&&NE_16(*clas,INACTIVE_CLAS))
            {
                *clas = UNVOICED_CLAS;
                move16();
            }
            ...
            IF( Opt_AMR_WB )
            {
                *locattack = 0;
                ...
                test();
                IF( EQ_16(*clas, UNVOICED_CLAS)&&NE_16(coder_type,INACTIVE))
                {
                ...
                *amr_io_class = *clas;
                ...
                test();
                test();
                if ( (GT_16(diff_ener, 6*256/*Q8*/)&&EQ_16(*clas,AUDIO_CLAS))||GT_16(diff_ener,9*256/*Q8*/))
                {
                    *locattack = 1;
                    move16();
                }
                ...
                IF( NE_16(coder_type, INACTIVE))
                {
                    ...
                    test();
                    IF ( EQ_16(*amr_io_class, AUDIO_CLAS)&&GT_16(tmp16,5*256/*Q8*/))
                    {
                        *amr_io_class = *clas;
                        move16();
...
)
{
    ...
    IF( NE_16(coder_type,VOICED))
    {
        ...
        ELSE IF( EQ_16(tmpS,1))
        {
            IF( GE_16(last_good,VOICED_TRANSITION))
            {
    ...
    test();
    test();
    IF( GE_32(bitrate,ACELP_14k80)&&NE_16(coder_type,TRANSITION)&&LT_16(coder_type,AUDIO))
    {
...
)
{
    ...
    T0 = 0;
    move16();
    IF( GT_16(coder_type,UNVOICED))
    {
        /* decode the clas and energy information */
        IF( LT_16(coder_type,AUDIO))
        {
            *clas = FEC_dec_class_fx( st_fx, core_brate, coder_type, enr_q, last_good);
            ...
            test();
            test();
            IF( EQ_16(coder_type,GENERIC)&&EQ_16(*clas,VOICED_CLAS)&&(LE_16(last_good,UNVOICED_CLAS)||EQ_16(last_good,INACTIVE_CLAS)))
            {
                *clas = SIN_ONSET;
                move16();
            }
        ...
        test();
        IF( EQ_16(coder_type,GENERIC)&&GT_32(core_brate,ACELP_24k40))
        {
            nBits = 0;
            move16();
