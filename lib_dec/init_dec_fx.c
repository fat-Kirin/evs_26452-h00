...
)
{
    ...
    move16();
    st_fx->last_total_brate_ber_fx = -1;
    move32();
    ...
    /* HF synth init */
    hf_synth_amr_wb_init_fx( &st_fx->prev_r_fx, &st_fx->fmerit_w_sm_fx, st_fx->mem_syn_hf_fx,
    ...
    set16_fx( st_fx->mem_hp_interp_fx, 0, INTERP_3_1_MEM_LEN );
    ...
    set16_fx( st_fx->last_bitallocation_band_fx, 0, 6 );
    ...
    move16();
    st_fx->lp_ener_FEC_av = 500000;
    move32();
    st_fx->lp_ener_FEC_max = 500000;
    move32();
    st_fx->prev_bfi_fx = 0;
    move16();
    ...
    move16();
    st_fx->ho_sid_bw_fx = L_deposit_l(0);
    ...
    set16_fx( st_fx->old_out_LB_fx, 0, L_FRAME32k );
    ...
    move16();
    set32_fx( st_fx->oldIMDCTout_fx, 0, L_FRAME8k/2 );
    ...
    set16_fx(st_fx->Norm_gain_fx, 1, SFM_N_NB);  /*check if it is in Q0  */
    ...
    move16();
    set16_fx(st_fx->mem_norm_fx+1, 39, SFM_N_ENV_STAB-1);
     ...
     * HQ FEC
     *----------------------------------------------------------------------------------*/
    st_fx->old_synthFB_fx = st_fx->synth_history_fx + NS2SA_fx2(st_fx->output_Fs_fx, PH_ECU_MEM_NS);
    ...
    move16();
    set16_fx( st_fx->oldgapsynth_fx, 0, L_FRAME48k );
    ...
    IF( EQ_32(st_fx->output_Fs_fx,48000))
    {
        ...
        set16_fx( st_fx->fbbwe_hpf_mem_fx_Q, 0, 4 );
    }

    set16_fx( st_fx->mem_resamp_HB_fx, 0, INTERP_3_1_MEM_LEN );
    ...
    st_fx->tilt_mem_fx = 0;
    move16();
    set16_fx( st_fx->prev_lsf_diff_fx, 16384, LPC_SHB_ORDER-2 );
    ...
    set16_fx( st_fx->shape1_filt_mem_dec_fx, 0, 10 );
    set16_fx( st_fx->shape2_filt_mem_dec_fx, 0, 10 );
    set16_fx( st_fx->shape3_filt_mem_dec_fx, 0, 10 );
    /* IGF */
    st_fx->igf = 0;
    move16();
