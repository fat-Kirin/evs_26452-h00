...
{
    test();
    IF ( (st->tcx_hm_LtpPitchLag > 0) && GT_16(st->tcxltp_gain, kLtpHmGainThr))
    {
...
)
{
    ...
    *hm_bits = 0;
    ...
    test();
    IF( !(EQ_16(coder_type,VOICED)||EQ_16(coder_type,GENERIC)))
    {
    ...
    *hm_bits = add(sub(NumTargetBits,  targetBits), 1);
    ...
    UnmapIndex(
        ...
        (L_frame_m_256 >= 0),
        LtpPitchLag,
        (( LE_16(NumTargetBits,kSmallerLagsTargetBitsThreshold) ) || ( L_frame_m_256 < 0 )),
        &fract_res,
        &lag
    );
    test();
    ...
    /* Dequantize gain */
    IF( tcx_hm_dequantize_gain((Word16)EQ_16(coder_type,VOICED),prm_hm[2],&gain))
    {
        /* A bit error was encountered */
        *hm_bits = -1;
        return;
