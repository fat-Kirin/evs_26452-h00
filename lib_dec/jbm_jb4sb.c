...
Word16 JB4_PushDataUnit( JB4_HANDLE h, JB4_DATAUNIT_HANDLE dataUnit, Word32 rcvTime )
{
    ...
    IF( h->firstDataUnitPopped &&
        GE_32(JB4_rtpTimeStampDiff(h->lastReturnedTs, dataUnit->timeStamp),
              L_mult0(50 * 3, extract_l(dataUnit->duration))))
    {
    ...
    {
        IF(EQ_16(dataUnit->partialCopyOffset, 0))
        {
        ...
        ELSE IF(EQ_16(dataUnit->partialCopyOffset, 2))
        {
        ...
        ELSE IF(EQ_16(dataUnit->partialCopyOffset, 3))
        {
            h->rfOffset2Active = 0;
            move16();
        ...
        ELSE IF(EQ_16(dataUnit->partialCopyOffset, 5))
        {
            h->rfOffset2Active = 0;
            move16();
            h->rfOffset3Active = 0;
        ...
        ELSE IF(EQ_16(dataUnit->partialCopyOffset, 7))
        {
            h->rfOffset2Active = 0;
            move16();
            h->rfOffset3Active = 0;
    ...
    IF(dataUnit->partial_frame != 0)
    {
        ...
        IF( h->firstDataUnitPopped )
        {
            IF( LE_16(dataUnit->partialCopyOffset, 3)&&JB4_rtpTimeStampDiff(h->nextExpectedTs,dataUnit->timeStamp)<0)
            {
                JB4_FreeDataUnit(h, dataUnit);
                return 0;
            }
            ELSE IF( EQ_16(dataUnit->partialCopyOffset, 5)&&LT_32(JB4_rtpTimeStampDiff(h->nextExpectedTs,dataUnit->timeStamp),-40))
            {
                JB4_FreeDataUnit(h, dataUnit);
                return 0;
            }
            ELSE IF( EQ_16(dataUnit->partialCopyOffset, 7)&&LT_32(JB4_rtpTimeStampDiff(h->nextExpectedTs,dataUnit->timeStamp),-80))
            {
                JB4_FreeDataUnit(h, dataUnit);
                return 0;
            }
        ...
        IF(JB4_INPUTBUFFER_Enque( h->inputBuffer, dataUnit, (void**)&droppedDataUnit ) == 0)
        {
            ...
            IF( LE_16(dataUnit->partialCopyOffset, 3))
            {
                        ...
                        JB4_DATAUNIT_HANDLE *pDataUnit, Word16 *scale, Word16 *maxScaling )
{
    ...
    if( GT_32(sysTime, L_add(h->prevPopSysTime, h->frameDuration)))
    {
                                    ...
                                    Word32 extBufferedTime, Word16 *scale, Word16 *maxScaling )
{
    ...
    IF( GT_32(currPlayoutDelay, targetMax))   /* time shrinking */
    {
        ...
        IF( gap > 0 &&
            ...
            ( convertToLateLoss ||
              GT_32(L_add(L_add(buffered, h->frameDuration), extBufferedTime), targetMax) ) )
        {
    ...
    ELSE   /* time stretching */
    {
        ...
        IF( LE_32(L_add(delayWithClearedExternalBuffer, h->frameDuration), targetMaxStretch)&&
        LT_32(currPlayoutDelay, targetMaxStretch)  && LT_32(currPlayoutDelay, L_add(110, L_shr(h->rfDelay, 2))) )
        {
            *scale = 120;
            move16();
...
static void JB4_adaptDtxPlayout( JB4_HANDLE h, Word32 sysTime, Word16 *stretchTime )
{
    ...
    IF( !JB4_INPUTBUFFER_IsEmpty( h->inputBuffer ) )
    {
        ...
        IF( !firstDu->silenceIndicator )
        {
            ...
            /*  decided between shrinking/stretching */
            IF( GT_32(currPlayoutDelay, L_add(targetStartUp, headRoom)))   /* time shrinking */
            {
            ...
            ELSE IF( LT_32(L_add(currPlayoutDelay, headRoom), targetStartUp))   /* time stretching */
            {
                *stretchTime = true;
                move16();
            }
    ...
    /*  decided between shrinking/stretching */
    IF( GE_32(currPlayoutDelay, L_add(targetDtx, h->frameDuration)))   /* time shrinking */
    {
        IF( JB4_checkDtxDropping( h ) )
        {
            JB4_dropFromBuffer( h, sysTime );
    ...
    ELSE IF( LT_32(L_add(currPlayoutDelay, L_shr(h->frameDuration, 1)), targetDtx))   /* time stretching */
    {
        *stretchTime = true;
        move16();
    }
...
static void JB4_adaptFirstPlayout( JB4_HANDLE h, Word32 sysTime, Word16 *prebuffer )
{
    ...
    IF(LT_32(targetStartUp, h->frameDuration))
    {
        return;
    }
    ...
    IF( LT_32(L_add(currPlayoutDelay, L_shr(h->frameDuration, 1)), targetStartUp))   /* time stretching */
    {
        *prebuffer = true;
        move16();
    }
...
static void JB4_popFromBuffer( JB4_HANDLE h, Word32 sysTime, JB4_DATAUNIT_HANDLE *pDataUnit )
{
    ...
    move16();
    test();
    IF ( ( GT_32(h->totWin , 3000))||(GT_32(h->FecOffWinLen,100)))
    {
    ...
    IF( tsDiff < 0 )
    {
        ...
        FOR ( i=0; i < readlen; i++)
        {
            ...
            IF ( tempDataUnit->partial_frame == 0 && h->lastPoppedWasSilence == 0 )
            {
                ...
                test();
                IF ( frameoffset > 0  && (LT_32( frameoffset, MAXOFFSET)))
                {
    ...
    *pDataUnit = nextDataUnit;
    ...
    IF( EQ_16(h->pre_partial_frame,1)||EQ_16(nextDataUnit->partial_frame,1))
    {
        ...
        FOR(searchpos = 0; searchpos < endpos; searchpos++)
        {
            ...
            IF ( EQ_32(partialCopyDu->timeStamp,L_add(nextDataUnit->timeStamp,partialCopyDu->duration)))
            {
    ...
    IF ( EQ_16(nextDataUnit->partial_frame,1))
    {
        ...
        const JB4_INPUTBUFFER_ELEMENT arrayElement, Word16 *replaceWithNewElementIfEqual )
{
    ...
    *replaceWithNewElementIfEqual = 0;
    ...
    ELSE   /* equal timestamps */
    {
        ...
        ELSE IF(EQ_16(newDataUnit->partial_frame, arrayDataUnit->partial_frame)&&
        GT_32(newDataUnit->dataSize, arrayDataUnit->dataSize))
        {
