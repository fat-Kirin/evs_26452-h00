                             ...
                             Word32 *maxConv, Word16 *maxConv_bits, Word16 *pitch)
{
    ...
    FOR (t = s; t < e; t++)
    {
        ...
        IF (GT_16(t,s))                               /* don't use the first value */
        {
            ...
            IF((GT_32(r1_high, r2_high))||
               (EQ_32(r1_high, r2_high)  && GT_32((Word32)r1_low, (Word32)r2_low) ))
            {
            ...
            ELSE
            {
                ...
                move16();
                move16(); /* moves are for the (Word32) casts */
                IF((LT_32(r1_high, r2_high))||
                (EQ_32(r1_high, r2_high) && LT_32((Word32)r1_low, (Word32)r2_low)))
                {
                    /* otherwise */
                     ...
                     Word32 ener_mean, Word16  pitch, Word16  Framesize)
{
    ...
    test();
    test();
    IF (LT_32(ener, L_shl(50,8))||(LT_32(ener,L_sub(ener_mean,L_shl(8,8)))
                                         && LT_16(cov_max, 29491)  ))
    {
        flag = 0;
        move16();
    }
    ELSE IF (GT_16(cov_max, 26214))
    {
        flag = 1;
        move16();
    }
    ELSE IF (GT_16(zp, 100))
    {
        flag = 0;
        move16();
    }
    ELSE IF (LT_32(ener, L_sub(ener_mean,L_shl(6,8))))
    {
        flag = 0;
        move16();
    }
    ELSE IF (GT_32(ener, L_add(ener_mean, L_shl(1,8)))&&GT_16(cov_max,19661))
    {
        flag = 1;
        move16();
    }
                       ...
                       )
{
    ...
    *voicing = 0;
    ...
    IF (LE_16(Framesize, 256))
    {
        IF (GT_16(zp_current, 70))
        {
            return 0;
        }
    }
    ...
    IF (LE_16(Framesize, 320))
    {
        test();
        IF ((0==tilt_enr2) ||
            (LT_32(tilt_enr1, L_shr(tilt_enr2, 1)) ))
        {
            return 0;
        }
    }
    ELSE
    {
        test();
        IF ((0==tilt_enr2) ||
        (LT_32(tilt_enr1, Mpy_32_16_1(tilt_enr2, 22938)) ))
        {
            return 0;
        }
    }
                            ...
                            Word16  n, Word16  pitch, Word16  Framesize)
{
    ...
    n2 = Framesize_sub_n;
    move16();
    if(LT_16(Loverlap, Framesize_sub_n))
    {
        n1 = Loverlap;
        move16();
    }
    if(LT_16(pitch125, Framesize_sub_n))
    {
        n2 = pitch125;
        move16();
    }
                        ...
                        Word16 pitch)
{
    ...
    test();
    test();
    IF (T_bfi && (LE_16(pitch , Framesizediv2))
        && (GT_16(Framesize ,256)) && (EQ_16(curr_mode , 1)))
    {
        Word16 i1 = 0, i2 = 0;
        ...
        test();
        test();
        IF ((LT_16(pos1,pos2))&&(GT_16(pos3,pitch))&&(LT_16(pos1,Framesizediv2)))
        {
...
void concealment_signal_tuning_fx(Word16 bfi, Word16 curr_mode, Word16 *outx_new_fx, void *_plcInfo, Word16 nbLostCmpt, Word16 pre_bfi, Word16 *OverlapBuf_fx, Word16 past_core_mode, Word16 *outdata2_fx, Decoder_State_fx *st)
{
    ...
    ELSE
    {
        ...
        IF (pre_bfi &&
        past_core_mode != 0 &&
        GE_32(st->last_total_brate_fx, 48000)  &&
