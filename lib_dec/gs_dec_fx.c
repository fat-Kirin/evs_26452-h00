...
)
{
    ...
    test();
    IF( st_fx->GSC_noisy_speech_fx && NE_16(st_fx->bwidth_fx,SWB))
    {
        st_fx->BER_detect = 1;
        move16();
    ...
    IF( LT_32(st_fx->core_brate_fx,CFREQ_BITRATE))
    {
        ...
        move16();
        test();
        if( LT_32(st_fx->core_brate_fx,ACELP_9k60)&&(EQ_16(coder_type,INACTIVE)))
        {
            nbits = 1;
            move16();
        }
    ...
    test();
    IF( LT_32(st_fx->core_brate_fx,ACELP_9k60)&&NE_16(coder_type,INACTIVE))
    {
        pit_band_idx = 1;
        move16();
    }
...
)
{
    ...
    test();
    IF( st_fx->bfi_fx || st_fx->BER_detect )
    {
        /* copy old gain */
    ...
    *last_bin = 0;
    ...
    test();
    IF( (EQ_16(st_fx->bfi_fx,1))||st_fx->BER_detect)
    {
        /*--------------------------------------------------------------------------------------*
         * Copy old spectrum
