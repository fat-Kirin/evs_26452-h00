...
)
{
    ...
    IF ( EQ_32(core_brate,SID_1k75)||EQ_32(core_brate,SID_2k40))
    {
        ...
        ELSE
        {
            lsf_dec_fx( st_fx, 0,  L_frame, INACTIVE, -1, Aq, &LSF_Q_prediction, lsf_new, lsp_new, 0 );
            {
                Word16 enr_new, Aq_tmp[M + 1];
    ...
    *allow_cn_step = 0;
    ...
    IF( EQ_32(core_brate,SID_1k75)||EQ_32(core_brate,SID_2k40))
    {
        ...
        IF( LE_32(st_fx->last_core_brate_fx, SID_2k40) || EQ_16(st_fx->prev_bfi_fx, 1) )
        {
            tmp1 = add(st_fx->old_enr_index_fx, 20);
        }
        ELSE
        {
            tmp1 = add(st_fx->old_enr_index_fx, 40);
        }
        IF( GT_16(L_enr_index, tmp1) && st_fx->old_enr_index_fx >= 0 )  /* Likely bit error and not startup */
        {
            L_enr_index = tmp1; move16();
        ...
        test();
        test();
        if (GT_32(st_fx->last_core_brate_fx,SID_1k75)&&
                st_fx->first_CNG_fx != 0 &&
                st_fx->old_enr_index_fx >= 0 &&
                GT_16(L_enr_index, add(st_fx->old_enr_index_fx,1)))
        {
            *allow_cn_step = 1;
            move16();
        }
    ...
    ELSE
    {
        ...
        FOR( ll = burst_ho_cnt; ll > 0; ll-- )
        {
            ...
            test();
            test();
            IF( ( EQ_16(L_frame,L_FRAME16k)&&st_fx->ho_16k_lsp_fx[s_ptr]==0)||(EQ_16(L_frame,L_FRAME)&&EQ_16(st_fx->ho_16k_lsp_fx[s_ptr],1)))
            {
    ...
    test();
    IF( EQ_32(st_fx->core_brate_fx, SID_1k75)||EQ_32(st_fx->core_brate_fx,SID_2k40))
    {
        /* Update hangover memory during CNG */
        test();
        IF ( *allow_cn_step == 0 && LT_32(st_fx->Enew_fx,L_add(st_fx->lp_ener_fx,L_shr(st_fx->lp_ener_fx,1))))
        {
            ...
            move32();
            test();
            IF ( EQ_32(core_brate,SID_2k40)&&*sid_bw==0)
            {
...
)
{
    ...
    IF ( st_fx->core_brate_fx == FRAME_NO_DATA || EQ_32(st_fx->core_brate_fx, SID_2k40))
    {
        ...
        test();
        IF (st_fx->cng_type_fx == LP_CNG && EQ_16(st_fx->extl_fx, SWB_CNG))
        {
    ...
    ELSE
    {
        ...
        if ( GT_16(st_fx->burst_cnt_fx, 10))
        {
            st_fx->burst_cnt_fx = 0;
            move16();
        }
...
)
{
    ...
    {
        test();
        IF ( EQ_32(st_fx->core_brate_fx, SID_2k40)&&EQ_16(sid_bw,1))
        {
    ...
    test();
    IF ( EQ_16(allow_cn_step_fx,1)||GT_32(st_fx->last_core_brate_fx,SID_2k40))
    {
    ...
    IF ( EQ_16(st_fx->last_vad_fx, 1))
    {
        ...
        move16();
        test();
        if ( GT_16(st_fx->burst_cnt_fx, 3)&&NE_16(st_fx->last_core_fx,HQ_CORE))
        {
