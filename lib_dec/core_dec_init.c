...
void open_decoder_LPD( Decoder_State_fx *st, Word32 bit_rate, Word16 bandwidth_mode )
{
    ...
    move16();
    test();
    if( st->tcxonly == 0 && LE_32(st->sr_core, 16000))
    {
    ...
    ELSE
    {
        ...
        test();
        IF( (NE_16(st->L_frame_fx, st->last_L_frame_fx))||(EQ_16(st->last_codec_mode,MODE1)))
        {
            set16_fx(st->old_synth, 0, OLD_SYNTH_INTERNAL_DEC);
        }
        ...
        test();
        test();
        IF( (NE_16(st->L_frame_fx,st->last_L_frame_fx))||(EQ_16(st->last_core_fx,AMR_WB_CORE))
        || EQ_16(st->last_core_fx,HQ_CORE) )
        {
            /*LPC quant. mem*/
        ...
        test();
        IF( EQ_16(st->last_codec_mode, MODE1)&&EQ_16(st->last_core_fx,ACELP_CORE))
        {
            st->last_core_bfi = ACELP_CORE;
            move16();
        ...
        IF( EQ_16(st->last_codec_mode,MODE2)&&
            ...
            ( ( EQ_16(st->m_frame_type, SID_FRAME)  && GT_16(st->last_core_fx, ACELP_CORE) ) ||
              ( GT_16(st->last_core_fx, ACELP_CORE)  && GT_16(st->core_fx, ACELP_CORE) ) ||
              st->prev_bfi_fx ) )
        {
        ...
        ELSE IF( (NE_16(st->L_frame_fx,st->last_L_frame_fx))&&(LE_16(st->L_frame_fx,L_FRAME16k))&&(LE_16(st->last_L_frame_fx,L_FRAME16k))) /* Rate switching between 12.8 and 16 kHz*/
        {
        ...
        ELSE IF( NE_16(st->L_frame_fx,st->last_L_frame_fx)) /* Rate switching involving TCX only modes */
        {
        ...
        ELSE IF( !st->tcxonly && EQ_16(st->L_frame_fx,L_FRAME16k)&&GT_32(st->last_total_brate_fx,ACELP_32k))
        {
            lsp2lsf_fx( st->lsp_old_fx, st->lsf_old_fx, M, st->sr_core );
        }
    }
    ...
    move16();
    test();
    IF( EQ_16(st->last_codec_mode,MODE1)||st->ini_frame_fx==0)
    {
    ...
    IF (st->prev_bfi_fx)
    {
        ...
        test();
        IF( EQ_16(st->core_fx,ACELP_CORE)&&EQ_16(st->last_core_fx,HQ_CORE))
        {
    ...
    IF(GE_32(st->total_brate_fx, 48000))
    {
        ...
        test();
        test();
        IF (st->ini_frame_fx == 0 || LT_32(st->last_total_brate_fx, HQ_48k)||EQ_16(st->last_codec_mode,MODE1)||st->force_lpd_reset)
        {
