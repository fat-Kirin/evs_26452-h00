...
)
{
	...
	for (j = 0; j<31; j++)
	{
           L_tmp = L_mac(L_tmp, x[i+j], fir_6k_7k_fx[j]);
	}
...
)
{
    ...
    test();
    IF( GT_16(*frame_count, FRAME_COUNT)&&*amr_io_class==UNVOICED_CLAS)
    {
        *frame_count = 0;
        move16();
        *ne_min = -7680;
    ...
    ELSE
    {
        ...
        *frame_count = add(*frame_count ,1);
        if(GT_16(tmp1, 2*FRAME_COUNT))
        {
            *frame_count = 2*FRAME_COUNT;
            move16();
        }
        if ( LT_16(ng_ener_ST, *ne_min))
        {
            *ne_min = ng_ener_ST;
            move16();/*Q8; */
        }
    ...
    IF ( EQ_32(core_brate, ACELP_23k85))
    {
        ...
        FOR( i = 240; i < L_FRAME16k; i++ )
        {
            *pt1 = mult_r(*pt1, scale); /*qdct */ move16();
            ...
            IF ( GE_16(i, sub(320, filt_weight_coeff)))
            {
...
)
{
    ...
    *unvoicing_fx = round_fx(L_mac(L_mult(16384, *unvoicing_fx), 16384, unvoicing_tmp_fx));  /*Q15 */
    ...
    if ( GT_16(sub(*unvoicing_fx, *unvoicing_sm_fx),3277))
    {
        *unvoicing_flag = 1;
    }

    if ( LT_16(sub(*unvoicing_fx, *unvoicing_sm_fx),1638))
    {
        *unvoicing_flag = 0;
    }
    test();
...
)
{
    ...
    {
        test();
        IF ( LT_16(rr, 1024)&&LT_16((*prev_r),1024))
        {
...
)
{
    ...
    test();
    test();
    if( GT_16(voicing_fac_fx, 6554)||(GT_16(voicing_fac_fx,4915)&&GE_16(clas,VOICED_CLAS))||EQ_16(clas,AUDIO_CLAS))
    {
        *voicing_flag = 1;
        move16();
    }
    ...
    test();
    if( LT_16(voicing_fac_fx, 3277)&&LT_16(clas,VOICED_CLAS))
    {
        *voicing_flag = 0;
        move16();
    }
    ...
    *start_band = 160;
    ...
    IF( LT_32(rate, ACELP_23k05))
    {
        ...
        move16();
        M2 = sub(M, 2);
        IF( EQ_16(*voicing_flag,1))
        {
            IF( LE_32(rate, ACELP_8k85))
            {
                M2 = sub(M, 8);
            }
            ELSE IF( LE_32(rate, ACELP_12k65))
            {
                M2 = sub(M, 6);
            }
            ELSE IF( LE_32(rate, ACELP_15k85))
            {
                M2 = sub(M, 4);
            }
        }
        ...
        *start_band = s_min(s_max(sub(tmp1, 40), 40), 160);
             ...
             ( LT_32(OptCrit_fx, L_tmp)  && GT_32(*OptCrit_old_fx, 858993) > 0 ) )
        {
            *OptCrit_old_fx = OptCrit_fx;
            ...
            test();
            test();
            if ( LT_16(abs_s(sub((*start_band),(*start_band_old))), 20)&&EQ_16(*voicing_flag,1)&&EQ_16(voicing_flag_old,1))
            {
        ...
        {
            test();
            if (LT_32(OptCrit_fx, (*OptCrit_old_fx))&&EQ_16((*voicing_flag),1))
            {
                *OptCrit_old_fx = OptCrit_fx;
                move16();
            }
