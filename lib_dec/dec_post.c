...
)
{
    ...
    IF (EQ_16(L_frame, L_FRAME))
    {
        ...
        IF (LT_32(lp_noise, LP_NOISE_THRESH))
        {
            ...
            ELSE IF (LT_32(rate, ACELP_24k40))
            {
                /*Low rates*/
    ...
    ELSE
    {
        ...
        ELSE IF (EQ_32(rate, ACELP_13k20))
        {
            post_G1 = 26870/*0.82f Q15*/;
            move16();
        }
        ELSE IF (EQ_32(rate, ACELP_16k40))
        {
            post_G1 = 26214/*0.80f Q15*/;
            move16();
        }
        ELSE IF (EQ_32(rate, ACELP_24k40)||EQ_32(rate,ACELP_32k))
        {
...
)
{
    ...
    ELSE
    {
        ...
        ELSE
        {
            ...
            IF (EQ_16(select_ltp(num_gltp, den_gltp, sh_num, sh_den, num2_gltp, den2_gltp, sh_num2, sh_den2), 1))
            {
                /* select short filter */
                temp = sub(phase, 1);
...
)
{
    ...
    L_den_int = L_acc; /* sets to 'L_acc' in 1 clock */
	move32();
    ...
    ptr_y_up = y_up;
    L_den_max = L_den_int; /* sets to 'L_acc' in 1 clock */
	move32();
    ptr_L_den0 = L_den0;
    ptr_L_den1 = L_den1;
    ptr_h = Tab_hup_s;
    ...
    FOR (phi = 1; phi < F_UP_PST; phi++)
    {
        ...
        *ptr_L_den1 = L_acc;
        ...
        IF (GT_16(abs_s(ptr_y_up[0]), abs_s(ptr_y_up[L_SUBFR])))
        {
    ...
    FOR (phi = 1; phi < F_UP_PST; phi++)
    {
        ...
        IF (L_temp > 0L)
        {
            num_max = num;
            move16();
            L_numsq_max = L_temp1; /* sets to 'L_temp1' in 1 clock */
			move32();
            den_max = den0;
            move16();
            ioff = 0;
...
)
{
    ...
    IF (L_acc != 0L)
    {
        ...
        /* Compute o   gain */
        {
            ...
            FOR (i = 0; i < L_subfr; i++)
            {
                acc64 = W_mac0_16_16(acc64, 1, abs_s(sig_out[i]));
            }
            L_acc = W_sat_l(acc64);
        }
        IF (L_acc == 0L)
