...
)
{
    ...
    *Npeaks = vq_peaks;
    ...
    FOR (i = 1; i < vq_peaks; i++)
    {
        ...
        /* safety check in case of bit errors */
        test();
        IF( GT_16(pgain_difidx[i], 44)||pgain_difidx[i]<0)
        {
            st_fx->BER_detect = 1;
            move16();
            pgain_difidx[i] = 44;
...
)
{
    ...
    ELSE
    {
        ...
        IF(LE_32(absPeakGain1, absPeakGain))
        {
            Mpy_32_16_ss(*peak_gain, xq[0], &vect_out[0], &dontCare); /* vect_out in Q12 */
            Mpy_32_16_ss(*peak_gain, xq[1], &vect_out[1], &dontCare); /* Q12 */
        }
...
)
{
    ...
    IF (mode == HVQ_CP_DELTA)
    {
        ...
        FOR (i = 1; i < num_peaks; i++)
        {
            ...
            move16();
            /* safety check in case of bit errors */
            IF (GE_16(peak_idx[i], HVQ_THRES_BIN_32k))
            {
...
)
{
    ...
    FOR (j = 0; j < layer_length; j++)
    {
        IF (EQ_16(layer2[j], 1))
        {
            ...
            IF ( j == 0 && GT_16(val, 4)) /* out[0] and out[1] are invalid positions */
            {
                st_fx->BER_detect = 1;
                move16();
                val = 4;
