...
)
{
    ...
    test();
    IF ( GE_16(length, L_FRAME32k)&&LE_32(core_brate,HQ_32k))
    {
        *hqswb_clas = get_next_indice_fx( st_fx, 2 );
        move16();
        test();
        if (( EQ_16(length, L_FRAME48k))&&(EQ_16(*hqswb_clas,HQ_NORMAL)))
        {
    ...
    *is_transient = 0;
    ...
    test();
    test();
    if ( *hqswb_clas == HQ_NORMAL && EQ_16(length, L_FRAME32k)&&LE_32(core_brate,HQ_32k))
    {
