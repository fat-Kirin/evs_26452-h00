...
)
{
    ...
    st_fx->bpf_off_fx = 0;
    move16();
    if( EQ_16(st_fx->last_core_fx,HQ_CORE))
    {
    ...
    test();
    test();
    if( st_fx->last_con_tcx && NE_16(st_fx->L_frameTCX_past, st_fx->L_frame_fx)&&st_fx->last_core_fx!=0)
    {
     ...
     *----------------------------------------------------------------*/
    test();
    IF( NE_16(st_fx->last_L_frame_fx,st_fx->L_frame_fx)&&NE_16(st_fx->last_core_fx,HQ_CORE))
    {
        if( st_fx->pfstat.on != 0 )
        {
    ...
    IF( NE_16(st_fx->last_L_frame_fx,st_fx->L_frame_fx))
    {
        /* update buffer of old subframe pitch values */
        IF( EQ_16(st_fx->L_frame_fx,L_FRAME))
        {
            move16();
            IF( EQ_16(st_fx->last_L_frame_fx,L_FRAME32k))
            {
                /* (float)12800/(float)32000; */
                k = 13107;
            }
            ELSE IF( EQ_16(st_fx->last_L_frame_fx,512))
            {
            ...
            {
                st_fx->old_pitch_buf_fx[i-1] = Mpy_32_16_1(st_fx->old_pitch_buf_fx[i], k);
                move32();
            }

            FOR( i=2*NB_SUBFR16k-NB_SUBFR; i<2*NB_SUBFR16k; i++ )
            {
                st_fx->old_pitch_buf_fx[i-2] =  Mpy_32_16_1(st_fx->old_pitch_buf_fx[i], k);
                move32();
            }
        }
        ELSE
        {
            ...
            {
                st_fx->old_pitch_buf_fx[i+2] = L_add(st_fx->old_pitch_buf_fx[i], Mpy_32_16_1(st_fx->old_pitch_buf_fx[i], k));
                move32();
            }
            ...
            {
                st_fx->old_pitch_buf_fx[i+1] = L_add(st_fx->old_pitch_buf_fx[i], Mpy_32_16_1(st_fx->old_pitch_buf_fx[i], k));
                move32();
            }
    ...
    IF( NE_16(st_fx->bfi_pitch_frame_fx, st_fx->L_frame_fx))
    {
        IF( EQ_16(st_fx->L_frame_fx,L_FRAME))
        {
            move16();
            IF( EQ_16(st_fx->bfi_pitch_frame_fx,L_FRAME32k))
            {
                /* (float)12800/(float)32000; */
                k = 13107;
            }
            ELSE IF( EQ_16(st_fx->bfi_pitch_frame_fx,512))
            {
                /* (float)12800/(float)25600; */
                k = 16384;
            }
            ELSE /* st->bfi_pitch_frame_fx == L_FRAME16k */
            {
                /* (float)12800/(float)16000; */
                k = 26214;
            }
        ...
        ELSE
        {
            ...
            ELSE /* st->bfi_pitch_frame_fx == L_FRAME12k8 */
            {
                /* tmpF = (float)16000/(float)12800; */
                k = 8192;
            }
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        tc_subfr_fx = -1;
        move16();
        if( EQ_16(coder_type_fx,TRANSITION))
        {
         ...
         *----------------------------------------------------------------*/
        test();
        IF ( st_fx->core_brate_fx == FRAME_NO_DATA || EQ_32(st_fx->core_brate_fx,SID_2k40))
        {
            ...
            move16();
            test();
            if( LT_32(st_fx->lp_ener_fx,40)&&EQ_16(st_fx->cng_type_fx,LP_CNG)) /* very low energy frames, less than 0.3125 */
            {
                delta_mem_scale = 0;
                move16();
            }
            ...
            Rescale_mem( st_fx->Q_exc, &st_fx->prev_Q_syn, &st_fx->Q_syn, st_fx->mem_syn2_fx, st_fx->mem_syn_clas_estim_fx, delta_mem_scale,
                         &st_fx->mem_deemph_fx, st_fx->pst_old_syn_fx,&st_fx->pst_mem_deemp_err_fx, &st_fx->agc_mem_fx[1], &st_fx->pfstat, 0, 0, NULL );
        ...
        ELSE
        {
            ...
            test();
            IF ( st_fx->last_core_brate_fx == FRAME_NO_DATA || EQ_32(st_fx->last_core_brate_fx,SID_2k40))
            {
            ...
            lsf_dec_fx( st_fx, tc_subfr_fx, st_fx->L_frame_fx, coder_type_fx, st_fx->bwidth_fx, Aq_fx,
            &LSF_Q_prediction, lsf_new_fx, lsp_new_fx, lsp_mid_fx );
            ...
            IF( GE_32(st_fx->core_brate_fx,ACELP_11k60))
            {
                ...
                T0_tmp = FEC_pos_dec_fx( st_fx, coder_type_fx, st_fx->last_good_fx, &last_pulse_pos, &st_fx->clas_dec, &enr_q_fx, st_fx->core_brate_fx );
                move16();

                test();
                test();
                IF( NE_16(st_fx->last_core_fx,HQ_CORE)||(EQ_16(st_fx->last_core_fx,HQ_CORE)&&st_fx->last_con_tcx))
                {
                    test();
                    test();
                    test();
                    ...
                    test();
                    test();
                    IF( EQ_16(st_fx->clas_dec,SIN_ONSET)&&last_pulse_pos!=0&&EQ_16(st_fx->prev_bfi_fx,1))
                    {
                        st_fx->Q_exc =
                        FEC_SinOnset_fx( old_exc_fx+L_EXC_MEM_DEC-L_EXC_MEM, last_pulse_pos, T0_tmp, enr_q_fx, Aq_fx, st_fx->L_frame_fx, st_fx->Q_exc );
                    }
                    ELSE IF( (EQ_16(coder_type_fx,GENERIC)||EQ_16(coder_type_fx,VOICED))&&last_pulse_pos!=0&&EQ_16(st_fx->old_bfi_cnt_fx,1)&&EQ_16(output_frame,L_FRAME16k))
                    {
                        do_WI_fx = FEC_enhACB_fx( st_fx->L_frame_fx, st_fx->last_L_frame_fx, old_exc_fx+L_EXC_MEM_DEC-L_EXC_MEM , T0_tmp, last_pulse_pos, st_fx->bfi_pitch_fx );
                    }
                }
            }
            ...
            test();
            test();
            IF( st_fx->stab_fac_fx == 0 && st_fx->old_bfi_cnt_fx > 0 && NE_16(st_fx->clas_dec,VOICED_CLAS)&&NE_16(st_fx->clas_dec,ONSET)&&st_fx->relax_prev_lsf_interp_fx==0)
            {
                int_lsp4_fx(st_fx->L_frame_fx, st_fx->lsp_old_fx, lsp_mid_fx, lsp_new_fx, Aq_fx,  M, 2 );
            }
            ...
            test();
            test();
            IF( ( NE_16(coder_type_fx,UNVOICED)&&
                  NE_16(coder_type_fx,AUDIO)     &&
                  NE_16(coder_type_fx,INACTIVE)   &&
                  !(LE_32(st_fx->core_brate_fx,ACELP_8k00)  && NE_16(coder_type_fx,TRANSITION) ) ) ||
                (EQ_16(coder_type_fx,INACTIVE)  && GE_32(st_fx->total_brate_fx,ACELP_32k) )
              )
            {
            ...
            *------------------------------------------------------------*/
            ...
            ELSE IF( EQ_16(coder_type_fx,UNVOICED))
            {
                /* UNVOICED frames */
            ...
            ELSE IF( EQ_16(coder_type_fx,TRANSITION ))
            {
            ...
            ELSE IF( EQ_16(coder_type_fx,AUDIO)||(EQ_16(coder_type_fx,INACTIVE)&&LE_32(st_fx->core_brate_fx,ACELP_24k40)))
            {
            ...
            Inac_swtch_ematch_fx( exc2_fx, dct_exc_tmp, st_fx->lt_ener_per_band_fx, coder_type_fx, st_fx->L_frame_fx, st_fx->core_brate_fx, st_fx->Q_exc, st_fx->bfi_fx, st_fx->last_core_fx, st_fx->last_codec_mode );

            /*------------------------------------------------------------*
             * Decode information and modify the excitation signal of stationary unvoiced frames
             *------------------------------------------------------------*/

            IF ( NE_16(st_fx->nelp_mode_dec_fx,1))
            {
            ...
            test();
            IF( EQ_16(coder_type_fx, AUDIO )&&st_fx->GSC_noisy_speech_fx==0)
            {
                ...
                LD_music_post_filter_fx( dct_buffer_fx, dct_buffer_fx, st_fx->core_brate_fx, &st_fx->LDm_last_music_flag_fx,
                st_fx->LDm_thres_fx, &st_fx->LDm_nb_thr_1_fx, &st_fx->LDm_nb_thr_3_fx, st_fx->LDm_lt_diff_etot_fx,
                &st_fx->LDm_mem_etot_fx, st_fx->LDm_enh_min_ns_gain_fx, st_fx->LDm_bckr_noise_fx,
                ...
                &st_fx->pst_mem_deemp_err_fx, &st_fx->agc_mem_fx[1], &st_fx->pfstat, temp_buf_fx, mem_tmp_fx );
            }
            ELSE
            {
                ...
                i = 1;
                move16();
                if( EQ_16(coder_type_fx,INACTIVE))
                {
                    i = 0;
                    move16();
                }
                ...
                test();
                test();
                if( EQ_16(coder_type_fx, INACTIVE)&&st_fx->flag_cna&&GE_16(round_fx(L_shl(st_fx->lp_noise,1)),15<<7))
                {
                    k = 1;
                    move16();
                }
                ...
                st_fx->pst_old_syn_fx, &st_fx->pst_mem_deemp_err_fx, &st_fx->agc_mem_fx[1], &st_fx->pfstat, i, k, temp_buf_fx );
            ...
            syn_fx, &st_fx->lp_ener_FER_fx, &st_fx->decision_hyst_fx,
            ...
            0, 0, 0, st_fx->last_core_brate_fx );

            /*------------------------------------------------------------*
             * FEC - Estimate pitch
             *------------------------------------------------------------*/

            FEC_pitch_estim_fx( st_fx->Opt_AMR_WB_fx, st_fx->last_core_fx, st_fx->L_frame_fx, st_fx->clas_dec, st_fx->last_good_fx, pitch_buf_fx, st_fx->old_pitch_buf_fx,
            ...
            FEC_scale_syn_fx( st_fx->L_frame_fx, &update_flg, st_fx->clas_dec, st_fx->last_good_fx, syn_fx, pitch_buf_tmp, st_fx->enr_old_fx, enr_q_fx, coder_type_fx, LSF_Q_prediction,
            &st_fx->scaling_flag, &st_fx->lp_ener_FEC_av, &st_fx->lp_ener_FEC_max, st_fx->bfi_fx, st_fx->total_brate_fx, st_fx->prev_bfi_fx, st_fx->last_core_brate_fx,
            exc_fx, exc2_fx, Aq_fx, &st_fx->old_enr_LP, mem_tmp_fx, st_fx->mem_syn2_fx, st_fx->Q_exc, st_fx->Q_syn , avoid_lpc_burst_on_recovery, 0 );

            test();
            if( (EQ_32(st_fx->total_brate_fx,ACELP_7k20))||(EQ_32(st_fx->total_brate_fx,ACELP_8k00)))
            {
                frame_ener_fx( st_fx->L_frame_fx, st_fx->clas_dec, syn_fx,  pitch_buf_tmp[sub(shr(st_fx->L_frame_fx,6),1)], &st_fx->enr_old_fx, st_fx->L_frame_fx, st_fx->Q_syn, 3, 0 );
            }
        }
    ...
    {
        /* SC-VBR */
        if ( EQ_16(st_fx->last_nelp_mode_dec_fx,1))
        {
        ...
        FEC_lsf2lsp_interp( st_fx, st_fx->L_frame_fx, Aq_fx, lsf_new_fx, lsp_new_fx );
        ...
        Rescale_mem( st_fx->Q_exc, &st_fx->prev_Q_syn, &st_fx->Q_syn, st_fx->mem_syn2_fx, st_fx->mem_syn_clas_estim_fx, 4, &st_fx->mem_deemph_fx,
        st_fx->pst_old_syn_fx, &st_fx->pst_mem_deemp_err_fx, &st_fx->agc_mem_fx[1], &st_fx->pfstat, 1, k, temp_buf_fx );
        ...
        FOR( i = 1; i<MAX_LT; i++ )
        {
            st_fx->LDm_lt_diff_etot_fx[i-1] = st_fx->LDm_lt_diff_etot_fx[i];
            move16();
        }
    ...
    IF( EQ_16(st_fx->last_bwidth_fx,NB))
    {
        k = 0;
        move16();
        FOR (i = 0; i < st_fx->L_frame_fx; i += L_SUBFR)
        ...
        IF(EQ_16(st_fx->bwidth_fx,NB))
        {
            st_fx->pfstat.on = 1;
            move16();
            nb_post_filt( st_fx->L_frame_fx, &(st_fx->pfstat), &st_fx->psf_lp_noise_fx, tmp_noise_fx, syn_fx, Aq_fx, pitch_buf_tmp, coder_type_fx, st_fx->BER_detect, 0 );
        }
        ELSE
        {
            st_fx->pfstat.on = 0;
            move16();
            nb_post_filt( st_fx->L_frame_fx, &(st_fx->pfstat), &st_fx->psf_lp_noise_fx, tmp_noise_fx, syn_fx, Aq_fx, pitch_buf_tmp, AUDIO, st_fx->BER_detect, 0 );
        }
    }
    ELSE
    {
    ...
    test();
    test();
    IF( GE_16(st_fx->last_bwidth_fx,WB)&&GT_32(st_fx->core_brate_fx,ACELP_24k40)&&LE_32(st_fx->core_brate_fx,ACELP_32k))
    {
    ...
    ELSE IF( GE_16(st_fx->last_bwidth_fx,WB))
    {
        if( st_fx->pfstat.on )
        {
            Copy( st_fx->pfstat.mem_pf_in+L_SYN_MEM-M, temp_buf, M );
            Copy( syn_fx, temp_buf + M, L_SUBFR );
            Residu3_fx ( Aq_fx, temp_buf + M, temp_buf+M+L_SUBFR, L_SUBFR, 1 );
            E_UTIL_synthesis ( 1, Aq_fx, temp_buf+M+L_SUBFR, temp_buf, L_SUBFR, st_fx->pfstat.mem_stp+L_SYN_MEM-M, 0, M );
            scale_st ( syn_fx, temp_buf, &st_fx->pfstat.gain_prec, L_SUBFR );
    ...
    test();
    test();
    IF( st_fx->flag_cna || (EQ_16(st_fx->cng_type_fx,FD_CNG)&&LE_32(st_fx->total_brate_fx,ACELP_32k))||(EQ_16(st_fx->cng_type_fx,LP_CNG)&&LE_32(st_fx->total_brate_fx,SID_2k40)))
    {
        ...
        ApplyFdCng( syn_fx, st_fx->Q_syn, realBuffer, imagBuffer, 0, st_fx->hFdCngDec_fx, st_fx->m_frame_type, st_fx, 0,
                    (EQ_16(coder_type_fx, AUDIO ) && st_fx->GSC_noisy_speech_fx == 0) );
        ...
        test();
        test();
        IF( st_fx->flag_cna && NE_16(coder_type_fx,AUDIO))
        {
    ...
    IF( st_fx->flag_cna == 0 && EQ_16(st_fx->L_frame_fx,L_FRAME16k)&&st_fx->last_flag_cna==1&&((st_fx->last_core_fx==ACELP_CORE&&st_fx->last_coder_type_fx!=AUDIO)||st_fx->last_core_fx==AMR_WB_CORE))
    {
        FOR (i=0; i < st_fx->L_frame_fx/2; i++)
        {
            syn_fx[i] = add( syn_fx[i], shr_r( st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth2[i+5*st_fx->L_frame_fx/4], -st_fx->Q_syn ) );
            move16();
        }
    }

    test();
    IF( st_fx->flag_cna == 0 || EQ_16(coder_type_fx,AUDIO))
    {
    ...
    *----------------------------------------------------------------*/
    ...
    test();
    IF( NE_16(st_fx->L_frame_fx,st_fx->last_L_frame_fx)&&NE_16(st_fx->last_codec_mode,MODE2))
    {
        IF( EQ_16(st_fx->L_frame_fx,L_FRAME))
        {
            retro_interp5_4_fx( st_fx->pst_old_syn_fx );
        }
        ELSE IF( EQ_16(st_fx->L_frame_fx,L_FRAME16k))
        {
    ...
    addBassPostFilterFx( bpf_error_signal, realBuffer, imagBuffer, st_fx->cldfbBPF_fx, workBuffer, negate(st_fx->Q_syn),
                         i, st_fx->cldfbAna_fx->no_col, st_fx->cldfbAna_fx->no_channels, &scaleFactor );
    ...
    ELSE if( NE_16(st_fx->cldfbSyn_fx->bandsToZero,sub(st_fx->cldfbSyn_fx->no_channels,st_fx->cldfbAna_fx->no_channels)))
    {
    ...
    test();
    test();
    IF( ( EQ_32(st_fx->core_brate_fx,FRAME_NO_DATA)||EQ_32(st_fx->core_brate_fx,SID_2k40))&&(EQ_16(st_fx->cng_type_fx,FD_CNG))&&(LT_16(st_fx->hFdCngDec_fx->hFdCngCom->numCoreBands,st_fx->cldfbSyn_fx->no_channels)))
    {
    ...
    test();
    test();
    IF( (EQ_16(st_fx->L_frame_fx,L_FRAME)&&NE_16(st_fx->bwidth_fx,NB)&&GE_16(output_frame,L_FRAME16k)&&
         ( EQ_16(st_fx->extl_fx,-1)  || EQ_16(st_fx->extl_fx,SWB_CNG)  || (EQ_16(st_fx->extl_fx,WB_BWE)  && st_fx->extl_brate_fx == 0 && NE_16(coder_type_fx,AUDIO) )) ) )
    {
    ...
    test();
    test();
    IF( ( !st_fx->bfi_fx && ( st_fx->prev_bfi_fx )) || ((EQ_16(st_fx->last_vbr_hw_BWE_disable_dec_fx,1))&&(st_fx->vbr_hw_BWE_disable_dec_fx==0))||((EQ_16(st_fx->extl_fx,SWB_TBE)||EQ_16(st_fx->extl_fx,WB_TBE)||EQ_16(st_fx->extl_fx,FB_TBE))&&NE_16(st_fx->last_extl_fx,SWB_TBE)&&NE_16(st_fx->last_extl_fx,WB_TBE)&&NE_16(st_fx->last_extl_fx,FB_TBE)))
    {
    ...
    test();
    if( EQ_32(st_fx->core_brate_fx,FRAME_NO_DATA)||EQ_32(st_fx->core_brate_fx,SID_2k40))
    {
        st_fx->bwe_non_lin_prev_scale_fx = L_deposit_l(0);
    }
    ...
    IF( GT_32(st_fx->core_brate_fx,SID_2k40))
    {
