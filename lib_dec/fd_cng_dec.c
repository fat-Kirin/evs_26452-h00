                       ...
                       )
{
    ...
    test();
    if ( bitrate != FRAME_NO_DATA && NE_32(bitrate, SID_2k40))
    {
    ...
    ELSE IF ( EQ_16(bandwidth, WB))
    {
        ...
        test();
        test();
        IF ( LE_32(hsCom->CngBitrate, ACELP_8k00)
        ...
        ELSE IF ( LE_32(hsCom->CngBitrate, ACELP_13k20)
                  || EQ_16(L_frame,L_FRAME)
                )
        {
                  ...
                  )
{
    ...
    if(EQ_16(st->hFdCngCom->frame_type_previous,ACTIVE_FRAME))
    {
    ...
    case ACTIVE_FRAME:
        ...
        **************************/
        ...
        IF (EQ_16(concealWholeFrame,1)&&EQ_16(stdec->nbLostCmpt,1)&&(L_shl(L_tmp,L_tmp_exp)>21474836 /*0.01f Q31*/))
        {
    ...
    case ZERO_FRAME:
        ...
        *************************************/
        ...
        IF ( EQ_16(st->hFdCngCom->inactive_frame_counter,1))
        {
        ...
        IF ( EQ_16(m_frame_type,SID_FRAME))
        {
            IF ( LT_32(st->hFdCngCom->msFrCnt_init_counter,L_deposit_l(st->hFdCngCom->msFrCnt_init_thresh)))
            {
            ...
            ELSE
            {
                ...
                IF ( GT_16( st->hFdCngCom->regularStopBand, st->hFdCngCom->numCoreBands ))
                {
                    scalebands (
                        st->hFdCngCom->sidNoiseEst,
                        st->hFdCngCom->part,
        ...
        IF ( EQ_16(stdec->codec_mode, MODE2))
        {
                                  ...
                                  )
{
    ...
    fac = 20972/*0.64 Q15*/;
    move16();
    if( EQ_16(st->hFdCngCom->fftlen,512))
    {
        fac = 16384/*0.5 Q15*/;
        move16();
    }

    if ( EQ_16(st->hFdCngCom->fftlen,640))
    {
        s = 18;
        move16();
    }
    ...
    if ( EQ_16(st->hFdCngCom->fftlen,320))
    {
        s = 16;
        move16();
    }
                            ...
                            )
{
    ...
    *flag_noisy_speech = 0;
    move16();
    if ( LT_32(L_sub(*lp_speech,*lp_noise),234881024l/*28.0 Q23*/))
    {
        *flag_noisy_speech = 1;
        move16();
    }
                           ...
                           )
{
    ...
    */
...

    {
        ...
        IF( NE_16(stdec->bwidth_fx, NB))
        {
            IF( EQ_16(stdec->bwidth_fx,WB))
            {
    ...
    */
    test();
    IF ( bufferReal!=NULL && (LT_16(st->numCoreBands,st->regularStopBand)))
    {
                              ...
                              )
{
    ...
    IF ( LT_16(st->numCoreBands,st->regularStopBand))
    {
                            ...
                            )
{
    ...
    IF(core!=AMR_WB_CORE)
    {
        ...
        FOR (i=0; i < scaleTableSize; i++)
        {
            test();
            test();
            IF (   ( EQ_16(st->CngBandwidth,scaleTable_cn_only[i].bwmode))
                   && (GE_32(st->CngBitrate,scaleTable_cn_only[i].bitrateFrom) )
                   && ( LT_32(st->CngBitrate,scaleTable_cn_only[i].bitrateTo) ))
    ...
    ELSE
    {
        ...
        FOR (i=0; i < scaleTableSize; i++)
        {
            IF ( GE_32(st->CngBitrate,scaleTable_cn_only_amrwbio[i][0]))
            {
