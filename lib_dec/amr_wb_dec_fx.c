...
)
{
    ...
    Word16 syn_tmp_fx[L_FRAME+2], *syn_fx;                /* synthesis signal buffer               */
    Word32 L_tmp, L_tmp1;
    ...
    IF( st_fx->ini_frame_fx == 0 )
    {
        ...
        E_LPC_isf_isp_conversion( st_fx->lsf_old_fx, st_fx->lsp_old_fx, M );
    }
    ...
    test();
    test();
    IF( NE_16(st_fx->last_core_fx,AMR_WB_CORE)&&EQ_16(st_fx->last_L_frame_fx,L_FRAME16k)&&NE_16(st_fx->last_core_fx,HQ_CORE))
    {
    ...
    IF( NE_16(st_fx->bfi_pitch_frame_fx, L_FRAME))
    {
        move16();
    ...
    test();
    if( EQ_16(st_fx->last_bwidth_fx,NB)&&st_fx->ini_frame_fx!=0)
    {
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        test();
        IF ( EQ_32(st_fx->core_brate_fx,FRAME_NO_DATA)||EQ_32(st_fx->core_brate_fx,SID_1k75))
        {
            ...
            i = st_fx->Q_exc;
            Rescale_exc( st_fx->dct_post_old_exc_fx, exc_fx, NULL, st_fx->last_exc_dct_in_fx, st_fx->L_frame_fx,
            ...
            syn_12k8_fx( L_FRAME, Aq_fx, exc2_fx, dummy_buf_fx, st_fx->mem_syn3_fx, 1, st_fx->Q_exc, st_fx->Q_syn );

            /* reset the decoder */
            CNG_reset_dec_fx( st_fx, pitch_buf_fx, dummy_buf_fx+L_FRAME );

            /* update st_fx->mem_syn1 for ACELP core switching */
            Copy_Scale_sig( st_fx->mem_syn3_fx, st_fx->mem_syn1_fx, M, sub(-1,st_fx->Q_syn) ); /*Q-1*/
        ...
        ELSE
        {
            ...
            test();
            IF ( EQ_32(st_fx->last_core_brate_fx,FRAME_NO_DATA)||EQ_32(st_fx->last_core_brate_fx,SID_1k75))
            {
                Copy( st_fx->lspCNG_fx, st_fx->lsp_old_fx, M );
                E_LPC_isp_isf_conversion( st_fx->lspCNG_fx, st_fx->lsf_old_fx, M );
            ...
            test();
            test();
            if(EQ_16(coder_type,INACTIVE)&&st_fx->flag_cna&&GE_16(st_fx->psf_lp_noise_fx,15<<7))
            {
                tmp16 = 1;
                move16();
            }
            Rescale_mem( st_fx->Q_exc, &st_fx->prev_Q_syn, &st_fx->Q_syn, st_fx->mem_syn2_fx, st_fx->mem_syn_clas_estim_fx, 5, &st_fx->mem_deemph_fx,
            st_fx->pst_old_syn_fx, &st_fx->pst_mem_deemp_err_fx, &st_fx->agc_mem_fx[1], &st_fx->pfstat, vad_flag, tmp16, tmp_buffer_fx );
            ...
            FEC_clas_estim_fx( st_fx, 1, L_FRAME, &st_fx->clas_dec, coder_type, pitch_buf_fx,
            syn_fx, &st_fx->lp_ener_FER_fx,
            &st_fx->decision_hyst_fx, &st_fx->UV_cnt_fx, &st_fx->LT_UV_cnt_fx, &st_fx->Last_ener_fx, &locattack, st_fx->lt_diff_etot_fx,
            &amr_io_class, st_fx->core_brate_fx, st_fx->Q_syn, &class_para_fx, st_fx->mem_syn_clas_estim_fx, &st_fx->classifier_Q_mem_syn,
            ...
            test();
            test();
            IF( NE_16(amr_io_class,UNVOICED_CLAS)&&NE_16(coder_type,INACTIVE)&&LT_16(st_fx->psf_lp_noise_fx,15<<8))
            {
                tmp_coder_type = AUDIO;
                move16();
                test();
                if (EQ_16(st_fx->last_coder_type_fx,INACTIVE)||EQ_16(st_fx->last_coder_type_fx,UNVOICED))
                {
                ...
                Post_music_postP_fx( dct_buffer_fx, exc2_fx, mem_tmp_fx, st_fx->mem_syn2_fx, Aq_fx, syn_fx, &st_fx->Q_exc, &st_fx->prev_Q_syn,
                                     ...
                                     &st_fx->pst_mem_deemp_err_fx, &st_fx->agc_mem_fx[1], &st_fx->pfstat, NULL, NULL );
            }
            ELSE
            {
                /*------------------------------------------------------------*
                ...
                locattack, Aq_fx, exc2_fx, st_fx->Q_exc, mem_tmp_fx, syn_fx, st_fx->Q_syn, pitch_buf_fx, st_fx->Last_ener_fx, st_fx->rate_switching_reset, st_fx->last_coder_type_fx );
            ...
            FEC_pitch_estim_fx( 1, st_fx->last_core_fx, L_FRAME, st_fx->clas_dec, st_fx->last_good_fx, pitch_buf_fx, st_fx->old_pitch_buf_fx, &st_fx->bfi_pitch_fx,
            ...
            FEC_scale_syn_fx( L_FRAME, &update_flg, st_fx->clas_dec, st_fx->last_good_fx, syn_fx, pitch_buf_tmp, st_fx->enr_old_fx, L_enr_q_fx, -1, MOVING_AVERAGE,
    ...
    ELSE
    {
        ...
        st_fx->pst_old_syn_fx, &st_fx->pst_mem_deemp_err_fx, &st_fx->agc_mem_fx[1], &st_fx->pfstat, 1, 0, tmp_buffer_fx );
        ...
        FEC_scale_syn_fx( L_FRAME, &update_flg, st_fx->clas_dec, st_fx->last_good_fx, syn_fx, pitch_buf_tmp, st_fx->enr_old_fx, L_enr_q_fx, -1,
        MOVING_AVERAGE, &st_fx->scaling_flag, &st_fx->lp_ener_FEC_av, &st_fx->lp_ener_FEC_max, st_fx->bfi_fx, st_fx->total_brate_fx,
        st_fx->prev_bfi_fx, st_fx->last_core_brate_fx, exc_fx, exc2_fx, Aq_fx, &st_fx->old_enr_LP, mem_tmp_fx, st_fx->mem_syn2_fx,
     ...
     *--------------------------------------------------------*/
    test();
    IF( EQ_16(output_frame,L_FRAME8k)||EQ_16(st_fx->last_bwidth_fx,NB))
    {
        FOR( i=0; i<NB_SUBFR; i++ )
        {
            pitch_buf_tmp[i] = mult_r(pitch_buf_fx[i], 512);
        ...
        IF( EQ_16(output_frame,L_FRAME8k))
        {
            st_fx->pfstat.on = 1;
            move16();
            nb_post_filt( L_FRAME, &(st_fx->pfstat), &st_fx->psf_lp_noise_fx, tmp_noise_fx, syn_fx, Aq_fx, pitch_buf_tmp, coder_type, st_fx->BER_detect, 0 );
        }
        ELSE
        {
            st_fx->pfstat.on = 0;
            move16();
            nb_post_filt( L_FRAME, &(st_fx->pfstat), &st_fx->psf_lp_noise_fx, tmp_noise_fx, syn_fx, Aq_fx, pitch_buf_tmp, AUDIO, st_fx->BER_detect, 0 );
        }
    }
    ...
    IF( NE_16(output_frame,L_FRAME8k)&&NE_16(st_fx->last_bwidth_fx,NB))
    {
        st_fx->pfstat.on = 1;
        move16();
        test();
        formant_post_filt( &(st_fx->pfstat), tmp_buffer_fx + L_SYN_MEM, Aq_fx, syn_fx, L_FRAME, L_shl(st_fx->psf_lp_noise_fx,15), st_fx->total_brate_fx, sub(amr_io_class,AUDIO_CLAS) == 0);
    }
    ...
    IF( (GE_16(st_fx->psf_lp_noise_fx,15<<8))||(coder_type==INACTIVE))
    {
        ...
        test();
        IF( EQ_16(st_fx->VAD, 1)&&NE_16(coder_type,INACTIVE))
        {
            st_fx->VAD = 1;
            move16();
        }
        ...
        ApplyFdCng( syn_fx, st_fx->Q_syn, NULL, NULL, 0, st_fx->hFdCngDec_fx, st_fx->m_frame_type, st_fx, 0, 0 );
    ...
    IF( flag_cna == 0 )
    {
        ...
        test();
        test();
        IF( EQ_16(st_fx->last_flag_cna,1)&&((EQ_16(st_fx->last_core_fx,ACELP_CORE)&&NE_16(st_fx->last_coder_type_fx,AUDIO))||EQ_16(st_fx->last_core_fx,AMR_WB_CORE)))
        {
    ...
    if( NE_16(st_fx->cldfbSyn_fx->bandsToZero,sub(st_fx->cldfbSyn_fx->no_channels,st_fx->cldfbAna_fx->no_channels)))
    {
     ...
     *--------------------------------------------------------*/
    frame_ener_fx( L_FRAME, st_fx->clas_dec, syn_fx, mult_r(pitch_buf_fx[3],512), &L_Ng_ener, L_FRAME, st_fx->Q_syn, 3, 0 );
    /*--------------------------------------------------------*
    ...
    test();
    test();
    IF( GE_16(output_frame,L_FRAME16k)&&(NE_16(st_fx->cldfbSyn_fx->bandsToZero,sub(st_fx->cldfbSyn_fx->no_channels,10))||NE_16(st_fx->last_flag_filter_NB,1)))
    {
    ...
    Copy( Aq_fx, st_fx->old_Aq_fx, NB_SUBFR * (M+1) );
    ...
    move16();
    IF( EQ_16(last_core_ori,HQ_CORE))
    {
    ...
    if( GT_32(st_fx->core_brate_fx,SID_1k75))
    {
    ...
    test();
    IF( GT_32(st_fx->core_brate_fx,SID_1k75)&&st_fx->first_CNG_fx)
    {
    ...
    Copy_Scale_sig( synth_out_fx+NS2SA_fx2( st_fx->output_Fs_fx,ACELP_LOOK_NS+DELAY_BWE_TOTAL_NS), st_fx->old_synth_sw_fx, NS2SA_fx2(st_fx->output_Fs_fx,FRAME_SIZE_NS-ACELP_LOOK_NS-DELAY_BWE_TOTAL_NS), sub(st_fx->Q_old_postdec, st_fx->Q_syn2) );
