...
)
{
     ...
     *                             in first subframe(s) )
     *---------------------------------------------------------------------*/
    IF(GT_16(tc_subfr, add(i_subfr,TC_0_192)))
    {
    ...
    ELSE IF(((GE_16(tc_subfr,i_subfr))&&(LE_16(sub(tc_subfr,i_subfr),TC_0_192))))
    {
    ...
    ELSE IF ( LT_16(tc_subfr,i_subfr))
    {
        IF( EQ_16(L_frame,L_FRAME))
        {
            *Jopt_flag = 1;
            move16();
            test();
            IF( (GE_16(sub(i_subfr,tc_subfr),L_SUBFR))&&(LE_16(sub(i_subfr,tc_subfr),L_SUBFR+TC_0_192)))
            {
                pit_flag = 0;
                move16();
            }
            ...
            ELSE IF( (EQ_16(i_subfr,3*L_SUBFR))&&(EQ_16(tc_subfr,TC_0_192)))
            {
                ...
                IF( LT_16(index,shl(sub(pit_limit,pit_start),1)))
                {
                    *T0 = add( pit_start, shr(index,1));
                    move16();
                    *T0_frac = shl(sub(index, shl(sub((*T0), pit_start),1)),1);
                ...
                IF( GT_16(add((*T0<<2),*T0_frac),add((PIT_MAX<<2),2)))
                {
                    *T0 = L_SUBFR;
                    move16();
                    *T0_frac = 0;
            ...
            **pt_pitch = shl(add(shl(*T0,2),*T0_frac),4);
            ...
            test();
            test();
            IF( (GE_16(tc_subfr,2*L_SUBFR))&&(EQ_16(i_subfr,3*L_SUBFR)))
            {
            ...
            ELSE IF( (EQ_16(tc_subfr,L_SUBFR))&&(EQ_16(i_subfr,2*L_SUBFR)))
            {
                tmp = shl(add(shl(*T0,2),*T0_frac),4);
                (*pt_pitch) -= 2;
                move16();
            ...
            ELSE IF( (EQ_16(tc_subfr,TC_0_64))&&(EQ_16(i_subfr,L_SUBFR)))
            {
                tmp = shl(add(shl(*T0,2),*T0_frac),4);
                (*pt_pitch) -= 1;
                move16();
            ...
            ELSE IF( (EQ_16(tc_subfr,TC_0_128))&&(EQ_16(i_subfr,2*L_SUBFR)))
            {
                tmp = shl(add(shl(*T0,2),*T0_frac),4);
                (*pt_pitch) -= 2;
                move16();
            ...
            ELSE IF( (EQ_16(tc_subfr,TC_0_192))&&(EQ_16(i_subfr,3*L_SUBFR)))
            {
                tmp = shl(add(shl(*T0,2),*T0_frac),4);
                (*pt_pitch) -= 3;
                move16();
        ...
        ELSE  /* L_frame == L_FRAME16k */
        {
            ...
            IF( EQ_16(sub(i_subfr, tc_subfr), L_SUBFR))
            {
            ...
            IF( EQ_16(nBits,10))
            {
                pit16k_Q_dec_fx( index, nBits, limit_flag, T0, T0_frac, T0_min, T0_max, &st_fx->BER_detect );
            }
            ELSE IF( EQ_16(nBits,8))     /* tc_subfr==0 && i_subfr==L_SUBFR */
            {
                ...
                IF( LT_16(index,(PIT16k_FR2_TC0_2SUBFR-PIT16k_MIN)*4))/*(PIT16k_FR2_TC0_2SUBFR-PIT16k_MIN)*4*/
                {
                ...
                /* biterror detection mechanism */
                IF( GT_16(add((*T0<<2),*T0_frac),((2*L_SUBFR)<<2)))
                {
                    *T0 = L_SUBFR;
                    move16();
                    *T0_frac = 0;
...
)
{
    ...
    IF( EQ_16(L_frame,L_FRAME))
    {
        ...
        test();
        test();
        IF ( ((i_subfr == 0) && ((tc_subfr == 0) || (EQ_16(tc_subfr,TC_0_64))||(EQ_16(tc_subfr,TC_0_128))||(EQ_16(tc_subfr,TC_0_192))))||(EQ_16(tc_subfr,L_SUBFR)))
        {
            *T0 = L_SUBFR;
            move16();
            *T0_frac = 0;
    ...
    ELSE  /* L_frame == L_FRAME16k */
    {
        ...
        IF( EQ_16(nBits,10))
        {
            IF( LT_16(i,(PIT16k_FR2_EXTEND_10b-PIT16k_MIN_EXTEND)*4))
            {
