...
)
{
    ...
    IF (!st_fx->bfi_fx
        && st_fx->prev_bfi_fx
        && (EQ_16(st_fx->last_codec_mode, MODE2) )
        && (EQ_16(st_fx->last_core_bfi, TCX_20_CORE) 
            || EQ_16(st_fx->last_core_bfi, TCX_10_CORE) ))
    {
        /* v_multc(st_fx->old_out_fx, st_fx->plcInfo.recovery_gain, */
        /*         st_fx->old_out_fx, st_fx->L_frameTCX); */
        FOR( i = 0; i < st_fx->L_frameTCX; i++ )
        {
            st_fx->old_out_fx[i]    = shl(mult_r(st_fx->old_out_fx[i]   , st_fx->plcInfo.recovery_gain), 1);
        }
        FOR( i = 0; i < st_fx->L_frame_fx; i++ )
        {
            st_fx->old_out_LB_fx[i] = shl(mult_r(st_fx->old_out_LB_fx[i], st_fx->plcInfo.recovery_gain), 1);
        }
        ...
        IF ( 0 == st_fx->tcx_cfg.last_aldo )
        {
            ...
            Word16 s;
            Word16 tmp1;
            f = L_deposit_l(st_fx->conceal_eof_gain); /*Q14*/
            s = norm_l(f);
            s = sub(16,s);
            ...
            FOR( i=0; i < st_fx->tcx_cfg.tcx_mdct_window_lengthFB; i++ )
            {
                ...
                st_fx->syn_OverlFB[i] = shl(mult(tmp1,st_fx->syn_OverlFB[i]), add(s,1));
                move16();
            ...
            {
                st_fx->syn_Overl[i]   = shl(mult(tmp1,st_fx->syn_Overl[i]),s);
                move16();
            }
        }
    }
    ...
    set16_fx( hb_synth_fx, 0, L_FRAME48k );
    hb_synth_fx_exp = 0;
    move16();
    ...
    IF( EQ_16(st_fx->last_core_fx,AMR_WB_CORE))
    {
        updt_IO_switch_dec_fx( output_frame, st_fx );
    }

    IF( NE_16(frameMode,FRAMEMODE_MISSING))  /* frame mode normal or future frame */
    {
    ...
    test();
    IF( EQ_16(st_fx->rf_frame_type,RF_NO_DATA)&&EQ_16(st_fx->use_partial_copy,1))
    {
    ...
    test();
    test();
    IF( EQ_16(st_fx->use_partial_copy,1)&&EQ_16(st_fx->core_fx,TCX_20_CORE)&&
        EQ_16(st_fx->prev_bfi_fx,1) && EQ_16(st_fx->last_core_fx,ACELP_CORE) )
    {
        st_fx->bfi_fx = 1;
        move16();
        st_fx->codec_mode = st_fx->last_codec_mode;
    ...
    /* if previous frame was HQ Core or TCX10, drop partial copy info and continue HQ Core/TCX10 concealment */
    IF( st_fx->use_partial_copy && ((sub(st_fx->last_core_fx, HQ_CORE) == 0) || (sub(st_fx->last_core_fx, TCX_10_CORE) == 0) || ((sub(st_fx->last_core_fx, TCX_20_CORE) == 0) && getTcxonly(st_fx->last_total_brate_fx))) )
    {
        st_fx->bfi_fx = 1;
        move16();
        st_fx->codec_mode = st_fx->last_codec_mode;
        ...
        st_fx->use_partial_copy = 0;
        move16();
        st_fx->core_fx = st_fx->last_core_fx;
        move16();
    }
    ...
    IF( EQ_16(st_fx->codec_mode,MODE1))
    {
        ...
        test();
        test();
        IF ( EQ_16(st_fx->extl_fx,SWB_TBE)||EQ_16(st_fx->extl_fx,FB_TBE)
             || (NE_16(coder_type,AUDIO)  && NE_16(coder_type,INACTIVE)  && GT_32(st_fx->core_brate_fx,SID_2k40) && EQ_16(st_fx->core_fx,ACELP_CORE) 
                 && GE_32(st_fx->output_Fs_fx,32000)  && GT_16(st_fx->bwidth_fx,NB)  && st_fx->bws_cnt_fx > 0 && !st_fx->ppp_mode_dec_fx
                 && !( EQ_16( st_fx->nelp_mode_dec_fx, 1) && EQ_16( st_fx->bfi_fx, 1) ) ) )

        {
            ...
            test();
            IF ( EQ_16(output_frame,L_FRAME48k)&&EQ_16(st_fx->extl_fx,FB_TBE))
            {
                fb_tbe_dec_fx( st_fx, fb_exc_fx, Q_fb_exc, hb_synth_fx, hb_synth_fx_exp);
            }
        }
        ELSE IF( EQ_16(st_fx->extl_fx,SWB_BWE)||EQ_16(st_fx->extl_fx,FB_BWE)||
                 (GE_32(st_fx->output_Fs_fx,32000)  && EQ_16(st_fx->core_fx,ACELP_CORE)  && GT_16(st_fx->bwidth_fx,NB)  && st_fx->bws_cnt_fx > 0 && !st_fx->ppp_mode_dec_fx
                  && !( EQ_16( st_fx->nelp_mode_dec_fx, 1)  && EQ_16( st_fx->bfi_fx, 1)  ) ) )
        {
            /* SWB BWE decoder */
            hb_synth_fx_exp = swb_bwe_dec_fx( st_fx, synth_fx, hb_synth_fx, output_frame, &Qpostd, coder_type );
        }
        ELSE IF( EQ_16(st_fx->extl_fx,SWB_BWE_HIGHRATE)||EQ_16(st_fx->extl_fx,FB_BWE_HIGHRATE))
        {
        ...
        test();
        test();
        IF ( st_fx->prev_bfi_fx && EQ_16(st_fx->last_core_fx,HQ_CORE)&&NE_16(st_fx->extl_fx,-1))
        {
        ...
        IF( GE_16(output_frame,L_FRAME32k))
        {
            ...
            test();
            if( LE_32(st_fx->core_brate_fx, SID_2k40)&&st_fx->bws_cnt_fx==0)
            {
                hb_synth_fx_exp = 3;
                move16();
            }
        ...
        IF (EQ_16(st_fx->core_fx,ACELP_CORE)
            && !st_fx->bfi_fx
            && st_fx->prev_bfi_fx
            && GE_32(st_fx->last_total_brate_fx, HQ_48k) 
            && EQ_16(st_fx->last_codec_mode, MODE2) 
            && (EQ_16(st_fx->last_core_bfi, TCX_20_CORE)  || EQ_16(st_fx->last_core_bfi, TCX_10_CORE) )
            && st_fx->plcInfo.concealment_method == TCX_NONTONAL
            && LT_32(st_fx->plcInfo.nbLostCmpt, 4)  )
        {
            tmps = 0;
            IF( GE_16(output_frame,L_FRAME16k))
            {
        ...
        test();
        test();
        IF ( NE_16(st_fx->extl_fx,-1)||(st_fx->bws_cnt_fx>0&&EQ_16(st_fx->core_fx,ACELP_CORE)))
        {
            ...
            {
                test();
                IF( EQ_16(st_fx->extl_fx,SWB_BWE_HIGHRATE)||EQ_16(st_fx->extl_fx,FB_BWE_HIGHRATE))
                {
            ...
            test();
            test();
            IF ( (NE_16(st_fx->extl_fx,st_fx->last_extl_fx)||(EQ_16(st_fx->extl_fx,st_fx->last_extl_fx)&&EQ_16((st_fx->core_fx^st_fx->last_core_fx),HQ_CORE)))
                 && !(EQ_16(st_fx->extl_fx,SWB_CNG)  && EQ_16(st_fx->last_extl_fx,SWB_TBE)) )
            {
            ...
            ELSE IF ( LT_16(tmps,st_fx->old_bwe_delay_fx))
            {
            ...
            ELSE IF (GT_16(tmps,st_fx->old_bwe_delay_fx))
            {
            ...
            test();
            test();
            IF( ( st_fx->ppp_mode_dec_fx || (EQ_16(st_fx->nelp_mode_dec_fx,1)&&EQ_16(st_fx->bfi_fx,1)))&&EQ_16(st_fx->L_frame_fx,st_fx->last_L_frame_fx)&&(st_fx->bws_cnt_fx>1||st_fx->last_extl_fx!=-1))

            {
                Copy( st_fx->old_hb_synth_fx, hb_synth_fx, output_frame );
                IF(LT_16(st_fx->prev_hb_synth_fx_exp, 14))
                {
            ...
            test();
            IF ( GE_16(output_frame, L_FRAME32k)&&GT_16(st_fx->extl_fx,SWB_CNG))
            {
                SWITCH (output_frame)
                {
                case L_FRAME8k:
    ...
    ELSE /* MODE2 PART */
    {
        ...
        if( EQ_16(frameMode, FRAMEMODE_NORMAL))
        {
            st_fx->m_decodeMode = DEC_NO_FRAM_LOSS;
            move16();
        }

        IF( EQ_16(frameMode, FRAMEMODE_MISSING))
        {
            test();
            test();
            IF( st_fx->use_partial_copy && GE_16(st_fx->rf_frame_type, RF_TCXFD)&&LE_16(st_fx->rf_frame_type,RF_TCXTD2))
            {
                st_fx->m_decodeMode = DEC_NO_FRAM_LOSS;
                move16();
            }
        ...
        IF( st_fx->igf )
        {
            ...
            test();
            test();
            if( EQ_32(st_fx->output_Fs_fx,8000)||(EQ_32(st_fx->output_Fs_fx,16000)&&EQ_16(st_fx->L_frame_fx,L_FRAME16k)))
            {
                st_fx->extl_fx = -1;
                move16();
            }
            ...
            IF( EQ_16(st_fx->m_frame_type, ACTIVE_FRAME))
            {
                test();
                test();
                IF( ( st_fx->bfi_fx == 0 || st_fx->last_core_fx == ACELP_CORE ) && st_fx->core_fx == ACELP_CORE )
                {
                    ...
                    ELSE IF( EQ_16(st_fx->extl_fx, SWB_TBE)||EQ_16(st_fx->extl_fx,FB_TBE))
                    {
                        ...
                        test();
                        IF( EQ_16(st_fx->extl_fx, FB_TBE)&&EQ_16(output_frame,L_FRAME48k))
                        {
                            fb_tbe_dec_fx( st_fx, fb_exc_fx, Q_fb_exc, hb_synth_fx, hb_synth_fx_exp );
                        }
                    }
                ...
                ELSE
                {
                    IF( EQ_16(st_fx->last_core_fx,ACELP_CORE))
                    {
                        test();
                        test();
                        test();
                        test();
                        IF( ( EQ_16(st_fx->bwidth_fx, SWB)||EQ_16(st_fx->bwidth_fx,FB))&&
                        (( EQ_16(st_fx->last_extl_fx, SWB_TBE) || EQ_16(st_fx->last_extl_fx, FB_TBE) ) && EQ_16(st_fx->last_codec_mode, MODE2)  ) )
                        {
                            GenTransition_fx( st_fx->syn_overlap_fx, st_fx->old_tbe_synth_fx, 2*NS2SA(st_fx->output_Fs_fx, DELAY_BWE_TOTAL_NS), hb_synth_fx,
                                              st_fx->genSHBsynth_Hilbert_Mem_fx, st_fx->genSHBsynth_state_lsyn_filt_shb_local_fx, 
                                             st_fx->mem_resamp_HB_32k_fx,
                                             &(st_fx->syn_dm_phase_fx), st_fx->output_Fs_fx, st_fx->int_3_over_2_tbemem_dec_fx, st_fx->rf_flag, st_fx->total_brate_fx );

                            hb_synth_fx_exp = st_fx->prev_Q_bwe_syn2;
                            move16();
                        }
                        ELSE IF ( st_fx->bwidth_fx == WB && st_fx->last_extl_fx == WB_TBE )
                        {
                            GenTransition_WB_fx( st_fx->syn_overlap_fx, st_fx->old_tbe_synth_fx, st_fx->prev_Qx, 2*NS2SA(st_fx->output_Fs_fx, DELAY_BWE_TOTAL_NS), hb_synth_fx,
                                                 st_fx->state_lsyn_filt_shb_fx, st_fx->state_lsyn_filt_dwn_shb_fx, st_fx->output_Fs_fx, st_fx->mem_resamp_HB_fx );
        ...
        {
            nab = s_min( st_fx->cldfbAna_fx->no_channels, st_fx->cldfbSyn_fx->no_channels );
            ...
            test();
            test();
            IF ( st_fx->hFdCngDec_fx != NULL && (EQ_32(st_fx->sr_core,8000)||EQ_32(st_fx->sr_core,12800)||EQ_32(st_fx->sr_core,16000))&&LE_32(st_fx->total_brate_fx,ACELP_32k))
            {
                /***************************************
                 In CLDFB domain:
                ...
                ****************************************/
                ...
                hFdCngDec->hFdCngCom->likelihood_noisy_speech = mult_r(hFdCngDec->hFdCngCom->likelihood_noisy_speech, 32440/*0.99 Q15*/);
                IF ( hFdCngDec->hFdCngCom->flag_noisy_speech != 0 )
                {
                    hFdCngDec->hFdCngCom->likelihood_noisy_speech = add(hFdCngDec->hFdCngCom->likelihood_noisy_speech, 328/*0.01 Q15*/);
                    move16();
                }
                st_fx->lp_noise = hFdCngDec->lp_noise;
                move32();
                ApplyFdCng( output_sp, 0, realBuffer, imagBuffer, &st_fx->scaleFactor.hb_scale, hFdCngDec, st_fx->m_frame_type, st_fx, concealWholeFrame, 0);
            ...
            test();
            test();
            IF( st_fx->flag_cna == 0 && EQ_16(st_fx->L_frame_fx,L_FRAME16k)&&EQ_16(st_fx->last_flag_cna,1)
                && ( (st_fx->last_core_fx == ACELP_CORE && NE_16(st_fx->last_coder_type_fx,AUDIO) ) || EQ_16(st_fx->last_core_fx,TCX_20_CORE) || EQ_16(st_fx->last_core_fx,AMR_WB_CORE)  ) )
            {
                FOR (i=0; i < st_fx->L_frame_fx/2; i++)
                {
                    output_sp[i] = add( output_sp[i], st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth2[i+5*st_fx->L_frame_fx/4] );
                    move16();
                }
            }
            ...
            IF( EQ_16(st_fx->m_frame_type,ACTIVE_FRAME))
            {
                timeIn_e = s_max(0, sub(getScaleFactor16( output_sp, st_fx->L_frame_fx ), 3));
                IF( NE_16(st_fx->core_fx,ACELP_CORE))
                {
                    timeIn_e = s_max(0, s_min(sub(getScaleFactor16( pcmbufFB, st_fx->L_frameTCX ), 3),timeIn_e));
                }
                Scale_sig( output_sp, st_fx->L_frame_fx, timeIn_e );
                timeIn_e = negate( timeIn_e );
                cldfbAnalysisFiltering( st_fx->cldfbAna_fx, realBuffer, imagBuffer, &st_fx->scaleFactor, output_sp, timeIn_e, CLDFB_NO_COL_MAX, workBuffer );
            ...
            ELSE
            {
                ...
                ELSE IF( EQ_16(st_fx->L_frame_fx,L_FRAME16k)&&EQ_16(st_fx->last_flag_cna,1)&&((EQ_16(st_fx->last_core_fx,ACELP_CORE)&&NE_16(st_fx->last_coder_type_fx,AUDIO))||EQ_16(st_fx->last_core_fx,TCX_20_CORE)||EQ_16(st_fx->last_core_fx,AMR_WB_CORE)))
                {
                    FOR( i=0; i < st_fx->L_frame_fx/2; i++ )
                    {
                        timeDomainBuffer[i] = add( timeDomainBuffer[i], st_fx->hFdCngDec_fx->hFdCngCom->olapBufferSynth2[i+5*st_fx->L_frame_fx/4] );
                        move16();
                    }
                }
                /* check if the CLDFB works on the right sample rate */
                IF( NE_16((st_fx->cldfbAna_fx->no_channels * st_fx->cldfbAna_fx->no_col),st_fx->L_frame_fx))
                {
                    Word16 newCldfbBands = CLDFB_getNumChannels(L_mult0(st_fx->L_frame_fx, 50));
                ...
                move16();
                test();
                IF ( EQ_16( st_fx->bwidth_fx, NB )&&GT_16(st_fx->cldfbSyn_fx->no_channels,10))
                {
                ...
                timeIn_e = negate(timeIn_e);
                cldfbAnalysisFiltering( st_fx->cldfbAna_fx, realBuffer, imagBuffer, &st_fx->scaleFactor, timeDomainBuffer, timeIn_e, CLDFB_NO_COL_MAX, workBuffer) ;
            }
            ...
            IF( st_fx->p_bpf_noise_buf )
            {
                ...
                addBassPostFilterFx( st_fx->p_bpf_noise_buf, realBuffer, imagBuffer, st_fx->cldfbBPF_fx, workBuffer,
                                     timeInBpf_e, CLDFB_NO_COL_MAX, st_fx->cldfbAna_fx->no_col, st_fx->cldfbAna_fx->no_channels, &st_fx->scaleFactor );
            ...
            IF (GT_32(st_fx->output_Fs_fx, 8000))
            {
            ...
            cldfbSynthesisFiltering(st_fx->cldfbSyn_fx, realBuffer, imagBuffer, &st_fx->scaleFactor, output_sp, timeIn_e, CLDFB_NO_COL_MAX, workBuffer );
            ...
            test();
            IF( EQ_16(st_fx->last_codec_mode,MODE1)&&GT_16(st_fx->last_core_bfi,ACELP_CORE))
            {
                Copy_Scale_sig( st_fx->delay_buf_out_fx, output_sp, delay_comp, negate(timeIn_e) );  /* copy the HQ/ACELP delay synchronization buffer at the beginning of ACELP frame */
                IF( EQ_16(st_fx->core_fx,ACELP_CORE))
                {
                    ...
                    FOR (i = 0; i < tmps; i++)
                    {
                        output_sp[i+delay_comp] = msu_r(L_mult(output_sp[i+delay_comp], alpha), shr(st_fx->old_out_fx[i+nz], timeIn_e), add(alpha, -32768));
                        move16();
                ...
                ELSE
                {
                    IF( EQ_32(st_fx->output_Fs_fx,8000))
                    {
            ...
            IF( EQ_16(st_fx->core_fx,ACELP_CORE)&&!(st_fx->con_tcx))
            {
                ...
                Copy_Scale_sig( output_sp, st_fx->previoussynth_fx, output_frame, timeIn_e );
            }
            ELSE
            {
        ...
        IF( GE_16(output_frame,L_FRAME16k))
        {
            ...
            Copy_Scale_sig( st_fx->prev_synth_buffer_fx, output_sp, delay_tdbwe, negate(timeIn_e) );
            Copy( tmp_buffer_fx, output_sp + delay_tdbwe, output_frame - delay_tdbwe );
            Copy_Scale_sig( tmp_buffer_fx + output_frame - delay_tdbwe, st_fx->prev_synth_buffer_fx, delay_tdbwe, timeIn_e);
        }

        test();
        IF( st_fx->igf != 0 && EQ_16( st_fx->m_frame_type, ACTIVE_FRAME ))
        {
            test();
            test();
            test();
            test();
            IF( st_fx->bfi_fx == 0 && EQ_16(st_fx->core_fx, ACELP_CORE)&&(st_fx->tec_flag!=0||st_fx->tfa_flag!=0)&&GT_32(st_fx->output_Fs_fx,8000))
            {
                tmp16 = 0;
                move16();
                if (EQ_16(st_fx->tec_flag, 2))
                {
                    tmp16 = 1;
                    move16();
                }
            ...
            test();
            IF( (( ( st_fx->bfi_fx == 0 || st_fx->last_core_fx == ACELP_CORE ) && st_fx->core_fx == ACELP_CORE ) ||
                 ( ( EQ_16(st_fx->last_core_fx, ACELP_CORE)  ) && (NE_16(st_fx->bwidth_fx, NB)  && EQ_16(st_fx->last_codec_mode, MODE2) ) ))
                && (GT_32( st_fx->output_Fs_fx, 8000 ) ) )
            {
        ...
        test();
        test();
        IF ( (st_fx->bfi_fx && GT_16(st_fx->last_core_fx, ACELP_CORE))||GT_16(st_fx->core_fx,ACELP_CORE))
        {
            test();
            test();
            test();
            test();
            IF ( GT_16(st_fx->last_core_bfi, ACELP_CORE)||(st_fx->bfi_fx&&st_fx->last_core_fx>ACELP_CORE)||(st_fx->prev_bfi_fx&&st_fx->last_con_tcx))
            {
                Copy_Scale_sig(st_fx->FBTCXdelayBuf, output_sp, tmps, negate(timeIn_e));
                Copy_Scale_sig(pcmbufFB, output_sp + tmps, sub(st_fx->L_frameTCX, tmps), negate(timeIn_e));
            }
            ELSE
            {
                Word16 step, alpha;
                ...
                Copy_Scale_sig( pcmbufFB + tmps, output_sp + shl(tmps,1), sub(st_fx->L_frameTCX, shl(tmps,1)), negate(timeIn_e) );
            }

            Copy( pcmbufFB + st_fx->L_frameTCX - tmps, st_fx->FBTCXdelayBuf, tmps );

            test();
            IF( st_fx->bfi_fx && GT_16(st_fx->last_core_fx, ACELP_CORE))
            {
                IF( EQ_32(st_fx->output_Fs_fx, 8000))
                {
        ...
        ELSE IF( (EQ_16(st_fx->last_codec_mode,MODE2))&&(GT_16(st_fx->last_core_fx,ACELP_CORE)))
        {
            Word16 step, alpha;

            Copy_Scale_sig(st_fx->FBTCXdelayBuf, output_sp, delta, negate(timeIn_e));
            ...
            {
                output_sp[i] = msu_r(L_mult(output_sp[i], alpha), shr(st_fx->FBTCXdelayBuf[i], timeIn_e), add(alpha, -32768));
                move16();
                alpha = add(alpha, step);
            }
        }
        ...
        Scale_sig( output_sp, output_frame, timeIn_e );
    ...
    IF (EQ_16(st_fx->codec_mode, MODE1))
    {
        ...
        Copy( st_fx->synth_history_fx+output_frame, st_fx->synth_history_fx, output_frame-post_hq_delay+NS2SA_fx2( st_fx->output_Fs_fx, PH_ECU_MEM_NS ));
        ...
        set16_fx( st_fx->old_synthFB_fx+2*output_frame-post_hq_delay, 0, post_hq_delay );
    ...
    test();
    IF( st_fx->last_is_cng == 0 && EQ_16(st_fx->codec_mode,MODE2))
    {
        st_fx->bfi_fx = 0;
        move16();
        IF( st_fx->use_partial_copy && GE_16(st_fx->rf_frame_type, RF_TCXFD)&&LE_16(st_fx->rf_frame_type,RF_TCXTD2))
        ...
        ELSE IF( EQ_16(st_fx->m_decodeMode, DEC_CONCEALMENT_EXT))
        {
            st_fx->bfi_fx = 1;
            move16();
        }
    ...
    test();
    IF ( GT_32(st_fx->core_brate_fx,SID_2k40)&&EQ_16(st_fx->first_CNG_fx,1))
    {
        if( GE_16(st_fx->act_cnt_fx,BUF_DEC_RATE))
        {
            st_fx->act_cnt_fx = 0;
            move16();
        }
        ...
        test();
        if( (EQ_16(st_fx->act_cnt_fx,BUF_DEC_RATE))&&(st_fx->ho_hist_size_fx>0))
        {
            st_fx->ho_hist_size_fx = sub(st_fx->ho_hist_size_fx,1);
        }

        st_fx->act_cnt2_fx = add(st_fx->act_cnt2_fx,1);
        if( GE_16(st_fx->act_cnt2_fx,MIN_ACT_CNG_UPD))
        {
    ...
    test();
    test();
    if ( LE_32(st_fx->core_brate_fx,SID_2k40)&&st_fx->first_CNG_fx==0&&EQ_16(st_fx->cng_type_fx,LP_CNG))
    {
    ...
    IF( EQ_16(st_fx->codec_mode, MODE1))
    {
        updt_bw_switching_fx( st_fx, synth_fx, Qpostd );
    }
    ELSE
    {
    ...
    test();
    test();
    IF( st_fx->bfi_fx || (NE_32(st_fx->core_brate_fx,FRAME_NO_DATA)&&NE_32(st_fx->core_brate_fx,SID_2k40)))
    {
    ...
    st_fx->last_total_brate_ber_fx = st_fx->total_brate_fx;
    move32();
    if( st_fx->bfi_fx == 0 )
    {
