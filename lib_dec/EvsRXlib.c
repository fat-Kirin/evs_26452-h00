                         ...
                         Word16 jbmSafetyMargin)
{
    ...
    *phEvsRX = (EVS_RX_HANDLE) calloc(1, sizeof(struct EVS_RX) );
    ...
    ELSE IF(EQ_32(st->output_Fs_fx, 16000))
    {
        wss = 2;
        move16();
        css = 1;
    ...
    ELSE IF(EQ_32(st->output_Fs_fx, 32000))
    {
        wss = 4;
        move16();
        css = 2;
    ...
    ELSE IF(EQ_32(st->output_Fs_fx, 48000))
    {
        wss = 6;
        move16();
        css = 3;
...
)
{
    ...
    test();
    IF(NE_16(partialCopyFrameType, RF_NO_DATA)&&partialCopyOffset!=0)
    {
                  ...
                  Word32 systemTimestamp_ms)
{
    ...
    WHILE(LT_16(pcmdsp_fifo_nReadableSamples(hEvsRX->hFifoAfterTimeScaler), soundCardFrameSize))
    {
        ...
        IF(dataUnit)
        {
            IF( NE_16(st->codec_mode,0))
            {
                tmp = 0;
                if (EQ_16(dataUnit->partial_frame,1))
                {
                    tmp = 1;
                }
        ...
        ELSE IF( EQ_16(st->codec_mode, MODE2))
        {
            IF(st->bfi_fx == 0)
            {
                evs_dec_fx(st, pcmBuf, FRAMEMODE_NORMAL);
            }
            ELSE IF ( EQ_16(st->bfi_fx,2))
            {
        ...
        END_SUB_WMOPS;
        test();
        IF( EQ_16(st->codec_mode, MODE1)||EQ_16(st->codec_mode,MODE2))
        {
        ...
        if(LT_16(scale, APA_MIN_SCALE))
            scale = APA_MIN_SCALE;
        move16();
        if(GT_16(scale, APA_MAX_SCALE))
            scale = APA_MAX_SCALE;
        move16();
...
static Word16 isSidFrame( Word16 size )
{
    ...
    ret = 0;
    move16();
    if(EQ_16(size, SID_1k75 / 50))
    {
        ret = 1; /* AMR-WB SID */                                               move16();
    }
    if(EQ_16(size, SID_2k40 / 50))
    {
