...
)
{
    ...
    Word16 en_noncoded, min_env, max_env;
    ...
    st_fx->bwe_highrate_seed_fx = extract_l(L_mult0(pitch_buf[0],pitch_buf[3]));
    ...
    test();
    IF( EQ_16(st_fx->last_core_fx, HQ_CORE)||NE_16(st_fx->last_extl_fx,st_fx->extl_fx))
    {
    ...
    IF( st_fx->bfi_fx )
    {
        ...
        test();
        test();
        IF( (EQ_16(st_fx->clas_dec, VOICED_CLAS)||EQ_16(st_fx->clas_dec,INACTIVE_CLAS))&&LE_16(st_fx->nbLostCmpt,3))
        {
        ...
        IF( is_transient )
        {
            ...
            FOR( k=0; k<NUM_TIME_SWITCHING_BLOCKS; k++ )
            {
                ...
                temp4 = Find_Max_Norm32(t_audio32 + temp, tmpS);
                Copy_Scale_sig32_16(t_audio32 + temp, st_fx->t_audio_prev_fx + i_mult2(k, tmpS), tmpS, temp4);
        ...
        ELSE
        {
            ...
            temp4 = Find_Max_Norm32(t_audio32 + temp, tmpS);
            Copy_Scale_sig32_16(t_audio32 + temp, st_fx->t_audio_prev_fx, tmpS, temp4);
    ...
    ELSE
    {
        ...
        IF( is_transient )
        {
            ...
            test();
            IF( EQ_16(st_fx->extl_fx, SWB_BWE_HIGHRATE)||EQ_16(output_frame,L_FRAME32k))
            {
            ...
            FOR( k = 0; k < NUM_TIME_SWITCHING_BLOCKS; k++ )
            {
                ...
                IF( EQ_16(st_fx->extl_fx, FB_BWE_HIGHRATE))
                {
        ...
        ELSE    /* !is_transient */
        {
            ...
            test();
            IF( GE_16(nBits, 9 + NBITS_GLOB_GAIN_BWE_HR)&&sum16_fx(nq,Nsv)>0)
            {
                ind1 = (Word16) get_next_indice_fx( st_fx, NBITS_GLOB_GAIN_BWE_HR );
            ...
            IF( GT_16(Nsv2, Nsv))
            {
                ...
                set16_fx( nq_tmp + Nsv, 0, sub(Nsv2, Nsv) );
            }
            ...
            IF( GT_16(nBits_total, NBITS_THRESH_BWE_HR))
            {
            ...
            ELSE
            {
                ...
                if( EQ_16(output_frame,L_FRAME48k))
                {
                    tmpS = sub(width_noncoded,2*WIDTH_NONTRANS_FREQ_COEF);
                }

                ptr32 = &t_audio32[NUM_NONTRANS_END_FREQ_COEF];
                L_temp = L_mult(18022 /* 2.2 in Q13*/,en_noncoded);  /* Q13 * Q9 -> Q23 */
                L_temp2 = Mult_32_16(L_temp,32768/160); /* step */
                FOR( j=0; j<tmpS; j++ )
                {
                ...
                L_temp = L_mult(21299 /* 0.65 in Q15*/,en_noncoded);  /* Q15 * Q9 -> Q25 */
                L_temp2 = Mult_32_16(L_temp,32768/320); /* step */
            ...
            IF( EQ_16(output_frame, L_FRAME48k))
            {
                ...
                {
                    L_temp = Mult_32_16(*ptr32, overlap_coefs_48kHz_fx[i]);
                    *ptr32++ = L_temp;
                    move32();
                }
            }
            ELSE
            {
                FOR( i=0; i<NSV_OVERLAP*WIDTH_BAND; i++ )
                {
                    L_temp = Mult_32_16(*ptr32, overlap_coefs_fx[i]);
                    *ptr32++ = L_temp;
                    move32();
                }
            }
    ...
    window_ola_fx( t_audio32_tmp, hb_synth_fx, &t_audio_exp, st_fx->L_old_wtda_swb_fx, &st_fx->Q_old_wtda, output_frame,
    ...
    IF( !st_fx->bfi_fx )
    {
        ...
        FOR( j=0; j<4; j++ )
        {
            ...
            test();
            test();
            IF (temp3 < 0 || (GT_32(L_temp, L_EnergyLT)&&temp3==0))
            {
                IsTransient = 1;
                move16();
                pos = j;
        ...
        IF( IsTransient != 0 && pos > 0 && LT_32(L_tilt_wb, 16777216L*3/*tilt_wb in Q24*/)&&GT_16(pitch,500*16/*Q4*/))
        {
            ...
            IF( EQ_16(st_fx->last_extl_fx, st_fx->extl_fx))
            {
    ...
    IF( EQ_16(st_fx->last_core_fx, HQ_CORE)||NE_16(st_fx->last_extl_fx,st_fx->extl_fx))
    {
        IF( LT_32(L_tilt_wb, 16777216L*3/*tilt_wb in Q24*/))
        {
            ...
            {
                st_fx->L_old_wtda_swb_fx[i] = mult_r(st_fx->L_old_wtda_swb_fx[i], temp);
                move16();
            }
            ...
            FOR( i=0; i<tmpS; i++ )
            {
                st_fx->t_audio_prev_fx[i] = mult_r(st_fx->t_audio_prev_fx[i], temp);
                move16();
            }
        }
    }
