...
)
{
    ...
    *tmp_tc = st_fx->bfi_pitch_fx;
    ...
    IF( EQ_16(L_frame,L_FRAME))
    {
        test();
        test();
        IF ( (LT_16(round_fx(L_shl(st_fx->old_pitch_buf_fx[2*NB_SUBFR-1], 6)), shl(mult(29491, st_fx->bfi_pitch_fx), 1))&&
             ...
             GE_16(st_fx->upd_cnt_fx, MAX_UPD_CNT) )      /* or last update too far in the past */
        {
    ...
    ELSE  /* L_frame == L_FRAME16k */
    {
        test();
        test();
        IF ( (LT_16(round_fx(L_shl(st_fx->old_pitch_buf_fx[2*NB_SUBFR16k-1], 6)), shl(mult(29491, st_fx->bfi_pitch_fx), 1))&&
    ...
    test();
    test();
    IF( st_fx->last_coder_type_fx == UNVOICED && LE_16(st_fx->nbLostCmpt, 3))
    {
        /* last good frame was clearly unvoiced */
        alpha = _ALPHA_UU_FX;
        move16();
    }
    ELSE IF( EQ_16(st_fx->last_coder_type_fx,AUDIO)||EQ_16(st_fx->last_good_fx,INACTIVE_CLAS))
    {
        test();
        IF( st_fx->Last_GSC_pit_band_idx_fx > 0 && GT_16(st_fx->nbLostCmpt,1))
        {
            alpha = 26214;
            move16();
        }
        ELSE IF( LE_16(st_fx->nbLostCmpt,5))
        {
            alpha = 32604;
            move16();
        }
    ...
    ELSE IF( EQ_16(st_fx->last_good_fx,UNVOICED_CLAS))
    {
    ...
    ELSE IF( EQ_16(st_fx->last_good_fx,UNVOICED_TRANSITION))
    {
        alpha = _ALPHA_UT_FX;
        move16();
    }
    ELSE IF( EQ_16(st_fx->last_good_fx,ONSET)&&LE_16(st_fx->nbLostCmpt,3)&&(EQ_16(st_fx->last_coder_type_fx,GENERIC)||EQ_16(st_fx->last_coder_type_fx,TRANSITION)))
    {
        alpha = 26214;
        move16();    /* mild convergence to 0 for the first 3 erased frames 0.8 in Q15 */
    }
    ELSE IF( ( EQ_16(st_fx->last_good_fx,VOICED_CLAS)||EQ_16(st_fx->last_good_fx,ONSET))&&LE_16(st_fx->nbLostCmpt,3))
    {
        alpha = _ALPHA_V_FX;
        move16();   /* constant for the first 3 erased frames */
    }
    ELSE IF( EQ_16(st_fx->last_good_fx,SIN_ONSET))
    {
        alpha = _ALPHA_S_FX;
        move16();
    }
    test();
    test();
    IF( GE_16(st_fx->last_good_fx,VOICED_CLAS)&&LT_16(st_fx->last_good_fx,INACTIVE_CLAS)&&NE_16(st_fx->last_coder_type_fx,AUDIO))
    {
        IF( EQ_16(st_fx->nbLostCmpt,1)) /* if first erased frame in a block, reset harmonic gain */
        {
    ...
    test();
    test();
    IF( (GE_16(st_fx->last_good_fx,UNVOICED_TRANSITION)&&LT_16(st_fx->last_good_fx,INACTIVE_CLAS))||
        ( (EQ_16(st_fx->last_coder_type_fx,AUDIO)  || EQ_16(st_fx->last_good_fx,INACTIVE_CLAS) ) && st_fx->Last_GSC_pit_band_idx_fx > 0) )
    {
        ...
        test();
        test();
        IF( EQ_16(st_fx->last_good_fx,UNVOICED_TRANSITION)&&(EQ_16(st_fx->last_coder_type_fx,GENERIC)||EQ_16(st_fx->last_coder_type_fx,TRANSITION)))
        {
        ...
        ELSE
        {
            ...
            IF(!(GE_16(st_fx->last_good_fx,VOICED_CLAS)&&LT_16(st_fx->last_good_fx,INACTIVE_CLAS)&&NE_16(st_fx->last_coder_type_fx,AUDIO)&&GT_16(st_fx->nbLostCmpt,1)))
            {
        ...
        test();
        test();
        IF( (EQ_16(st_fx->last_coder_type_fx,AUDIO)||EQ_16(st_fx->last_good_fx,INACTIVE_CLAS))&&st_fx->Last_GSC_pit_band_idx_fx>0)
        {
    ...
    *-----------------------------------------------------------------*/
    ...
    test();
    test();
    IF( EQ_16(st_fx->last_coder_type_fx,AUDIO)||(EQ_16(st_fx->last_good_fx,INACTIVE_CLAS)&&LE_32(st_fx->total_brate_fx,ACELP_24k40)&&!st_fx->Opt_AMR_WB_fx))
    {
    ...
    ELSE
    {
        ...
        test();
        test();
        if( EQ_16(st_fx->last_good_fx,UNVOICED_TRANSITION)&&(EQ_16(st_fx->last_coder_type_fx,GENERIC)||EQ_16(st_fx->last_coder_type_fx,TRANSITION))&&gainCNG>0)
        {
        ...
        test();
        test();
        if( (EQ_16(st_fx->last_good_fx,UNVOICED_CLAS)||EQ_16(st_fx->last_good_fx,INACTIVE_CLAS))&&NE_16(st_fx->last_coder_type_fx,UNVOICED))
        {
    ...
    ELSE IF( GE_16(st_fx->last_good_fx,UNVOICED_TRANSITION)&&LT_16(st_fx->last_good_fx,INACTIVE_CLAS))
    {
    ...
    test();
    IF( EQ_16(st_fx->last_coder_type_fx,AUDIO)||EQ_16(st_fx->last_good_fx,INACTIVE_CLAS))
    {
...
)
{
    ...
    av_pred_en = s_max(av_pred_en, -14 * (1 << 10));
