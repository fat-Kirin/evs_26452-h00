...
)
{
    ...
    test();
    test();
    IF ((LT_32(L_deposit_h(nFrameLength),pitchStart))
        || (pitchStart <= 0) ||  (pitchEnd <= 0)
        || (LT_16(nSubframes, 1) )
        || (GT_16(nSubframes, 5) )
        || (LE_32(Mpy_32_16_1(pitchEnd,add(nSubframes,1)),Mpy_32_16_1(pitchStart,sub(nSubframes,1))))
        || (src_exc-dst_exc >= 0))
    {
        /* This is error handling and recovery that should never occur. */
        test();
        IF (src_exc != dst_exc && LE_16(nFrameLength, 1200))
        {
    ...
    IF ((k >= 0) && LE_16(add(k,1) , NB_PULSES_MAX))
    {
        ...
        FOR (i = 1; i <= k; i++)
        {
            ...
            IF (GT_32(L_deposit_h(roundedCycleDelta), cycleDelta32))
            {
        ...
        IF (nSamplesDelta < 0)
        {
            ...
            IF (GT_16(iMinPos1 , add(roundedPitchStart , shr(iDeltaSamples[0],1))))
            {
        ...
        {
            /* Find the location of the minimum energy before the first pulse */
            IF (GT_16(iMinPos1 , roundedPitchStart))
            {
