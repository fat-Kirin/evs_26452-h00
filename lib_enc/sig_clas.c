...
)
{
    ...
    test();
    test();
    IF (localVAD == 0 || EQ_16(*coder_type,UNVOICED)||LT_16(relE,-1536))
    {
        clas = UNVOICED_CLAS;
        move16();
    }
    ...
    *uc_clas = clas;
    ...
    if( ( (EQ_16(*coder_type,UNVOICED))||
            (NE_16(st->input_bwidth_fx,NB)  && LT_16(fmerit1,13435)  && GT_16(st->mold_corr_fx,21299)  ) ||                  /* WB case */
            (EQ_16(st->input_bwidth_fx,NB)  && LT_16(mult_r(fmerit1,28836),13435)  && GT_16(st->mold_corr_fx,18022)  ) ) &&  /* NB case */
            GT_16(relE,-3840)  && LT_16(st->lt_dec_thres_fx,768)  )       /* to compute unvoiced on frame that tends to speech */
    {
    ...
    test();
    if( GE_16(clas,VOICED_TRANSITION)&&st->tc_cnt_fx>=0)
    {
        st->tc_cnt_fx = add(st->tc_cnt_fx,1);
        move16();
    }

    if( GT_16(st->tc_cnt_fx,2))
    {
        st->tc_cnt_fx = -1;
        move16();
    }
    ...
    IF ( EQ_16(st->codec_mode,MODE1))
    {
        ...
        test();
        if( GT_32(st->total_brate_fx,ACELP_9k60)&&EQ_16(*coder_type,UNVOICED))
        {
            *coder_type = GENERIC;
            move16();
        }
        ...
        test();
        test();
        if( EQ_32(st->total_brate_fx,ACELP_9k60)&&EQ_16(*coder_type,UNVOICED)&&st->audio_frame_cnt_fx!=0)
        {
            *coder_type = GENERIC;
            move16();
        }
        ...
        if( localVAD == 0 && (
                    (
                        EQ_16(*coder_type,UNVOICED) 
                        && ( ( st->Opt_SC_VBR_fx == 0) || ( ( st->Opt_SC_VBR_fx == 1 ) && st->vbr_generic_ho_fx == 0 && GT_16(st->last_coder_type_fx,UNVOICED)  ))  )
                    || EQ_16(*coder_type,TRANSITION)  || EQ_16(*coder_type,VOICED)  )
        ...
        test();
        test();
        if( EQ_16(*coder_type,GENERIC)&&EQ_16(unmod_coder_type,UNVOICED)&&(st->Opt_SC_VBR_fx==1))
        {
        ...
        test();
        if ( GT_16(*coder_type,UNVOICED)&&(st->Opt_SC_VBR_fx==1))
        {
        ...
        move16();
        test();
        if( localVAD == 0 && EQ_16( *coder_type, UNVOICED ))
        {
        ...
        test();
        IF( localVAD != 0 && GE_16(st->tc_cnt_fx,1))   /* TC mode is allowed only in active signal */
        {
        ...
        test();
        test();
        if( (GE_32(st->total_brate_fx,ACELP_24k40))&&NE_16(*coder_type,GENERIC)&&NE_16(*coder_type,TRANSITION))
        {
            *coder_type = GENERIC;
            move16();
        }
        ...
        test();
        test();
        if( EQ_16(*coder_type,VOICED)&&EQ_16(st->input_bwidth_fx,NB)&&LT_16(relE,-2560)&&LE_32(st->total_brate_fx,ACELP_8k00))
        {
            *coder_type = GENERIC;
            move16();
        }
