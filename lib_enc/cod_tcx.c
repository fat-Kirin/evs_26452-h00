...
)
{
       ...
       stack memory usage. */
    {
        ...
        IF (st->tcx_lpc_shaped_ari == 0)   /* old arithmetic coder */
        {
            ...
            IF ((tcx_cfg->ctx_hm != 0) && (st->last_core_fx != ACELP_CORE))
            {
                ...
                test();
                IF ((tcxonly == 0) && (LT_16(st->tcxltp_pitch_int, st->L_frame_fx)))
                {
            ...
            if (GT_16(LPDmem->tcx_target_bits_fac, 0x5000))
            {
                LPDmem->tcx_target_bits_fac = 0x5000;
                move16();
            }
            if (LT_16(LPDmem->tcx_target_bits_fac, 0x3000))
            {
        ...
        ELSE    /* low rates: new arithmetic coder */
        {
            ...
            IF (LT_16(st->tcxltp_pitch_int, st->L_frame_fx))
            {
                tmp32 = L_shl(L_mult0(st->L_frame_fx, st->pit_res_max), 1+kLtpHmFractionalResolution+1);
                tmp1 = add(imult1616(st->tcxltp_pitch_int, st->pit_res_max), st->tcxltp_pitch_fr);
                LtpPitchLag = div_l(tmp32, tmp1);
...

        {
            ...
            ELSE
            {
                ...
                ELSE {
                    ...
                    (GT_16(abs_s(st->noiseLevelMemory), 5)))
                    {
     ...
     *-----------------------------------------------------------*/
    {
         ...
         * Apply gain                                                *
         *-----------------------------------------------------------*/
        IF (EQ_16(st->tcx_cfg.coder_type, INACTIVE))
        {
        ...
        *spectrum_e = add(*spectrum_e, gain_tcx_e);
        ...
        IF ((EQ_16(L_frame, shr(st->L_frame_fx, 1)))&&(tcxonly!=0))
        {
            ...
            test();
            test();
            if (((tcx_cfg->fIsTNSAllowed != 0) && (fUseTns != 0)) || (GT_16(L_spec, L_frame)))
            {
                L = L_spec;
                move16();
            }
...

        {
            ...
            *-----------------------------------------------------------*/
            ...
            test();
            test();
            IF ((st->last_core_fx > ACELP_CORE) && (((EQ_16(L_frameTCX, shr(st->L_frameTCX, 1)))&&(st->tcxonly!=0))||(EQ_16(st->tcx_cfg.tcx_last_overlap_mode,TRANSITION_OVERLAP))))
            {
                ...
                ELSE
                {
                    ...
                    move16();
                    test();
                    if ((stop == 0) || (EQ_16(tcx_cfg->tcx_last_overlap_mode, MIN_OVERLAP)))
                    {
                        tmp1 = tcx_cfg->tcx_last_overlap_mode;
                        move16();
                    }
...
)
{
     ...
     * Windowing                                                 *
     *-----------------------------------------------------------*/
    IF (EQ_16(st->tcx_cfg.tcx_last_overlap_mode, TRANSITION_OVERLAP))
    {
