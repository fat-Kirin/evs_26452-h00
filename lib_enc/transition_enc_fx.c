    ...
    IF( i_subfr == 0 )
    {
        ...
        ELSE
        {
            *position = emaximum_fx( Q_new, res_fx, add(T_op_fx[0],2), &temp );
            ...
            test();
            test();
            IF( (EQ_16(L_frame,L_FRAME)&&GT_16(T_op_fx[0],2*PIT_MIN))||
            (EQ_16(L_frame,L_FRAME16k)  && GT_16(T_op_fx[0],2*PIT16k_MIN) )
              )
            {
                ...
                test();
                IF( GT_32(temp2, L_shl(L_temp2,(31-exp2)-(31-exp)))&&
                    LT_32(aver,  L_shl(L_temp1,(31-exp_aver)-(31-exp)))   )
                {
        ...
        {
            test();
            IF( EQ_16(L_frame,L_FRAME)&&LT_16(T_op_fx[1],PIT_MIN))
            {
                mult_Top = 2;
                move16();
            }
            test();
            if( EQ_16(L_frame,L_FRAME16k)&&LT_16(T_op_fx[1],PIT16k_MIN))
            {
                mult_Top = 2;
                move16();
            }
...
)
{
    ...
    *T0_frac = 0;
    ...
    test();
    test();
    if( i_subfr == 0 && EQ_16(L_frame,L_FRAME)&&(LT_16(*T0,L_SUBFR)||EQ_16(*tc_subfr,3*L_SUBFR)))
    {
        *tc_subfr = TC_0_0;
        move16();
    }
    ...
    *gain_pit_fx = corr_xy1_fx( xn_fx, yy1_fx, g_corr_fx, L_SUBFR, 0 );
    ...
    IF( EQ_16(L_frame,L_FRAME))
    {
        test();
        test();
        IF( ( (i_subfr != 0) || (EQ_16(*tc_subfr,TC_0_0)))
            && (NE_16(*tc_subfr,L_SUBFR) ))
        {
            test();
            /* write pitch index */
...
)
{
    ...
    *quant_index = N_GAIN_TC-1;
    ...
    {
        IF (LE_16(gain_trans,tbl_gain_trans_tc_fx[i]))
        {
            *quant_index = i;
            move16();
            BREAK;
