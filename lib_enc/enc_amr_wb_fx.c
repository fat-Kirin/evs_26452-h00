...
)
{
    ...
    FOR ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_SUBFR )
    {
        ...
        *pt_pitch = pit_encode_fx(st, st->core_brate_fx, 1, L_FRAME, -1, &pitch_limit_flag, i_subfr, exc,
        ...
        clip_gain = gp_clip_fx( st->core_brate_fx, voicing, i_subfr, 0, xn, st->clip_var_fx, sub(shift_wsp,1));
        ...
        inov_encode_fx( st, st->core_brate_fx, 1, L_FRAME, st->last_L_frame_fx, -1, -1, 0, i_subfr, -1, p_Aq, gain_pit, cn,
                        exc, h2, mem->tilt_code, *pt_pitch, xn2, code, y2, &unbits, shift );
        /*-----------------------------------------------------------------*
         * Gain encoding
        ...
        IF ( EQ_32(st->core_brate_fx,ACELP_23k85))
        {
            IF( GE_32(st->input_Fs_fx,16000))
            {
                hf_cod_fx( st->core_brate_fx, &speech16k_fx[i_subfr * L_SUBFR16k/L_SUBFR], Aq, &exc[i_subfr], &syn[i_subfr],
                           &st->seed2_enc_fx, st->mem_hp400_enc_fx, st->mem_syn_hf_enc_fx, st->mem_hf_enc_fx, st->mem_hf2_enc_fx,
                           st->hangover_cnt_fx, &st->gain_alpha_fx, &hf_gain_fx[i_subfr/L_SUBFR], add(Q_new,1), st->Q_syn);
            }
