                      ...
                      )
{
    ...
    test();
    test();
    IF( (LT_16(st->frameloop, 60))&&(GT_16(st->frameloop,5))&&(LT_16(f_tonality_rate[0],9174/* 0.56 Q14 */))
        && (LT_16(f_tonality_rate[1],8192/* 0.5 Q14 */)) && (LT_16(ltd_stable_rate[1],1966/* 0.06 Q15 */))
        && LT_32(snr, 83886080) )
    {
	IF( LT_16(st->frameloop, 50) )
	{
    ...
    {
        IF(LT_16(st->bg_update_count, 16))
        {
            IF( LT_16(st->frameloop, 50) )
            {
		exp_frame_energy_amendment.s32Mantissa = VAD_L_div(exp_frame_energy.s32Mantissa, 10, exp_frame_energy.s16Exp, 0, &q_divout);
    ...
    test();
    test();
    IF( (EQ_16(music_backgound_f, 1))&&(LT_32(st->lt_snr_org,107374179/* 3.2 Q25 */))
    ...
    IF( (EQ_32(st->tbg_energy_count, 64)))
    {
