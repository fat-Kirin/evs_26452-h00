                ...
                )
{
    ...
    if(LT_16(vad_state->frameloop, 1000))
    {
    ...
    IF( vad_flag== 0)
    {
        vad_state->continuous_speech_num2 = 0;
        move16();
        IF(GT_16(vad_state->continuous_noise_num, 10))
        {
            vad_state->continuous_speech_num = 0;
            move16();
        }
        ELSE IF(GT_32(vad_state->continuous_speech_num, 9))
        {
        ...
        if(GT_16(vad_state->continuous_noise_num, 2048))
        {
    ...
    ELSE
    {
        ...
        if(GT_16(vad_state->continuous_speech_num, 2048))
        {
            vad_state->continuous_speech_num = 2048;
            move16();
        }

        if(GT_16(vad_state->continuous_speech_num2, 2048))
        {
               ...
               )
{
    ...
    IF(LT_16(numBands, 20))
    {
        bandwidth = 1;
        move16();
    }
    ELSE IF(LT_16(numBands, 40))
    {
        bandwidth = 2;
        move16();
    }
