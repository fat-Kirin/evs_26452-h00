...
)
{
    ...
    IF (vect_out[0] != 0)
    {
        ...
        IF (GT_32(absPeakGain1, absPeakGain))
        {
            weights[0] = 0;
            move16();
            if (vect_out[1] != 0)
...
)
{
    ...
    FOR (j = 0; j < layer_length; j++)
    {
        IF (layer2[j] != 0)
        {
            ...
            {
                IF (EQ_16(hvq_cp_layer1_map5[idx], val))
                {
                    BREAK;
                }
            }
...
)
{
    ...
    test();
    IF (GT_16(delta_bits, sparse_bits)||delta_bits<0)
    {
