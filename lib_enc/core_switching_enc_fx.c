...
)
{
    ...
    test();
    IF( EQ_16(st_fx->last_core_fx, -1) && EQ_16(st_fx->core_fx, HQ_CORE))
    {
    ...
    test();
    test();
    IF( GE_32(st_fx->input_Fs_fx, 16000)&&NE_16(st_fx->last_extl_fx,WB_BWE)&&EQ_16(st_fx->extl_fx,WB_BWE))
    {
        test();
...
)
{
    ...
    IF( EQ_16(st_fx->core_fx, HQ_CORE))
    {
        ...
        test();
        IF( ( EQ_16(st_fx->last_core_fx, ACELP_CORE)||EQ_16(st_fx->last_core_fx,AMR_WB_CORE)))  /* core switching ==> CELP subframe encoding */
        {
            acelp_core_switch_enc_fx( st_fx, &(st_fx->LPDmem),old_inp_12k8 + L_INP_MEM - NS2SA_fx2(INT_FS_FX, ACELP_LOOK_NS),
                                      old_inp_16k  + L_INP_MEM - NS2SA_fx2(INT_FS_16k, ACELP_LOOK_NS), T_op, voicing, A, Qshift, Q_new );
        }
        ...
        *Qmus=Qsp; /* Write Qout */
        ...
        IF( (EQ_16(st_fx->extl_fx, SWB_TBE)||EQ_16(st_fx->extl_fx,FB_TBE))&&
            ( EQ_16(st_fx->last_core_fx,HQ_CORE)  || NE_16(st_fx->L_frame_fx, st_fx->last_L_frame_fx)  ||
              ( NE_16(st_fx->last_extl_fx, SWB_TBE)  && NE_16(st_fx->last_extl_fx, FB_TBE)  ) )
          )
        {
