...
)
{
    ...
    IF ( EQ_16(st_fx->numactive_fx,RATEWIN))  /* goes into rate control only the numactive ==RATEWIN. So rate control is triggered after each RATEWIN avtive frames */
    {
        ...
        IF ( LT_32(st_fx->frame_cnt_ratewin_fx,1000))
        {
            ...
            IF(GT_32(st_fx->frame_cnt_ratewin_fx,1))
            {
        ...
        IF ( LT_32(target_fx,st_fx->global_avr_rate_fx))  /* Action is taken to reduce the averge rate. Only initiated if the global rate > target rate */
        {
            /* Check the vad snr values to table the noisey/not noisey decision */

            test();
            IF ( LT_16(st_fx->SNR_THLD_fx , 17152)) /*Q8  */ /* Currently in QFF mode. The bumpup thresholds are slightly relaxed for noisy speech. */
            {
            ...
            ELSE IF ( st_fx->mode_QQF_fx == 0 && GT_32(st_fx->sum_of_rates_fx, target_fx)) /* Now SNR_THLD is in the max allowed. Sill the global average is higher and
            ...
            ELSE IF ( GT_32(st_fx->sum_of_rates_fx , target_fx))  /* Actions (1) and (2) are not sufficient to control the rate. Still the last RATEWIN active
                                                      ...
                                                      bump up thresholds (less bump ups->reduced rate)*/
            {
                ...
                IF ( GT_32(st_fx->global_avr_rate_fx ,L_add(target_fx,3440640))) /* Last resort rate control. This is a safer rate control mechanism by increasing NELPS */
                {
            ...
            ELSE IF ( LT_32(st_fx->sum_of_rates_fx, target_fx )) /* If the average rate of last RATEWIN frames is controlled by above actions, disable the most
        ...
        ELSE
        {
            ...
            IF ( EQ_16(st_fx->rate_control_fx,1))
            {
                st_fx->rate_control_fx = 0;
                move16();
            }
            ELSE IF ( EQ_16(st_fx->mode_QQF_fx,1)) /* now rate control is not active and still the global rate is below the target. so go to QFF mode */
            {
                st_fx->mode_QQF_fx = 0;
                move16();
            }
            ELSE
            {
                IF ( GE_16(st_fx->SNR_THLD_fx, 15360))
                {
        ...
        IF ( LT_32(st_fx->global_avr_rate_fx , L_sub(target_fx,983040)))  /* In QFF mode and global rate is less than target rate-0.2kbps. We can send some Q frames
                                                    to F frames to improve the quality */
        {
            ...
            if ( GT_16(st_fx->pattern_m_fx,1000))
            {
