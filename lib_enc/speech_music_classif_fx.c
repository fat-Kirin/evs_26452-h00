...
)
{
    ...
    *sp_aud_decision1 = sp_mus_classif_gmm_fx( st, localVAD_HE_SAD, pitch, voicing, lsp_new, cor_map_sum,
    ...
    test();
    IF ( EQ_16( st->codec_mode, MODE1)||EQ_32(st->sr_core,12800))
    {


        /* Improvement of the 1st stage decision on mixed/music content */
        test();
        IF ( st->Opt_SC_VBR_fx == 0 && ( NE_32(st->total_brate_fx, ACELP_24k40)))
        {
        ...
        *sp_aud_decision2 = *sp_aud_decision1;
        ...
        IF (  GT_16(st->bwidth_fx,NB))
        {
            ...
            test();
            test();
            IF ( flag_spitch && EQ_16(st->bwidth_fx,WB)&&LT_32(st->total_brate_fx,ACELP_13k20))
            {
                *sp_aud_decision2 = 0;
                move16();
            }
        ...
        test();
        test();
        IF ( !st->Opt_SC_VBR_fx && EQ_32(st->total_brate_fx, ACELP_13k20)&&EQ_16(vad_flag,1)&&
             ( EQ_16(st->bwidth_fx, WB) || EQ_16(st->bwidth_fx, SWB)) )
        {
        ...
        test();
        test();
        IF ( EQ_16(vad_flag,1)&&GE_32(st->total_brate_fx,ACELP_13k20)&&LT_32(st->total_brate_fx,ACELP_24k40)&&
             GT_16(st->lp_noise_fx,3072) && *sp_aud_decision1 == 0 && GE_16(st->bwidth_fx,SWB) &&
             EQ_16(st->coder_type_raw_fx,UNVOICED))
        {
        ...
        test();
        test();
        IF ( EQ_16(st->codec_mode,MODE1)&&(*sp_aud_decision2||st->GSC_noisy_speech_fx))
        {
            *coder_type = AUDIO;
            move16();
...
)
{
    ...
    test();
    IF ( EQ_16(st_fx->tc_cnt_fx,1)||EQ_16(st_fx->tc_cnt_fx,2))
    {
        *pFV++ = pitch[2];
        move16();
    }
    ...
    *pFV++ = add(ps_diff_16, st_fx->past_ps_diff_fx);
    ...
    FOR ( i = LOWEST_FBIN; i < HIGHEST_FBIN; i++ )
    {
        ...
        IF (GT_32(PS_norm[i],st_fx->past_PS_fx[i-LOWEST_FBIN]))
        {
            mx = PS_norm[i];
            move16();  /*Q25 */
        }
    ...
    *high_lpn_flag_ptr = 0;
    ...
    IF ( vad )
    {
        ...
        test();
        test();
        IF ( LT_16(relE,-20*256)||(LE_16(lps,-5*512)&&LE_16(lpm,-5*512)))
        {
            IF ( st_fx->sp_mus_state_fx > 0 )
            {
                if ( LT_16(st_fx->sp_mus_state_fx,HANG_LEN))
                {
            ...
            ELSE IF ( GT_16(st_fx->sp_mus_state_fx,-HANG_LEN))
            {
        ...
        ELSE IF ( st_fx->sp_mus_state_fx > 0 && LT_16(st_fx->sp_mus_state_fx,HANG_LEN))
        {
    ...
    {
        test();
        IF ( st_fx->sp_mus_state_fx > 0 && LT_16(st_fx->sp_mus_state_fx,HANG_LEN))
        {
            st_fx->inact_cnt_fx = 0;
            move16();
        }
    ...
    test();
    IF ( dlp < 0 && LT_16(dlp,st_fx->past_dlp_fx[0]))
    {
    ...
    if ( EQ_16(st_fx->sp_mus_state_fx,-HANG_LEN))
    {
    ...
    test();
    test();
    IF ( !vad && EQ_16(st_fx->sp_mus_state_fx,-HANG_LEN))
    {
        /* inactive state */
        dec = 0;
        move16();
...
)
{
    ...
    *attack_flag = 0;
    ...
    test();
    test();
    IF( EQ_16(sp_aud_decision1,1))
    {
        test();
        test();
        test();
        IF( LT_16(st->ener_RAT_fx,5898)&&GT_16(st->lt_dec_thres_fx,7680))
        {
            *sp_aud_decision2 = 0;
            move16();
        }
        ELSE IF( EQ_16(st->high_stable_cor_fx, 1)&&GE_16(pitch[0],130))
        {
            ...
            *sp_aud_decision2 = 0;
            ...
            test();
            IF( EQ_16(st->codec_mode,MODE1)&&EQ_16(*coder_type,TRANSITION))
            {
                *coder_type = GENERIC;
                move16();
            }
        }
        ELSE IF( GT_16(st->gsc_lt_diff_etot_fx[MAX_LT-1],1152)&&
                 GT_16(sub(st->gsc_lt_diff_etot_fx[MAX_LT-1], st->gsc_lt_diff_etot_fx[MAX_LT-2]),2560))  /* 10.0f in Q8 */
        {
            IF ( EQ_16(st->tc_cnt_fx,1))
            {
                *sp_aud_decision2 = 0;
                move16();
            ...
            ELSE
            {
                ...
                ELSE IF( GE_16(attack,ATT_SEG_LEN/2))
                {
    ...
    ELSE IF( EQ_16(localVAD,1)&&EQ_16(*coder_type,GENERIC)&&
             ( (GE_16(attack,ATT_3LSUB_POS) && LT_32(st->total_brate_fx,ACELP_24k40)) ||
               (GE_16(attack,ATT_3LSUB_POS_16k) && GE_32(st->total_brate_fx,ACELP_24k40) && LT_32(st->total_brate_fx,ACELP_48k)) )
           )
    {
        /* do TC coding if attack is located in the last subframe */
        *attack_flag = 1;
...
)
{
    ...
    *high_stable_cor = 0;
    move16();
    test();
    IF( GT_16(*mold_corr,26214)&&LT_16(var_cor,2))
    {
        *high_stable_cor = 1;
        move16();
    }
...
)
{
    ...
    ELSE IF( attack > 0 )
    {
        ...
        FOR( i=2; i<att_3lsub_pos-2; i++ )
        {
            ...
            test();
            IF( NE_16(i,attack)&&GT_32(finc[i],etmp))
            {
                attack = 0;
                move16();
                BREAK;
...
)
{
    ...
    ELSE
    {
        IF ( LT_16(len,10))
        {
            ...
            test();
            test();
            IF ( (GT_16(M_pkh,2200)||LT_32(V_epsP_tilt,171799)||GT_16(M_cor_map_sum,25600))&&voiced_cnt<4)
            {
                mode = 1;
                move16();
            }
            ELSE IF ( GT_16(M_Ntonal,108)&&voiced_cnt<4) /*27 in Q2 */
            {
                mode = 1;
                move16();
            }
        ...
        ELSE
        {
            ...
            test();
            test();
            IF ( (LT_16(M_flux10,4352)||(LT_32(V_epsP_tilt,2147484)&&LT_16(M_flux10,6144))||GT_16(M_pkh,2100)||GT_16(M_cor_map_sum,25600))&&
                 LT_16(voiced_cnt,3)  && LT_16(tmp,7680)  )
            {
                mode = 1;
                move16();
                *dec_mov = 32767;
            ...
            test();
            test();
            IF ( GT_16(M_flux10,8192)||(GT_16(M_flux10,7680)&&voiced_cnt>2)||GT_16(tmp,9728)||(GE_16(buf_flux[59],2560)&&GT_16(st->lps_fx,st->lpm_fx)))
            {
                mode = 0;
                move16();
                *dec_mov = 0;
            ...
            FOR ( i=10; i<len; i++ )
            {
                ...
                test();
                test();
                IF ( ((LT_16(M_flux,add(6144,mult_r(1638,shl(sub(len,10),9))))&&LT_16(M_flux10,7680))||
                      LT_32(V_epsP_tilt,L_add(214748,L_shl(L_mult0(19327,(len-10)),1))) ||
                      GT_16(M_pkh,sub(2100,extract_l(L_mult0(10,sub(len,10))))) ||
                      GT_16(M_cor_map_sum,sub(24320,extract_l(L_mult0(77,sub(len,10)))))) && LT_16(voiced_cnt,3))
                {
                    mode = 1;
                    move16();
                    return( mode );
            ...
            IF ( EQ_16(len,BUF_LEN))
            {
                ...
                test();
                IF ( GT_16(M_Ntonal,72)||LT_16(lf_Ntonal_ratio,6554))
                {
                    mode = 1;
                    move16();
                }
                ELSE IF ( LT_16(M_Ntonal,4))
                {
                    mode = 0;
                    move16();
                }
...
)
{
    ...
    FOR ( i=0; i<64; i++ )
    {
        if ( GT_16(p2v_map[i],7040))
        {
            Ntonal = add(Ntonal,1);
        }

        IF ( GT_16(p2v_map[i],10240))
        {
...
)
{
    ...
    test();
    if ( GT_16(flux,2560)&&GT_16(dec_mov,26214))
    {
        flux = 2560;
        move16(); /*20 in Q7 */
    }
...
)
{
    ...
    {
        test();
        IF ( GT_16(Bin_E[i],Bin_E[i-1])&&GT_16(Bin_E[i],Bin_E[i+1]))
        {
            peak[k] = Bin_E[i];
            move16();
            peak_idx[k] = i;
...
)
{
    ...
    test();
    IF ( vad_flag == 0 || LT_16(sub(st->lp_speech_fx,st->lp_noise_fx),6400))  /* 25 in Q8 */
    {
        /* st->dec_mov = 0.5f; */
        /* st->dec_mov1 = 0.5f; */
        st->dec_mov_fx = 16384;
    ...
    test();
    test();
    IF ( GT_16(sub(st->buf_etot_fx[1],st->buf_etot_fx[0]),1536)&&
         LT_16(st->buf_etot_fx[2],st->buf_etot_fx[1]) &&
         GT_16(sub(st->buf_etot_fx[1],st->lp_speech_fx),768) )    /* 3 in Q8 */
    {
        ...
        test();
        test();
        IF ( GT_16(sub(st->buf_etot_fx[1],st->buf_etot_fx[3]),768)&&
             LT_16(st->buf_etot_fx[3],st->buf_etot_fx[2]) &&
             LT_16(tmp,24576))     /* 0.75 in Q15 */
        {
            ...
            ELSE
            {
                ...
                test();
                test();
                IF ( LT_16(old_cor,24576)&&LT_16(voicing[0],24576)&&LT_16(voicing[1],24576)&&GT_16(st->old_lt_diff_fx[0],1280))
                {
                    percus_flag = 1;
                    move16();
                }
    ...
    test();
    test();
    IF (    GT_16(sub(st->buf_etot_fx[3],st->buf_etot_fx[2]),1536)
            && GT_16(st->dec_mov_fx,29491)
            && GT_16(sub(etot,st->lp_speech_fx),1280)
            && GT_16(st->old_lt_diff_fx[0],640))
    {
    ...
    test();
    IF ( GT_16(voicing[0],29491)&&GT_16(voicing[1],29491))
    {
        IF ( GT_16(log_max_spl,st->mov_log_max_spl_fx))
        {
    ...
    FOR ( i=1; i<16; i++ )
    {
        ...
        ELSE IF(EQ_32(epsP[i+1],epsP_max))
        {
            tmp = -32768;
            move16();
            L_tmp = Mult_32_16(epsP[i],tmp); /*Q_epsP */
    ...
    test();
    test();
    IF ( (EQ_16(st->coder_type_raw_fx,UNVOICED)||EQ_16(st->coder_type_raw_fx,INACTIVE))&&
         GT_16(etot,384) && LT_16(st->buf_Ntonal2_fx[59],2)  )
    {
    ...
    test();
    test();
    IF ( EQ_16(dec,1)&&LT_16(st->dec_mov1_fx,6554)&&LT_16(st->LT_UV_cnt1_fx,12800))
    {
        dec = 0;
        move16();
    }
...
)
{
    ...
    test();
    IF( EQ_16(st_fx->hangover_cnt_fx,10)&&EQ_16(vad_flag,1))
    {
    ...
    IF ( *sp_aud_decision1 == 1 &&
         ...
         ( LT_32(st_fx->lt_tonality,655360000) ) &&
         ( ( GT_32(st_fx->lt_tonality,32768000) && GT_16(s_max(st_fx->lt_voicing, voi_mean),32440) ) ||
           ( GT_32(st_fx->lt_tonality,49152000) && GT_16(st_fx->lt_corr,32440)  ) ||
           ( GT_32(st_fx->lt_tonality,98304000) && GT_16(st_fx->lowrate_pitchGain,15729) ) ||
           ( lt_pitch_diff == 0 && GT_16(st_fx->lowrate_pitchGain,14582))))
    {
        IF( LT_16(sum16_fx(st_fx->lt_old_mode, 2),2))
        {
    ...
    test();
    test();
    IF ( (EQ_16(*sp_aud_decision1, 1))&&st_fx->lt_music_state_fx==0&&st_fx->lt_music_hangover_fx==0&&
         (LT_16(t2_fx, 8847)) && (GT_16(t2_fx, 4260)) && (GT_16(t3_fx, 3604)) && (LT_16(tL_fx, 8847)) && (GT_16(tL_fx, 4260)) && (GT_16(err_fx, 8192)) )
    {
    ...
    ELSE IF( EQ_16(st_fx->lt_music_state_fx, 1)&&st_fx->lt_music_hangover_fx==0&&
             (LT_16(t2_fx, 5571) ) && (LT_16(t3_fx, 4260) ) && (LT_16(tL_fx, 7373) ) )
    {
        st_fx->lt_music_state_fx = 0;
        move16();
        st_fx->lt_music_hangover_fx = 6;
    ...
    test();
    test();
    IF ( (EQ_16(*sp_aud_decision1, 1))&&st_fx->lt_speech_state_fx==0&&st_fx->lt_speech_hangover_fx==0&&
         (GT_16(cor_fx, 13107)) && (LT_16(dft_fx, 1638)) && GT_16(shr(voi_fv,1), add(cor_map_sum_fv, 1966)) &&
         (LT_16(t2_fx, shr(cor_fx, 1))) && (LT_16(t3_fx, shr(cor_fx, 1))) && (LT_16(tL_fx, shr(cor_fx, 1))) &&
         (LT_16(cor_map_sum_fv, cor_fx)) && (GT_16(voi_fv, cor_fx)) && (GT_16(voi_fv, 24903) ) )
    {
    ...
    ELSE IF ( (EQ_16(st_fx->lt_speech_state_fx, 1))&&st_fx->lt_speech_hangover_fx==0&&(LT_16(cor_fx,13107)))
    {
        st_fx->lt_speech_state_fx = 0;
        move16();
        st_fx->lt_speech_hangover_fx = 6;
    ...
    test();
    test();
    IF ( EQ_16(*sp_aud_decision1,1)&&EQ_16(st_fx->lt_speech_state_fx,1))
    {
    ...
    ELSE IF ( *sp_aud_decision1 == 0 && EQ_16(st_fx->lt_speech_state_fx,1))
    {
...
)
{
    ...
    IF (EQ_16(st_fx->bwidth_fx, WB))
    {
        ...
        ELSE
        {
            ...
            FOR (i = 0; i < HANG_LEN_INIT; i++)
            {
                FOR (j = 0; j < 4; j++)
                {
                    IF (GT_16(st_fx->sparse_buf_fx[i], tmp_buf[j]))
                    {
    ...
    IF (EQ_16(*sp_aud_decision1, 1))
    {
        ...
        ELSE IF (EQ_16(st_fx->gsc_hangover_fx, 1))
        {
            IF (GT_16(sparse, 85))
            {
                *sp_aud_decision1 = 0;
        ...
        IF (EQ_16(st_fx->bwidth_fx, WB))
        {
            ...
            test();
            test();
            IF (GT_16(st_fx->LT_sparse_fx, 15360)&&GT_16(sparse,50)&&LT_16(Mlpe,-1331)&&GT_16(Mv,27853)&&
                lb_sp_high_flag == 0 && ((hb_sp_high_flag == 0 && GT_16(sumh, mult_r(4915, sum))) || LE_16(sumh, mult_r(4915, sum))))
            {
                *sp_aud_decision1 = 0;
                move16();
                *sp_aud_decision2 = 1;
            ...
            ELSE IF (EQ_16(st_fx->gsc_hangover_fx, 1)&&!(*sp_aud_decision1==0&&*sp_aud_decision2==1))
            {
                IF (LT_16(abs_s(sub(sparse, mean_fx(&st_fx->sparse_buf_fx[HANG_LEN_INIT-1-st_fx->gsc_cnt_fx], st_fx->gsc_cnt_fx))), 7))
                {
                    *sp_aud_decision1 = 0;
                    move16();
                    *sp_aud_decision2 = 1;
    ...
    test();
    IF (*sp_aud_decision1 == 0 && EQ_16(*sp_aud_decision2, 1))
    {
        st_fx->gsc_cnt_fx = add(st_fx->gsc_cnt_fx, 1);
        IF (GT_16(st_fx->gsc_cnt_fx, 7))
        {
            st_fx->gsc_cnt_fx = 7;
            move16();
        }
