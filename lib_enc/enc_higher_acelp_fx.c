...
)
{
    ...
    test();
    test();
    IF( NE_16(coder_type,INACTIVE)&&LE_32(core_brate,ACELP_32k)&&GT_32(core_brate,ACELP_24k40)&&!harm_flag_acelp)
    {
    ...
    ELSE
    {
        ...
        test();
        IF( LE_32(core_brate,ACELP_32k)&&GT_32(core_brate,ACELP_24k40))
        {
    ...
    *unbits = nBits;
    ...
    test();
    test();
    IF( EQ_16(i_subfr,4*L_SUBFR)&&NE_16(extl,SWB_BWE_HIGHRATE)&&NE_16(extl,FB_BWE_HIGHRATE))
    {
        WHILE( *unbits > 0 )
        {
            i = s_min(*unbits, 16);
