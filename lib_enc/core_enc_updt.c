                       ...
                       )
{
    ...
    *---------------------------------------------------------------*/
    ...
    IF ( s_or( s_or(st->core_fx == ACELP_CORE, s_or( (Word16)EQ_32(st->core_brate_fx, SID_2k40),(Word16)EQ_32(st->core_brate_fx,FRAME_NO_DATA))),(Word16)EQ_32(st->core_fx,AMR_WB_CORE)))
    {
    ...
    test();
    test();
    IF( ((LE_32(st->core_brate_fx,SID_2k40))&&EQ_16(st->cng_type_fx,FD_CNG))
        || (st->tcxonly && EQ_16(st->codec_mode,MODE2))
      )
    {
        /* reset LP memories */
