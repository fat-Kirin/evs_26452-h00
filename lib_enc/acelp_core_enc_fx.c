...
)
{
    ...
    IF ( EQ_32(st_fx->core_brate_fx,SID_2k40)||EQ_32(st_fx->core_brate_fx,FRAME_NO_DATA))
    {
        ...
        /* reset the encoder */
        CNG_reset_enc_fx( st_fx, mem, pitch_buf_fx, voice_factors_fx, 0 );
        ...
        Copy( syn1_fx+st_fx->L_frame_fx-M-1, mem->syn, M+1 );
    ...
    ELSE
    {
        ...
        calc_residu_fx( st_fx, inp_fx, res_fx, Aq_fx, vad_hover_flag_fx, vad_flag_fx );
        ...
        test();
        test();
        IF( ( NE_16(coder_type_fx,UNVOICED)&&NE_16(coder_type_fx,AUDIO)&&NE_16(coder_type_fx,INACTIVE)
        ...
        ELSE IF( st_fx->ppp_mode_fx )
        {
            ...
            IF( st_fx->bump_up_fx )   /* PPP failed, bump up */
            {
                ...
                calc_residu_fx( st_fx, inp_fx, res_fx, Aq_fx, 0, 0 );
                st_fx->burst_ho_cnt_fx = 0;
                move16();
        ...
        E_UTIL_deemph2( sub(Q_new,1), syn1_fx, st_fx->preemph_fac, st_fx->L_frame_fx, &mem->syn[M] );
        Copy( syn1_fx+st_fx->L_frame_fx-M-1, mem->syn, M+1 );
    ...
    IF ( NE_32(st_fx->core_brate_fx,SID_2k40)&&NE_32(st_fx->core_brate_fx,FRAME_NO_DATA)&&NE_32(st_fx->core_brate_fx,PPP_NELP_2k80))
    {
        ...
        test();
        test();
        IF ( EQ_16(coder_type_fx,AUDIO)||(EQ_16(coder_type_fx,INACTIVE)&&LE_32(st_fx->core_brate_fx,ACELP_24k40)))
        {
            nBits = 0;
            move16();
        }
        ELSE IF( EQ_16(st_fx->L_frame_fx,L_FRAME))
        {
    ...
    test();
    IF( (st_fx->Opt_DTX_ON_fx != 0 ) && (GT_32(st_fx->core_brate_fx,SID_2k40)))
    {
        ...
        IF ( GT_16(st_fx->burst_ho_cnt_fx,1))
        {
