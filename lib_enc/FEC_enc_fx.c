...
)
{
    ...
    test();
    test();
    IF( GT_16(coder_type,UNVOICED)&&LT_16(coder_type,AUDIO)&&GE_32(core_brate,ACELP_11k60))
    {
        /*-----------------------------------------------------------------*
         ...
         *-----------------------------------------------------------------*/
        test();
        IF( GE_32(total_brate,ACELP_16k40)&&NE_16(coder_type,TRANSITION))    /* GENERIC and VOICED frames */
        {
        ...
        * Encode last glottal pulse position (8 bits)
         *-----------------------------------------------------------------*/
        test();
        IF( GE_32(total_brate,ACELP_32k)&&NE_16(coder_type,TRANSITION))   /* GENERIC frames */
        {
