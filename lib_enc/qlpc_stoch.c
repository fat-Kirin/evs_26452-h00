...
)
{
    ...
    ELSE IF ( EQ_16(lpcQuantization, 1))
    {
        ...
        test();
        test();
        IF( st->tc_cnt_fx >= 1 || LE_32(st->last_core_brate_fx,SID_2k40)||(EQ_16(st->next_force_safety_net_fx,1)))
        {
            force_sf = 1;
            move16();
        ...
        test();
        IF ( EQ_16(st->next_force_safety_net_fx,1)&&EQ_16(st->Opt_RF_ON,1))
        {
            force_sf = 1;
            st->next_force_safety_net_fx = 0;
        }
