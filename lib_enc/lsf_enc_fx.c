...
)
{
    ...
    ELSE
    {
        ...
        ELSE IF ( EQ_16(st_fx->nelp_mode_fx, 1))
        {
            nBits = 30;
            move16();
        ...
        ELSE IF ( EQ_16(st_fx->ppp_mode_fx, 1))
        {
            nBits = 26;
            move16();
        }
    ...
    IF ( EQ_16(st_fx->next_force_safety_net_fx ,1))
    {
        force_sf = 1;
        move16();
    ...
    *-------------------------------------------------------------------------------------*/
    ...
    test();
    IF ( EQ_16(st_fx->last_core_fx, HQ_CORE)&&EQ_16(st_fx->core_fx,ACELP_CORE))
    {
    ...
    *-------------------------------------------------------------------------------------*/
    ...
    IF ( (EQ_16(st_fx->L_frame_fx, L_FRAME16k))&&(LT_16(stab,STAB_FAC_LIMIT_FX))&&(EQ_16(coder_type,GENERIC)))
    {
        st_fx->next_force_safety_net_fx = 1;
        move16();
    }
    ELSE IF((LT_16(stab, STAB_FAC_LIMIT_FX))&&(EQ_16(st_fx->clas_fx,VOICED_CLAS)||(LT_16(st_fx->clas_fx,VOICED_CLAS)&&EQ_16(coder_type,AUDIO))))
    {
        st_fx->next_force_safety_net_fx = 1;
        move16();
    }
    ...
    *-------------------------------------------------------------------------------------*/
    ...
    /* LSP interpolation and conversion of LSPs to A(z) */
    int_lsp4_fx( L_frame, st_fx->lsp_old_fx, lsp_mid, lsp_new, Aq,  M, 0 );
...
)
{
    ...
    test();
    test();
    IF(EQ_16(coder_type_org, GENERIC)&&EQ_32(int_fs,INT_FS_16k)&&(rf_flag==0)&&(mode2_flag==0))
    {
    ...
    *------------------------------------------------------------------------------------ -*/
    ...
    Unified_weighting_fx(&Bin_Ener[L_FFT/2], Q_ener, lsf, wghts, (Word16)EQ_16(bwidth, NB),(Word16)EQ_16(coder_type,UNVOICED),int_fs,M);
    ...
    *--------------------------------------------------------------------------------*/
    ...
    ELSE
    {
        ...
        test();
        test();
        IF ( ((GT_16(*pstreaklen, (STREAKLEN+3)))&&(EQ_16(coder_type,VOICED)))||((GT_16(*pstreaklen,(STREAKLEN)))&&(NE_16(coder_type,VOICED))))
        {
        ...
        /* VOICED_WB@16kHz */
        test();
        IF ( EQ_32(int_fs, INT_FS_16k)&&EQ_16(coder_type,VOICED))
        {
        ...
        ELSE
        {
            ...
            IF ( force_sf || LT_32(Mult_32_16(Err[0],(*streaklimit)),L_add(Err[1],Mult_32_16(Err[1],PREFERSFNET_FX)))||LT_32(Err[0],abs_threshold))
            {
                safety_net = 1;
                move16();
                *pstreaklen = 0;
    ...
    *--------------------------------------------------------------------------*/
    ...
    IF (mode2_flag == 0)
    {
        ...
        test();
        IF(EQ_16(coder_type_org, GENERIC)&&EQ_32(int_fs,INT_FS_16k))
        {
        ...
        test();
        IF ( EQ_16(coder_type, VOICED)&&EQ_32(int_fs,INT_FS_16k))
        {
...
)
{
    ...
    FOR (state = 0; state < NUM_STATE; state ++)
    {
        ...
        if (LE_32(brDist_fx[0], brDist_fx[1]))
        {
            branch = 0;
            move16();
        }
...
)
{
    ...
    branch = FixBranch[shr(inis,2)][fins][stage4];
    move16();
    index  = NTRANS2[add(branch,2)][*prev_state];
    move16();
...
)
{
    ...
    {
        if (LT_32(opDist_fx[state], minDist_fx))
        {
            fBlock  = state;
            move16();
        }
...
)
{
    ...
    FOR (state = 0; state < NUM_STATE; state++)
    {
        ...
        FOR (i = 0; i < 4; i++)
        {
            ...
            prev_state = state;
            move16();
            fDist_fx = BcTcvq_FixSearch_fx(X_fx, TCVQ_CB_SUB3_fx, fCodeword, quant_fx, FixBranch_tbl, N_STAGE_VQ-4, inis, i, &prev_state, W_fx, IntraCoeff_fx);

            FOR (stage = N_STAGE_VQ-3; stage < N_STAGE_VQ; stage++)
            {
                fDist_fx = L_add(fDist_fx, BcTcvq_FixSearch_fx(X_fx, TCVQ_CB_SUB3_fx, fCodeword, quant_fx, FixBranch_tbl, stage, inis, i, &prev_state, W_fx, IntraCoeff_fx));
            }
            IF (LT_32(fDist_fx, minDist_fx))
            {
