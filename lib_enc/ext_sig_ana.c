                                      ...
                                      )
{
    ...
    *--------------------------------------------------------------------------*/
    ...
    FOR (frameno = 0; frameno < nSubframes; frameno++)
    {
        ...
        test();
        IF ((NE_16(transform_type[frameno], TCX_20))||(EQ_16(st->tcx_cfg.tcx_last_overlap_mode,TRANSITION_OVERLAP)))
        {
        ...
        ELSE /* transform_type[frameno] != TCX_5 */
        {
            ...
            move16();
            test();
            IF ((EQ_16(transform_type[frameno], TCX_20))&&(NE_16(st->tcx_cfg.tcx_last_overlap_mode,TRANSITION_OVERLAP)))
            {
            ...
            test();
            test();
            IF (EQ_16(st->tcxMode, TCX_20)&&((LT_32(st->total_brate_fx,HQ_96k))||st->igf))
            {

                pMdstWin = tcx20Win;
                test();
                if (((EQ_16(st->tcxMode, TCX_20))&&(NE_16(st->tcx_cfg.tcx_last_overlap_mode,TRANSITION_OVERLAP))))
                {
                    pMdstWin = mdstWin;
                }
