    ...
    *-----------------------------------------------------------------*/
    ...
    if( ( EQ_16(st_fx->last_coder_type_raw_fx,UNVOICED))||             /* previous frame was unvoiced  */
            ( ( LT_32(ee[0],ee0_th)  ) && ( GT_32(hp_E[0],L_shl(E_MIN_FX,Q_new))  ) &&         /* energy is concentrated in high frequencies provided that some energy is present in HF */
              ( LT_16(add(voicing[0],corr_shift),voi_th))))             /* normalized correlation is low */
    {
        tmp_offset_flag = 0;
        move16();
    }
    ...
    IF ( st_fx->Opt_SC_VBR_fx )
    {
        ...
        test();
        if ( EQ_16(st_fx->input_bwidth_fx,NB)&&LT_16(st_fx->vadnoise_fx,20<<8)) /* vadnoise in Q8, constant Q0<<8 */
        {
    ...
    IF ( EQ_16(st_fx->input_bwidth_fx,NB))
    {
        ...
        if( ( ( LT_16(add(mean_voi3, corr_shift),add(22282,mean_voi3_offset)))&&     /* normalized correlation low  */
                (  LT_16(add(voicing[2], corr_shift),25887) ) &&               /* normalized correlation low on look-ahead - onset detection */
    ...
    test();
    test();
    IF( EQ_16(localVAD,1)&&EQ_16(coder_type,GENERIC)&&NE_16(last_core_orig,AMR_WB_CORE))
    {
        ...
        ELSE IF ( st_fx->Opt_SC_VBR_fx && EQ_16(st_fx->input_bwidth_fx,NB)&&LT_16(st_fx->vadnoise_fx,20<<8))
        {
            test();
            test();
            test();
        ...
        IF ( *flag_spitch || ( LE_16(dpit1,3<<6)&&LE_16(dpit2,3<<6)&&LE_16(dpit3,3<<6)&&
                               GT_16(voicing_m, 31130)  && GT_16(voicing_sm, 31785)  ) )
        {
            coder_type = VOICED;
            move16();
            *flag_spitch = 1;
    ...
    IF ( EQ_16 ( coder_type, GENERIC )  )
    {
        ...
        IF( ( LT_16(voicing_fr[0],6554))&&          /* normalized correlation high in 2st sf.  */
            ...
            ( GT_16(st_fx->vadnoise_fx, 25 << 8 ) )) /* when speech is clean */
