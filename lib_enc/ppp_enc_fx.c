...
)
{
    ...
    test();
    IF (GT_16(add(tmp_fx,12),28)||LT_16(tmp_fx,-12))
    {
        tmp_fx = 0;
        move16();
        returnFlag = 0;
...
)
{
    ...
    IF (  EQ_16( *vad_flag, 1)&&
          ( EQ_16( noisy_speech_HO , 1 )  || EQ_16( clean_speech_HO, 1 )  || EQ_16( NB_speech_HO, 1)  ) &&
    ...
    test();
    test();
    IF ( EQ_16( *coder_type , INACTIVE )&&(*vad_flag==0)&&EQ_16(st_fx->last_nelp_mode_fx,1)) /* avoid HO frame go to GSC */
    {
        *coder_type = UNVOICED;
        move16();
    }
    ...
    test();
    IF ( NE_16( *coder_type, VOICED )||EQ_16(st_fx->last_coder_type_fx,TRANSITION))
    {
    ...
    ELSE
    {
        ...
        test();
        test();
        IF ( ( EQ_16( st_fx->pppcountE_fx, 1 )&&NE_16(st_fx->last_last_ppp_mode_fx,1)&&st_fx->rate_control_fx==0)||
        ( EQ_16( st_fx->pppcountE_fx, 1 )  && st_fx->mode_QQF_fx != 0) )
        {
            st_fx->ppp_mode_fx = 1;
            move16();
        ...
        ELSE IF ( EQ_16(st_fx->pppcountE_fx, 2 ))
        {
            test();
    ...
    test();
    IF ( st_fx->ppp_mode_fx == 0 && EQ_16( st_fx->set_ppp_generic_fx, 1))
    {
    ...
    IF ( (st_fx->last_ppp_mode_fx != 0 ) && ( st_fx->ppp_mode_fx == 0 ) && ( sp_aud_decision1 != 0)
         && EQ_16(st_fx->bwidth_fx, NB)&&st_fx->Opt_SC_VBR_fx!=0) /*if it were about to go from ppp->HQ*/
    {
    ...
    test();
    test();
    IF ( (st_fx->last_nelp_mode_fx != 0) && ( sp_aud_decision1 != 0) && EQ_16( st_fx->bwidth_fx, NB)&&(st_fx->Opt_SC_VBR_fx!=0)) /*if it were about to go from nelp->HQ*/
    {
        st_fx->avoid_HQ_VBR_NB = 1;
        move16();
        *coder_type = GENERIC;
    ...
    test();
    test();
    IF( (  GT_16(st_fx->old_pitch_buf_fx[(2*NB_SUBFR)-1], PPP_LAG_THRLD_Q6)||
           GT_16(T_op_fx[1], PPP_LAG_THRLD) || !st_fx->last_Opt_SC_VBR_fx ) &&
        EQ_16(st_fx->ppp_mode_fx,1) )
    {
        st_fx->ppp_mode_fx=0;
        move16();
