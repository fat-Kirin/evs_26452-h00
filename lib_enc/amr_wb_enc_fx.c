...
)
{
    ...
    core_switching_pre_enc_fx( st,&(st->LPDmem), NULL, NULL );
    ...
    {
        hf_cod_init_fx( st->mem_hp400_enc_fx, st->mem_hf_enc_fx, st->mem_syn_hf_enc_fx, st->mem_hf2_enc_fx, &st->gain_alpha_fx );
    }
    ...
    IF( NE_16(st->last_core_fx,AMR_WB_CORE)&&EQ_16(st->last_L_frame_fx,L_FRAME16k)&&NE_16(st->last_core_fx,HQ_CORE))
    {
        ...
        Copy( st->LPDmem.mem_syn2, st->LPDmem.mem_syn3, M );
    ...
    IF( NE_16(st->last_L_frame_fx,L_FRAME))
    {
        ...
        {
            st->old_pitch_buf_fx[i-1] = mult_r(tmp, st->old_pitch_buf_fx[i]);
            move16();
        }
    ...
    vad_param_updt_fx( st, pitch, voicing, st->pitO_fx, corr_shift, vad_flag, A );
    ...
    IF ( EQ_32(st->core_brate_fx,SID_1k75)||EQ_32(st->core_brate_fx,FRAME_NO_DATA))
    {
        ...
        /* synthesis */
        syn_12k8_fx( L_FRAME, Aq, exc2, dummy_buf, st->LPDmem.mem_syn3, 1, Q_new, st->Q_syn );

        /* reset the encoder */
        CNG_reset_enc_fx( st,  &(st->LPDmem), pitch_buf, dummy_buf+L_FRAME, 0 );

        /* update st->mem_syn1 for ACELP core switching */
        Copy( st->LPDmem.mem_syn3, st->mem_syn1_fx, M );

        /* update ACELP core synthesis filter memory */
        Copy( st->LPDmem.mem_syn3, st->LPDmem.mem_syn, M );

        /* update old synthesis buffer - needed for ACELP internal sampling rate switching */
        Copy( dummy_buf + L_FRAME - L_SYN_MEM, st->LPDmem.mem_syn_r, L_SYN_MEM );
        /* Update MODE2 core switching memory */
        E_UTIL_deemph2( sub(Q_new,1), dummy_buf, PREEMPH_FAC, L_FRAME, &(st->LPDmem.syn[M]) );
        Copy( dummy_buf+L_FRAME-M-1, st->LPDmem.syn, M+1 );
    ...
    ELSE
    {
        ...
        isf_enc_amr_wb_fx( st, isf_new, isp_new, Aq, &st->stab_fac_fx);

        /*---------------------------------------------------------------*
         * Calculation of LP residual (filtering through A[z] filter)
         *---------------------------------------------------------------*/

        calc_residu_fx( st, inp, res, Aq, 0, 0 );
    ...
    if( LT_16(st->ini_frame_fx,MAX_FRAME_COUNTER))
    {
        st->ini_frame_fx = add(st->ini_frame_fx,1);
    }

    if( GT_32(st->core_brate_fx,SID_1k75))
    {
    ...
    test();
    IF ( GT_32(st->core_brate_fx,SID_1k75)&&st->first_CNG_fx)
    {
        if( GE_16(st->act_cnt_fx,BUF_DEC_RATE))
        {
            st->act_cnt_fx = 0;
            move16();
        }
        ...
        test();
        if( EQ_16(st->act_cnt_fx,BUF_DEC_RATE)&&st->ho_hist_size_fx>0)
        {
            st->ho_hist_size_fx = sub(st->ho_hist_size_fx,1);
        }

        st->act_cnt2_fx = add(st->act_cnt2_fx,1);
        if( GE_16(st->act_cnt2_fx, MIN_ACT_CNG_UPD))
        {
