...
)
{
    ...
    test();
    IF( NE_16(coder_type,INACTIVE )&&GE_32(st_fx->total_brate_fx,ACELP_13k20))
    {
     ...
     *---------------------------------------------------------------*/
    test();
    IF ( GE_32(st_fx->core_brate_fx,ACELP_9k60)&&LE_32(st_fx->core_brate_fx,ACELP_13k20))
    {
        FOR( i = 0; i < 5; i++)
        {
            test();
            if( GT_16(abs_s(st_fx->gsc_lt_diff_etot_fx[MAX_LT-i-1]),1536)&&EQ_16(st_fx->cor_strong_limit_fx,1))
            {
    ...
    ELSE
    {
        test();
        test();
        IF( (st_fx->cor_strong_limit_fx == 0 || EQ_16(coder_type,INACTIVE))&&GE_32(st_fx->core_brate_fx,ACELP_9k60))
        {
            nb_subfr = 2;
            move16();
            nb_subfr_flag = 0;
        ...
        IF( GE_32(st_fx->core_brate_fx,ACELP_9k60))
        {
    ...
    test();
    IF( st_fx->GSC_noisy_speech_fx && EQ_16(nb_subfr,NB_SUBFR ))
    {
     ...
     * DCT transform
     *---------------------------------------------------------------*/
    edct_16fx( exc, dct_epit, L_FRAME, 7 );
    edct_16fx( res, dct_res, L_FRAME, 7 );
    /*---------------------------------------------------------------*
    ...
    IF( GT_16(m_mean,st_fx->mid_dyn_fx))
    {
    ...
    test();
    if( EQ_16(coder_type,INACTIVE)&&LE_32(st_fx->core_brate_fx,ACELP_9k60))
    {
    ...
    edct_16fx( dct_epit, exc, L_FRAME, 7 );
    edct_16fx( exc_wo_nf, exc_wo_nf, L_FRAME, 7 );
    /*--------------------------------------------------------------------------------------*
...
)
{
    ...
    mean_gain = gsc_gainQ_fx( st_fx, Ener_per_bd_iQ, Ener_per_bd_iQ, st_fx->core_brate_fx, coder_type, st_fx->bwidth_fx );
    *tmp_noise = mult_r(320,mean_gain);  /*10 in Q5  lp_gainc in Q3 */
    ...
    IF( NE_16(st_fx->last_coder_type_fx, AUDIO) /* First audio frame */
        && NE_16(st_fx->last_coder_type_fx, UNVOICED) )/* last_coder_type == INACTIVE is overwritten in update_enc to UNVOICED */
    {
    ...
    freq_dnw_scaling_fx( st_fx->cor_strong_limit_fx, coder_type, st_fx->noise_lev_fx, st_fx->core_brate_fx, exc_diffQ, Q_PVQ_OUT );
    ...
    highband_exc_dct_in_fx( st_fx->core_brate_fx, mfreq_bindiv_loc_fx, last_bin, Diff_len, st_fx->noise_lev_fx, tmp_band, exc_diffQ,
                            ...
                            st_fx->bwidth_fx, exc_wo_nf, Q_PVQ_OUT, Q_exc, st_fx->GSC_noisy_speech_fx, NULL );
