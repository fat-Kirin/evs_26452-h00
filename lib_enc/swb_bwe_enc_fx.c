...
)
{
    ...
    IF(GT_16(add(max1_exp, *Q_old), add(max2_exp, *Q_new)))
    {
    ...
    ELSE IF(LT_16(add(max1_exp, *Q_old), add(max2_exp, *Q_new)))
    {
...
)
{
    ...
    IF( EQ_32(st_fx->total_brate_fx, ACELP_13k20))
    {
        /*---------------------------------------------------------------------*
...
)
{
    ...
    *----------------------------------------------------------------------*/
    ...
    IF(EQ_16(st_fx->last_extl_fx, SWB_BWE)||EQ_16(st_fx->last_extl_fx,FB_BWE))
    {
        exp = norm_l(st_fx->EnergyLT_fx);
        IF(add(st_fx->EnergyLT_fx_exp, exp) > shl(sub(Q_synth_hf, 4), 1))
        {
            Q_shb = sub(Q_synth_hf, 4);
            st_fx->EnergyLT_fx = L_shr(st_fx->EnergyLT_fx, sub(st_fx->EnergyLT_fx_exp, shl(Q_shb, 1)));
        }
        ELSE
        {
            Q_shb = shr(add(st_fx->EnergyLT_fx_exp, exp), 1);
...
)
{
    ...
    test();
    IF( EQ_16(extl,SWB_BWE)||EQ_16(extl,FB_BWE))
    {
    ...
    ELSE
    {
        ...
        IF ( EQ_16(offset,HQ_GENERIC_FOFFSET_24K4 ))
        {
...
)
{
    ...
    IF ( EQ_16(st_fx->extl_fx, SWB_BWE)||EQ_16(st_fx->extl_fx,FB_BWE))
    {
        ...
        test();
        test();
        IF ( ( EQ_16(st_fx->last_extl_fx, SWB_BWE)&&EQ_16(st_fx->extl_fx,SWB_BWE))||(EQ_16(st_fx->last_extl_fx,FB_BWE)&&EQ_16(st_fx->extl_fx,FB_BWE)))
        {
            ...
            test();
            IF (EQ_16(st_fx->prev_mode_fx,TRANSIENT))
            {
                numharmonic = shl(numharmonic, 1);
            }
            ELSE IF (EQ_16(st_fx->prev_mode_fx, NORMAL)||EQ_16(st_fx->prev_mode_fx,NOISE))
            {
    ...
    ELSE IF (EQ_16(st_fx->extl_fx, WB_BWE))
    {
    ...
    FOR(i = 0; i < numsharp; i ++)
    {
        ...
        IF(NE_32(mean[i], L_deposit_l(peak)))
        {
        ...
        test();
        IF (GT_16(sharp, 4608)&&GT_16(peak,shl(1,add(Q_syn,3))))
        {
            k = add(k, 1);
            move16();
        }
        ELSE IF (LT_16(sharp, 3072))
        {
            noise = add(noise, 1);
            move16();
        }

        IF (GT_16(sharp, sharpPeak))
        {
            sharpPeak = sharp;
            move16();
        }
    ...
    IF ( EQ_16(st_fx->extl_fx, SWB_BWE)||EQ_16(st_fx->extl_fx,FB_BWE))
    {
        ...
        test();
        test();
        IF(GE_16(k, numharmonic)&&GT_16(gain_tmp,8192)&&LT_16(gain_tmp,29491)&&GT_16(sharpPeak,shl(sharplimit,10)))
        {
            sharpMod = 1;
            move16();
        }
    ...
    {
        test();
        IF (GE_16(k,numharmonic)&&GT_16(sharpPeak,shl(sharplimit,10)))
        {
            sharpMod = 1;
            move16();
        }
    ...
    test();
    test();
    IF (sharpMod && LT_16(st_fx->modeCount_fx, 12))
    {
    ...
    ELSE IF ( EQ_16(st_fx->extl_fx, SWB_BWE)||EQ_16(st_fx->extl_fx,FB_BWE))
    {
        ...
        test();
        test();
        IF (GT_16(noise, 4)&&(LT_32(L_tmp,L_meanH)||L_meanH==0)&&LT_16(tilt_nb,10240))
        {
            mode = NOISE;
            move16();
        }
...
static void freq_weights_fx(
    ...
    Word16 w_env[],               /* o  : Freq. weighting         Q13 */
...
)
{
    ...
    FOR( i=1; i<Nbands; i++ )
    {
        IF( LT_16(Band_Ener[i], min_b))
        {
            min_b = Band_Ener[i];
            move16();/*Q8 */
        }

        IF( GT_16(Band_Ener[i],max_b))
        {
            max_b = Band_Ener[i];
            move16();/*Q8 */
        }
...
)
{
    ...
    IF ( EQ_16(core,ACELP_CORE))
    {
        ...
        move16();/*.35 in Q15 */
        test();
        IF ( NE_16(coder_type,AUDIO)&&LT_32(st_fx->total_brate_fx,ACELP_8k85))
        {
            core_type = 0;
            move16();
        }
    ...
    ELSE  /* HQ core */
    {
        ...
        band_step = 1;
        move16();
        IF ( EQ_16(offset,HQ_GENERIC_FOFFSET_32K))
        {
            max_band = 12;
            move16();
        }
    ...
    FOR( n_band=0; n_band<max_band; )
    {
        ...
        IF( LT_16(SFM_gen_fx[n_band],mult_r(24576,SFM_org_fx[n_band])))
        {
            ...
            IF( LT_16(energy_factor_fx[n_band],gamma_fx))
            {
...
)
{
    ...
    FOR ( n_band = 0; n_band < 4; n_band++ )
    {
        ...
        IF(GT_32(Mult_32_16(energy_fx, 5958), st_fx->EnergyLF_fx))
        {
            IsTransient_LF = 1;
            move16();
        }
    ...
    test();
    test();
    IF( EQ_16(IsTransient,1)&&(GT_16(tilt_fx,16384)||GT_16(st_fx->clas_fx,1)))
    {
        IsTransient = 0;
        move16();
    ...
    IF( EQ_16(IsTransient,1))
    {
        ...
        {
            IF(GT_16(SWB_tenv_fx[n_band],max_fx))
            {
        ...
        test();
        test();
        IF(IsTransient_LF == 0 && EQ_16(coder_type,INACTIVE)&&EQ_16(st_fx->TransientHangOver_fx,1))
        {
            FOR(n_band = 0; n_band < SWB_TENV; n_band++)
            {
                SWB_tenv_fx[n_band] = mult_r(SWB_tenv_fx[n_band], 16384);
        ...
        FOR(n_band = 0; n_band < SWB_TENV; n_band++)
        {
            ...
            IF (GT_16(SWB_tenv_tmp_fx[n_band], 30720))
            {
                index = 15;
                move16();
            }
...
)
{
    ...
    FOR ( n_coeff=0; n_coeff<length; n_coeff++ )
    {
        ...
        IF (LT_32(max_fx , org_spec_fx[n_coeff]))
        {
    ...
    FOR ( n_coeff=0; n_coeff<length; n_coeff++ )
    {
        ...
        IF (LT_32(max_fx , gen_spec_fx[n_coeff]))
        {
...
)
{
    ...
    IF ( core == ACELP_CORE )
    {
        ...
        move16();
        test();
        IF ( NE_16(coder_type , AUDIO)&&LE_32(st_fx->total_brate_fx,ACELP_8k00))
        {
            core_type = 0;
            move16();
        }
...
)
{
    ...
    IF (GT_32(avgp2a_fx , 187227))/*8.6 / 10log10(2), Q16 */
    {
...
)
{
    ...
    IF ( EQ_16(st_fx->hq_generic_speech_class_fx , 1))
    {
    ...
    FOR ( n_band = 0; n_band < nenv; n_band++ )
    {
        ...
        {
            IF (LT_32(max_coefs_fx , L_abs(coefs_fx[n_coeff])))
            {
        ...
        ELSE
        {
            hq_generic_fenv_fx[n_band] = -24576;
            move16();
        }
    }
    ...
    IF ( EQ_16(st_fx->bwidth_fx , FB))
    {
        FOR ( n_band = 0; n_band < DIM_FB; n_band++ )
        {
            ...
            ELSE
            {
                hq_generic_fenv_fx[add(n_band,nenv)] = -24576;
                move16();
            }
        }
    }
