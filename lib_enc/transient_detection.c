                             ...
                             const Word16 nCurrentSubblocks, const Word16 nPrevSubblocks)
{
    ...
    IF ( s_or(pTransientDetector->bIsAttackPresent, isTCX10) )    /* frame is TCX-10 */
    {
        ...
        {
            if ( LT_32(nrgMax, pSubblockNrg[i]))
            {
                idxMax = i;
                move16();
            }
        ...
        if ( LT_32(Mpy_32_16_1(nrgMax, 12288/*0.375f Q15*/), nrgMin) )
        {
    ...
    IF( EQ_16(st->codec_mode,MODE2))
    {
        ...
        IF ( st->tcx10Enabled != 0 && st->tcx20Enabled != 0 )
        {
            ...
            IF ( ((pTransientDetection->transientDetector.bIsAttackPresent != 0)
                  ||  (GT_32(Mpy_32_16_1(st->currEnergyHF_fx, 840/*1.0f/39.0f Q15*/), st->prevEnergyHF_fx) ))
                 && ((NE_16(st->last_core_fx, ACELP_CORE) ) && (NE_16(st->last_core_fx, AMR_WB_CORE) )) )
            {
                move16();
                st->tcxMode = TCX_10;
            }
        ...
        ELSE IF ( (EQ_16(st->tcxMode, TCX_10))&&(EQ_16(st->tcx_cfg.tcx_curr_overlap_mode,ALDO_WINDOW)))
        {
            move16();
        ...
        ELSE IF ( EQ_16(st->tcxMode, TCX_20))
        {
            IF (EQ_16(pTransientDetection->transientDetector.attackIndex, 7))
            {
                move16();
                *tcxModeOverlap = HALF_OVERLAP;
            }
            ELSE IF (EQ_16(pTransientDetection->transientDetector.attackIndex, 6))
            {
                move16();
        ...
        test();
        if ((EQ_16(st->tcx_cfg.tcx_last_overlap_mode, TRANSITION_OVERLAP))&&(EQ_16(*tcxModeOverlap,ALDO_WINDOW)))
        {
            move16();
            *tcxModeOverlap = FULL_OVERLAP;
        }
...
static void UpdatedAndStoreAccWindowNrg(Word32 newWindowNrgF, Word32 * pAccSubblockNrg, Word16 facAccSubblockNrg, Word32 * pOutAccWindowNrgF)
{
    ...
    *pAccSubblockNrg = Mpy_32_16_1(*pAccSubblockNrg, facAccSubblockNrg);
    ...
    if ( GT_32(newWindowNrgF, *pAccSubblockNrg))
    {
        move32();
...
static void CalculateSubblockEnergies(Word16 const * input, Word16 nSamplesAvailable, SubblockEnergies * pSubblockEnergies)
{
    ...
    IF (nWindows > 0)
    {
        ...
        FOR (w = 0; w < nWindows; w++)
        {
            ...
            IF ( LT_16(k,SUBBLOCK_NRG_CHANGE_E))
            {
                move16();
