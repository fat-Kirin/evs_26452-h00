...
)
{
    ...
    {
        test();
        IF( GT_16(add(prev_pitch,i),st->pit_max)||LT_16(add(prev_pitch,i),st->pit_min))
        {
            CONTINUE;
        }
        ps        = L_deposit_l(0);
...
void enc_prm_side_Info( HANDLE_PLC_ENC_EVS hPlc_Ext, Encoder_State_fx *st )
{
    ...
    IF( GT_16(hPlc_Ext->nBits,1))
    {
        ...
        test();
        if( (GT_16(diff_pitch,sub(search_range,1)))||(LT_16(diff_pitch,add(-search_range,1))))
        {
            diff_pitch = -8;
            move16();
        }
...
)
{
    ...
    IF(sw)
    {
        ...
        test();
        test();
        if(GT_32(cum_dist1, L_add(cum_dist2, Mpy_32_16_1(cum_dist2, 4915)))
                && GT_16(sub(lsf4_mean, *prev_lsf4_mean), th_dif)
                && LT_16(*prev_lsf4_mean, th) 
                && GT_16(cnt_imprv, 2) 
