...
)
{
    ...
    IF ( cng_sort_fx[idx_cv] )
    {
        ...
        permute_fx(pTmp1, perm_MSLVQ_fx[idx_cv]);
        permute_fx(w1, perm_MSLVQ_fx[idx_cv]);

        dist = q_data_fx( pTmp1, w1, quant, cv_out, idx_lead, idx_scale, p_sigma, p_inv_sigma, p_scales, p_no_scales, p_no_lead );
        /* permute back */
        permute_fx( quant, perm_MSLVQ_fx[idx_cv] );
    }
    ELSE
    {
...
)
{
    ...
    FOR(j=0; j<no_leaders[0]; j++)
    {
        ...
        IF (EQ_16(l, LATTICE_DIM-1))
        {
            p = *pl_crt;
            /* if it went up to 7th position */
            IF ( pl_par_fx[j] )
            {
                IF ( NE_16(sig,pl_par_fx[j]))
                {
        ...
        IF ( LT_32(tmp_dist,min_dist))
        {
            min_dist = L_add(tmp_dist, 0);
            best_scale = 0;
            move16(); /* scale(truncation) index of potential codevector */
            best_idx = j;
            move16(); /* leader class index of potential codevector */
        }
    }
    ...
    FOR(k=1; k<no_scales; k++)
    {
        ...
        FOR(j=0; j<no_leaders[k]; j++)
        {
            ...
            IF ( LT_32(tmp_dist,min_dist))
            {
                min_dist = L_add(tmp_dist, 0);
                best_scale = k;
                move16(); /* scale(truncation) index of the potential codevector */
                best_idx = j;
                move16();  /* leader class index of the potential codevector */
            }
        }
        low_prec = 0;
        move16();
...
)
{
    ...
    IF ( no_scales > 0 )
    {
        ...
        IF ( GT_16(best_scale,-1)  )
        {
            ...
            FOR(j=0; j<LATTICE_DIM; j++)
            {
                cv_out[j] = i_mult2(sign[j], pl_HQ_fx[best_idx*LATTICE_DIM+id[j]]);
                move16();
            }
            IF(pl_par_fx[best_idx] )
            {
                IF ( NE_16(sig,pl_par_fx[best_idx]))
                {
...
)
{
    ...
    idx64 = W_add_nosat(idx64, index2_64);
...
)
{
    ...
    index = extract_l(L_mult0(index, C_VQ_fx[dim_loc][no_vals_ind_fx[idx_lead][1]]));
    ...
    index = extract_l(L_mult0(index, C_VQ_fx[dim_loc][no_vals_ind_fx[idx_lead][2]]));
...
)
{
    ...
    ELSE
    {
        ...
        FOR( i=1; i<=p[0]; i++ )
        {
            skip = add(skip, C_VQ_fx[sub(n,i)][sub(k,1)]);
        }
