...
)
{
    ...
    st_fx->Q_old_wtda = new_input_fx_exp;
    move16();
    new_input_fx_exp=0;
    wtda_fx( new_input_fx, &new_input_fx_exp, L_t_audio_tmp_fx, st_fx->L_old_wtda_swb_fx,
             &st_fx->Q_old_wtda, ALDO_WINDOW, ALDO_WINDOW, input_frame );
    ...
    IF( is_transient )
    {
        ...
        IF( EQ_16(st_fx->extl_fx, SWB_BWE_HIGHRATE))
        {
        ...
        FOR( k = 0; k < NUM_TIME_SWITCHING_BLOCKS; k++ )
        {
            ...
            ELSE
            {
                IF( LT_16(ind2, NUM_ENVLOPE_CODE_HR_TR2 ))
                {
            ...
            IF( NE_16(st_fx->extl_fx, SWB_BWE_HIGHRATE))
            {
    ...
    ELSE /* !is_transient */
    {
        ...
        {
            if (LT_16(en_band_fx[i], en_band_fx[pos]))
            {
                pos = i;
                move16();
            }
