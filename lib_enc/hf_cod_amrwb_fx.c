...
)
{
    ...
    set16_fx( mem_hf1_enc, 0, L_FIR-1 );
    set16_fx( mem_syn_hf_enc, 0, M );
    set16_fx( mem_hf2_enc, 0, L_FIR-1 );
...
)
{
    ...
    *-----------------------------------------------------------------*/
    ...
    test();
    IF( EQ_32(core_brate_fx, SID_1k75)||core_brate_fx==FRAME_NO_DATA)
    {
    ...
    *-----------------------------------------------------------------*/
    ...
    Syn_filt_s( 0, Ap_fx, M, HF_fx, HF_syn_fx, L_SUBFR16k, mem_syn_hf_enc_fx, 1 ); /*Q0 */

    /*-----------------------------------------------------------------*
    * high pass filtering (0.9375ms of delay = 15 samples@16k)
    *-----------------------------------------------------------------*/

    fir_fx(HF_syn_fx, fir_6k_8k_fx, HF_syn_fx, mem_hf1_enc_fx, L_SUBFR16k, L_FIR-1, 1, 0);
    fir_fx(HF_SP_fx, fir_6k_8k_fx, HF_SP_fx, mem_hf2_enc_fx, L_SUBFR16k, L_FIR-1, 1, 0);
    ...
    IF ( GT_16(4, dtxHangoverCount_fx))
    {
        *gain_alpha_fx = 16384;
        move16();
    }
