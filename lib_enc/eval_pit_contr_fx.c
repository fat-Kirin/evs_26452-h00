...
)
{
    ...
    IF( GT_16(F8th_harm,mfreq_loc_Q2fx[last_pit_band]))
    {
        DO
        {
            last_pit_band = add(last_pit_band,1);
        }
        WHILE( GE_16(F8th_harm,mfreq_loc_Q2fx[last_pit_band]));
    }
    test();
    test();
    test();
    ...
    test();
    test();
    IF( (st_fx->mem_last_pit_band_fx > 0 && GT_16(st_fx->old_corr_fx,16384)&&GT_16(st_fx->mold_corr_fx,16384)&&GE_16(st_fx->lt_gpitch_fx,19661)/*1.5f*GPIT_THR*/)
        || (GT_16(last_pit_band,6) )
        || (GE_16(last_pit_band,4)  && GE_16(st_fx->lt_gpitch_fx,19661) /*1.5f*GPIT_THR*/ && GT_16(st_fx->old_corr_fx,22938) )
        || (GT_16(last_pit_band,BAND1k2)  &&  GT_16(st_fx->mold_corr_fx,26214)  && GE_16(st_fx->lt_gpitch_fx,13107) /*GPIT_THR*/)
      )
    {
        tmp_dec = 1;
        move16();
    ...
    test();
    test();
    IF ( (st_fx->mem_last_pit_band_fx == 0 && EQ_16(tmp_dec,1))||(st_fx->mem_last_pit_band_fx>0&&tmp_dec==0))
    {
        IF( *hangover == 0 )
        {
            time_flg = tmp_dec;
    ...
    test();
    test();
    IF( EQ_16(time_flg,1)||NE_16(coder_type,INACTIVE)||st_fx->GSC_noisy_speech_fx)
    {
        test();
        test();
        /*if(st_fx->core_brate_fx  <ACELP_9k60)*/
        IF(LT_32(st_fx->core_brate_fx,ACELP_9k60)&&LT_16(low_pit,4096))
        {
