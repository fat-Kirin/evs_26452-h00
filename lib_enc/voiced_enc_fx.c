...
)
{
    ...
    test();
    IF (EQ_16(st_fx->bwidth_fx,WB)||EQ_16(st_fx->bwidth_fx,SWB))
    {
    ...
    IF ( EQ_16(st_fx->last_ppp_mode_fx,1)&&!st_fx->mode_QQF_fx)
    {
        st_fx->bump_up_fx = 1;
        move16();
        free(CURRP_NQ_FX);
    ...
    pl = s_min(MAX_LAG_PIT, rint_new_fx(L_shl(st_fx->old_pitch_buf_fx[(2*NB_SUBFR)-1],10)));
    move16();
    l = s_min(MAX_LAG_PIT, rint_new_fx(L_deposit_h(delay_fx)));
    move16();
    /* st_fx->old_pitch_buf_fx in Q6*/

    /* Bump up if the lag is out_fx of range */
    test();
    IF (GT_16(sub(l,pl),13)||LT_16(sub(l,pl),-11)||LT_16(l,19)||LT_16(pl,19))
    {
        st_fx->bump_up_fx = 1;
        move16();
        free(CURRP_NQ_FX);
    ...
    IF (NE_16(st_fx->last_ppp_mode_fx,1))
    {
    ...
    if (EQ_16(st_fx->last_coder_type_raw_fx,UNVOICED))
    {
        pl = l;
    ...
    ELSE
    {
        if (LT_32(L_shr(Lacc1,negate(sft)),Lacc))
        {
            res_enratio_fx=0x7FFF;
            move16();
        }
    ...
    IF (NE_32(res_enratio_fx,0x7FFF))
    {
    ...
    *******************************************************************************/
    ...
    /*******************************************************************************/
    IF (EQ_16(PPP_MODE_E,'Q'))
    {
        /* Bump up if the lag is out_fx of range */
        test();
        IF (GT_16(sub(l,pl),13)||LT_16(sub(l,pl),-11))
        {
            PPP_MODE_E = 'B' ;
            move16();
        }
        ...
        IF ( LT_16(shl(vadsnr_fx,1) ,st_fx->SNR_THLD_fx)) /*Q8  */
        {
            /*if ( res_enratio > 5.0 && tmp < 0.65 ) */
            /* 5 in Q11, 0.65 in Q15 // L_shl(tmp_fx,sub(31,Qtmp)) makes tmp_fx FIXED Q31 */
            test();
            IF ((GT_32(res_enratio_fx,10240))&&(LT_16(extract_h(L_shl(tmp_fx,sub(31,Qtmp))),21299)))
            {
                PPP_MODE_E = 'B';
                move16();
            }
        ...
        ELSE
        {
            ...
            test();
            IF ( (GT_32(res_enratio_fx,6144))&&(LT_16(extract_h(L_shl(tmp_fx,sub(30,Qtmp))),19661)))
            {
                PPP_MODE_E = 'B';
                move16();
            }
    ...
    IF ( LT_16(shl(vadsnr_fx,1) ,st_fx->SNR_THLD_fx)) /*Q8  */
    {
        ...
        IF (LT_32(L_shl(res_enratio_fx,4),819))  /*0x0333 = 0.025 in Q15, res_enratio_fx in Q15 after shl 4 */
        {
            st_fx->bump_up_fx = 1;
            move16();
            free(CURRP_NQ_FX);
    ...
    ELSE
    {
        ...
        if ( LT_32(L_shl(res_enratio_fx,4), 3015))/*3015 = 0.092 in Q15, res_enratio_fx in Q15 after shl 4 */
        {
            st_fx->bump_up_fx = 1;
            move16();
        }
    ...
    test();
    if ( LT_32(L_min(L_shl(res_enratio_fx,4), sp_enratio_fx), 2458)&&LT_32(tmp_fx,shl(-1,sub(Qtmp,1))))
    {
        st_fx->bump_up_fx = 1;
        move16();
    }
    ...
    IF ( LT_16(shl(vadsnr_fx,1) ,st_fx->SNR_THLD_fx)) /*Q8  */
    {
        IF (GT_32(res_enratio_fx,29696))  /*14.5 in Q11 */
        {
            st_fx->bump_up_fx = 1;
            move16();
            free(CURRP_NQ_FX);
    ...
    ELSE
    {
        if (GT_32(res_enratio_fx,14336))  /* 7.0 in Q11 */
        {
            st_fx->bump_up_fx = 1;
            move16();
        }
    ...
    IF (EQ_16(PPP_MODE_E,'Q'))
    {
        ...
        IF (flag)
        {
            ...
            IF ( GT_16(CURRP_Q_E_FX->upper_cut_off_freq_fx , 4000))
            {
                ...
                test();
                IF ( LT_32(low_band_en_fx , 204800 )&&LT_32(sp_hb_enratio_fx,858993459))
                {
                    PPP_MODE_E = 'B';
                    move16();
                }
            ...
            IF ( LT_16(shl(vadsnr_fx,1) ,st_fx->SNR_THLD_fx)) /*Q8  */
            {
                ...
                test();
                test();
                IF ( GT_32( Ltmp_32, Ltmp1_32)&&GT_32(L_max(pos_nq_fx,neg_nq_fx),805306368)&&st_fx->rate_control_fx)
                {
                    /*if ( pos_nq > neg_nq && pos_nq > 2.0f * pos_q )  */
                    test();
                    IF ( GT_32(pos_nq_fx , neg_nq_fx)&&GT_32(Mult_32_16(pos_nq_fx,16384),pos_q_fx))
                    {
                        PPP_MODE_E = 'B';
                        move16();
                    }
                    ...
                    IF ( LT_32(pos_nq_fx , neg_nq_fx)&&GT_32(Mult_32_16(neg_nq_fx,16384),neg_q_fx))
                    {
                        PPP_MODE_E = 'B';
                        move16();
                    }
            ...
            ELSE
            {
                ...
                test();
                test();
                IF ((((L_sub(Ltmp_32 ,(st_fx->prev_cw_en_fx)>0))&&(GT_32(L_max(pos_nq_fx,neg_nq_fx),939524096)))&&(st_fx->rate_control_fx))||
                (((GT_32( Ltmp_32, Ltmp1_32)  ) && (GT_32(L_max(pos_nq_fx,neg_nq_fx),805306368)))&&(!st_fx->rate_control_fx)))
                {
                    ...
                    test();
                    test();
                    IF ((GT_32(pos_nq_fx , neg_nq_fx)&&GT_32(Mult_32_16(pos_nq_fx,13107),pos_q_fx)&&(st_fx->rate_control_fx))||
                    (GT_32(pos_nq_fx , neg_nq_fx) && GT_32(Mult_32_16(pos_nq_fx,16384),pos_q_fx) && (!st_fx->rate_control_fx)))
                    {
                        PPP_MODE_E='B';
                        move16();
                    }
                    ...
                    test();
                    test();
                    IF ((LT_32(pos_nq_fx , neg_nq_fx)&&GT_32(Mult_32_16(neg_nq_fx,13107),neg_q_fx)&&(st_fx->rate_control_fx))||
                        (LT_32(pos_nq_fx , neg_nq_fx) && GT_32(Mult_32_16(neg_nq_fx,16384),neg_q_fx)  && (!st_fx->rate_control_fx)))
                    {
                        PPP_MODE_E='B';
                        move16();
                    }
                ...
                IF ( st_fx->rate_control_fx )
                {
                    ...
                    test();
                    test();
                    IF ((GT_32(Ltmp_32, st_fx->prev_cw_en_fx ))&&(GT_32(L_max(pos_q_fx,neg_q_fx),939524096))&&(GT_32(energy_impz_fx,30720))
                        && (GT_32(Mult_32_16(tmpres_fx,23265),shl(1,sub(Qtmpres,1))))   )
                    {
                        /* if ((pos_q > neg_q) && ((pos_q>3.0*pos_nq0) || ((pos_q > 1.5*pos_nq0) && (neg_q < 1.5*neg_nq0)))) */
                        test();
                        test();
                        test();
                        IF ( (GT_32(pos_q_fx , neg_q_fx))
                             && ((GT_32(Mult_32_16(pos_q_fx,10923), L_shr(pos_nq0_fx,sub(Qposnq,28)) ))
                                 || ((GT_32( Mult_32_16(pos_q_fx,21845),L_shr(pos_nq0_fx,sub(Qposnq,28)) ))
                                     && (LT_32(Mult_32_16(neg_q_fx,21846), L_shr(neg_nq0_fx,sub(Qnegnq,28)) )) ))
                           )
                        ...
                        IF ( (LE_32(pos_q_fx ,neg_q_fx))
                             && ((GT_32(Mult_32_16(neg_q_fx,10923),  L_shr(neg_nq0_fx,sub(Qnegnq,28))))
                                 || ((GT_32(Mult_32_16(neg_q_fx,21846),  L_shr(neg_nq0_fx,sub(Qnegnq,28))))
                                     && (LT_32( Mult_32_16( pos_q_fx,21846),L_shr(pos_nq0_fx,sub(Qposnq,28))))))
                           )
                        {
                            PPP_MODE_E='B';
                            move16();
            ...
            *TMPDTFS3_FX = DTFS_sub_fx(*TMPDTFS_FX,*TMPDTFS2_FX);
            ...
            test();
            IF (GT_32(Ltemp_fx , 6710886)&&(!st_fx->rate_control_fx)) /* 0.05 in Q27 = 6710886 */
            {
                ...
                IF(LT_32(Ltmp,1638)) /* 1638 = 0.1 in Q14 */
                {
                    /* if (res_enratio > 0.8) */
                    if (GT_32(res_enratio_fx , 1638)) /* 1638 = 0.8 in Q11, res_enratio_fx in Q11 */
                    {
                        PPP_MODE_E = 'B';
                        move16();
                    }
            ...
            IF ( LT_16(shl(vadsnr_fx,1) ,st_fx->SNR_THLD_fx)) /* Q8 */
            {
                ...
                test();
                test();
                if ((( LT_32(Ltemp, 25585254 )&&GT_32(L_max(L_shl(res_enratio_fx,4),sp_enratio_fx),6554))&&(st_fx->rate_control_fx))||
                        (( LT_32(Ltemp, 23488102 )  && GT_32(L_max(L_shl(res_enratio_fx,4),sp_enratio_fx) , 5325) )&&(!st_fx->rate_control_fx)))
                {
                    PPP_MODE_E = 'B';
                    move16();
                }
            ...
            ELSE
            {
                ...
                test();
                test();
                if ((( LT_32(Ltemp ,20132659 )&&GT_32(L_max(L_shl(res_enratio_fx,4),sp_enratio_fx),7700))&&(st_fx->rate_control_fx))||
                (( LT_32(Ltemp, 37748736 )  && GT_32(L_max(L_shl(res_enratio_fx,4),sp_enratio_fx) , 4096) )&&(!st_fx->rate_control_fx)))
                {
                    PPP_MODE_E = 'B';
                    move16();
                }
    ...
    IF ( st_fx->Q_to_F_fx )
    {
        ...
        IF (GE_16(st_fx->patterncount_fx , 1000))
        {
