...
static Word16 pyramidSearchProjInit_fx(
    Word16 L,
    Word16 Ptot)
{
    return (sub(Ptot, extract_l(L_shr(L_mult0(8223, (Word32)L), 14))) > 0 );
}
                        ...
                        Word16 max_xabs)         /* current accumulated max amplitude for pulses */
{
    ...
    *L_xy_ptr = L_mac(*L_xy_ptr, x_abs[imax], 1);                              /*      Q12+1 */
...
)
{
    ...
    ELSE
    {
        ...
        num = sub(pulses, PYR_OFFSET);
        high_pulse_density_flag = pyramidSearchProjInit_fx(dim, pulses );
        ...
        L_yy = L_shl(L_yy,1);                    /* compensate search loop analysis energy downshift by 1,
    ...
    IF( neg_gain != 0  )
    {
        L_isqrt = Isqrt(L_shr(L_yy,1));      /* Note: one single gain factor as not computed */
    }
