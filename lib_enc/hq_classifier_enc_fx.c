...
)
{
    ...
    *hqswb_clas = HQ_NORMAL;
    ...
    test();
    test();
    IF( EQ_16(length, L_FRAME32k)&&NE_16(is_transient,1)&&LE_32(st_fx->core_brate_fx,HQ_32k)&&EQ_16(st_fx->bwidth_fx,st_fx->last_bwidth_fx))
    {
    ...
    test();
    test();
    IF ( EQ_16(length, L_FRAME48k)&&NE_16(is_transient,1)&&LE_32(st_fx->core_brate_fx,HQ_32k)&&EQ_16(st_fx->bwidth_fx,st_fx->last_bwidth_fx))
    {
        /* Detect HQ_HARMONIC mode */
        *hqswb_clas = peak_avrg_ratio_fx( st_fx->total_brate_fx, coefs, NUMC_N+96, &st_fx->mode_count_fx, &st_fx->mode_count1_fx, 12);
        /* Detect harmonic VQ mode HQ_HVQ */
    ...
    test();
    test();
    IF( EQ_16(length, L_FRAME48k)&&LE_32(st_fx->core_brate_fx,HQ_32k)&&EQ_16(*hqswb_clas,HQ_NORMAL))
    {
    ...
    test();
    IF( GE_16( length, L_FRAME32k)&&LE_32(st_fx->core_brate_fx,HQ_32k))
    {
        bits = 2;
        move16();
    }
    ...
    test();
    IF ( EQ_16( length, L_FRAME48k)&&LE_32(st_fx->core_brate_fx,HQ_32k))
    {
    ...
    test();
    test();
    IF ( EQ_16( *hqswb_clas, HQ_NORMAL)&&EQ_16(length,L_FRAME32k)&&LE_32(st_fx->core_brate_fx,HQ_32k))
    {
...
)
{
    ...
    FOR( i = 3; i < 17; i ++ )
    {
        ...
        FOR(j = 0; j < 32; j ++)
        {
            ...
            IF (GT_32(input_abs_fx[q] , peak_fx))
            {
        ...
        IF(LT_16(i,8))
        {
            if(GT_32(peak_fx, Mult_32_16(mean_fx, 4608))) /* Q15 0.140625 */
            {
                k = add(k,1);
            }
        }
        ...
        {
            test();
            if(GT_32(peak_fx, Mult_32_16(mean_fx, 3686))  /*Q15 0.1125 */
            && GT_32(peak_fx, peak_th_fx)) /*Q27 10 */
            {
                k1 = add(k1,1);
            }
        }
    ...
    test();
    IF( GE_16(add(k,k1),10)&&GT_16(k1,5))
    {
    ...
    test();
    test();
    if ((GE_16(add(k, k1), 5)&&GT_16(k1,2)&&EQ_32(total_brate,HQ_24k40))
            || (((GE_16(add(k, k1), 10)  && GT_16(k1, 5) ) || GE_16(*mode_count, 5) ) && LT_16(*mode_count1, 5) ))
    {
...
)
{
    ...
    test();
    test();
    IF ( EQ_16(*hqswb_clas, HQ_HARMONIC)&&last_core!=ACELP_CORE&&NE_16(last_core,AMR_WB_CORE))
    {
        FOR ( i = 0; i < N; i++ )
        {
        ...
        *Npeaks = 0;
        ...
        FOR ( i = 0; i < nsub; i++ )
        {
            ...
            IF( LT_32(L_nf_mean[i], L_tmp))
            {
                IF ( LT_16(sharp[i], HVQ_PA_SHARP_THRES3_FX))
                {
                    avail_peaks[i] = HVQ_PA_PEAKS_SHARP3;
                    move16();
                }
                ELSE IF( LT_16(sharp[i], HVQ_PA_SHARP_THRES2_FX))
                {
        ...
        FOR ( i = 0; i < N-2; i++ )
        {
            IF ( LT_32(L_input_abs[i], L_thr[i]))
            {
        ...
        *Npeaks = i;
        move16();
        IF ( GT_16(*Npeaks, HVQ_MIN_PEAKS))
        {
            test();
            IF ( GT_16(num_sharp_bands, sub(nsub, 3))&&LE_16(*Npeaks,peak_th))
            {
                sharp_dist = mult(sharp_dist, inv_nsub);    /*x+15-15=x */
                test();
                IF ( LE_16(sharp_dist, SHARP_DIST_THRES_FX)&&*hvq_hangover<0)
                {
