...
)
{
    ...
    test();
    IF (GT_32(core_brate, ACELP_13k20)&&!Opt_AMR_WB)
    {
        acelpautoc = 1;
        move16();

        cb_shape_fx( 1, 1, 0, sharpFlag, 0, g1, g2, p_Aq, h2, tilt_code, shr(add(pt_pitch,26),6), 0 );
        /* h2: Q11, Rw: (Rw_e)Q */
    ...
    test();
    test();
    IF( NE_16(L_frame,last_L_frame)&&GT_32(core_brate,ACELP_13k20)&&(LT_32(core_brate,ACELP_32k)||EQ_16(bwidth,WB)))
    {
    ...
    cb_shape_fx( 1, 1, 0, sharpFlag, 0, g1, g2, p_Aq, code, tilt_code, shr(add(pt_pitch,26),6), 0 );
