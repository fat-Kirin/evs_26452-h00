...
)
{
    ...
    *hq_core_type = -1;
    ...
    IF ( st_fx->Opt_SC_VBR_fx )
    {
        ...
        test();
        test();
        IF ( EQ_16(st_fx->ppp_mode_fx,1))
        {
            /* PPP mode */
            st_fx->core_brate_fx = PPP_NELP_2k80;
            move16();
        }
        ELSE IF ( ( ( EQ_16(coder_type,UNVOICED)||EQ_16(coder_type,TRANSITION))&&sp_aud_decision1==0)||NE_16(st_fx->bwidth_fx,NB))
        {
            test();
            test();
            test();
            ...
            test();
            test();
            IF ( EQ_16(coder_type,UNVOICED)&&EQ_16(vad_flag,1)&&
            ...
            ELSE IF ( EQ_16(coder_type,TRANSITION)||(EQ_16(coder_type,UNVOICED)&&NE_16(st_fx->nelp_mode_fx,1))||
                      ( ( EQ_16(coder_type,AUDIO)  || EQ_16(coder_type,INACTIVE)  ) && NE_16(st_fx->bwidth_fx, NB)  ) )
    ...
    ELSE IF ( EQ_16(st_fx->bwidth_fx,NB))
    {
        ...
        test();
        if ( GE_32(st_fx->total_brate_fx,HQCORE_NB_MIN_RATE)&&EQ_16(sp_aud_decision1,1))
        {
    ...
    ELSE IF ( EQ_16(st_fx->bwidth_fx,WB))
    {
        ...
        test();
        test();
        IF ( ( GE_32(st_fx->total_brate_fx,HQCORE_WB_MIN_RATE)&&EQ_16(sp_aud_decision1,1))||
        ...
        ELSE
        {
            ...
            test();
            test();
            IF ( EQ_16(st_fx->bwidth_fx,WB)&&LT_32(st_fx->total_brate_fx,ACELP_9k60))
            {
            ...
            ELSE IF ( EQ_16(st_fx->bwidth_fx,WB)&&GE_32(st_fx->total_brate_fx,ACELP_9k60)&&LE_32(st_fx->total_brate_fx,ACELP_16k40))
            {
                ...
                test();
                test();
                IF ( EQ_16(sp_aud_decision1,1)||EQ_16(coder_type,INACTIVE)||(sp_aud_decision1==0&&EQ_16(sp_aud_decision2,1)))
                {
                    st_fx->extl_fx = WB_BWE;
                    move16();
    ...
    ELSE IF ( EQ_16(st_fx->bwidth_fx,SWB)||EQ_16(st_fx->bwidth_fx,FB))
    {
        test();
        test();
        IF ( ( GE_32(st_fx->total_brate_fx,HQCORE_SWB_MIN_RATE)&&EQ_16(sp_aud_decision1,1))||
             GE_32(st_fx->total_brate_fx,HQ_96k) )
        {
            st_fx->core_fx = HQ_CORE;
            move16();
        ...
        ELSE
        {
            ...
            test();
            test();
            IF ( GE_32(st_fx->total_brate_fx,ACELP_13k20)&&LT_32(st_fx->total_brate_fx,ACELP_48k))
            {
                ...
                test();
                test();
                IF ( (EQ_16(sp_aud_decision1,1)||EQ_16(coder_type,INACTIVE)||(sp_aud_decision1==0&&EQ_16(sp_aud_decision2,1)))&&st_fx->GSC_noisy_speech_fx==0)
                {
...
)
{
    ...
    IF( EQ_16(st_fx->core_fx,ACELP_CORE ))
    {
        ...
        test();
        test();
        IF( EQ_16(st_fx->ppp_mode_fx,1)||EQ_16(st_fx->nelp_mode_fx,1))
        {
            ...
            test();
            test();
            IF ( EQ_16(coder_type,VOICED)&&EQ_16(st_fx->bwidth_fx,NB)&&EQ_16(st_fx->ppp_mode_fx,1))
            {
                push_indice_fx( st_fx, IND_PPP_NELP_MODE, 0, 2 );
            }
            ELSE IF ( EQ_16(coder_type,VOICED)&&NE_16(st_fx->bwidth_fx,NB)&&EQ_16(st_fx->ppp_mode_fx,1))
            {
                push_indice_fx( st_fx, IND_PPP_NELP_MODE, 1, 2 );
            }
            ELSE IF ( EQ_16(coder_type,UNVOICED)&&EQ_16(st_fx->bwidth_fx,NB)&&EQ_16(st_fx->nelp_mode_fx,1))
            {
                push_indice_fx( st_fx, IND_PPP_NELP_MODE, 2, 2);
            }
            ELSE IF ( EQ_16(coder_type,UNVOICED)&&NE_16(st_fx->bwidth_fx,NB)&&EQ_16(st_fx->nelp_mode_fx,1))
            {
