...
)
{
    ...
    IF( st_fx->ini_frame_fx == 0 )
    {
        ...
        move16();
        test();
        if( EQ_16( st_fx->codec_mode, MODE1 )||st_fx->Opt_AMR_WB_fx)
        {
        ...
        st_fx->Opt_AMR_WB_fx == 0 )
    {
        ...
        test();
        test();
        IF ( GT_32(st_fx->last_core_brate_fx,SID_2k40)&&
             NE_32(st_fx->last_total_brate_cng_fx,-1)  &&
             NE_32(st_fx->last_total_brate_cng_fx,st_fx->total_brate_fx)  &&
             ( (LE_32(st_fx->last_total_brate_cng_fx, ACELP_24k40) ) || (LT_16(st_fx->lp_noise_fx, 1280) ) ) )
        {
        ...
        IF ( LE_32(st_fx->last_core_brate_fx,SID_2k40)&&
             NE_32(st_fx->last_total_brate_fx,st_fx->total_brate_fx)&&
             ( LE_32(st_fx->last_total_brate_fx,ACELP_24k40)  || LT_16(st_fx->lp_noise_fx, 1280) ) )
    ...
    IF( st_fx->Opt_DTX_ON_fx && vad == 0 &&
        GT_16(st_fx->ini_frame_fx,2)   &&                     /* CNG coding starts after 2 frames */
        ( LE_32(st_fx->total_brate_fx,ACELP_24k40)  || LT_16(st_fx->lp_noise_fx, 1280)) &&
        st_fx->fd_cng_reset_flag == 0 )
    {
        /* reset counter */
        ...
        test();
        IF( EQ_16(st_fx->cng_type_fx,FD_CNG)&&LE_32(st_fx->total_brate_fx,ACELP_24k40))     /* at highest bit-rates, use exclusively LP_CNG */
        {
            test();
            test();
            IF ( EQ_32(st_fx->total_brate_fx,ACELP_9k60)||EQ_32(st_fx->total_brate_fx,ACELP_16k40)||EQ_32(st_fx->total_brate_fx,ACELP_24k40))
            {
                st_fx->codec_mode = MODE2;
                move16();
            }
    ...
    test();
    test();
    IF ( (NE_32(st_fx->core_brate_fx ,SID_2k40))&&(NE_32(st_fx->core_brate_fx,SID_1k75))&&(st_fx->core_brate_fx!=0))
    {
        st_fx->cnt_SID_fx = 0;
        move16();
        ...
        IF ( LT_16(st_fx->interval_SID_fx,st_fx->max_SID_fx))
        {
        ...
        test();
        IF( GE_16(st_fx->active_fr_cnt_fx,CNG_TYPE_HO)&&st_fx->Opt_AMR_WB_fx==0)
        {
            test();
            test();
            test();
    ...
    IF ( st_fx->Opt_DTX_ON_fx )
    {
        ...
        IF ( EQ_16(st_fx->clas_fx,VOICED_CLAS))
        {
            ...
            move16();
            if ( GT_32(st_fx->frame_ener_fx,st_fx->lt_ener_voiced_fx))
            {
        ...
        ELSE IF( !st_fx->Opt_AMR_WB_fx )
        {
            alpha = ALPHA_ENER_SLOW_FX;
            move16();
            if (LT_32(st_fx->frame_ener_fx,st_fx->lt_ener_noise_fx))
            {
                alpha = ALPHA_ENER_FAST_FX;
                move16();
            }
...
)
{
    ...
    test();
    test();
    IF( EQ_32(st_fx->core_brate_fx, SID_2k40)||EQ_32(st_fx->core_brate_fx,SID_1k75)||st_fx->core_brate_fx==FRAME_NO_DATA)
    {
        /* Adapt the SID interval */
        test();
        test();
        IF ( st_fx->var_SID_rate_flag_fx != 0 && EQ_16(st_fx->VarDTX_cnt_voiced_fx, MIN_CNT)&&EQ_16(st_fx->VarDTX_cnt_noise_fx,MIN_CNT))
        {
            ...
            st_fx->interval_SID_fx = s_min(s_max(st_fx->interval_SID_fx, INT_L), INT_H);

            test();

            if( st_fx->Opt_AMR_WB_fx == 0 || NE_16(st_fx->max_SID_fx,3))
            {
                st_fx->max_SID_fx = st_fx->interval_SID_fx;
                move16();    /* change SID update rate */
            }
        ...
        IF( st_fx->Opt_DTX_ON_fx != 0 && st_fx->cnt_SID_fx != 0 )
        {
            ...
            test();
            test();
            if ( LT_16(delta,LTE_VAR_FX)&&EQ_16(st_fx->VarDTX_cnt_voiced_fx,MIN_CNT)&&EQ_16(st_fx->VarDTX_cnt_noise_fx,MIN_CNT))
            {
        ...
        IF( st_fx->var_SID_rate_flag_fx )
        {
            ...
            if( st_fx->Opt_AMR_WB_fx != 0 && EQ_16(st_fx->max_SID_fx,3)&&EQ_16(st_fx->cnt_SID_fx,3))
            {
            ...
            if( NE_16(st_fx->max_SID_fx,3)&&EQ_16(st_fx->cnt_SID_fx,DTX_HIST_SIZE))
            {
        ...
        test();
        IF( st_fx->var_SID_rate_flag_fx == 0 && GT_16(st_fx->interval_SID_fx,1))
        {
            /* set the size of CNG history buffer for averaging to interval_SID frames */
            st_fx->cng_hist_size_fx = st_fx->interval_SID_fx;
            move16();
            if ( GT_16(st_fx->cng_hist_size_fx, DTX_HIST_SIZE))
            {
                st_fx->cng_hist_size_fx = DTX_HIST_SIZE;
                move16();
            }
        }
        IF( GE_16(st_fx->cnt_SID_fx,st_fx->max_SID_fx))
        {
...
)
{
    ...
    FOR ( i=0; i<st_fx->burst_ho_cnt_fx-1; i++ )
    {
        ...
        ptr = add(ptr,1);
        if ( EQ_16(ptr,st_fx->ho_circ_size_fx))
        {
            ptr = 0;
            move16();
        }
    ...
    test();
    test();
    IF ( ( LT_16(Dlsp,13107)&&LT_16(Denr,359)&&LT_16(S_max,3277)
