...
)
{
    ...
    Word16 headroom;
    Word16 cldfb_addition = 0; move16(); 
    Word16 old_pitch1;
    ...
    *attack_flag = 0;
    ...
    last_core_orig = st->last_core_fx;
    move16();
    ...
    modify_Fs_fx( signal_in, input_frame, st->input_Fs_fx, new_inp_12k8, 12800, st->mem_decim_fx, (const Word16)(EQ_16(st->max_bwidth_fx,NB)));
    ...
    test();
    test();
    /* reserve an extra bit of headroom in case of NB coding and if there is a chance of energy above 4 kHz */
    /* st->bwidth_fx refers to the coded bandwidth of the previous frame */
    if( ((EQ_16(st->bwidth_fx, NB))||(EQ_16(st->max_bwidth_fx,NB)))&&(GT_32(st->input_Fs_fx,8000)))
    {
        headroom = add(headroom, 1);
    }
    Preemph_scaled( new_inp_12k8, Q_new, &st->mem_preemph_fx, st->Q_max,
                    PREEMPH_FAC, 0, headroom, L_Q_MEM, L_FRAME, st->last_coder_type_fx, 1 );
    ...
    *vad_flag = wb_vad_fx( st, fr_bands, localVAD, &noisy_speech_HO, &clean_speech_HO, &NB_speech_HO,
    ...
    test();
    IF ( GT_16(relE, 384) )     /*relE > 1.5 */
    {
        st->Pos_relE_cnt = 0;
        move16();
    }
    ELSE IF (relE < 0 && *vad_flag == 1)
    {
        st->Pos_relE_cnt = add(st->Pos_relE_cnt, 1);
    }
     ...
     *----------------------------------------------------------------*/
    test();
    IF( (NE_32(st->last_total_brate_fx,st->total_brate_fx))||(NE_16(st->last_bwidth_fx,st->bwidth_fx)))
    {
        L_tmp = st->total_brate_fx;
        move32();
        test();
        if( st->rf_mode && EQ_32(st->total_brate_fx,ACELP_13k20))
        {
            L_tmp = ACELP_9k60;
            move32();
        }
        configureFdCngEnc( st->hFdCngEnc_fx, st->bwidth_fx, L_tmp );
    }
    test();
    ...
    *----------------------------------------------------------------*/
    ...
    /* 1/4 pitch precision improvement */
    IF( LE_32(st->total_brate_fx,ACELP_24k40))
    {
        /* 1/4 pitch precision improvement */
    ...
    vad_param_updt_fx( st, pitch, voicing, old_pitch1, corr_shift, *vad_flag, A );
    ...
    *coder_type = find_uv_fx( st, pitch_fr, voicing_fr, voicing, inp_12k8, *localVAD, ee, corr_shift,
                              relE, *Etot, hp_E, *Q_new, &flag_spitch, st->voicing_sm_fx, *shift, last_core_orig );
    ...
    *-----------------------------------------------------------------*/
    ...
    ELSE IF( st->rf_mode && NE_32(st->core_brate_fx,FRAME_NO_DATA)&&NE_32(st->core_brate_fx,SID_2k40))
    {
    ...
    IF( EQ_16(st->codec_mode,MODE1))
    {
        ...
        ELSE IF ( EQ_32(st->core_brate_fx,SID_2k40)&&GE_16(st->bwidth_fx,WB)&&st->first_CNG_fx&&LT_16(st->act_cnt2_fx,MIN_ACT_CNG_UPD))
        {
            /* prevent "L_frame" changes in SID frame after short segment of active frames */
            st->L_frame_fx = st->last_CNG_L_frame_fx;
            move16();
        }
        ELSE IF ( ( EQ_32(st->core_brate_fx,SID_2k40)&&GE_32(st->total_brate_fx,ACELP_9k60)&&((EQ_16(st->bwidth_fx,WB)&&!(EQ_32(st->total_brate_fx,ACELP_13k20)&&EQ_16(st->cng_type_fx,FD_CNG)))||(EQ_16(st->cng_type_fx,LP_CNG)&&GT_16(st->bwidth_fx,WB)&&GE_32(st->total_brate_fx,ACELP_16k40))))||
    ...
    IF( EQ_16(st->codec_mode,MODE1))
    {
        ...
        test();
        test();
        IF( NE_16(st->last_L_frame_fx,st->L_frame_fx)&&NE_32(st->core_brate_fx,FRAME_NO_DATA)&&NE_32(st->core_brate_fx,SID_2k40)&&(NE_16(st->coder_type_raw_fx,VOICED)))
        {
            /* enforce TC frame in case of ACELP@12k8 <-> ACELP@16k core switching */
            *coder_type = TRANSITION;
            move16();
        }
        ELSE IF( EQ_16(st->last_core_fx,HQ_CORE)||EQ_16(st->last_core_fx,TCX_10_CORE)||EQ_16(st->last_core_fx,TCX_20_CORE))
        {
            /* enforce TC frame in case of HQ/TCX -> ACELP core switching */
            *coder_type = TRANSITION;
            move16();
        }
        ELSE IF( LE_32(st->last_core_brate_fx,SID_2k40)&&EQ_16(st->cng_type_fx,FD_CNG))
        {
        ...
        ELSE IF( LE_32(st->total_brate_fx,ACELP_24k40)&&*vad_flag==0)
        {
            /* inactive frames will be coded by GSC technology */
            /* except for the VBR mode. VBR mode uses NELP for that */
            test();
            IF ( !( EQ_16(st->Opt_SC_VBR_fx, 1)&&EQ_16(vad_flag_dtx,1)))
            {
                *coder_type = INACTIVE;
                move16();
        ...
        ELSE IF( GT_32(st->total_brate_fx,ACELP_24k40)&&
                 ( (*vad_flag == 0 && GE_16(st->bwidth_fx,SWB)  && GE_16(st->max_bwidth_fx,SWB) ) || (*localVAD == 0 && (LE_16(st->bwidth_fx,WB)  || LE_16(st->max_bwidth_fx,WB))) )
               )
        {
    ...
    ELSE
    {
        ...
        ELSE IF( GT_16(*coder_type,GENERIC))
        {
            *coder_type = GENERIC;
            move16();
        }
    ...
    IF( EQ_16(st->codec_mode,MODE1))
    {
        ...
        IF ( EQ_16(st->core_fx,HQ_CORE)) /* Decision matrix decided for MDCT coding */
        {
            test();
            test();
            IF( (EQ_16(st->bwidth_fx,SWB)||EQ_16(st->bwidth_fx,FB))&&EQ_32(st->total_brate_fx,32000))
            {
                /* Select MDCT Core */
                st->core_fx = mdct_classifier_fx(fft_buff,st,*vad_flag, enerBuffer
                                                 ,sub(enerBuffer_exp, 31)
                                                );
            }
            test();
            IF( (EQ_32(st->total_brate_fx,13200))&&(NE_16(st->bwidth_fx,FB)))
            {
    ...
    test();
    test();
    IF( EQ_32(st->total_brate_fx, ACELP_32k)&&EQ_16(loc_harm,1)&&GT_16(cor_map_sum,50<<8)
        && EQ_16(st->clas_fx, VOICED_CLAS) && EQ_16(*coder_type,GENERIC)  )
    {
    ...
    ELSE IF (NE_16(*coder_type,INACTIVE))
    {
    ...
    *sharpFlag = 0;
    ...
    IF( EQ_16(*coder_type,TRANSITION))
    {
        ...
        test();
        test();
        IF( ( GT_32(st->total_brate_fx,ACELP_48k)&&LT_16(st->bwidth_fx,SWB))||                                     /* Deactivate for core bitrates higher than 48.0 kb/s */
            ( GE_32(st->total_brate_fx,ACELP_13k20)  && LE_32(st->total_brate_fx,ACELP_16k40)  ) ||                    /* Deactivate for bitrates <13.2, 16.4> kb/s (this is basically due to lack of signaling configurations */
            ( GT_32(st->total_brate_fx,ACELP_16k40)&&GT_16(st->lp_noise_fx,FORMANT_SHARPENING_NOISE_THRESHOLD_FX)))  /* Deactivate for bitrates >= 24.4 kb/s if the long-term noise level exceeds 34 dB */
        {
            *sharpFlag= 0;
            move16();
        }
    ...
    test();
    IF( EQ_16(*coder_type,GENERIC)||EQ_16(*coder_type,VOICED))
    {
        test();
        test();
        test();
        ...
        test();
        IF( *vad_hover_flag ||
            ( GT_32(st->total_brate_fx,ACELP_48k)  && LT_16(st->bwidth_fx,SWB)  ) ||               /* Deactivate for core bitrates higher than 48.0 kb/s */
            ( GE_32(st->total_brate_fx,ACELP_13k20)&&GT_16(st->lp_noise_fx,FORMANT_SHARPENING_NOISE_THRESHOLD_FX)   /* Deactivate for bitrates >= 13.2 kb/s if the long-term noise level exceeds 34 dB */
              && GT_32(st->total_brate_fx,CNA_MAX_BRATE) ) )
        {
            *sharpFlag = 0;
            move16();
        }
    ...
    test();
    test();
    IF( EQ_16(st->rf_mode,1)&&(EQ_16(*coder_type,VOICED)||EQ_16(*coder_type,GENERIC)))
    {
        *sharpFlag = 1;
    }
    ...
    *Voicing_flag = 0;
    ...
    test();
    test();
    if ( *sp_aud_decision1 == 0 && ( EQ_16(*coder_type,VOICED)||EQ_16(*coder_type,GENERIC)))
    {
        *Voicing_flag = 1;
        move16();
    }
    ...
    sr_core_tmp = L_max(INT_FS_16k_FX,st->sr_core);     /* indicates the ACELP sampling rate for MODE2, 16 kHz for MODE1 */
    if( EQ_16(st->codec_mode,MODE1))
    {
        sr_core_tmp = INT_FS_16k_FX;
        move32();
    }

    L_frame_tmp = s_max(L_FRAME16k,st->L_frame_fx);
    if( EQ_16(st->codec_mode,MODE1))
    {
    ...
    test();
    IF( EQ_32(st->input_Fs_fx,sr_core_tmp))
    {
    ...
    test();
    test();
    IF( ((st->tcxonly == 0) ||  (EQ_16(st->codec_mode,MODE1)))&&GT_32(st->input_Fs_fx,8000))
    {
        st->mem_preemph_enc = shl(new_inp_16k[sub(L_frame_tmp,1)],1);
    }

    test();
    IF( GT_32(st->input_Fs_fx,8000)&&EQ_32(sr_core_tmp,16000))
    {
    ...
    ELSE IF( GT_32(st->input_Fs_fx,8000))    /* keep memory up-to-date in case of bit-rate switching */
    {
    ...
    test();
    test();
    IF( ( ((st->tcxonly == 0) || !(NE_32(st->core_brate_fx,FRAME_NO_DATA)||NE_32(st->core_brate_fx,SID_2k40)))&&EQ_16(st->L_frame_fx,L_FRAME16k)&&EQ_16(st->codec_mode,MODE2))||
        ( EQ_16(st->L_frame_fx,L_FRAME16k)  && EQ_16(st->codec_mode,MODE1)  ) )
    {
        *Q_new = Q_new_16k;
        move16();
    }
        ...
        ( EQ_16(st->L_frame_fx,L_FRAME16k)  && EQ_16(st->codec_mode,MODE1)  ) )
    {
        ...
        test();
        IF( EQ_16(st->last_L_frame_fx,L_FRAME)&&EQ_16(st->codec_mode,MODE1))
        {
    ...
    ELSE
    {
        ...
        test();
        test();
        IF( EQ_16(st->codec_mode,MODE2)&&st->tcxonly==0&&Q_exp!=0)
        {
            Scale_sig( st->buf_speech_enc_pe, st->encoderPastSamples_enc+st->encoderLookahead_enc, Q_exp );
            Scale_sig( &(st->mem_wsp_enc), 1, Q_exp );
        }
    ...
    test();
    IF ( EQ_16(st->codec_mode,MODE2)&&st->tcxonly==0)
    {
        IF (Q_wsp_exp != 0)
        {
            Scale_sig(st->buf_wspeech_enc, st->L_frame_fx+L_SUBFR, Q_wsp_exp);
        }
        IF( EQ_16(excitation_max_test,1))
        {
    ...
    IF ( EQ_16(st->codec_mode,MODE2))
    {
        test();
        test();
        IF((NE_32(st->core_brate_fx,FRAME_NO_DATA)&&NE_32(st->core_brate_fx,SID_2k40)&&st->tcxonly==0))
        {
        ...
        test();
        IF (EQ_16(st->mdct_sw_enable,MODE2)&&!st->rf_mode)
        {
            IF (EQ_16(st->core_fx,TCX_20_CORE)) /* Switching only possible from TCX_20 frames, not from TCX_10 frames */
            {
                /* Select MDCT Core */
                test();
                test();
                IF ((EQ_16(st->bwidth_fx,SWB)||EQ_16(st->bwidth_fx,FB))&&EQ_32(st->total_brate_fx,24400))
                {
                    st->core_fx = mdct_classifier_fx(fft_buff,st,*vad_flag, enerBuffer
                                                     ,sub(enerBuffer_exp, 31)
                                                    );
                ...
                test();
                IF ((EQ_32(st->total_brate_fx,16400))&&(NE_16(st->bwidth_fx,FB)))
                {
                    MDCT_selector( st, sp_floor, *Etot, cor_map_sum, voicing, enerBuffer, enerBuffer_exp, *vad_flag );
                }
            }
            ...
            ELSE IF( EQ_16(st->last_codec_mode,MODE1)&&EQ_16(st->last_core_fx,HQ_CORE))
            {
    ...
    ELSE IF( GT_32(st->input_Fs_fx,8000))
    {
    ...
    test();
    test();
    IF( (EQ_32(sr_core_tmp,16000))&&st->tcxonly&&EQ_16(st->codec_mode,MODE2))
    {
