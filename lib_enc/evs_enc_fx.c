...
)
{
    ...
    IF (EQ_16(st->mdct_sw,MODE2))
    {
        ...
        test();
        test();
        IF ((GT_32(st->total_brate_fx,ACELP_24k40)&&LT_32(st->total_brate_fx,HQ_96k))||(EQ_32(st->total_brate_fx,ACELP_24k40)&&GE_16(st->bwidth_fx,WB)))
        {
    ...
    IF( EQ_16(st->codec_mode,MODE1))
    {
        ...
        core_switching_pre_enc_fx( st,&(st->LPDmem), old_inp_12k8, old_inp_16k);
    ...
    ELSE   /* MODE2 */
    {
        ...
        enc_acelp_tcx_main( old_inp_16k + L_INP_MEM, st, coder_type, pitch, voicing, Aw, lsp_new, lsp_mid,
                            ...
                            vad_hover_flag, vad_flag, &Q_new, &shift );
        ...
        test();
        test();
        IF( st->igf != 0 && EQ_16(st->core_fx,ACELP_CORE)&&GT_32(st->core_brate_fx,SID_2k40))
        {
    ...
    test();
    IF ( GE_32(st->input_Fs_fx,16000 )&&(LT_16(st->bwidth_fx,SWB)))
    {
    ...
    if( GT_32(st->core_brate_fx,SID_2k40))
    {
    ...
    test();
    IF( GT_32(st->core_brate_fx,SID_2k40)&&EQ_16(st->first_CNG_fx,1))
    {
    ...
    test();
    test();
    if ( LE_32(st->core_brate_fx,SID_2k40)&&st->first_CNG_fx==0&&EQ_16(st->cng_type_fx,LP_CNG))
    {
        st->first_CNG_fx = 1;
        move16();
    }
    ...
    IF (EQ_16(st->rf_mode,1))
    {
        ...
        {
            st->last_nelp_mode_fx = 1;
        }
        ELSE
        {
            st->last_nelp_mode_fx = 0;
        }
    }
...
)
{
    ...
    IF( NE_32(st->core_brate_fx, SID_2k40)&&NE_32(st->core_brate_fx,FRAME_NO_DATA))
    {
        ...
        test();
        test();
        if( !st->tcxonly && !localVAD && EQ_16(st->tcx_cfg.coder_type,GENERIC))
        {
