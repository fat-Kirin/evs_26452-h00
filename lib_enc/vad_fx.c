...
)
{
    ...
    test();
    test();
    if ( ( ( LT_16(lp_snr,(16*256)))
            && ( NE_16(st_fx->input_bwidth_fx, NB)   ))
            ||  ( GT_16(st_fx->prim_act_he_fx, 31130)  ) )  /*.95*Q15*/
    {
    ...
    if (GT_16(st_fx->vad_prim_cnt_16_fx, 12))  /* 12 requires roughly > 80% primary activity */
    {
        hangover_short_dtx = add(hangover_short_dtx,2);
    }

    if (GT_16(st_fx->vad_flag_cnt_50_fx, 40)) /* 40 requires roughtly > 80% flag activity */
    {
    ...
    test();
    test();
    if (   ( NE_16(st_fx->input_bwidth_fx, NB)) /* WB or SWB or FB */
            && ( NE_16(st_fx->core_fx, AMR_WB_CORE)  )
            && ( GT_16(lp_snr, 25*256)  )
       )
    {
        tmp = 2;
    ...
    IF ( tmp != 0 )
    {
        ...
        test();
        test();
        if ( (GT_16(hangover_short_dtx, tmp))
                && ( (LT_16(st_fx->vad_prim_cnt_16_fx, 7)  )
                     || ( (GT_16(lp_snr, (16*256)) )
                          && (LT_16(st_fx->prim_act_he_fx, 27853) ) /*0.85f*2^15 */
                        )
                   )
           )
        {
    ...
    IF ( vad_flag != 0 )  /* Speech present */
    {
        ...
        test();
        if ( ( GE_16(st_fx->nb_active_frames_fx, ACTIVE_FRAMES_FX))
                || (GT_16(st_fx->vad_flag_cnt_50_fx,45))  ) /* 45 requires roughly > 90% flag activity */
        {
        ...
        /* inside HO period */
        test();
        if( ( LT_16(st_fx->hangover_cnt_dtx_fx, HANGOVER_LONG_FX))
                &&  (st_fx->hangover_cnt_dtx_fx != 0) )
        {
        ...
        test();
        test();
        if ( (GT_16(st_fx->prim_act_he_fx,31129))
                && (GT_16(st_fx->Etot_lp_fx,40*256) )
                && (GT_16(st_fx->vad_prim_cnt_16_fx,14) )
                && (GT_16(st_fx->vad_flag_cnt_50_fx,48) )  ) /* 45 requires roughly > 95% flag activity */
        {
        ...
        test();
        if( ( LT_16(st_fx->hangover_cnt_music_fx, HANGOVER_LONG_MUSIC_FX))
                &&  (st_fx->hangover_cnt_music_fx != 0) )
        {
    ...
    ELSE
    {
        ...
        if(LT_16(st_fx->hangover_cnt_dtx_fx,HANGOVER_LONG_FX))    /* inside HO period */
        {
            st_fx->hangover_cnt_dtx_fx = add(st_fx->hangover_cnt_dtx_fx,1);
        }
        if(LT_16(st_fx->hangover_cnt_music_fx,HANGOVER_LONG_MUSIC_FX))    /* inside HO period */
        {
            st_fx->hangover_cnt_music_fx = add(st_fx->hangover_cnt_music_fx,1);
        }
        ...
        IF (  st_fx->hangover_terminate_flag_fx != 0  )
        {
            ...
            if (LT_16(st_fx->Etot_lp_fx,20*256))
            {
        ...
        if( LE_16(st_fx->hangover_cnt_dtx_fx, hangover_short_dtx))  /* "hard" hangover  */
        {
            flag_dtx = 1;
            move16();
        }

        if( LE_16(st_fx->hangover_cnt_music_fx, 15))  /* "hard" hangover music */
        {
            flag_dtx = 1;
            move16();
        }
...
)
{
     ...
     *---------------------------------------------------------------------*/
    IF( EQ_16(st_fx->max_band_fx,19))  /* WB input */ /* or SWB input */
    {
        nk              = 3277;
        move16(); /*0.1 Q15 */
        nc              = 4122 ;
    ...
    *NB_speech_HO    = 0;
    ...
    snr_idx = 2;
    move16();
    if( GT_16(lp_snr,4608))  /*18.0 Q8*/
    {
        snr_idx = 1;
        move16();
    }
    if ( GT_16(lp_snr,6144)) /*24.0 Q8*/
    {
        snr_idx = 0;
        move16();
    }
    ...
    *snr_sum_he      = 0;
    ...
    test();
    IF(  ( EQ_16(st_fx->max_band_fx, 19))
         && LT_32(L_snr_outlier , MAX_SNR_OUTLIER_3_FX) 
         && GT_16(snr_outlier_index, 3)
         && LT_16(snr_outlier_index, MAX_SNR_OUTLIER_IND_FX) )
    {
    ...
    *---------------------------------------------------------------------*/
    ...
    sub(0,0);
    IF ( LT_16(lp_snr, st_fx->sign_dyn_lp_fx))
    {
        lp_snr = add(lp_snr,1<<8);  /*  lp_snr += 1; */

        if (GT_16(lp_snr,  st_fx->sign_dyn_lp_fx))
        {
    ...
    IF (GT_16(lp_snr, (Word16)20*(1<<8)))  /* if (lp_snr > 20.0f )*/
    {
        ...
        test();
        test();
        if( EQ_16(st_fx->max_band_fx,16)
                && GT_16(lp_snr,40*256) 
                && GT_16(thr1,6600) 
                && LT_16(st_fx->lp_speech_fx,11520)  )
        {
            thr1 = 6600;
        }
    }
    ...
    IF( NE_16(vad_bwidth_fx, NB))
    {
        ...
        IF( LT_16(lp_snr,th_clean))
        {
            ...
            test();
            test();
            IF( ( (LE_16(snr_outlier_index, 4))&&(GT_16(st_fx->last_coder_type_fx,UNVOICED))&&(st_fx->Opt_SC_VBR_fx!=0))||
                ( (LE_16(snr_outlier_index, 4)) && (GT_16(st_fx->last_7k2_coder_type_fx, UNVOICED) ) && ( st_fx->Opt_SC_VBR_fx == 0 ) ) )
            ...
            ELSE IF ( ((LE_16(st_fx->last_coder_type_fx, UNVOICED))&&(LT_32(L_snr_outlier,MAX_SNR_OUTLIER_2_FX))&&(st_fx->Opt_SC_VBR_fx!=0))||
                      ((LE_16(st_fx->last_7k2_coder_type_fx, UNVOICED) )  && (LT_32(L_snr_outlier,MAX_SNR_OUTLIER_2_FX))  && ( st_fx->Opt_SC_VBR_fx == 0 ) ) )
        ...
        IF ( GT_32(L_mssnr, L_vad_thr))
        {
            ...
            IF ( GE_16(st_fx->nb_active_frames_he1_fx,ACTIVE_FRAMES_FX))
            {
        ...
        IF ( GT_16(st_fx->voiced_burst_fx, 3))
        {
            IF ( LT_16(st_fx->bcg_flux_fx, 640)) /* Q4 */
            {
        ...
        IF ( LT_16(st_fx->bcg_flux_fx, 640))
        {
        ...
        {
            IF ( GT_32(L_mssnr_hov, L_vad_thr))
            {
                flag_he1 = 1;
                move16();
                st_fx->soft_hangover_fx=sub(st_fx->soft_hangover_fx,1);
        ...
        test();
        IF ( (flag_he1 == 0)
             && (LT_16(st_fx->hangover_cnt_he1_fx, hangover_hd)  )
             && (st_fx->soft_hangover_fx == 0 ) )
        {
            flag_he1 = 1;
            move16();
        ...
        {
            IF ( GT_16(snr_sumt, st_fx->bcg_flux_fx))
            {
                IF ( st_fx->bcg_flux_init_fx-- > 0 )
                {
                    IF ( GT_16(snr_sumt,add(st_fx->bcg_flux_fx,800)))
                    {
                ...
                ELSE
                {
                    IF ( GT_16(snr_sumt,add(st_fx->bcg_flux_fx,160)))
                    {
            ...
            ELSE
            {
                IF ( st_fx->bcg_flux_init_fx-- > 0 )
                {
                    IF ( LT_16(snr_sumt,sub(st_fx->bcg_flux_fx,480)))
                    {
                ...
                ELSE
                {
                    IF ( LT_16(snr_sumt,sub(st_fx->bcg_flux_fx,160)))
                    {
        ...
        *localVAD = 0;
        ...
        IF ( (GT_16(snr_sum, thr1))&&(EQ_16(flag_he1,1))) /* Speech present */
        {
            flag      = 1;
            move16();
            *localVAD = 1;
            ...
            /* inside HO period */
            test();
            if( LT_16(st_fx->hangover_cnt_fx,HANGOVER_LONG_FX)
                    && st_fx->hangover_cnt_fx != 0 )
            {
        ...
        ELSE
        {
            ...
            if( LT_16(st_fx->hangover_cnt_fx,HANGOVER_LONG_FX))    /* inside HO period */
            {
                st_fx->hangover_cnt_fx = add(st_fx->hangover_cnt_fx,1);
            }


            IF( LE_16(st_fx->hangover_cnt_fx, hangover_short))  /* "hard" hangover  */
            {
                test();
                test();
                if ( (LT_16(lp_snr,th_clean))
                && (st_fx->Opt_SC_VBR_fx != 0 )
                && (GE_16(st_fx->hangover_cnt_fx, 2)) )
                {
                    *noisy_speech_HO = 1;
                    move16();
                }
        ...
        *localVAD_HE_SAD = 0;
        ...
        test();
        IF ( (GT_16(snr_sum_HE_SAD, thr1))
             && (EQ_16(flag_he1, 1) ) ) /* Speech present */
        {
    ...
    ELSE                                          /* NB input */
    {
        ...
        *localVAD = 0;
        ...
        IF ( GT_16(snr_sum,thr1))                     /* Speech present,  possibly in hangover */
        {
            st_fx->nb_active_frames_fx = add(st_fx->nb_active_frames_fx,1);                /* Counter of consecutive active speech frames */
            IF ( GE_16(st_fx->nb_active_frames_fx,ACTIVE_FRAMES_FX))
            {
        ...
        IF( LT_16(st_fx->hangover_cnt_fx,HANGOVER_LONG_NB_FX))
        {
            st_fx->hangover_cnt_fx = add(st_fx->hangover_cnt_fx,1);

            IF( LT_16(lp_snr, 4864 ))   /*19.0f Q8*/         /* very low SNR */
            {
                thr1_nb_mod  = sub(thr1_nb_mod , 1331);        /*thr1 -= 5.2f;*/
            }
            ELSE IF( LT_16(lp_snr, 8960))    /*35 in Q8 */  /* low SNR */
            {
        ...
        test();
        IF ( (LT_16(snr_sum, thr1_nb_mod))
             && (GT_16(snr_sum, thr2)) )           /* Speech present */
        {
            flag          = 1;
            move16();
            *localVAD     = 0;
    ...
    *flag_noisy_speech_snr = 0;
    ...
    IF( vad_bwidth_fx != NB )
    {
        if(LT_16(lp_snr, TH16_2_NFLAG_FX ))           /*now 27, original threshold: 35dB*/
        {
    ...
    move16();
    if (LE_16(st_fx->prim_act_quick_fx,st_fx->prim_act_slow_fx))
    {
    ...
    move16();
    if (LE_16(st_fx->prim_act_quick_he_fx,st_fx->prim_act_slow_he_fx))
    {
