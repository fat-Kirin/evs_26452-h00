...
)
{
    ...
    IF( !Opt_AMR_WB )
    {
        ...
        IF( EQ_16(coder_type,AUDIO))
        {
            ...
            test();
            test();
            if ( EQ_16(L_subfr,L_frame/2)&&i_subfr!=0&&EQ_16(L_frame,L_FRAME))
            {
                pit_flag = L_SUBFR;
                move16();
            }
...
)
{
    ...
    ELSE  /* double-extended Q range */
    {
        IF( EQ_16(fr_steps,2))
        {
            ...
            IF( LT_16(T0,PIT_FR1_DOUBLEEXTEND_8b))
            {
...
)
{
    ...
    ELSE IF(EQ_16(coder_type,9)) /* 8/0/8/0 (RF mode Gen pred) */
    {
        IF (i_subfr == 0)
        {
            limit_T0_voiced( 4, shr(pit_res_max,1), T_op[0], 0, 1, T0_min, T0_min_frac, T0_max, T0_max_frac, pit_min, pit_max );
