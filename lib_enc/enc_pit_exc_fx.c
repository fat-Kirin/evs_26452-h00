    ...
    *saved_bit_pos = st_fx->next_bit_pos_fx;
    ...
    FOR ( i_subfr = 0; i_subfr < L_FRAME; i_subfr += L_subfr )
    {
        ...
        *pt_pitch = pit_encode_fx( st_fx, Pitch_BR, 0, L_FRAME, Pitch_CT, &pitch_limit_flag, i_subfr, exc,
        ...
        clip_gain = gp_clip_fx(st_fx->core_brate_fx, voicing, i_subfr, AUDIO, xn, st_fx->clip_var_fx,sub(shift_wsp, 1));
        ...
        move16(); /*Q14*/
        test();
        IF( st_fx->GSC_noisy_speech_fx == 0 || NE_16(L_subfr,L_SUBFR ))
        {
        ...
        gp_clip_test_gain_pit_fx( st_fx->core_brate_fx, gain_pit, st_fx->clip_var_fx );
