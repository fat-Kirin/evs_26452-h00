...
)
{
    ...
    FOR (k = 0; k < 32; k++)
    {
        ...
        IF (GT_32(magSq[k + 20], peak_l))
        {
            peak_l = L_add(magSq[k + 20], 0);
        }
        IF (GT_32(magSq[k + 96], peak_h))
        {
    ...
    IF(GT_32(peak_h, L_shl(L_tmp1, 1)))
    {
        exp3 = norm_l(peak_h);
    }
    ELSE
    {
        exp3 = sub(norm_l(L_tmp1), 1);
    }
    ...
    test();
    test();
    IF ((GT_32(gain4, Mult_32_16(gain11, 26214))&&GT_32(Mult_32_32(L_shl(peak_l,exp),L_shl(avrg_h,exp1)),Mult_32_32(L_shl(Mult_32_16(peak_h,12800),exp),L_shl(avrg_l,exp1)))
         && LT_32(Mult_32_32(L_shl(Mult_32_16(peak_l, 6400), exp2), L_shl(avrg_h, exp1)), Mult_32_32(L_shl(peak_h, exp2), L_shl(avrg_l, exp1))))
        || (GT_32(gain4, Mult_32_16(gain11, 9830))&& LT_32(Mult_32_16(peak_h, 21845), avrg_h)&& LT_32(Mult_32_16(peak_H2, 21845), avrg_H2))
        || (LT_32(Mult_32_32(L_shl(peak_l, exp), L_shl(avrg_h, exp1)), Mult_32_32(L_shl(Mult_32_16(peak_h, 12800), exp), L_shl(avrg_l, exp1))) && GT_32(Mult_32_16(peak_h, 21845), avrg_h))
        || (GT_32(Mult_32_32(L_shl(Mult_32_16(peak_l, 12800), exp3), L_shl(avrg_h, exp1)), Mult_32_32(L_shl(peak_h, exp3), L_shl(avrg_l, exp1))) > 0 && LT_32(Mult_32_16(peak_h, 21845), avrg_h)))
    {
        condition4 = 1;
        move16();
    }
    ...
    test();
    test();
    IF ((EQ_32(st_fx->total_brate_fx, HQ_32k)&&(s_xor(condition1,condition2)!=0||condition3))
        || (EQ_32(st_fx->total_brate_fx, HQ_24k40)&& condition4))
    {
    ...
    ELSE
    {
        st_fx->last_gain1 = L_shr(st_fx->last_gain1, sub(enerBuffer_exp, st_fx->last_enerBuffer_exp));
        move32();
        st_fx->last_gain2 = L_shr(st_fx->last_gain2, sub(enerBuffer_exp, st_fx->last_enerBuffer_exp));
        move32();
        gain1_tmp = gain1;
        move32();
        gain2_tmp = gain2;
    ...
    test();
    test();
    IF ((EQ_16(st_fx->clas_final_old_fx, HQ_CORE)||EQ_16(st_fx->clas_final_old_fx,TCX_20_CORE))
        && ((GT_32(st_fx->last_gain1, L_shr(gain1_tmp, 1)) && LT_32(st_fx->last_gain1, L_shl(gain1_tmp, 1)))
            && (GT_32(st_fx->last_gain2, L_shr(gain2_tmp, 1)) && LT_32(st_fx->last_gain2, L_shl(gain2_tmp, 1)))))
    {
        clas_final = st_fx->clas_final_old_fx;
        move16();
    }
    ELSE IF (GT_16(clas_sec, st_fx->clas_sec_old_fx)&&GT_16(clas_sec,MDCT_CLASSIFER_THRESH_UP)) /* Going up? */
    {
    ...
    if ( EQ_16(st_fx->mdct_sw_enable, MODE2)&&(EQ_16(st_fx->flag_noisy_speech_snr,1)||vadflag==0)&&EQ_16(clas_final,HQ_CORE))
    {
        clas_final = TCX_20_CORE;
        move16();
    }
    ...
    move32();
    st_fx->last_enerBuffer_exp = enerBuffer_exp;
    move16();
