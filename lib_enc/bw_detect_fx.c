...
)
{
    ...
    Word32 spect32[BWD_TOTAL_WIDTH],in_win32[BWD_TOTAL_WIDTH];
    Word16 e_tmp, f_tmp;
    Word32 L_tmp, sum32;
    ...
    */
    ...
    IF( GT_32(st->input_Fs_fx,8000))
    {
        ...
        ELSE
        {
            ...
            edct_fx(in_win32, spect32, BWD_TOTAL_WIDTH, &Q_dct);
        ...
        test();
        IF( localVAD  || GT_16(st->lp_noise_fx,7680))
        {
            ...
            test();
            IF( EQ_16(st->last_input_bwidth_fx,WB)&&GT_32(st->input_Fs_fx,16000))
            {
                IF( GT_16(st->count_SWB_fx,BWD_COUNT_WIDER_BW))
                {
                    st->input_bwidth_fx = SWB;
            ...
            test();
            IF( EQ_16(st->last_input_bwidth_fx,SWB)&&GT_32(st->input_Fs_fx,32000))
            {
                IF( GT_16(st->count_FB_fx,BWD_COUNT_WIDER_BW))
                {
                    st->input_bwidth_fx = FB;
    ...
    IF ( EQ_16(st->codec_mode,MODE1))
    {
        ...
        test();
        test();
        IF ( LE_32(total_brate_fx, ACELP_9k60)&&NE_16(st->bwidth_fx,NB)&&NE_16(st->bwidth_fx,WB))
        {
            st->bwidth_fx = WB;
            move16();
        }
