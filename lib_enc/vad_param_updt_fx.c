...
)
{
    ...
    IF( !st_fx->Opt_AMR_WB_fx )
    {
        ...
        move16();
        test();
        if( (NE_32(st_fx->core_brate_fx, (Word32)SID_2k40))&&(st_fx->core_brate_fx!=0)) /* Note,  core_brate_fx can be -1 */
        {
            tmp_active_flag = 1;
        ...
        test();
        test();
        IF( (st_fx->Opt_DTX_ON_fx != 0)  && (tmp_active_flag == 0) && ( GT_16(st_fx->ini_frame_fx,3)))


        {
            /* update the counter of consecutive inactive frames in DTX */
            st_fx->consec_inactive_fx = add(st_fx->consec_inactive_fx,1);
            IF( GT_16(st_fx->consec_inactive_fx,5))
            {
                st_fx->consec_inactive_fx = 5;
                move16();
            }

            IF( EQ_16(st_fx->consec_inactive_fx,5))
            {
                ...
                IF( EQ_16(st_fx->spectral_tilt_reset_fx,1))
                {
                ...
                IF( GT_16(abs_s(st_fx->ra_deltasum_fx), 6553)) /*0.2 in Q15*/
                {
    ...
    pitch_tmp = abs_s( sub( pitch[0], old_pitch1 ) ); /*Q0 */
    ...
    move16();
    test();
    if ( ( LE_16(voice_tmp,21299))    /* 0.65 in Q15 */
            ||  ( GE_16(pitch_tmp,42)  ) )  /*3*14 = 42   Q0 */
    {
