                                 ...
                                 Word16 lowpassLine)
{
    ...
    test();
    IF (powerSpec != NULL && LT_16(add(startLine, 6), L_frame))
    {
        lastTone = 0;
        move16();
...
void detectLowpassFac(const Word32 *powerSpec, Word16 powerSpec_e, Word16 L_frame, Word8 rectWin, Word16 *pLpFac, Word16 lowpassLine)
{
    ...
    {
        IF (GT_32(powerSpec[i], threshold))
        {
            BREAK;
        }
    }
...
)
{
    ...
    *powerSpec_e = add(shl(sub(mdctSpectrum_e, s1), 1), 1);
    ...
    if ( L_msu0(L_mult0(st->L_frame_fx, extract_l(st->last_sr_core)), st->L_frame_past, extract_l(st->sr_core)) != 0
            || NE_16(st->last_core_fx, TCX_20_CORE )  )
    {
        tmp8 = 1;
        move16();
    }
                     ...
                     )
{
    ...
    IF (tcx_lpc_shaped_ari == 0)
    {
        ...
        FOR (i = 0; i < lg_4; i++)
        {
            ...
            test();
            IF ((GE_16(abs_s(xq[i]), 2))&&(tmp32>=0x3A000))   /* 0x3A000 -> 3.625 (15Q16) */
            {
...
)
{
    ...
    i = sub(L_frame, 1);
    WHILE ((memQuantZeros[i] != 0) && (LT_32(L_abs(L_shl(x[i], tmp16)), tmp32)))
    {
        test();
        xq[i] = 0;
        move16();
...
)
{
                   ...
                   );
    {
        ...
        *flatteningTrigger = 0;
        test();
        IF (LT_32(tns_predictionGain, 9646899l/*1.15 Q23*/)&&
            LT_16(predictionGain, 147/*1.15 Q7*/))
        {
            *flatteningTrigger = 1;
        }
    }
