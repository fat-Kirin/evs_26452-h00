...
static Word32 dot(const Word16 *X, const Word16 *Y, Word16 n)
{
    ...
    FOR (i = 0; i < n; i++)
    {
        acc_64 = W_mac0_16_16(acc_64, X[i], Y[i]);
    }
    acc = W_sat_l( acc_64 );
...
)
{
    ...
    win = E_ROM_inter4_1;
    if (EQ_16(frac_max, 6))win=E_ROM_inter6_1;
                   ...
                   )
{
    ...
    IF (tcxltp_on != 0 || SideInfoOnly != 0)
    {
        ...
        if ( (
                    tcxOnly == 0 &&
                    EQ_16(tcxMode, TCX_20) &&
                    GT_16(mult(norm_corr, *norm_corr_past), 0x2000)  &&
                    LT_16(tempFlatness, 448/*3.5f Q7*/) 
                ) ||
                (
                    tcxOnly != 0 &&
                    EQ_16(tcxMode, TCX_10)  &&
                    GT_16(s_max(norm_corr, *norm_corr_past), 0x4000)  &&
                    LT_16(maxEnergyChange, 448/*3.5f Q7*/) 
                ) ||
                ( /* Use LTP for lower correlation when pitch lag is big, L_frame*(1.2f-norm_corr) < pitch_int <=> norm_corr > 1.2f-pitch_int/L_frame */
                    tcxOnly != 0 &&
                    GT_16(norm_corr, 14418/*0.44f Q15*/)  &&
                ...
                (
                    ...
                    GT_16(norm_corr, 14418/*0.44f Q15*/)  &&
                    (
                        LT_16(tempFlatness, 768/*6.0f Q7*/)  ||
                        (
                            LT_16(tempFlatness, 896/*7.0f Q7*/)  &&
                            LT_16(maxEnergyChange, 2816/*22.0f Q7*/) 
                        )
                    )
                )
           )
