...
)
{
    ...
    IF( EQ_32(st_fx->input_Fs_fx,8000))
    {
        st_fx->min_band_fx = 1;
        move16();
    ...
    move16();
    set16_fx(st_fx->past_dec_fx, 0, HANG_LEN-1 );
    set16_fx(st_fx->past_dlp_fx, 0, HANG_LEN-1 );
    ...
    move16();
    set32_fx( st_fx->past_PS_fx, 0, HIGHEST_FBIN-LOWEST_FBIN );
    ...
    noise_est_init_fx( &st_fx->totalNoise_fx, &st_fx->first_noise_updt_fx, st_fx->bckr_fx, st_fx->enrO_fx,
                       ...
                       ,&st_fx->Etot_st_est_fx,&st_fx->Etot_sq_st_est_fx
                     );
    ...
    st_fx->old_input_fhb_fx_Q = 0;
    move16();
    st_fx->cldfbHBLT = 8192/*1.0f Q13*/;
    ...
    set16_fx( st_fx->old_out_fx, 0, L_FRAME32k );
    ...
    st_fx->lt_voicing = 16384/*0.5f Q15*/;
    st_fx->lt_corr = 16384/*0.5f Q15*/;
    st_fx->lt_tonality = 0;
    move32();
    ...
    set16_fx(st_fx->last_bitalloc_max_band_fx, 0, 2);
    ...
    test();
    test();
    IF( st_fx->Opt_RF_ON == 0 || (NE_16(st_fx->bwidth_fx,WB)&&NE_16(st_fx->bwidth_fx,SWB))||NE_32(st_fx->total_brate_fx,ACELP_13k20))
    {
    ...
    L_tmp = st_fx->total_brate_fx;
    move32();
    test();
    if( st_fx->rf_mode && EQ_32(st_fx->total_brate_fx,ACELP_13k20))
    {
        L_tmp = ACELP_9k60;
        move32();
    }
    configureFdCngEnc( st_fx->hFdCngEnc_fx, st_fx->bwidth_fx, L_tmp );
