...
)
{
    ...
    test();
    test();
    IF ( (EQ_32(st_fx->last_total_brate_fx, ACELP_6k60))||
         (EQ_32(st_fx->last_total_brate_fx, ACELP_8k85)) ||
         (EQ_32(st_fx->last_total_brate_fx, ACELP_12k65)) ||
         (EQ_32(st_fx->last_total_brate_fx, ACELP_14k25)) ||
         (EQ_32(st_fx->last_total_brate_fx, ACELP_15k85)) ||
         (GE_32(st_fx->last_total_brate_fx, ACELP_18k25) && LE_32(st_fx->last_total_brate_fx, ACELP_23k85)) )
    {
        fSwitchFromIO = 1;
        move16();
    }
    ...
    test();
    test();
    IF ( EQ_16(st_fx->extl_fx, WB_BWE)||EQ_16(st_fx->extl_fx,WB_TBE)||st_fx->igf!=0)
    {
        ramp_flag = 0;
        ...
        test();
        test();
        IF( (NE_16(st_fx->last_extl_fx, WB_TBE)&&NE_16(st_fx->last_extl_fx,WB_BWE)&&st_fx->igf==0)||
    ...
    test();
    test();
    IF ( (NE_16(st_fx->extl_fx, WB_BWE)||(EQ_16(st_fx->extl_fx,WB_BWE)&&LE_32(st_fx->total_brate_fx,ACELP_8k00)))&&!st_fx->ppp_mode_fx)
    {
...
)
{
    ...
    IF( EQ_32(st_fx->input_Fs_fx, 32000))
    {
        Copy(input_fx, new_swb_speech_fx, L_FRAME32k); /*Q0 */
        test();
        test();
        IF( NE_16(st_fx->last_extl_fx, SWB_BWE)&&NE_16(st_fx->last_extl_fx,FB_BWE)&&NE_16(st_fx->extl_fx,SWB_BWE_HIGHRATE))
        {
        ...
        test();
        IF( NE_16(st_fx->extl_fx, SWB_BWE)&&NE_16(st_fx->extl_fx,FB_BWE))
        {
    ...
    ELSE /* 48 kHz */
    {
        ...
        IF( EQ_16(st_fx->codec_mode,MODE1))
        {
            test();
            test();
            IF( NE_16(st_fx->extl_fx,SWB_BWE)&&NE_16(st_fx->extl_fx,FB_BWE)&&EQ_16(st_fx->core_fx,ACELP_CORE))
            {
            ...
            ELSE
            {
                ...
                IF( NE_16(st_fx->last_extl_fx,SWB_BWE)&&NE_16(st_fx->last_extl_fx,FB_BWE))
                {
                    ...
                    IF( EQ_16(st_fx->last_bwidth_fx,FB))
                    {
    ...
    test();
    test();
    IF( ( EQ_16(st_fx->core_fx, ACELP_CORE)&&NE_16(st_fx->extl_fx,SWB_BWE_HIGHRATE)&&NE_16(st_fx->extl_fx,FB_BWE_HIGHRATE))
        || ( ( EQ_32(st_fx->total_brate_fx, 9600)  || st_fx->rf_mode != 0 ) && EQ_16(st_fx->bwidth_fx, SWB)  ) )
    {
        ...
        /* Highband energy computation for gain shape control in case of bandwidth switching fix*/
        {
            ...
            st_fx->cldfbHBLT = mac_r(CldfbHB_ener, 29491/*0.9 Q15*/, st_fx->cldfbHBLT); /* cldfbHBLT is in Q13 */
        }
        ...
        *Q_shb_spch = 0; /*shb_speech_fx : Q0*/
        ...
        test();
        test();
        IF( NE_16(st_fx->extl_fx, WB_TBE)&&NE_16(st_fx->extl_fx,SWB_TBE)&&NE_16(st_fx->extl_fx,FB_TBE))
        {
    ...
    ELSE
    {
        IF( EQ_16(st_fx->bwidth_fx, FB)||EQ_16(st_fx->core_fx,ACELP_CORE))
        {
