...
)
{
    ...
    st_lp_sp_enr = st_fx->lp_sp_enr_fx;
    ...
    ELSE
    {
        ...
        test();
        test();
        IF( GT_32(st_fx->last_core_brate_fx, SID_2k40) && ( EQ_16( st_fx->last_core_fx, HQ_CORE ) || burst_ho_cnt > 0 ) && LT_16(st_fx->lp_sp_enr_fx, 1536) &&
        GT_16(sub(sp_enr, st_fx->lp_sp_enr_fx), 1024) && GT_16(sp_enr, 1536) )
        {
            st_fx->lp_sp_enr_fx = sp_enr;
            move16();
            force_cn_step = 1;
    ...
    if(EQ_16(st_fx->cng_hist_ptr_fx, DTX_HIST_SIZE))
    {
    ...
    *-----------------------------------------------------------------*/
    test();
    test();
    IF( (EQ_32(st_fx->core_brate_fx, SID_2k40)||EQ_32(st_fx->core_brate_fx,SID_1k75))&&GE_16(st_fx->cng_cnt_fx,sub(st_fx->cng_hist_size_fx,1)))
    {
        set32_fx( max, 0, 2 );
    ...
    *allow_cn_step = 0;
    ...
    IF( EQ_32(st_fx->core_brate_fx, SID_2k40)||EQ_32(st_fx->core_brate_fx,SID_1k75))
    {
        ...
        {
            isf_enc_amr_wb_fx( st_fx, lsf_new, lsp_new, 0, 0 );
        }
        ELSE
        {
        ...
        test();
        test();
        if ( EQ_16(st_fx->bwidth_fx,WB)&&st_fx->first_CNG_fx!=0&&NE_16(st_fx->L_frame_fx,st_fx->last_CNG_L_frame_fx))
        {
    ...
    *---------------------------------------------------------------------*/
    ...
    IF( LE_32(st_fx->last_core_brate_fx, SID_2k40))
    {
        ...
        test();
        IF( LT_16(st_fx->num_ho_fx,3)||LT_32(Mult_32_16(st_fx->Enew_fx,21845 /*1/1.5f, Q15*/), st_fx->lp_ener_fx))
        {
            FOR( i=0; i<M; i++ )
            {
                /* AR low-pass filter  */
    ...
    ELSE
    {
        ...
        test();
        test();
        IF( ( st_fx->Opt_AMR_WB_fx || EQ_16(st_fx->bwidth_fx,WB))
        && ( !st_fx->first_CNG_fx || GE_16(st_fx->act_cnt2_fx,MIN_ACT_CNG_UPD)  ) )
        {
            IF( GT_32(st_fx->last_active_brate_fx,ACELP_16k40))
            {
                st_fx->CNG_mode_fx = -1;
                move16();
            }
            ELSE IF( GT_32(st_fx->last_active_brate_fx,ACELP_13k20))
            {
                st_fx->CNG_mode_fx = 4;
                move16();
            }
            ELSE IF( GT_32(st_fx->last_active_brate_fx,ACELP_9k60))
            {
                st_fx->CNG_mode_fx = 3;
                move16();
            }
            ELSE IF( GT_32(st_fx->last_active_brate_fx,ACELP_8k00))
            {
                st_fx->CNG_mode_fx = 2;
                move16();
            }
            ELSE IF( GT_32(st_fx->last_active_brate_fx,ACELP_7k20))
            {
                st_fx->CNG_mode_fx = 1;
                move16();
            }
        ...
        FOR( ll = burst_ho_cnt; ll > 0; ll-- )
        {
            ...
            if( EQ_16(st_fx->ho_hist_ptr_fx, HO_HIST_SIZE))
            {
            ...
            test();
            test();
            IF( EQ_16(L_frame,L_FRAME )&&EQ_16(st_fx->ho_16k_lsp_fx[s_ptr],1))
            {
                /* Conversion from 16k LPSs to 12k8 */
                lsp_convert_poly_fx( &(st_fx->ho_lsp_circ_fx[s_ptr*M]), L_frame, 0 );
            }
            ELSE IF ( EQ_16(L_frame,L_FRAME16k)&&st_fx->ho_16k_lsp_fx[s_ptr]==0)
            {
            ...
            st_fx->ho_sid_bw_fx = L_shl(L_and(st_fx->ho_sid_bw_fx, (Word32) 0x3fffffffL ), 1);
            ...
            if (GT_16(st_fx->ho_hist_size_fx, HO_HIST_SIZE))
            {
            ...
            if( EQ_16(s_ptr, st_fx->ho_circ_size_fx))
            {
                s_ptr = 0;
                move16();
            }
        ...
        IF ( *allow_cn_step == 0 && st_fx->ho_hist_size_fx > 0 )
        {
            ...
            m1 = 0;
            move16();
            IF( L_and(st_fx->ho_sid_bw_fx, (Word32) 0x1) == 0 )
            {
            ...
            FOR( k=1; k<st_fx->ho_hist_size_fx; k++ )
            {
                ...
                test();
                IF ( LT_32(Mult_32_16(st_fx->ho_ener_hist_fx[ptr],ONE_OVER_BUF_H_NRG_FX),st_fx->ho_ener_hist_fx[st_fx->ho_hist_ptr_fx])&&
                     GT_32(st_fx->ho_ener_hist_fx[ptr],Mult_32_16(st_fx->ho_ener_hist_fx[st_fx->ho_hist_ptr_fx], BUF_L_NRG_FX)) )
                {
                    ...
                    IF( L_and(st_fx->ho_sid_bw_fx, L_shl((Word32)0x1,k)) == 0 )
                    {
                        Copy32( &st_fx->ho_env_hist_fx[ptr*NUM_ENV_CNG], &tmp_env[m1*NUM_ENV_CNG], NUM_ENV_CNG );
                        m1 = add(m1,1);
                    }
                    m = add(m,1);
                }
    ...
    *-----------------------------------------------------------------*/
    ...
    IF( NE_16(st_fx->bwidth_fx,NB))
    {
        test();
        IF( EQ_16(st_fx->bwidth_fx,WB)&&st_fx->CNG_mode_fx>=0)
        {
    ...
    *-----------------------------------------------------------------*/
    ...
    IF( EQ_32(st_fx->core_brate_fx, SID_2k40)||EQ_32(st_fx->core_brate_fx,SID_1k75))
    {
        IF( GE_16(st_fx->cng_cnt_fx,sub(st_fx->cng_hist_size_fx,1)))
        {
        ...
        test();
        IF( st_fx->old_enr_index_fx >= 0 && GT_16(enr_index, add(st_fx->old_enr_index_fx, MAX_DELTA)))
        {
            IF( *allow_cn_step != 0 )
            {
        ...
        test();
        IF ( *allow_cn_step == 0 && LT_32(Mult_32_16(st_fx->Enew_fx,21845 /*1/1.5f, Q15*/), st_fx->lp_ener_fx))
        {
            /* update the pointer to circular buffer of old LSP vectors */
            st_fx->ho_hist_ptr_fx = add(st_fx->ho_hist_ptr_fx, 1);
            if( EQ_16(st_fx->ho_hist_ptr_fx,HO_HIST_SIZE))
    ...
    IF ( EQ_32(st_fx->core_brate_fx, SID_2k40))
    {
        ...
        IF ( LT_32(st_fx->input_Fs_fx, 32000))
        {
...
)
{
    ...
    IF ( EQ_16(update_fx, 1))
    {
        ...
        st_fx->ho_sid_bw_fx = L_shl(L_and(st_fx->ho_sid_bw_fx, (Word32) 0x3fffffffL ), 1);
        st_fx->ho_sid_bw_fx = L_or(st_fx->ho_sid_bw_fx, 0x1L);
    }
    ELSE
    {
        IF ( EQ_32(st_fx->core_brate_fx, SID_2k40))
...
)
{
    ...
    if ( GT_16(abs_s(sub(log_wb_ener_fx, st_fx->mov_wb_cng_ener_fx)), 3072))
    {
        allow_cn_step_fx = 1;
        move16();
    }
    ...
    IF ( EQ_32(st_fx->core_brate_fx, SID_2k40))
    {
        test();
        test();
        IF ( st_fx->first_CNG_fx == 0 || EQ_16(st_fx->last_vad_fx, 1)||GE_16(st_fx->shb_NO_DATA_cnt_fx,100))
        {
            update_fx = 1;
            move16();
        }
        ELSE
        {
            ...
            ELSE
            {
                IF ( GT_16(abs_s(sub(sub(st_fx->mov_wb_cng_ener_fx, st_fx->mov_shb_cng_ener_fx), sub(st_fx->last_wb_cng_ener_fx, st_fx->last_shb_cng_ener_fx))), 768))
                {
                    update_fx = 1;
                    move16();
                }
                ...
                {
                    test();
                    IF ( GE_16(st_fx->bwidth_fx, SWB)&&LT_16(st_fx->last_SID_bwidth_fx,SWB))
                    {
                        update_fx = 1;
                        move16();
                    }
                    ...
                    {
                        test();
                        IF ( LT_16(st_fx->bwidth_fx, SWB)&&GE_16(st_fx->last_SID_bwidth_fx,SWB))
                        {
                            update_fx = 1;
                            move16();
                        }
