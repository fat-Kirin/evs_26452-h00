...
static Word16 msvq_enc_find_p_max_8(Word32 dist[])
{
    ...
    if (GT_32(dist[1], dist[p_max]))
    {
        p_max = 1;
        move16();
    }
    if (GT_32(dist[2], dist[p_max]))
    {
        p_max = 2;
        move16();
    }
    if (GT_32(dist[3], dist[p_max]))
    {
        p_max = 3;
        move16();
    }
    if (GT_32(dist[4], dist[p_max]))
    {
        p_max = 4;
        move16();
    }
    if (GT_32(dist[5], dist[p_max]))
    {
        p_max = 5;
        move16();
    }
    if (GT_32(dist[6], dist[p_max]))
    {
        p_max = 6;
        move16();
    }
    if (GT_32(dist[7], dist[p_max]))
    {
        p_max = 7;
        move16();
    }
...
)
{
    ...
    Unified_weighting_fx( Bin_Ener, Q_ener, lsf, weights, narrowband,(Word16)EQ_16(coder_type,UNVOICED),12800,M);
