...
void noise_est_init_fx(
    ...
    ,Word16 *Etot_st_est,           /* Q8 */
    Word16 *Etot_sq_st_est         /* Q2 */
...
)
{
    ...
    *Etot_st_est=5120;         /* 20.0f in Q8 */
    *Etot_sq_st_est=1600;      /* 400 in Q2 */
...
)
{
    ...
    ELSE
    {
        ...
        *Etot_l = add(*Etot_l, 20);
        ...
        IF ( GT_16(harm_cor_cnt,HE_LT_CNT_PRE_FX))
        {
            test();
            IF( ( LT_16( ini_frame_fx , s_min(HE_LT_CNT_INIT_FX ,MAX_FRAME_COUNTER-1) ))
            &&   (LT_16(sub(*Etot_h, *Etot_lp),(Word16)3*256)   ) /* 3.0 Q8 */
              )
            {
            ...
            test();
            IF ( ( GT_16(sub(*Etot_last, *Etot_l), HE_LT_THR2_FX ))  /* 30.0f*Q8 */
                 && (GT_16(harm_cor_cnt,HE_LT_CNT_PRE_SHORT_FX))
               )
            {
            ...
            ELSE IF (GT_16(sub(*Etot_last, *Etot_l), HE_LT_THR1_FX )) /* 10.0 in Q8*/
            {
                *Etot_l = add(*Etot_l, 20);
        ...
        *Etot_l = s_min(*Etot_l, Etot);
        ...
        IF ( LT_16(ini_frame_fx,100)&&LT_16(*Etot_l,*Etot_l_lp))
        {
            /**Etot_l_lp = 0.1f * *Etot_l + (1.0f - 0.1) * *Etot_l_lp; */
            *Etot_l_lp = mac_r(L_mult(3277, *Etot_l), 29491, *Etot_l_lp);
            move16();
        }
        ELSE
        {
            ...
            test();
            test();
            IF ( ( (GT_16(harm_cor_cnt, HE_LT_CNT_FX))
            &&  (GT_16(sub(*Etot_last, *Etot_l), HE_LT_THR2_FX )  )
                 )
            ||  ( (sub(harm_cor_cnt, HE_LT_CNT_FX) > 0 )  &&  (LT_16(ini_frame_fx, HE_LT_CNT_INIT_FX)  ) )
            || (GT_16(sub(*Etot_l_lp, *Etot_l), HE_LT_THR2_FX )  )
               )
            {
...
)
{
    ...
    *loc_harm = multi_harm_fx( EspecdB, st_fx->old_S_fx, st_fx->cor_map_fx, &st_fx->multi_harm_limit_fx, st_fx->total_brate_fx,
    ...
    spec_div = 0;
    move16();
    if (GT_32(Mult_32_16(Lsum_num, 6554), Lsum_den)) /* Qx+Q15+1-16  ==> Qx */
    {
        spec_div = 1;
        move16();
    }
    ...
    *Q_sp_div = add(15,sub(ExpNum ,ExpDen));
    ...
    noise_char = 0;
    move16();
    if (GT_16(st_fx->noise_char_fx, nchar_thr))
    {
        noise_char = 1;
        move16();
    }
    ...
    *non_staX = 0;
    ...
    FOR( i = st_fx->min_band_fx; i <= st_fx->max_band_fx; i++ )
    {
        ...
        test();
        IF ( GE_16(i,START_BAND_SPMUS)&&LT_16(i,NB_BANDS_SPMUS+START_BAND_SPMUS))
        {
            ...
            *non_staX                 = add(*non_staX, wtmp);
            ...
            IF ( GE_16(st_fx->ini_frame_fx, 100 ))
            {
    ...
    if ( st_fx->ini_frame_fx < 150 )
    {
        /* Allow use of quicker filter during init - if needed */
        /* st->Etot_st_est = 0.25f * lim_Etot + (1.0f-0.25F) * st->Etot_st_est; */
        st_fx->Etot_st_est_fx = mac_r(L_mult(8192, lim_Etot_fx), 24576, st_fx->Etot_st_est_fx);
        move16();
        /* st->Etot_sq_st_est = 0.25f * lim_Etot * lim_Etot + (1.0f-0.25f) * st->Etot_sq_st_est; */
        st_fx->Etot_sq_st_est_fx = mac_r(L_mult(8192, lim_Etot_sq_fx), 24576, st_fx->Etot_sq_st_est_fx);
        move16();
    }
    else
    {
    ...
    st_E_var_est_fx = sub(st_fx->Etot_sq_st_est_fx,extract_h(L_shl_r(L_mult(st_fx->Etot_st_est_fx,st_fx->Etot_st_est_fx),1)));
    ...
    *st_harm_cor_cnt = add(*st_harm_cor_cnt , 1);
    ...
    IF( (GT_16(*st_harm_cor_cnt,1))&&((LT_16(Etot,3840))||/* 15 in Q8 */
                                           ( GT_16(st_fx->ini_frame_fx,10)   &&
                                                   GT_16(sub(Etot,st_fx->Etot_lp_fx),1792) )) /* 7 in Q8 */
      )
    {
        *st_harm_cor_cnt = 1;
    }
    ...
    {
        test();
        if( EQ_16(*bg_cnt,-1)&&(LT_16(sub(Etot,Etot_l_lp),1280))/*5 in Q8*/)
        {
    ...
    alpha =        6554;
    move16();/* 0.2 Q15 */
    if (LT_16(epsP_0_2_ad, st_fx->epsP_0_2_ad_lp_fx))
    {
    ...
    alpha =        983 ;
    move16();/* 0.03  Q15 */
    if (GT_16(epsP_2_16 , st_fx->epsP_2_16_lp_fx))
    {
        alpha = 6554;
        move16();/* 0.2  Q15 */
    }
    ...
    alpha    = 1638;
    move16();/* 0.05  Q15 */
    if (LT_16(epsP_2_16_dlp , st_fx->epsP_2_16_dlp_lp2_fx))
    {
        alpha = 655;
        move16();/* 0.02  Q15 */
    }
    ...
    move16();
    move16();
    if( LT_16( st_fx->lt_tn_track_fx , 1638 ))      /* 0.05 in Q15*/
    {
    ...
    test();
    if ( ( GT_32(non_sta, th_sta))  /* float th_sta  NB 5e10 ,  WB 3.5e10*/
            || (*loc_harm > 0)
       )
    {
    ...
    if( (     ( LT_16(*st_harm_cor_cnt, (3*HC_CNT_SLOW_FX) ))
              && ( ( GT_32(non_sta, th_sta) ) ||  (LT_16(tmp_pc, TH_PC_FX) ) || ( GT_16(noise_char ,0))  ) )
            ||
            ( (GT_16(st_fx->ini_frame_fx, HE_LT_CNT_INIT_FX )  )  &&  ( GT_16(sub(Etot, Etot_l_lp), 2560)  )  ) ||
            (  GT_16(cor_tmp, cor_max)  ) ||  /* Q15 */
            ( GT_32(LepsP, th_eps)    ) ||  /* Q11 */
            ( GT_16(*loc_harm,0))  ||
            ((GT_16(st_fx->act_pred_fx, 26214) ) && (GT_32(Lnon_sta2, th_sta) )  ) /*act_pred in Q15 , th_sta in Q10 */
      )
    {
        vad_2nd_stage_fx = 1;
    ...
    /*                            Q15         Q12                 Q12              */
    comb_hcm_epsP = s_max(s_max(shr(st_fx->lt_haco_ev_fx,15-12), epsP_2_16_dlp_max), epsP_0_2_ad_lp_max); /* Q12 */
    ...
    tmp = 12288;
    move16();/* 1.5 Q13 */
    if( LT_16(st_fx->Etot_lp_fx, 12800 )) /* 50.0 in Q8 */
    {
    ...
    enr_bgd     = 0;
    move16();
    if(LT_16(Etot, Etot_l_lp_thr )) /* Q8 */
    {
        enr_bgd = 1;
        move16();/* Q0 */
    }
    ...
    move16();
    test();
    if(  (GT_16(epsP_0_2, 32563))          /* 7.95 in Q12 */
            && (LT_32(non_sta, 1024000L)  ) )     /* 1e3f in  Q10 ? */
    {
        cns_bgd = 1;
        move16(); /* Q0 */
    }
    ...
    lp_bgd = 0;
    move16();
    if( LT_16(epsP_2_16_dlp_max,  410)) /*0.10 Q12 */
    {
        lp_bgd = 1;
        move16(); /* Q0 */
    }
    ...
    ns_mask = 0;
    move16();
    if( LT_32(non_sta, (Word32)102400000L )) /*  (1e5f in  Q10)*/
    {
        ns_mask = 1;
        move16(); /* Q0 */
    }
    ...
    move16();
    test();
    if( (GT_16(epsP_0_2_ad,2048))         /* 0.5 in Q12 */
            &&  (GT_16(epsP_0_2, 32563) ))   /* 7.95 in Q12 */
    {
        SD_1 = 1;
        move16(); /* Q0 */
    }
    ...
    PD_1 = 0;
    move16();
    if( (LT_16( epsP_2_16_dlp_max,  410)))   /* 0.10  in Q12 */
    {
        PD_1 = 1;
        move16(); /* Q0 */
    }
    ...
    PD_2 = 0;
    move16();
    if( (LT_16( epsP_0_2_ad_lp_max,  410)))   /* 0.10  in Q12 */
    {
        PD_2 = 1;
        move16(); /* Q0 */
    }
    ...
    PD_3 = 0;
    move16();
    if( (LT_16(comb_ahc_epsP, 3482 )))   /* 0.85  in Q12 */
    {
        PD_3 = 1;
        move16(); /* Q0 */
    }
    ...
    PD_4 = 0;
    move16();
    if( (LT_16(comb_ahc_epsP, 614)))   /* 0.15  in Q12 */
    {
        PD_4 = 1;
        move16(); /* Q0 */
    }
    ...
    PD_5 = 0;
    move16();
    if( (LT_16(comb_hcm_epsP, 1229)))   /* 0.30  in Q12 */
    {
        PD_5 = 1;
        move16(); /* Q0 */
    }
    ...
    test();
    test();
    if(  ( (SD_1 == 0) || (LT_16(Etot, Etot_l_lp_thr)))
            && (bg_haco_mask != 0) && ( LT_16(st_fx->act_pred_fx, 27853 )  ) /* 0.85f in Q15 */
            && (LT_16(st_fx->Etot_lp_fx, 50*256)  ))    /* 50.0 in Q8 */
    {
        BG_1 = 1;
        move16();
    }
    ...
    tmp = 0;
    move16();/*Q0*/
    if( LT_16(Etot, 55*256)) /*55.0 in Q8 */
    {
        tmp = 1;
        move16();/*Q0*/
    }
    ...
    test();
    test();
    if (  ( GT_16(st_fx->sign_dyn_lp_fx, 15*256))                                /* 15 in Q8  */
            && ( LT_16(sub(Etot, st_fx->Etot_l_lp_fx ), shl(Etot_v_h2, 1) )  )  /*  Q8 , Etot_v_h2 has limited dynmics can be upscaled*/
    ...
    test();
    test();
    if (  ( (LT_16(st_fx->act_pred_fx, 19333))&&(LT_16(st_fx->lt_haco_ev_fx,7537))) /* .59 in Q15 .23 in Q15 */
            || (LT_16(st_fx->act_pred_fx, 12452)   )  /* .38 in Q15 */
            || (LT_16(st_fx->lt_haco_ev_fx, 4915)  ) /* .15 in Q15 */
            || (LT_16(non_staB, 50*256 )  ) /* 50.0 in Q8  */
            || aE_bgd != 0
            || ( (LT_16(Etot,10752))                         /* 42 in Q8 */
                 && (GT_16(st_fx->harm_cor_cnt_fx, 10)  )
                 && ( LT_16(st_fx->lt_haco_ev_fx, 11469)  ) /* 0.35 in Q15 */
    ...
    test();
    test();
    if ( ( LT_16(st_fx->ini_frame_fx, HE_LT_CNT_INIT_FX ))
            &&  ( GT_16(st_fx->harm_cor_cnt_fx,5))      /* > 5  Q0 */
            &&  ( LT_16(sub(Etot,st_fx->Etot_lp_fx),1792 )) /* 7 in Q8 */
            &&  ( NE_16(tmp, 0)) )
    {
        tn_ini = 1;
        move16();
    ...
    tmp = 0;
    move16();
    if(  GT_16(st_fx->lt_tn_track_fx, 29491 )) /* .90 in Q15 */
    {
        tmp = 1;
        move16();
    }
    ...
    IF (  s_or(s_and(bg_bgd2  , s_or(aE_bgd, s_or(sd1_bgd, s_or(tmp, NEW_POS_BG)))), tn_ini) )
    {
        ...
        test();
        test();
        IF( (   ( LT_16(st_fx->act_pred_fx, 27853))                                    /* 0.85 in Q15 */
                &&  ( NE_16(aE_bgd ,0) )
                &&  ( (LT_16(st_fx->lt_Ellp_dist_fx, 10*256) )  || ( NE_16(sd1_bgd, 0) )  )       /* 10.0 in Q8*/
                &&  ( LT_16(st_fx->lt_tn_dist_fx, 40*256 )  )                                /* 40.0 in Q8*/
                &&  ( LT_16(sub(Etot, st_fx->totalNoise_fx), 10*256)   )                     /* 10.0 in Q8*/
            )
            || ( (st_fx->first_noise_updt_fx == 0)  && (GT_16(st_fx->harm_cor_cnt_fx,80) )
                 && ( aE_bgd != 0 ) && (GT_16(st_fx->lt_aEn_zero_fx, 16384) )  /*.5 in Q15*/
               )
            ||  ( (tn_ini != 0 ) && ( ( aE_bgd != 0 ) || ( LT_16(non_staB, 10*256)  ) || (GT_16(st_fx->harm_cor_cnt_fx, 80)  ) ) /* 10.0 in Q8*/
                )
          )
        ...
        ELSE IF ( ( ( LT_16(st_fx->act_pred_fx, 26214))             /* .8 in Q15*/
                    && ( ( aE_bgd != 0 )   ||  ( PAU != 0 ) )
                    && (LT_16(st_fx->lt_haco_ev_fx, 3277)  ) )           /* .10 in q15*/
                  || (  ( LT_16(st_fx->act_pred_fx, 22938 )  )            /* 0.70 in Q15 */
                        && ( (aE_bgd!=0 ) || ( LT_16(non_staB, 17*256 )  ) )/* 17.0 in Q8 */
                        && ( PAU != 0 )
                        &&  ( LT_16(st_fx->lt_haco_ev_fx,4915)  )            /* 0.15 in Q15 */
                     )
                  || ( (GT_16(st_fx->harm_cor_cnt_fx, 80) ) && (GT_16(st_fx->totalNoise_fx, 5*256)  ) /* 5.0 in Q8 */
                       && ( LT_16(Etot, s_max((Word16)1*256, add(Etot_l_lp, add(st_fx->Etot_v_h2_fx,shr(st_fx->Etot_v_h2_fx,1))))) ) /* 1.5= 1.0+.5 */
                     )
                  || ( (GT_16(st_fx->harm_cor_cnt_fx,50) ) && (GT_16(st_fx->first_noise_updt_fx, 30) )
                       && (aE_bgd != 0)  && (GT_16(st_fx->lt_aEn_zero_fx, 16384) ) ) /*.5 in Q15*/
                  || ( tn_ini != 0 )
                )

        {
            ...
            test();
            IF (    (  aE_bgd==0  )
                    &&  (  LT_16(st_fx->harm_cor_cnt_fx, 50)  )
                    &&  ( ( GT_16(st_fx->act_pred_fx, 19661)   )                          /* 0.6 in Q15*/
                          || (    ( tn_ini==0  )
                                  &&  (LT_16(sub(Etot_l_lp, st_fx->totalNoise_fx),10*256)  )  /* 10.0 in Q8 */
                                  &&  (GT_16(non_staB, 8*256) )   /*  8.0 in in Q8*/
                             )
                        )
               )
            {
        ...
        ELSE IF  ( (aE_bgd !=0) || (GT_16(st_fx->harm_cor_cnt_fx, 100)))
        {
    ...
    ELSE
    {
        ...
        test();
        test();
        IF (   (GT_16(st_fx->low_tn_track_cnt_fx, 300))
        && (GT_16(st_fx->lt_haco_ev_fx, 29491 )  )  /*.9 in Q15 */
        && (st_fx->totalNoise_fx > 0 ) )
        {
            ...
            FOR( i=0; i< NB_BANDS;  i++ )
            {
                IF( GT_32(st_fx->bckr_fx[i], L_shl(Le_min_scaled, 1L) ))  /* 2*E_MIN(float) in float,  here we use  2*Le_min_scaled  Q_new+Q_SCALE  */
                {
