...
)
{
    ...
    IF ( GT_16(bits, sub(st_fx->rc_tot_bits_fx, st_fx->rc_num_bits_fx)))
    {
    ...
    move16();
    WHILE (LT_16(bits, sub(st_fx->rc_tot_bits_fx, 16)))
    {
...
)
{
    test();
    L_sub(0, 0);   /* For comparision in if */
    IF (st_fx->rc_low_fx < (0xff000000UL) || EQ_16(st_fx->rc_carry_fx, 1))
    {
...
{
    IF ( LE_16(add(rc_get_bits2_fx(st_fx->rc_num_bits_fx, st_fx->rc_range_fx), bits),  st_fx->rc_tot_bits_fx))
    {
        st_fx->rc_num_bits_fx = add(st_fx->rc_num_bits_fx, bits);
