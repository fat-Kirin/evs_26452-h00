...
)
{
    ...
    test();
    test();
    IF( GT_32(st_fx->core_brate_fx,ACELP_24k40)&&LE_32(st_fx->core_brate_fx,ACELP_32k)&&EQ_16(coder_type_fx,GENERIC))
    {
    ...
    FOR( i_subfr_fx=0; i_subfr_fx<L_frame_fx; i_subfr_fx+=L_SUBFR )
    {
        ...
        *pt_pitch_fx = pit_encode_fx( st_fx, st_fx->core_brate_fx, 0, L_frame_fx, coder_type_fx, &pitch_limit_flag, i_subfr_fx, exc_fx,
        ...
        clip_gain_fx = gp_clip_fx(st_fx->core_brate_fx,voicing_fx,i_subfr_fx,coder_type_fx,xn_fx,st_fx->clip_var_fx,sub(shift_wsp, 1));
        ...
        test();
        IF( GT_32(st_fx->core_brate_fx,ACELP_24k40)&&NE_16(coder_type_fx,INACTIVE))
        {
        ...
        if ( EQ_16(st_fx->last_ppp_mode_fx,1))
        {
            /* SC-VBR - all other st->clip_var values will be updated even in a PPP frame */
            st_fx->clip_var_fx[1] = gain_pit_fx;
            move16();
        }
        gp_clip_test_gain_pit_fx( st_fx->core_brate_fx, gain_pit_fx, st_fx->clip_var_fx );
        ...
        test();
        IF ( GT_32(st_fx->core_brate_fx,ACELP_24k40)&&EQ_16(coder_type_fx,INACTIVE))
        {
