...
)
{
    ...
    test();
    test();
    if( EQ_16(coder_type,INACTIVE)||(EQ_16(st->bpf_off_fx,1)&&NE_16(coder_type,AUDIO)&&NE_16(coder_type,TRANSITION)))
    {
    ...
    test();
    if ( EQ_16(coder_type,INACTIVE)&&GT_32(st->total_brate_fx,ACELP_24k40))
    {
    ...
    test();
    test();
    IF( st->Opt_AMR_WB_fx && EQ_16(coder_type,INACTIVE)&&st->core_brate_fx!=SID_1k75&&st->core_brate_fx!=FRAME_NO_DATA)
    {
...
)
{
    ...
    IF( EQ_16(st->last_core_fx,AMR_WB_CORE))  /* switching to EVS primary mode */
    {
        ...
        IF(  GE_16(input_frame,L_FRAME32k))
        {
        ...
        IF( EQ_16(input_frame,L_FRAME48k))
        {
            set16_fx(st->fb_state_lpc_syn_fx, 0, LPC_SHB_ORDER);
            st->fb_tbe_demph_fx = 0;
            move16();
            fb_tbe_reset_enc_fx( st->elliptic_bpf_2_48k_mem_fx, &st->prev_fb_energy_fx,st->elliptic_bpf_2_48k_mem_fx_Q,&st->prev_fb_energy_fx_Q);
        }
    ...
    ELSE            /* switching to AMR-WB IO mode */
    {
        ...
        set16_fx(st->past_qua_en_fx, (-14*1024), GAIN_PRED_ORDER );
