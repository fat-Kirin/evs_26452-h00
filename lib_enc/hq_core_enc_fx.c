...
)
{
    ...
    test();
    IF ( st_fx->last_core_fx == ACELP_CORE || EQ_16(st_fx->last_core_fx, AMR_WB_CORE))
    {
        /* Preprocessing in the first HQ frame after ACELP frame */
    ...
    test();
    IF ( EQ_16(st_fx->last_core_fx, HQ_CORE)&&GT_32(st_fx->core_brate_fx,MINIMUM_RATE_TO_ENCODE_VOICING_FLAG))
    {
        IF ( Voicing_flag > 0 )
        {
