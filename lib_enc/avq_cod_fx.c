...
)
{
    ...
    WHILE( (LE_16(Q_in, Q_in_ref))&&LE_16(tot_est_bits,mult(26214,NB_BITS))&&GT_16(tot_est_bits,600));  /* limited to 1 possible iteration */
...
)
{
    ...
    FOR( i=0; i<Nsv; i++ )
    {
        if( EQ_16(sub(i_mult2(5,nq[i]),1),bits)) /* check the overflow */
        {
            overflow = 1;
            move16();
        }
