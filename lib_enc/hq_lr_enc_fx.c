...
)
{
    ...
    Word16 trans_bit;
    Word16 adjustFlag;
    Word16 prev_SWB_peak_pos_tmp_fx[SPT_SHORTEN_SBNUM];
    Word16  k,j;
    Word16 flag_spt_fx;
    ...
    flag_spt_fx = 0;
    move16();
    set16_fx(prev_SWB_peak_pos_tmp_fx, 0, SPT_SHORTEN_SBNUM);
    adjustFlag = 0;
    move16();
    bw_low = 0;
    move16();
    ...
    test();
    test();
    IF( EQ_16(st_fx->bwidth_fx, SWB)&&(EQ_32(L_bwe_br,HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20)))
    {
        IF ( EQ_16(is_transient_fx, 1))
        {
    ...
    test();
    test();
    IF( EQ_16(st_fx->bwidth_fx, SWB)&&is_transient_fx==0&&(EQ_32(L_bwe_br,HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20)))
    {
        /* reserve bits for HQ_NORMAL2 and HQ_HARMONIC modes */
        test();
        IF( EQ_16(hqswb_clas_fx, HQ_NORMAL)||EQ_16(hqswb_clas_fx,HQ_HARMONIC))
        {
    ...
    test();
    test();
    IF(( EQ_32(L_bwe_br, HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20))&&EQ_16(st_fx->bwidth_fx,SWB))
    {
        IF( NE_16(st_fx->prev_hqswb_clas_fx, HQ_NORMAL))
        {
            j = 0;
            move16();
    ...
    test();
    IF ( GT_16(inner_frame_fx, length_fx)&&is_transient_fx)
    {
    ...
    IF( EQ_16(st_fx->bwidth_fx, SWB)&&(EQ_32(L_bwe_br,HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20)))
    {
        ...
        ELSE
        {
            ...
            IF( EQ_16(hqswb_clas_fx, HQ_NORMAL))
            {
                return_bits_normal2_fx( &bit_budget_fx, p2a_flags_fx, bands_fx, bits_lagIndices_modeNormal_fx );
            }
        }
    }
    ELSE
    ...
    test();
    test();
    IF ( is_transient_fx == 0 && EQ_16(inner_frame_fx, L_FRAME8k)&&LE_32(st_fx->core_brate_fx,ACELP_13k20))
    {
        lowband = 6;
        move16();
        trans_bit = 2;
        ...
        IF( EQ_32(st_fx->core_brate_fx, ACELP_13k20))
        {
            beta_fx = 13107;
            move16();/*14 1.25f; */
            gama_fx = 31130;
        ...
        test();
        test();
        IF(( (LT_32(L_tmp, L_shr(Ep_avrgL_fx,1))&&EQ_32(st_fx->core_brate_fx,ACELP_13k20))||LT_32(st_fx->core_brate_fx,ACELP_13k20))&&
           LT_32(L_tmp2, L_tmp3) && GT_32(L_tmp2, L_shr(Ep_avrg_fx,7)) )
        {
            FOR(i = lowband; i < bands_fx; i++)
            {
                L_tmp = Mult_32_16(Ep_avrg_fx,24576);/*Q(13+14-15 = 12) 1.5  */
                IF(LT_32(L_shr(Ep_tmp_fx[i],1), L_tmp))
                {
        ...
        ELSE
        {
            ...
            FOR(i = sub(bands_fx,trans_bit); i < bands_fx; i++)
            {
                ...
                IF(EQ_16(st_fx->last_bitalloc_max_band_fx[j++], 1))
                {
                    L_tmp = Mult_32_16(Ep_tmp_fx[i],sub(bands_fx,lowband));/*Q(13+0-15 = -2) */
        ...
        test();
        test();
        IF( GT_32(L_shr(Ep_avrg_fx,2), L_tmp2)&&LT_32(L_shr(Ep_avrg_fx,4),L_tmp2)&&GT_32(L_tmp,Ep_avrgL_fx))
        {
            adjustFlag = 1;
            move16();
            FOR (i = 0; i < lowband; i++)
        ...
        FOR(i = 0; i < 2; i++)
        {
            push_indice_fx ( st_fx, IND_HQ2_LAST_BA_MAX_BAND, st_fx->last_bitalloc_max_band_fx[i], 1 );
        }
    }
    ELSE IF( is_transient_fx == 0 && EQ_16(inner_frame_fx, L_FRAME16k))
    {
        ...
        {
            test();
            IF( EQ_16(p2a_flags_fx[i],1)||L_tmp2>0)
            {
            ...
            {
                tmp = sub(bands_fx,p2a_bands_fx);
                IF(EQ_16(st_fx->last_bitalloc_max_band_fx[sub(i, add(tmp, 1))], 1))
                {
                    tmp = sub(tmp,lowband);
        ...
        test();
        test();
        IF( (GT_32(L_shr(Ep_avrgL_fx,1), Ep_avrg_fx)&&GT_32(L_tmp,L_shr(Ep_avrgL_fx,2))&&LT_32(L_shr(Ep_avrgL_fx,1),L_tmp2))||
            (GT_32(L_shr(Ep_avrg_fx,1), Ep_avrgL_fx) && LT_32(L_shr(Ep_avrg_fx,3),L_tmp3)  && GT_32(L_tmp,L_shr(Ep_avrgL_fx,2))  ) )
        {
            adjustFlag = 1;
            move16();
            FOR (i = 0; i < lowband; i++)
    ...
    ELSE IF( EQ_16(st_fx->bwidth_fx, SWB)&&EQ_16(hqswb_clas_fx,HQ_HARMONIC)&&(EQ_32(L_bwe_br,HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20)))
    {
    ...
    IF((EQ_16(inner_frame_fx, L_FRAME8k)&&LE_32(st_fx->core_brate_fx,ACELP_13k20))||EQ_16(inner_frame_fx,L_FRAME16k))
    {
        j = 0;
        FOR(i = 2; i > 0; i--)
        {
            IF(npulses_fx[bands_fx-i] > 0)
            {
                st_fx->last_bitalloc_max_band_fx[j] = 1;
            }
            ELSE
            {
                st_fx->last_bitalloc_max_band_fx[j] = 0;
            }
            j++;
        }
    }
    ...
    ni_seed_fx = add(add(add(npulses_fx[0], npulses_fx[1]), npulses_fx[2]), npulses_fx[3]);
    ...
    IF( EQ_16(st_fx->bwidth_fx, SWB)&&(EQ_32(L_bwe_br,HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20)))
    {
        ...
        IF( EQ_16(hqswb_clas_fx, HQ_NORMAL)||EQ_16(hqswb_clas_fx,HQ_HARMONIC))
        {
            ...
            IF( EQ_16(hqswb_clas_fx, HQ_NORMAL))
            {
                ...
                {
                    test();
                    IF( p2a_flags_fx[bands_fx-SPT_SHORTEN_SBNUM+k] == 0 || npulses_fx[bands_fx-SPT_SHORTEN_SBNUM+k] == 0 )
                    {
                        prev_SWB_peak_pos_tmp_fx[k] = 0;
                        move16();
                    }
                }
            }
    ...
    updat_prev_frm_fx( L_y2, L_t_audio, L_bwe_br, length_fx, inner_frame_fx, bands_fx, st_fx->bwidth_fx, is_transient_fx, hqswb_clas_fx, &st_fx->prev_hqswb_clas_fx,
                       st_fx->prev_SWB_peak_pos_fx, prev_SWB_peak_pos_tmp_fx, &st_fx->prev_frm_hfe2_fx, &st_fx->prev_stab_hfe2_fx, 0 );

    IF( NE_16(st_fx->bwidth_fx, SWB))
    {
    ...
    test();
    test();
    IF( NE_16(hqswb_clas_fx, HQ_HARMONIC)&&(EQ_32(L_bwe_br,HQ_16k40)||EQ_32(L_bwe_br,HQ_13k20))&&EQ_16(st_fx->bwidth_fx,SWB))
    {
...
)
{
    ...
    IF ( flag_pack == 0 || ( EQ_16(flag_pack, 1)&&*hLCmode0==0))
    {
        ...
        FOR( i=1; i<BANDS; ++i )
        {
            ...
            test();
            if ( GT_16(qbidx[i], sub(ABS_ENG_OFFSET,1))||LT_16(qbidx[i],-ABS_ENG_OFFSET))
            {
                cnt_outlyer = add(cnt_outlyer, 1);
            }
        }
    ...
    FOR (k = sub(bands, Ngq); k < bands; k++)
    {
        ...
        IF ( L_Eyy > 0x0L && L_Exy > 0x0L )
        {
            ...
            FOR (i = band_start[k_sort[k]]; i <= band_end[k_sort[k]]; i++)
            {
                ...
                ELSE
                {
                    L_y2[i] = L_negate(L_shl(Mpy_32_16_1(L_abs(L_y2[i]), gamma_fx), 1));
                }
            }
        }
        ELSE
