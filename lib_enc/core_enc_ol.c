...
)
{
    ...
    *---------------------------------------------------------------*/
    ...
    IF ( EQ_16(st->core_fx, TCX_20_CORE))
    {
        ...
        target_bits = sub(target_bits,1);
        test();
        if (EQ_16(st->tcx_cfg.tcx_curr_overlap_mode, HALF_OVERLAP)||EQ_16(st->tcx_cfg.tcx_curr_overlap_mode,MIN_OVERLAP))
        {
            target_bits = sub(target_bits,1);
        }
    ...
    IF( st->Opt_DTX_ON_fx != 0 && vad_hover_flag != 0 )
    {
        ...
        if( GT_16(st->burst_ho_cnt_fx,HO_HIST_SIZE))
        {
    ...
    *---------------------------------------------------------------*/
    ...
    IF( EQ_16(st->rf_mode,1))
    {
        ...
        IF (EQ_16(st->core_fx, ACELP_CORE))
        {
            ...
            IF( NE_16(st->rf_frame_type,RF_NO_DATA))
            {
        ...
        ELSE
        {
            ...
            test();
            test();
            IF( (st->rf_gain_tcx[1] != 0) && (GT_16(st->rf_gain_tcx[0], tmp))&&(LE_16(st->tcxltp_gain,6554/*0.2 Q15*/)))
            {
            ...
            IF(
                ...
                && (EQ_16(st->rf_second_last_core, TCX_20_CORE) )/*&& (st->rf_second_last_core == TCX_20_CORE)*/
                && ( (LE_16(st->tcxltp_pitch_int, shr(st->L_frame_fx, 1)) ) || ( LE_16(st->tcxltp_gain, 13107/*0.4f Q15*/) ) )/*&& ((st->tcxltp_pitch_int <= 0.5f*st->L_frame) || ( st->tcxltp_gain <= 0.4f))*/
                && (EQ_16(st->tcxltp_pitch_int, st->rf_tcxltp_pitch_int_past) )/*&& (st->tcxltp_pitch_int == st->rf_tcxltp_pitch_int_past)*/
            ...
            ELSE IF (st->last_core_fx != 0)
            {
                ...
                test();
                test();
                IF ( ((LE_16(st->clas_fx, UNVOICED_TRANSITION))||(LE_16(st->last_clas_fx,UNVOICED_TRANSITION))||(LE_16(st->tcxltp_gain,13107/*0.4f Q15*/)))
            ...
            IF( rf_PLC_Mode == 0 && st->rf_gain_tcx[1] != 0 &&
                ( (st->transientDetection.transientDetector.bIsAttackPresent != 0 && LT_16(st->rf_gain_tcx[0], mult_r(st->rf_gain_tcx[1], 31785/*0.97f Q15*/)) ) ||
                  LT_16(st->rf_gain_tcx[0], mult_r(st->rf_gain_tcx[1], 29491/*0.90f Q15*/)) )
              )
            {
                TD_mode = 0;
            }
...
)
{
    ...
    ELSE
    {
        ...
        *---------------------------------------------------------------*/
        ...
        test();
        if (EQ_32(st->sr_core, 16000)||EQ_32(st->sr_core,12800))
        {
        ...
        *---------------------------------------------------------------*/
        ...
        if ((GT_16(snr_acelp, snr_tcx))&&
                ...
                (LT_16(add(st->prevTempFlatness_fx, currFlatness), 416/*3.25f Q7*/)  || EQ_16(stab_fac, 0x7fff)  || (EQ_32(st->sr_core, 12800)  && EQ_16(sp_aud_decision0,1) && LT_16(add(st->prevTempFlatness_fx, currFlatness), 2560/*20.f Q7*/) )) &&
        ...
        test();
        test();
        if( EQ_32(st->sr_core, 12800)&&(offset_tcx<0x18950F)&&GT_16(non_staX,1280 /*5.0f Q8*/ )&&(snr_acelp>=snr_tcx-1024 /*4.0f in Q8*/)&&GE_16(st->acelpFramesCount,1)&&((GT_16(st->lps_fx,st->lpm_fx)&&GE_16(tmp16,9830))||(GE_16(st->acelpFramesCount,6)&&(st->lps_fx>st->lpm_fx-768)))&&(sp_aud_decision0==0)&&vad_flag!=0)
        {
        ...
        test();
        test();
        if (EQ_32(st->sr_core, 12800)&&(LT_16(non_staX,512/*2.0f Q8*/)||(st->flag_noisy_speech_snr==0&&EQ_16(vad_flag,1)&&(offset_tcx==L_add(0xFFD57AB5,0))&&GE_16(st->acelpFramesCount,6)))&&(st->last_core_fx==ACELP_CORE||st->last_core_fx==TCX_20_CORE))
        {
