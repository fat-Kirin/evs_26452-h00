...
)
{
    ...
    test();
    if ((LT_16(last_class,VOICED_TRANSITION))&&(NE_16(bwidth,NB)))
    {
...
)
{
    ...
    WHILE (LE_16(pit_min, add(pit_max0, delta)))      /* check for section boundary */
    {
        IF (LE_16(abs_s(sub(pit_min, pitch1)), delta))        /* if multiple in the allowed range */
        {
...
)
{
    ...
    FOR ( k = sect0; k < NSECT; k++ )    /* loop for each section */
    {
        K = 3;
        move16();
        if (EQ_16(k, (NSECT-1)))  /* the number of tests depends on the section */
        {
            K = 2;
            move16();
        }
        ...
        FOR (i=0; i < K; i++)        /* for the 2 half-frames and look-ahead */
        {
            ...
            {
                IF (NE_16(j, i))  /* Exclude itself, of course */
                {
                    IF (GE_16(corr_tmp[j], CORR_TH1)) /* reinforcement can happen only if the correlation is high enough */
                    {
    ...
    FOR ( k = sect0; k < NSECT; k++ )    /* loop for each section */
    {
        ...
        FOR (i=0; i < K; i++)/* BRANCH(1); for the 2 half-frames and look-ahead */
        {
            ...
            FOR (j=0; j < K; j++)/* Verify pitch coherence with neighbours (including past pitch) */
            {
                IF (NE_16(j, i))/* Exclude itself, of course */
                {
                    IF (GE_16(corr_tmp[j+NHFR], CORR_TH1))/* reinforcement can happen only if the correlation is high enough */
                    {
                        ...
                        {
                            /* Favour stability across sections, favour closer values */
                            IF (EQ_16(ind_tmp[j+NHFR], add(NSECT, k)))
                            {
...
)
{
    ...
    move16();
    test();
    if( (LE_16(larger, extract_h(L_shl(L_mult(fac_max, smaller), 3))))&&    /* Changed to <= to keep BE */
            (LT_16(sub(larger, smaller), diff_max)))
    {
        pc = 1;
        move16();
    }
