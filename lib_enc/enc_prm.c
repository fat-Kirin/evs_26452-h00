               ...
               )
{
    ...
    IF ( GE_16(rf_frame_type,RF_TCXFD)&&LE_16(rf_frame_type,RF_TCXTD2))
    {
        ...
        test();
        test();
        IF( EQ_16(st->rf_clas[fec_offset], UNVOICED_CLAS))
        {
            index = 0;
            move16();
        }
        ELSE IF( (EQ_16(st->rf_clas[fec_offset], VOICED_TRANSITION))||(EQ_16(st->rf_clas[fec_offset],UNVOICED_TRANSITION)))
        {
            index = 1;
            move16();
        }
        ELSE IF( EQ_16(st->rf_clas[fec_offset], VOICED_CLAS))
        {
            index = 2;
            move16();
        }
        ...
        ELSE
        {
            ...
            /* LTP data */
            test();
            IF ( (EQ_16(rf_frame_type, RF_TCXTD1)||EQ_16(rf_frame_type,RF_TCXTD2))&&st->tcxltp!=0)
            {
...
)
{
            ...
            ELSE
            {
                ...
                ELSE
                {
                    ...
                    IF( EQ_16(st->rf_mode,1))
                    {
                        ...
                        test();
                        IF(EQ_16(st->tcx_cfg.coder_type,VOICED)||
                           EQ_16(st->tcx_cfg.coder_type,GENERIC)||
                           EQ_16(st->tcx_cfg.coder_type,TRANSITION))
                        {
                            st->sharpFlag=1;
                        }
                        ELSE
    ...
    IF ( core!=ACELP_CORE )
    {
        ...
        IF (EQ_16(st->tcx_cfg.tcx_curr_overlap_mode, MIN_OVERLAP))
        {
            nbits_tcx = 2;
            move16();
            overlap_code = 2;
        ...
        ELSE IF (EQ_16(st->tcx_cfg.tcx_curr_overlap_mode, HALF_OVERLAP))
        {
            nbits_tcx = 2;
            move16();
            overlap_code = 3;
    ...
    IF ( s_or((Word16)EQ_16(core,TCX_20_CORE),(Word16)EQ_16(core,HQ_CORE)))
    {
        ...
        /* LTP data */
        test();
        IF ( st->tcxltp || GT_32(st->sr_core, 25600))
        {
            IF ( prm[j] )
            {
        ...
        test();
        IF ( st->tcx_lpc_shaped_ari && NE_16(last_core, ACELP_CORE))
        {
        ...
        /*Context HM flag*/
        test();
        IF ( st->tcx_cfg.ctx_hm && NE_16(last_core, ACELP_CORE))
        {
            push_next_indice_fx(st, prm[j], 1);
    ...
    IF (EQ_16(core,TCX_10_CORE))
    {
        ...
        FOR (k = 0; k < 2; k++)
        {
            ...
            test();
            test();
            IF (  (k == 0) && (st->tcxltp!=0 || GT_32(st->sr_core, 25600)))/* PLC pitch info for HB */
            {
                IF ( prm[j] )
                {
                    push_next_indice_fx(st, 1       , 1);
