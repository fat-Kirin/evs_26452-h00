...
)
{
    ...
    if (LT_16(shl(1, shift), n))shift=add(shift,1);
...
{
    test();
    IF (EQ_16(st->mdct_sw_enable, MODE1)||EQ_16(st->mdct_sw_enable,MODE2))
    {
        ...
        move16();
        test();
        if (EQ_16(st->last_codec_mode, MODE1)&&EQ_16(last_core,TCX_20_CORE))
        {
            last_core = HQ_CORE;
            move16();
        }
        ...
        hi_sparse = 0;
        move16();
        if (LE_16(peak_count, mult_r(sub(hib_fft, lob_fft), MDCT_SW_HI_SPARSE_THR)))
        {
            hi_sparse = 1;
            move16();
        }
        ...
        sparse = 0;
        move16();
        if (LE_16(peak_count, mult_r(sub(hib_fft, lob_fft), MDCT_SW_SPARSE_THR)))
        {
            sparse = 1;
            move16();
        }
        ...
        test();
        test();
        if (st->prev_hi_sparse > 0 && sparse > 0 && GE_16(s_min(s_min(voicing[0], voicing[1]), voicing[2]), MDCT_SW_1_VOICING_THR))
        {
            hi_sparse = 1;
            move16();
        }
        ...
        test();
        test();
        switching_point = (NE_16(last_core, HQ_CORE)  && NE_16(last_core, TCX_20_CORE) ) || /* previous core was non-MDCT */
                          (LE_16(st->prev_hi_ener, MDCT_SW_HI_ENER_LO_THR)  || LE_16(hi_ener, MDCT_SW_HI_ENER_LO_THR) ) || /* hi band is close to silent */
                          (EQ_16(last_core, HQ_CORE) && (EQ_16(st->mdct_sw_enable, MODE1) || (hi_sparse > 0 && st->prev_hi_sparse >= 0 && LE_16(st->prev_hi_sparse, 1) ))) || /* HQ_CORE and hi band became sparse */
                          (EQ_16(last_core, TCX_20_CORE) && (hi_sparse == 0 && st->prev_hi_sparse > 0)); /* TCX and hi band became dense */
        ...
        test();
        test();
        prefer_tcx = (GE_16(sub(Etot, sp_floor), sig_hi_level_thr)) && /* noise floor is low */
                     (GE_16(cor_map_sum, cor_thr)  || GE_16(frame_voicing, voicing_thr)  || GE_16(sparseness, sparseness_thr) ) && /* strong tonal components */
                     (LE_16(hi_ener, hi_ener_lo_thr)  || hi_sparse > 0); /* high freqs have low energy or are sparse */
        ...
        ELSE IF (EQ_16(last_core, HQ_CORE)||EQ_16(last_core,TCX_20_CORE))
        {
        ...
        IF (EQ_16(st->mdct_sw_enable, MODE2)&&EQ_16(st->core_fx,HQ_CORE)&&(EQ_16(st->flag_noisy_speech_snr,1)||vadflag==0))
        {
            st->core_fx = TCX_20_CORE;
            move16();
        }
