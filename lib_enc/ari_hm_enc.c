...
)
{
    ...
    FOR (Index = Lo; Index < Hi; Index += Spacing)
    {
        ...
        if (GT_16(CurrentScore, BestScore))
        {
            BestIndex = Index;
            move16();
        }
...
)
{
    ...
    test();
    IF ((LtpPitchLag > 0) && (GT_16(LtpGain, kLtpHmGainThr)))
    {
        Bandwidth = 0;
        move16();
        ...
        FOR (Multiplier = 1; Multiplier <= tmp; ++Multiplier)
        {
            ...
            test();
            IF ((GE_32(Lag, 4<<kLtpHmFractionalResolution))&&(LE_32(Lag,tmp32)))
            {
                CurrentScore = SearchPeriodicityIndex_Single(
                                   AbsMdct3,
                                   NumToConsider,
    ...
    ELSE
    {
        IF (UnfilteredMdct != NULL)
        {
            ...
            {
                if (GT_32(AbsMdct3[i], AbsMdct3[MaxAt]))
                {
                    MaxAt = i;
                    move16();
                }
            ...
            if (GT_32(L_shr(AbsMdct3[MaxAt], 6), Mpy_32_16_1(A, 22938/*0.7 Q15*/)))
            {
        ...
        IF (LE_16(NumToConsider, 128))    /* no long lags for band-limited MDCTs */
        {
            SearchPeriodicityIndex_Range(
                AbsMdct3,
                NumToConsider,
        ...
        ELSE {
            test();
            IF (GT_16(TargetBits, kSmallerLagsTargetBitsThreshold)&&GE_16(NumToConsider,256))
            {
                SearchPeriodicityIndex_Range(
                    AbsMdct3,
                    NumToConsider,
...
)
{
    ...
    *gain = 0;
    ...
    IF ( LT_16(relative_score,kLowPeriodicityThr[s]))
    {
        return;
    }
...
)
{
    ...
    UnmapIndex(
        ...
        bw_flag,
        LtpPitchLag,
        (( LE_16(sub(targetBits, hm_bits),kSmallerLagsTargetBitsThreshold) ) || !bw_flag),
        &fract_res,
        &lag
    );
    test();
