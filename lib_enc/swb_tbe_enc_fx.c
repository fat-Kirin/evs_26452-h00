...
static void EstimateSHBFrameGain_fx( const Word16 length,
                                     ...
                                     Word32* GainFrame, const Word16* win_shb, const Word16* subwin_shb, const Word16 n_subfr_saturation );


static void EstimateSHBGainShape_fx( const Word16 length,
                                     const Word16* oriSHB, const Word16 Q_oriSHB,
                                     const Word16* synSHB, const Word16 Q_synSHB,
                                     Word16* subgain, const Word16* subwin, Word16* n_subfr_saturation );
...
)
{
    ...
    move16();
    set16_fx( st_fx->elliptic_bpf_2_48k_mem_fx_Q,  0, 4 );
    st_fx->prev_fb_energy_fx_Q = 0;
    move16();
...
)
{
    ...
    test();
    test();
    IF ( ( NE_16 (st_fx->last_extl_fx, WB_TBE)  && NE_16( st_fx->last_extl_fx, WB_BWE))
         && ( EQ_16( st_fx->clas_fx, UNVOICED_CLAS)  || ( LT_16( voicing_fx[0], 16384 ) && LT_16( voicing_fx[1], 16384 )  && LT_16( voicing_fx[2], 16384 )  ) )
         && st_fx->igf == 0 )
    {
    ...
    test();
    IF ( EQ_16(st_fx->rf_mode, 1)||EQ_32(st_fx->extl_brate_fx,WB_TBE_0k35))
    {
    ...
    move16();
    test();
    if( EQ_32(st_fx->extl_brate_fx, WB_TBE_1k05)&&EQ_16(st_fx->coder_type_raw_fx,UNVOICED))
    {
        uv_flag = 1;
        move16();
    }
...
)
{
    ...
    *prev_fb_energy_fx = 0;
    move16();
    set16_fx(elliptic_bpf_2_48k_mem_fx_Q,  0, 4);
    *prev_fb_energy_fx_Q = 0;
    move16();
...
)
{
    ...
    /* Spectral smoothing of autocorrelation coefficients */
    test();
    IF( (st_fx->rf_mode != 0) || EQ_32( st_fx->total_brate_fx, ACELP_9k60 ))
    {
    ...
    test();
    IF( (EQ_16(st_fx->rf_mode,1))||EQ_32(st_fx->total_brate_fx,ACELP_9k60))
    {
        ...
        weights_lsp[3] = mult_r( weights_lsp[3], 29789/*0.909091f Q15*/ );
        weights_lsp[4] = mult_r( weights_lsp[4], 29789/*0.909091f Q15*/ );
    ...
    test();
    IF( EQ_16(coder_type_fx, VOICED)||GT_16(mean_vf,13107/*0.4f Q15*/))
    {
        FOR( i = 1; i < NB_SUBFR; i++ )
        {
    ...
    IF(NE_16(st_fx->last_extl_fx,SWB_TBE))
    {
        FOR( i=1; i<LPC_SHB_ORDER-1; i++)
        {
            st_fx->prev_lsf_diff_fx[i-1] = mult(lsf_diff[i],16384);/*Q15*/
        }
    }
    ...
    IF( LE_32(st_fx->total_brate_fx,ACELP_16k40))
    {
        ...
        test();
        test();
        IF(!(GT_16(st_fx->prev_tilt_para_fx,5120)&&(EQ_16(coder_type_fx,TRANSITION)||LT_16(tilt_para,1024)))&&
    ...
    GenShapedSHBExcitation_fx( shaped_shb_excitation_fx + L_SHB_LAHEAD, lpc_shb_fx, White_exc16k_fx,
                               ...
                               &(st_fx->fb_tbe_demph_fx), &Q_bwe_exc,&Q_bwe_exc_fb, Q_shb, n_mem2, st_fx->prev_Q_bwe_syn, st_fx->total_brate_fx, 0 );
    ...
    *Q_white_exc = Q_bwe_exc_fb;
    ...
    IF( LT_16(st_fx->cldfbHBLT, 8192/*1.0f Q13*/))   /* cldfbHBLT in Q13 */
    {
        ...
        FOR(i = 0; i < NUM_SHB_SUBGAINS; i++)
        {
            ...
            GainShape_fx[i] = mac_r(676457349l/*0.315 Q31*/, 3277/*0.1 Q15*/, GainShape_fx[i]);
        }
    }
    ...
    test();
    test();
    IF( EQ_16(frGainAttenuate,1)||(GT_16(tmp,19661)&&GT_16(tmp1,6554)))
    ...
    ELSE IF( EQ_16(st_fx->prev_frGainAtten_fx, 1)&&GT_32(Mult_32_16(GainFrame_fx,10923),st_fx->prev_gainFr_SHB_fx))
    {
    ...
    {
        tmp1 = mult_r(6554/*0.2f Q15*/, tmp);  /* Q15, Q6  => Q6 */
        L_tmp = L_msu(8192l/*1.0f Q13*/, tmp1, tmp);  /* Q13 */
    }
    ELSE
    {
        tmp1 = mult_r(3277/*0.1f Q15*/, tmp);  /* Q15, Q6  => Q6 */
        L_tmp = L_msu(8192l/*1.0f Q13*/, tmp1, tmp);  /* Q13 */
    }
    /* sd_uq_q =  max(min(sd_uq_q, 1.0f), 0.5f); */
    L_tmp = L_min(L_tmp, 8192l/*1.0f Q13*/);
    L_tmp = L_max(L_tmp, 4096l/*0.5f Q13*/);  /* Q13 */
    ...
    tmp2 = 5462/*0.1667f Q15*/;
    if(GT_16(sum2, 4915/*0.6f Q13*/))
    {
        tmp2 = 10912/*0.333f Q15*/;   /* Q15 */
    }
    ...
    test();
    IF(EQ_16(st_fx->L_frame_fx, L_FRAME16k)||EQ_16(st_fx->rf_mode,1))
    {
        /* Compensate for energy increase mismatch due to memory-less synthesis*/
        GainFrame_fx = Mult_32_16(GainFrame_fx, 27853/*0.85f Q15*/ );  /* Q18 */
    }
...
)
{
    ...
    {
        scaling = negate(shr(n_subfr_saturation, 1) + 1);
        move16();
        oriNrg = 0;
        move32();
        FOR( i = 0; i < l_shb_lahead; i++ )
...
)
{
    ...
    ELSE
    {
        ...
        FOR( i = 0; i < num_gains; i++ )
        {
            ...
            IF(EQ_32(oriNrg, MAX_32))
            {
                if(n_subfr_saturation != NULL)
                {
                    *n_subfr_saturation = add(*n_subfr_saturation, 1);
                }
                scaling = -1;
                move16();
                oriNrg = 0;
    ...
    n = sub(31, add (n, (sub( 28 ,  n_max*2 ) )));
    normFact = Isqrt_lc( L_sum_gain, &n );

    FOR( i = 0; i < num_gains; i++ )
    {
        L_tmp = Mult_32_16( normFact, subgain[i] );  /*Q(31-n) * Q(31-norm[i])      */ /* Q(30-n-n_max) */
        subgain[i] = s_max( round_fx( L_shl( L_tmp, add( n, n_max + 1 ) ) ), 3277/*0.1f Q15*/); /* Q15 */
    }
...
)
{
    ...
    ELSE
    {
        ...
        IF( GT_16( idxFrameGain, ( 1 << NUM_BITS_SHB_FRAMEGAIN ) - 1 ))
        {
        ...
        WHILE( GT_32( Q_GainFrame, L_add( GainFrameLog, 4866 ) )&&
        idxFrameGain != 0 )
        {
    ...
    const   Word16 dims /* i : number of gains */ )
{
    ...
    FOR( j = 0; j < dims; j++ )
    {
        IF( GT_32(gain[j],8))/* 8 = 0.001 in Q13 */
        {
                              ...
                              )
{
    ...
    *MA_lsp_shb_spacing = 16384;
    ...
    test();
    IF( NE_16( st_fx->last_extl_fx, SWB_TBE )
        && NE_16(st_fx->last_extl_fx, FB_TBE) 
        && LT_16( lsp_spacing, 262 )  )
    {
    ...
    *MA_lsp_shb_spacing = mac_r(tempQ31, lsp_spacing, 13107);      /* 0.4f */
    ...
    test();
    test();
    IF( ( LT_16( lsp_spacing, 262 )&&(LT_16(*MA_lsp_shb_spacing,164)||EQ_16(st_fx->prev_frGainAtten_fx,1)))
        || LE_16( lsp_spacing, 105 )  )
    {
        *frGainAttenuate = 1;
        move16();

        IF( NE_32(st_fx->total_brate_fx , ACELP_24k40))
        {
...
)
{
    ...
    {
        s_max_value = s_max(s_max_value,abs_s(new_input[i]));
    }
    ...
    elliptic_bpf_48k_generic_fx( input_fhb, &exp_temp , tmp_vec, st->elliptic_bpf_2_48k_mem_fx, st->elliptic_bpf_2_48k_mem_fx_Q,full_band_bpf_2_fx );
    Sample_Delay_HP = NS2SA(48000, ACELP_LOOK_NS + DELAY_FD_BWE_ENC_12k8_NS + DELAY_FIR_RESAMPL_NS) - L_FRAME48k/2 ;

    IF( NE_16(st->last_extl_fx,FB_TBE))
    {
        set16_fx( st->old_input_fhb_fx, 0, Sample_Delay_HP );
        st->old_input_fhb_fx_Q = 0;
        move16();
        set16_fx( tmp_vec, 0, L_FRAME16k );
    }
    Copy_Scale_sig( st->old_input_fhb_fx, st->old_input_fhb_fx, Sample_Delay_HP, sub(exp_temp,st->old_input_fhb_fx_Q) );
    st->old_input_fhb_fx_Q = exp_temp;
    ...
    temp2 = L_add(temp2,L_shl(st->prev_fb_energy_fx,sub(sub(add(exp_temp,exp_temp),6),st->prev_fb_energy_fx_Q)));/* Q11 */
    st->prev_fb_energy_fx = sum2_fx_mod( input_fhb + L_FRAME48k/2, L_FRAME48k/2 );/*Q11*/
    st->prev_fb_energy_fx_Q = sub(add(exp_temp,exp_temp),6);
...
)
{
    ...
    test();
    test();
    IF ( ( st_fx->rf_mode || EQ_32( st_fx->total_brate_fx, ACELP_9k60 ))&&(EQ_16(st_fx->bwidth_fx,WB)))
    {
        /* WB LSF */
...
)
{
    ...
    ELSE IF( ( EQ_16( bandwidth, SWB ))||(EQ_16(bandwidth,FB)))
    {
        ...
        IF( EQ_16( bandwidth, FB ))
        {
            ...
            st_fx->fb_tbe_demph_fx = 0;
            move16();
            fb_tbe_reset_enc_fx( st_fx->elliptic_bpf_2_48k_mem_fx, &st_fx->prev_fb_energy_fx,st_fx->elliptic_bpf_2_48k_mem_fx_Q,&st_fx->prev_fb_energy_fx_Q);
        }
    }
}
